...
func TestTmpInspectSeq(t *testing.T) {
	...
	if len(s.recordId.chunk) > 0 { fmt.Println("recordId.chunk[0]", s.recordId.chunk[0]) }
	if len(s.start.chunk) > 0 { fmt.Println("start.chunk[0]", s.start.chunk[0]) }
	if len(s.stride.chunk) > 0 { fmt.Println("stride.chunk[0]", s.stride.chunk[0]) }
