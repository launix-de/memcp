			...
			for i, s := range shardlist {
				gls.Go(func(i int, s *storageShard) func() {
					return func() {
						...
						for !s.ComputeColumn(name, inputCols, computor, filterCols, filter, lazy, len(shardlist) == 1, currentTx) {
...
func (s *storageShard) ComputeColumn(name string, inputCols []string, computor scm.Scmer, filterCols []string, filter scm.Scmer, lazy bool, parallel bool, tx *TxContext) bool {
