...
func (s *StorageInt) JITEmit(ctx *scm.JITContext, thisptr scm.JITValueDesc, idx scm.JITValueDesc, result scm.JITValueDesc) scm.JITValueDesc {
	/* DO NEVER MANUALLY EDIT THIS SECTION. RUN make jitgen TO UPDATE */
			...
			ctx.FreeDesc(&d39)
			ctx.EnsureDesc(&d40)
			d41 := scm.JITValueDesc{Loc: scm.LocRegPair, Reg: r0, Reg2: r1}
			ctx.BindReg(r0, &d41)
			ctx.BindReg(r1, &d41)
			ctx.EnsureDesc(&d40)
			ctx.W.EmitMakeInt(d41, d40)
			if d40.Loc == scm.LocReg { ctx.FreeReg(d40.Reg) }
			ctx.W.EmitJmp(lbl0)
			...
			if thisptr.Loc == scm.LocImm {
				...
				d42 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r48}
				ctx.BindReg(r48, &d42)
			}
			ctx.EnsureDesc(&d35)
			ctx.EnsureDesc(&d42)
			...
			if d35.Loc == scm.LocImm && d42.Loc == scm.LocImm {
				...
				d43 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagBool, Reg: r49}
				ctx.BindReg(r49, &d43)
			...
			d45 := scm.JITValueDesc{Loc: scm.LocRegPair, Reg: r0, Reg2: r1}
			ctx.BindReg(r0, &d45)
			ctx.BindReg(r1, &d45)
			ctx.W.EmitMakeNil(d45)
			ctx.W.EmitJmp(lbl0)
