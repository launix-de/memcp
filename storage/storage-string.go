...
func (s *StorageString) JITEmit(ctx *scm.JITContext, thisptr scm.JITValueDesc, idx scm.JITValueDesc, result scm.JITValueDesc) scm.JITValueDesc {
	/* DO NEVER MANUALLY EDIT THIS SECTION. RUN make jitgen TO UPDATE */
			...
			if thisptr.Loc == scm.LocImm {
				...
				d139 = scm.JITValueDesc{Loc: scm.LocRegPair, Reg: r116, Reg2: r117}
			} else {
				...
				d139 = scm.JITValueDesc{Loc: scm.LocRegPair, Reg: r118, Reg2: r119}
			}
			r120 := ctx.AllocReg()
			r121 := ctx.AllocReg()
			if d139.Loc == scm.LocImm {
				ctx.W.EmitMovRegImm64(r120, uint64(d139.Imm.Int()))
			} else if d139.Loc == scm.LocRegPair {
				ctx.W.EmitMovRegReg(r120, d139.Reg)
			} else {
				ctx.W.EmitMovRegReg(r120, d139.Reg)
			}
			if d102.Loc == scm.LocImm {
				if d102.Imm.Int() != 0 {
					if d102.Imm.Int() >= -2147483648 && d102.Imm.Int() <= 2147483647 {
						ctx.W.EmitAddRegImm32(r120, int32(d102.Imm.Int()))
					} else {
						scratch := ctx.AllocReg()
						ctx.W.EmitMovRegImm64(scratch, uint64(d102.Imm.Int()))
				...
				ctx.W.EmitAddInt64(r120, d102.Reg)
			}
			if d137.Loc == scm.LocImm {
				ctx.W.EmitMovRegImm64(r121, uint64(d137.Imm.Int()))
			} else {
				ctx.W.EmitMovRegReg(r121, d137.Reg)
			}
			if d102.Loc == scm.LocImm {
				if d102.Imm.Int() >= -2147483648 && d102.Imm.Int() <= 2147483647 {
					ctx.W.EmitSubRegImm32(r121, int32(d102.Imm.Int()))
				} else {
					scratch := ctx.AllocReg()
					ctx.W.EmitMovRegImm64(scratch, uint64(d102.Imm.Int()))
				...
				ctx.W.EmitSubInt64(r121, d102.Reg)
			}
			...
			if thisptr.Loc == scm.LocImm {
				...
				d142 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r122}
			}
			...
			if d142.Loc == scm.LocImm {
				...
				r123 := ctx.AllocReg()
				ctx.W.EmitMovRegReg(r123, d142.Reg)
				ctx.W.EmitShlRegImm8(r123, 32)
				ctx.W.EmitShrRegImm8(r123, 32)
				d143 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r123}
			}
			ctx.FreeDesc(&d142)
			...
			if d34.Loc == scm.LocImm && d143.Loc == scm.LocImm {
				...
				d144 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagBool, Reg: r126}
			}
			ctx.FreeDesc(&d34)
			ctx.FreeDesc(&d143)
			...
			if idxInt.Loc == scm.LocImm {
				...
				d145 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r128}
			}
			ctx.FreeDesc(&idxInt)
			...
			if thisptr.Loc == scm.LocImm {
				...
				d146 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r129}
			}
			...
			if d146.Loc == scm.LocImm {
				...
				r130 := ctx.AllocReg()
				ctx.W.EmitMovRegReg(r130, d146.Reg)
				ctx.W.EmitShlRegImm8(r130, 56)
				ctx.W.EmitShrRegImm8(r130, 56)
				d147 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r130}
			}
			ctx.FreeDesc(&d146)
			...
			if d148.Loc == scm.LocImm {
				...
				r131 := ctx.AllocRegExcept(d148.Reg)
				ctx.W.EmitMovRegReg(r131, d148.Reg)
				ctx.W.EmitShrRegImm8(r131, 6)
				d149 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r131}
			}
			...
			if d149.Loc == scm.LocImm {
				ctx.W.EmitMovRegImm64(r132, uint64(d149.Imm.Int()) * 8)
			} else {
				ctx.W.EmitMovRegReg(r132, d149.Reg)
				ctx.W.EmitShlRegImm8(r132, 3)
			}
			if d107.Loc == scm.LocImm {
				ctx.W.EmitMovRegImm64(scm.RegR11, uint64(d107.Imm.Int()))
				ctx.W.EmitAddInt64(r132, scm.RegR11)
			} else {
				ctx.W.EmitAddInt64(r132, d107.Reg)
			}
			r133 := ctx.AllocRegExcept(r132)
			ctx.W.EmitMovRegMem(r133, r132, 0)
			ctx.FreeReg(r132)
			d150 := scm.JITValueDesc{Loc: scm.LocReg, Reg: r133}
			ctx.FreeDesc(&d149)
			var d151 scm.JITValueDesc
			if d148.Loc == scm.LocImm {
				d151 = scm.JITValueDesc{Loc: scm.LocImm, Type: scm.TagInt, Imm: scm.NewInt(d148.Imm.Int() % 64)}
			} else {
				r134 := ctx.AllocRegExcept(d148.Reg)
				ctx.W.EmitMovRegReg(r134, d148.Reg)
				ctx.W.EmitAndRegImm32(r134, 63)
				d151 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r134}
			}
			...
			if thisptr.Loc == scm.LocImm {
				...
				d153 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r135}
			}
			...
			r136 := ctx.AllocReg()
			ctx.EmitLoadFromStack(r136, 32)
			ctx.ProtectReg(r136)
			d154 := scm.JITValueDesc{Loc: scm.LocReg, Type: scm.JITTypeUnknown, Reg: r136}
			ctx.UnprotectReg(r136)
			...
			if thisptr.Loc == scm.LocImm {
				...
				d155 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r137}
			}
			...
			if d155.Loc == scm.LocImm {
				...
				r138 := ctx.AllocReg()
				ctx.W.EmitMovRegReg(r138, d155.Reg)
				ctx.W.EmitShlRegImm8(r138, 56)
				ctx.W.EmitShrRegImm8(r138, 56)
				d156 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r138}
			}
			ctx.FreeDesc(&d155)
			...
			if d148.Loc == scm.LocImm {
				...
				r139 := ctx.AllocRegExcept(d148.Reg)
				ctx.W.EmitMovRegReg(r139, d148.Reg)
				ctx.W.EmitAndRegImm32(r139, 63)
				d160 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r139}
			}
			...
			if thisptr.Loc == scm.LocImm {
				...
				d161 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r140}
			}
			...
			if d161.Loc == scm.LocImm {
				...
				r141 := ctx.AllocReg()
				ctx.W.EmitMovRegReg(r141, d161.Reg)
				ctx.W.EmitShlRegImm8(r141, 56)
				ctx.W.EmitShrRegImm8(r141, 56)
				d162 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r141}
			}
			ctx.FreeDesc(&d161)
			...
			if d163.Loc == scm.LocImm {
				...
				ctx.W.EmitSetcc(r142, scm.CcA)
				d164 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagBool, Reg: r142}
			}
			ctx.FreeDesc(&d163)
			...
			if d148.Loc == scm.LocImm {
				...
				d165 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r143}
			}
			...
			if d166.Loc == scm.LocImm {
				ctx.W.EmitMovRegImm64(r144, uint64(d166.Imm.Int()) * 8)
			} else {
				ctx.W.EmitMovRegReg(r144, d166.Reg)
			...
			if d107.Loc == scm.LocImm {
				...
				ctx.W.EmitAddInt64(r144, d107.Reg)
			}
			r145 := ctx.AllocRegExcept(r144)
			ctx.W.EmitMovRegMem(r145, r144, 0)
			ctx.FreeReg(r144)
			d167 := scm.JITValueDesc{Loc: scm.LocReg, Reg: r145}
			ctx.FreeDesc(&d166)
			...
			if d152.Loc == scm.LocImm && d171.Loc == scm.LocImm {
				...
				if d171.Imm.Int() >= -2147483648 && d171.Imm.Int() <= 2147483647 {
					ctx.W.EmitOrRegImm32(r147, int32(d171.Imm.Int()))
				} else {
					scratch := ctx.AllocReg()
					ctx.W.EmitMovRegImm64(scratch, uint64(d171.Imm.Int()))
					ctx.W.EmitOrInt64(r147, scratch)
					ctx.FreeReg(scratch)
				}
				d172 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r147}
			} else {
				r148 := ctx.AllocRegExcept(d152.Reg)
				ctx.W.EmitMovRegReg(r148, d152.Reg)
				ctx.W.EmitOrInt64(r148, d171.Reg)
				d172 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r148}
			}
			...
			ctx.W.ResolveFixups()
			d173 := scm.JITValueDesc{Loc: scm.LocReg, Reg: r127}
			ctx.FreeDesc(&idxInt)
			...
			if d173.Loc == scm.LocImm {
				...
				r149 := ctx.AllocReg()
				ctx.W.EmitMovRegReg(r149, d173.Reg)
			...
			if thisptr.Loc == scm.LocImm {
				...
				d175 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r150}
			}
			...
			if d176.Loc == scm.LocImm {
				...
				r151 := ctx.AllocReg()
				ctx.W.EmitMovRegReg(r151, d176.Reg)
				d177 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r151}
			}
			ctx.FreeDesc(&d176)
			...
			if d69.Loc == scm.LocImm {
				...
				d178 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r152}
			}
			...
			if d69.Loc == scm.LocImm && d177.Loc == scm.LocImm {
				...
				ctx.W.EmitAddInt64(r154, d177.Reg)
				d179 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r154}
			}
			...
			if d179.Loc == scm.LocImm {
				...
				r155 := ctx.AllocReg()
				ctx.W.EmitMovRegReg(r155, d179.Reg)
				d180 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagInt, Reg: r155}
			}
			ctx.FreeDesc(&d179)
			r156 := ctx.AllocReg()
			r157 := ctx.AllocReg()
			if d139.Loc == scm.LocImm {
				ctx.W.EmitMovRegImm64(r156, uint64(d139.Imm.Int()))
			} else if d139.Loc == scm.LocRegPair {
				ctx.W.EmitMovRegReg(r156, d139.Reg)
			} else {
				ctx.W.EmitMovRegReg(r156, d139.Reg)
			}
			if d178.Loc == scm.LocImm {
				if d178.Imm.Int() != 0 {
					if d178.Imm.Int() >= -2147483648 && d178.Imm.Int() <= 2147483647 {
						ctx.W.EmitAddRegImm32(r156, int32(d178.Imm.Int()))
					} else {
						scratch := ctx.AllocReg()
						ctx.W.EmitMovRegImm64(scratch, uint64(d178.Imm.Int()))
						ctx.W.EmitAddInt64(r156, scratch)
						ctx.FreeReg(scratch)
					}
				}
			} else {
				ctx.W.EmitAddInt64(r156, d178.Reg)
			}
			if d180.Loc == scm.LocImm {
				ctx.W.EmitMovRegImm64(r157, uint64(d180.Imm.Int()))
			} else {
				ctx.W.EmitMovRegReg(r157, d180.Reg)
			}
			if d178.Loc == scm.LocImm {
				if d178.Imm.Int() >= -2147483648 && d178.Imm.Int() <= 2147483647 {
					ctx.W.EmitSubRegImm32(r157, int32(d178.Imm.Int()))
				} else {
					scratch := ctx.AllocReg()
					ctx.W.EmitMovRegImm64(scratch, uint64(d178.Imm.Int()))
					ctx.W.EmitSubInt64(r157, scratch)
					ctx.FreeReg(scratch)
				}
			} else {
				ctx.W.EmitSubInt64(r157, d178.Reg)
			}
			d181 := scm.JITValueDesc{Loc: scm.LocRegPair, Reg: r156, Reg2: r157}
			ctx.FreeDesc(&d178)
			ctx.FreeDesc(&d180)
			...
			if thisptr.Loc == scm.LocImm {
				...
				d183 = scm.JITValueDesc{Loc: scm.LocReg, Reg: r158}
			}
			...
			if d69.Loc == scm.LocImm && d183.Loc == scm.LocImm {
				...
				d184 = scm.JITValueDesc{Loc: scm.LocReg, Type: scm.TagBool, Reg: r161}
			}
			ctx.FreeDesc(&d69)
			ctx.FreeDesc(&d183)
