...
func (t *storageShard) rebuild(all bool) *storageShard {
	...
	if all || maxInsertIndex > 0 || deletions.Count() > 0 {
		...
		for col, c := range columnSnapshot {
			...
			// StorageComputeProxy: port the proxy with its validMask instead of
			// evaluating the computor (which may scan other tables and cause
			// infinite recursion during concurrent rebuild/repartition).
				...
				if sortPerm != nil {
					for _, globalID := range sortPerm {
						if globalID < t.main_count {
							if oldProxy.compressed || oldProxy.validMask.Get(uint(globalID)) {
								oldProxy.mu.RLock()
								val, inDelta := oldProxy.delta[globalID]
								oldProxy.mu.RUnlock()
								if !inDelta && oldProxy.main != nil {
									val = oldProxy.main.GetValue(globalID)
								}
								newProxy.delta[newIdx] = val
								newProxy.validMask.Set(uint(newIdx), true)
							}
						} else {
							// delta row — read raw value
							val := t.getDelta(int(globalID-t.main_count), col)
							newProxy.delta[newIdx] = val
							newProxy.validMask.Set(uint(newIdx), true)
						}
						newIdx++
					...
					for idx := 0; idx < maxInsertIndex; idx++ {
						...
						val := t.getDelta(idx, col)
						newProxy.delta[newIdx] = val
						newProxy.validMask.Set(uint(newIdx), true)
						newIdx++
					}
				}
				newProxy.count = newIdx
				result.columns[col] = newProxy
				result.main_count = newIdx
				b.WriteString(col)
				b.WriteString(" compute-proxy")
				continue // skip normal column rebuild path
			}
