...
func (t *storageShard) rowValueByRecidLocked(recid uint32, col string) scm.Scmer {
	if recid < t.main_count {
		cs := t.getColumnStorageOrPanic(col, true, nil)
		...
		if len(a) > 0 {
			...
			func() {
				...
				for _, colDesc := range t.t.Columns {
					...
					k := colDesc.Name
					cs := t.getColumnStorageOrPanic(k, true, nil)
			...
			if withTrigger && triggerOldRow != nil {
				if alreadyLocked {
					t.runWithWriteLockReleased(currentTx, func() {
						t.t.ExecuteTriggers(AfterUpdate, triggerOldRow, triggerNewRow, currentTx)
					})
				} else {
					t.t.ExecuteTriggers(AfterUpdate, triggerOldRow, triggerNewRow, currentTx)
				}
			...
			if withTrigger && len(t.t.Triggers) > 0 {
				if alreadyLocked {
					...
					for i, col := range t.t.Columns {
						cs := t.getColumnStorageOrPanic(col.Name, true, nil)
						if idx < t.main_count {
				...
				if alreadyLocked {
					t.runWithWriteLockReleased(currentTx, func() {
						beforeDeleteOk = t.t.ExecuteBeforeDeleteTriggers(triggerDeletedRow, currentTx)
					})
				} else {
					beforeDeleteOk = t.t.ExecuteBeforeDeleteTriggers(triggerDeletedRow, currentTx)
				}
			...
			if withTrigger && triggerDeletedRow != nil {
				if batch != nil {
					...
					t.runWithWriteLockReleased(currentTx, func() {
						t.t.ExecuteTriggers(AfterDelete, triggerDeletedRow, nil, currentTx)
					})
				} else {
					t.t.ExecuteTriggers(AfterDelete, triggerDeletedRow, nil, currentTx)
				}
...
func (m *ShardMapReducer) MapOne(id uint32) scm.Scmer {
	if m.directRead {
		...
		return m.mapProgram.Call(m.args[1:])
	}
...
func (t *storageShard) initMapReducer(mr *ShardMapReducer, cols []string, mapReduceFn scm.Scmer, alreadyLocked bool, stride int, batchdata []scm.Scmer, currentTx *TxContext) {
	*mr = ShardMapReducer{
		shard:            t,
		currentTx:        currentTx,
		acidMode:         currentTx != nil && currentTx.Mode == TxACID,
	...
	for i, col := range cols {
		...
		if len(col) > 12 && col[:12] == "$orc_stored:" {
			proxy, ok := t.getColumnStorageOrPanic(col[12:], alreadyLocked, currentTx).(*StorageComputeProxy)
			if !ok || !proxy.isOrdered {
				panic("$orc_stored requires an ordered computed column")
			}
			getter := func(id uint32, _ uint32) scm.Scmer {
				return proxy.storedORCValue(id)
			}
			mr.mainGetters[i] = getter
			mr.deltaGetters[i] = getter
			continue
		...
		if col == "$update" {
			...
			if hasDeleteTriggers {
				mr.deleteBatch = t.t.BeginTriggerBatch(AfterDelete, true, currentTx)
			}
			...
			cacheColName := col[12:]
			cs := t.getColumnStorageOrPanic(cacheColName, alreadyLocked, currentTx)
			...
			mr.mainCols[i] = t.getColumnStorageOrPanic(col, alreadyLocked, currentTx)
		}
	}
...
func (t *storageShard) OpenMapReducer(cols []string, mapReduceFn scm.Scmer, alreadyLocked bool, stride int, batchdata []scm.Scmer, currentTx *TxContext) *ShardMapReducer {
	mr := new(ShardMapReducer)
	t.initMapReducer(mr, cols, mapReduceFn, alreadyLocked, stride, batchdata, currentTx)
	return mr
}

// OpenMapper creates a retained mapper for materialization and predicates that
// do not participate in a reduction.
func (t *storageShard) OpenMapper(cols []string, mapFn scm.Scmer, alreadyLocked bool, currentTx *TxContext) *ShardMapReducer {
	mr := new(ShardMapReducer)
	t.initMapReducer(mr, cols, scm.NewNil(), alreadyLocked, 0, nil, currentTx)
...
// Stream applies the fused map-reducer over a batch of record IDs. The recid list is
		...
		func() {
			...
			for i, getter := range m.mainGetters {
				if hasBulkValues && m.mainBulkReaders[i] != nil {
					m.args[i+1] = m.mainBulkValues[rowIndex*bulkWidth+i]
				} else {
					m.args[i+1] = getter(effectiveID, 0)
				}
			}
...
func (m *ShardMapReducer) processMainBlockBatch(acc scm.Scmer, recids []uint32, batchids []uint32) scm.Scmer {
	...
	for rowidx, id := range recids {
		func() {
			...
			for i, getter := range m.mainGetters {
				if hasBulkValues && m.mainBulkReaders[i] != nil {
					m.args[i+1] = m.mainBulkValues[rowidx*bulkWidth+i]
				} else {
					m.args[i+1] = getter(effectiveID, batchid)
				}
			}
			if needsPerRowLock {
				m.shard.mu.Unlock()
...
func (m *ShardMapReducer) FlushSideEffects() {
	...
	for proxy, recids := range m.invalidateBatch {
		invalidateComputedRows(proxy, recids, m.currentTx)
	}
	...
	for proxy, batch := range m.incrementBatch {
		for recid, delta := range batch {
			proxy.IncrementalUpdateTx(m.currentTx, recid, delta)
		}
	}
...
func (t *storageShard) Insert(columns []string, values [][]scm.Scmer, alreadyLocked bool, onFirstInsertId func(int64), isIgnore bool, currentTx *TxContext) uint32 {
	...
	if t.t.hasTableLock() {
		t.t.waitTableLock(ss, querySeqFromTx(currentTx), true)
	}
	...
	if len(beforeInsertTriggers) > 0 {
		...
		for _, row := range values {
			rowColumns, preparedRow, ok := t.t.executeBeforeInsertTriggerRow(columns, row, isIgnore, currentTx)
			if !ok {
				continue
			}
...
func (t *storageShard) lockForMutation(currentTx *TxContext) {
	ss := SessionStateFromTx(currentTx)
	for {
		if t.t.hasTableLock() {
			t.t.waitTableLock(ss, querySeqFromTx(currentTx), true)
...
func (t *storageShard) insertPreparedLocked(columns []string, values [][]scm.Scmer, onFirstInsertId func(int64), fireTriggers bool, propagateMaintenance bool, currentTx *TxContext) uint32 {
	...
	if fireTriggers && len(t.t.Triggers) > 0 {
		func() {
			t.mu.Unlock()
			defer t.lockForMutation(currentTx)
			for _, row := range triggerInsertRows {
				t.t.ExecuteTriggers(AfterInsert, nil, row, currentTx)
			}
		}()
	}
	return firstNewRecid
...
var uniqueLookupScratchPool = sync.Pool{
	New: func() any { return new(uniqueLookupScratch) },
}

func acquireUniqueLookupScratch() *uniqueLookupScratch {
	return uniqueLookupScratchPool.Get().(*uniqueLookupScratch)
}

func releaseUniqueLookupScratch(scratch *uniqueLookupScratch, columns int) {
	if columns > uniqueLookupInlineColumns {
	...
	if all || maxInsertIndex > 0 || deletions.Count() > 0 {
		...
		for _, idx := range result.Indexes {
			if idx.Savings >= 2.0 && !idx.baseState.active {
				...
				if allFound {
					getters := idx.buildGetters(nil, nil)
