...
((lambda () (begin
	...
	(print "DIRS=" (sqlx "SELECT d.ID, COALESCE((SELECT (CASE WHEN FALSE THEN TRUE ELSE NOT ((b.fopuser <> 'alice') AND (NOT (EXISTS (SELECT TRUE FROM era_share s WHERE s.mailbox = b.ID AND s.fopuser = 'alice' LIMIT 1)))) END) FROM era_box b WHERE b.ID = d.mailbox LIMIT 1), FALSE) AS ok FROM era_dir d ORDER BY d.ID"))
