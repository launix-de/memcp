...
((lambda () (begin
	...
	(print "EXISTS=" (sqlx "SELECT b.ID, EXISTS (SELECT TRUE FROM era_share s WHERE s.mailbox = b.ID AND s.fopuser = 'alice' LIMIT 1) AS ex FROM era_box b ORDER BY b.ID"))
	(print "NOT_EXISTS=" (sqlx "SELECT b.ID, NOT EXISTS (SELECT TRUE FROM era_share s WHERE s.mailbox = b.ID AND s.fopuser = 'alice' LIMIT 1) AS nex FROM era_box b ORDER BY b.ID"))
	(print "OK=" (sqlx "SELECT b.ID, NOT ((b.fopuser <> 'alice') AND (NOT (EXISTS (SELECT TRUE FROM era_share s WHERE s.mailbox = b.ID AND s.fopuser = 'alice' LIMIT 1)))) AS ok FROM era_box b ORDER BY b.ID"))
