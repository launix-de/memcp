		...
		Type: &TypeDescriptor{Kind: "func", Description: "applies OFFSET/LIMIT and a serial reducer to complete values emitted by a nested streaming producer without collecting an intermediate relation",
			...
			JITEmit: func(ctx *JITContext, sourceArgs []Scmer, args []JITValueDesc, result JITValueDesc) JITValueDesc {
				...
				bbs[1].RenderPS = func(ps PhiState) JITValueDesc {
					...
					ctx.ReclaimUntrackedRegs()
					_ = jitEmitGoVariadicCallFromDescs(ctx, declarations["stream_window_reduce"].Fn, args, result)
				...
				bbs[2].RenderPS = func(ps PhiState) JITValueDesc {
					...
					var d30 JITValueDesc
					if d29.Loc == LocImm {
						d30 = JITValueDesc{Loc: LocImm, Type: tagBool, Imm: NewBool(d29.Imm.Int() < -1)}
					} else {
						r3 := ctx.AllocReg()
						ctx.EmitCmpRegImm32(d29.Reg, -1)
				...
				bbs[4].RenderPS = func(ps PhiState) JITValueDesc {
					...
					var d54 JITValueDesc
					if d53.Loc == LocImm {
						d54 = JITValueDesc{Loc: LocImm, Type: tagBool, Imm: NewBool(d53.Imm.Int() == 0)}
					} else {
						r4 := ctx.AllocReg()
						ctx.EmitCmpRegImm32(d53.Reg, 0)
						ctx.EmitSetcc(r4, CondEqual)
						d54 = JITValueDesc{Loc: LocReg, Type: tagBool, Reg: r4}
						ctx.BindReg(r4, &d54)
					}
					ctx.FreeDesc(&d53)
					d55 = d54
					...
					ps58.OverlayValues = make([]JITValueDesc, 56)
					...
					ps59.OverlayValues = make([]JITValueDesc, 56)
				...
				bbs[6].RenderPS = func(ps PhiState) JITValueDesc {
					...
					ctx.EnsureDesc(&d82)
					if d82.Loc == LocImm {
						tmpPair := JITValueDesc{Loc: LocRegPair, Type: d82.Type, Reg: ctx.AllocReg(), Reg2: ctx.AllocReg()}
						if d82.Imm.GetTag() == tagBool {
							ctx.EmitMakeBool(tmpPair, d82)
						} else if d82.Imm.GetTag() == tagInt {
							ctx.EmitMakeInt(tmpPair, d82)
						} else if d82.Imm.GetTag() == tagFloat {
							ctx.EmitMakeFloat(tmpPair, d82)
						} else if d82.Imm.GetTag() == tagNil {
							ctx.EmitMakeNil(tmpPair)
						} else {
						...
						tmpPair := JITValueDesc{Loc: LocRegPair, Type: d82.Type, Reg: ctx.AllocRegExcept(d82.Reg), Reg2: ctx.AllocRegExcept(d82.Reg)}
						switch d82.Type {
						case tagBool:
							ctx.EmitMakeBool(tmpPair, d82)
						case tagInt:
					...
					if d82.Loc != LocRegPair && d82.Loc != LocStackPair {
						panic("jit: generic call arg expects 2-word value (PrepareSerialProc arg0)")
					}
					ctx.SyncDesc(&d82)
					d83 = ctx.EmitGoCallScalar(GoFuncAddr((func(arg0 Scmer) *SerialProc { value := PrepareSerialProc(arg0); return &value })), []JITValueDesc{d82}, 1)
					ctx.BindReg(d83.Reg, &d83)
					ctx.FreeDesc(&d82)
					ctx.SyncDesc(&d83)
					d84 = ctx.EmitGoCallScalar(GoFuncAddr(func() *SerialProc { return new(SerialProc) }), nil, 1)
					...
					if d85.Loc == LocImm {
						...
						if d85.Imm.GetTag() == tagBool {
							ctx.EmitMakeBool(tmpPair, d85)
						} else if d85.Imm.GetTag() == tagInt {
							ctx.EmitMakeInt(tmpPair, d85)
						} else if d85.Imm.GetTag() == tagFloat {
							ctx.EmitMakeFloat(tmpPair, d85)
						} else if d85.Imm.GetTag() == tagNil {
							ctx.EmitMakeNil(tmpPair)
						} else {
							ptrWord, auxWord := d85.Imm.RawWords()
							ctx.EmitMovRegImm64(tmpPair.Reg, uint64(ptrWord))
							ctx.EmitMovRegImm64(tmpPair.Reg2, auxWord)
						}
						d85 = tmpPair
					...
					if d85.Loc != LocRegPair && d85.Loc != LocStackPair {
						panic("jit: generic call arg expects 2-word value (PrepareSerialProc arg0)")
					}
					ctx.SyncDesc(&d85)
					d86 = ctx.EmitGoCallScalar(GoFuncAddr((func(arg0 Scmer) *SerialProc { value := PrepareSerialProc(arg0); return &value })), []JITValueDesc{d85}, 1)
					ctx.BindReg(d86.Reg, &d86)
					ctx.FreeDesc(&d85)
					ctx.EnsureDesc(&d86)
					ctx.EmitGoCallVoid(GoFuncAddr(func(dst, src *SerialProc) { *dst = *src }), []JITValueDesc{d84, d86})
					stackArray87 = ctx.AllocStack(int32(32))
					_ = stackArray87
					stackArray88 = ctx.AllocStack(int32(16))
					_ = stackArray88
					...
					ctx.ReclaimUntrackedRegs()
					ctx.ReclaimUntrackedRegs()
					d91 = ctx.EmitGoCallScalar(GoFuncAddr(func() *func(...Scmer) Scmer { return new(func(...Scmer) Scmer) }), nil, 1)
					ctx.BindReg(d91.Reg, &d91)
					ctx.ReclaimUntrackedRegs()
					ctx.EmitGoCallVoid(GoFuncAddr(func(dst *func(...Scmer) Scmer, value func(...Scmer) Scmer) { *dst = value }), []JITValueDesc{d91})
					ctx.ReclaimUntrackedRegs()
					r5 := ctx.AllocReg()
					r6 := ctx.AllocRegExcept(r5)
					ctx.EmitMovRegImm64(r5, 0)
					ctx.EmitMovRegImm64(r6, 0)
					d92 = JITValueDesc{Loc: LocRegPair, Type: JITTypeUnknown, Reg: r5, Reg2: r6}
					ctx.BindReg(r5, &d92)
					ctx.BindReg(r6, &d92)
					ctx.ReclaimUntrackedRegs()
					d93 = args[0]
					...
					d96.ID = 0
					ctx.ReclaimUntrackedRegs()
					d97 = JITValueDesc{Loc: LocImm, Type: tagInt, Imm: NewInt(8)}
					...
					var d101 JITValueDesc
					if d100.Loc == LocImm {
						d101 = JITValueDesc{Loc: LocImm, Type: tagInt, Imm: NewInt(int64(uint64(d100.Imm.Int()) << 8))}
					} else {
						ctx.EmitShlRegImm8(d100.Reg, 8)
						d101 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: d100.Reg}
						ctx.BindReg(d100.Reg, &d101)
					}
					if d101.Loc == LocReg && d100.Loc == LocReg && d101.Reg == d100.Reg {
					...
					var d102 JITValueDesc
					if d99.Loc == LocImm {
						d102 = JITValueDesc{Loc: LocImm, Type: tagInt, Imm: NewInt(d99.Imm.Int() & 255)}
					} else {
						ctx.EmitAndRegImm32(d99.Reg, int32(255))
						d102 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: d99.Reg}
						ctx.BindReg(d99.Reg, &d102)
					}
					if d102.Loc == LocImm {
						d102 = JITValueDesc{Loc: LocImm, Type: d102.Type, Imm: NewInt(int64(uint64(d102.Imm.Int()) & 0xff))}
					} else {
						ctx.EmitShlRegImm8(d102.Reg, 56)
						ctx.EmitShrRegImm8(d102.Reg, 56)
					}
					if d102.Loc == LocReg && d99.Loc == LocReg && d102.Reg == d99.Reg {
					...
					var d103 JITValueDesc
					if d102.Loc == LocImm {
						d103 = JITValueDesc{Loc: LocImm, Type: tagInt, Imm: NewInt(int64(uint64(uint8(d102.Imm.Int()))))}
					} else {
						r7 := ctx.AllocReg()
						ctx.EmitMovRegReg(r7, d102.Reg)
						ctx.EmitShlRegImm8(r7, 56)
						ctx.EmitShrRegImm8(r7, 56)
						d103 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: r7}
						ctx.BindReg(r7, &d103)
					}
					ctx.FreeDesc(&d102)
					ctx.ReclaimUntrackedRegs()
					...
					ctx.EnsureDesc(&d103)
					var d104 JITValueDesc
					if d101.Loc == LocImm && d103.Loc == LocImm {
						d104 = JITValueDesc{Loc: LocImm, Type: tagInt, Imm: NewInt(d101.Imm.Int() | d103.Imm.Int())}
					} else if d101.Loc == LocImm && d101.Imm.Int() == 0 {
						d104 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: d103.Reg}
						ctx.BindReg(d103.Reg, &d104)
					} else if d103.Loc == LocImm && d103.Imm.Int() == 0 {
						d104 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: d101.Reg}
						ctx.BindReg(d101.Reg, &d104)
					} else if d101.Loc == LocImm {
						scratch := ctx.AllocRegExcept(d103.Reg)
						ctx.EmitMovRegImm64(scratch, uint64(d101.Imm.Int()))
						ctx.EmitOrInt64(scratch, d103.Reg)
						d104 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: scratch}
						ctx.BindReg(scratch, &d104)
					} else if d103.Loc == LocImm {
						if d103.Imm.Int() >= -2147483648 && d103.Imm.Int() <= 2147483647 {
							ctx.EmitOrRegImm32(d101.Reg, int32(d103.Imm.Int()))
						} else {
							ctx.EmitMovRegImm64(RegR11, uint64(d103.Imm.Int()))
							ctx.EmitOrInt64(d101.Reg, RegR11)
						}
						d104 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: d101.Reg}
						ctx.BindReg(d101.Reg, &d104)
					} else {
						ctx.EmitOrInt64(d101.Reg, d103.Reg)
						d104 = JITValueDesc{Loc: LocReg, Type: tagInt, Reg: d101.Reg}
						ctx.BindReg(d101.Reg, &d104)
					}
					if d104.Loc == LocReg && d101.Loc == LocReg && d104.Reg == d101.Reg {
					...
					ctx.EnsureDesc(&d91)
					ctx.EnsureDesc(&d91)
					ctx.EmitMovToReg(d93.Reg, d91)
					ctx.ReclaimUntrackedRegs()
					ctx.EnsureDesc(&d104)
					ctx.EnsureDesc(&d104)
					ctx.EmitMovToReg(d96.Reg2, d104)
					ctx.FreeDesc(&d104)
					ctx.ReclaimUntrackedRegs()
					d105 = d92
					_ = d105
					...
					ctx.EnsureDesc(&d105)
					ctx.EnsureDesc(&d105)
					ctx.EmitStoreScmerToStack(d105, int32(stackArray88)+int32(0))
					d106 = JITValueDesc{Loc: LocVirtualSlice, Type: tagSlice, KnownSliceLen: int32(1), KnownSliceCap: int32(1), SliceSizeKnown: true}
					_ = d106
					ctx.EnsureDesc(&d84)
					ctx.EnsureDesc(&d84)
					if d84.Loc == LocRegPair || d84.Loc == LocStackPair || d84.Loc == LocRegTriple || d84.Loc == LocStackTriple {
						panic("jit: generic call arg expects 1-word value")
					}
					ctx.EnsureDesc(&d106)
					ctx.EnsureDesc(&d106)
					ctx.EnsureDesc(&d106)
					if d106.Loc != LocRegTriple && d106.Loc != LocStackTriple {
						panic("jit: generic call arg expects 3-word Go slice ((*SerialProc).Call arg1)")
					}
					ctx.SyncDesc(&d84)
					ctx.SyncDesc(&d106)
					d107 = ctx.EmitGoCallScalar(GoFuncAddr((*SerialProc).Call), []JITValueDesc{d84, d106}, 2)
