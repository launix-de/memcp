	...
	(if (not (equal? (qpir-kind node) (quote qpir-dep-join)))
		...
		(begin
			...
			(if (equal? (count correlated-free) 0)
				...
				(qpir-join (if (qpu-right-may-be-empty? right) (quote left) (quote inner))
					(qpir-dep-join-predicate node) left right
					(qpir-dep-join-rhs-alias node))
				node)))))
...
(define qpu-right-may-be-empty? (lambda (node)
	(match (qpir-kind node)
		(quote qpir-leaf) false
		(quote qpir-select) true
		(quote qpir-map) (qpu-right-may-be-empty? (qpir-map-child node))
...
(define qpu-merge-unique-atoms (lambda (a b)
	(reduce (coalesceNil b '()) (lambda (acc x)
		(if (or (nil? x) (has? acc x)) acc (merge acc (list x))))
		(coalesceNil a '()))))
...
(define qpu-unnest-right (lambda (node outer-aliases outer-ref-exprs repr)
	(match (qpir-kind node)
		...
		(quote qpir-groupby) (begin
			...
			(define sub-outer-refs-raw (qpu-substitute-exprs outer-ref-exprs repr))
			/* Only inner-side replacements may become GROUP BY keys. Any ref
			that still points at an outer alias after cclass substitution is a
			residual correlation and must stay in the extracted join predicate;
			pushing it into the helper would compile to a const[nil] key. */
			(define sub-outer-refs (filter sub-outer-refs-raw (lambda (k)
				(not (qpu-expr-references-aliases? k outer-aliases)))))
		...
		(quote qpir-join) (begin
			...
			(define left-outer-refs (qpu-filter-outer-refs-for-node
				outer-ref-exprs left-child))
			(define right-outer-refs (qpu-filter-outer-refs-for-node
				outer-ref-exprs right-child))
			(define left-result (qpu-unnest-right left-child
				outer-aliases left-outer-refs repr))
			(define right-result (qpu-unnest-right right-child
				outer-aliases right-outer-refs repr))
...
/* qpu-filter-outer-refs-for-node — keep only the outer refs actually free
in this child subtree. Parent dep-join domains must not be pushed into
unrelated nested helpers. */
(define qpu-filter-outer-refs-for-node (lambda (outer-ref-exprs node) (begin
	(define free (qpir-free-vars node))
	(filter (coalesceNil outer-ref-exprs '()) (lambda (expr) (match expr
		'((symbol get_column) tv _ col _) (has? free (list tv col))
		'((quote get_column)  tv _ col _) (has? free (list tv col))
		false))))))
	...
	(if (not (equal? (qpir-kind dj) (quote qpir-dep-join)))
		...
		(begin
			...
			(if (equal? (count outer-ref-exprs) 0)
				(qpir-join (if (qpu-right-may-be-empty? right) (quote left) (quote inner))
					(qpir-dep-join-predicate dj) left right
...
(define qpu-unnest-tree-topdown-helper (lambda (node ancestor-aliases)
	(match (qpir-kind node)
		(quote qpir-dep-join) (begin
			...
			(define extended (qpu-merge-unique-atoms ancestor-aliases own-outer))
