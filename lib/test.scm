...
(begin /* own enclosure */
	...
	(assert (equal? (qop (ir_root simple_ir)) 'project) true "untangle_query builds an operator root")
	(assert (equal? (qop (car (ir_sources simple_ir))) 'scan) true "untangle_query keeps physical table source as scan node")
	(assert (equal? (qassoc_get (qattrs (car (ir_sources simple_ir))) 'table nil) "t") true "scan node keeps physical table name")
	...
	(assert (strlike (try (lambda () (begin
			(untangle_query_term
				(list "memcp-tests" '()
				...
				nil)
			"unexpected-ok"))
		(lambda (e) e)) "NEUMANN_INVARIANT_BROKEN:%") true "untangle_query rejects table-backed scalar expression fallback")
	...
	(assert (strlike (try (lambda () (begin
			(untangle_query_term
				...
				nil)
			"unexpected-ok"))
		(lambda (e) e)) "NEUMANN_INVARIANT_BROKEN:%") true "untangle_query rejects correlated scalar expression fallback")
