...
(begin /* own enclosure */
	...
	(assert (equal? (match order1 '(((symbol get_column) "t" _ col _) dir) (list col) '()) '("id")) true "match order key extraction")
	...
	(assert (match expr_star '((symbol get_column) "t" ignorecase "*" _) "ok" "no") "ok" "match tbl.* with case-insensitive flag")
