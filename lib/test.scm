...
(begin /* own enclosure */
	...
	(define alias_map_flat (set_assoc (set_assoc '() "t" "memcp-tests.ticket") "ticket" "memcp-tests.ticket"))
	...
	(assert (equal? (canonicalize_expr canon_expr_t alias_map_flat)
		(canonicalize_expr canon_expr_t alias_map_fd)) true "canonicalize_expr flat assoc vs FastDict")
	(assert (equal? (serialize_canonical_expr (canonicalize_expr canon_expr_t alias_map_flat))
		(serialize_canonical_expr (canonicalize_expr canon_expr_t alias_map_fd))) true "serialize_canonical_expr flat assoc vs FastDict")
	...
	(assert (equal? (serialize_canonical_expr (canonicalize_expr case_expr_canon case_alias_map))
	...
	(assert (equal? (scalar_subselect_inline_reason nil true false false nil '() '(dummy-table) true)
		'legacy-fallback-non-aggregate) true "scalar_subselect_inline_reason names non-aggregate legacy fallback")
	...
	(assert (equal? (once_limit_scan_contract_partition_cols once_contract_implicit) 3) true "make_once_limit_scan_contract preserves partition column count")
	(assert (equal? (once_limit_scan_contract_once_limit once_contract_implicit) 2) true "make_once_limit_scan_contract defaults once_limit to 2")
	(assert (nil? (once_limit_scan_contract_promise_name once_contract_implicit)) true "make_once_limit_scan_contract leaves promise unset without runtime context")
	(define once_contract_explicit_limit (make_once_limit_scan_contract 1 0 0 nil nil nil nil nil))
	(assert (equal? (once_limit_scan_contract_limit once_contract_explicit_limit) 1) true "make_once_limit_scan_contract preserves explicit LIMIT 1")
	(assert (equal? (once_limit_scan_contract_once_limit once_contract_explicit_limit) 1) true "make_once_limit_scan_contract maps explicit LIMIT 1 to once_limit 1")
	(define once_contract_runtime (make_once_limit_scan_contract 2 0 1 2 "sq" true true "helper_tbl"))
	(assert (equal? (once_limit_scan_contract_limit once_contract_runtime) 2) true "make_once_limit_scan_contract keeps tagged helper LIMIT for runtime")
	(assert (equal? (once_limit_scan_contract_partition_cols once_contract_runtime) 1) true "make_once_limit_scan_contract keeps tagged helper partition width")
	(assert (equal? (once_limit_scan_contract_once_limit once_contract_runtime) 2) true "make_once_limit_scan_contract keeps tagged helper once_limit")
	(assert (not (nil? (once_limit_scan_contract_promise_name once_contract_runtime))) true "make_once_limit_scan_contract derives runtime promise names for once_limit 2")
	(define logical_scalar_order (list (list (list 'get_column "sq" false "dummy" false) '<)))
	(define logical_scalar_stage (make_scalar_partition_stage logical_scalar_order nil nil 3 '("sq") '()))
	(assert (equal? (stage_limit_partition_cols logical_scalar_stage) 3) true "make_scalar_partition_stage keeps domain partition width")
	(assert (equal? (stage_limit_val logical_scalar_stage) 2) true "make_scalar_partition_stage defaults scalar partition scans to LIMIT 2")
	(assert (equal? (stage_offset_val logical_scalar_stage) 0) true "make_scalar_partition_stage defaults scalar partition scans to OFFSET 0")
	(assert (equal? (stage_once_limit logical_scalar_stage) 2) true "make_scalar_partition_stage records scalar cardinality via once_limit")
	(define logical_scalar_stage_limit1 (make_scalar_partition_stage logical_scalar_order 1 4 1 '("sq") '()))
	(assert (equal? (stage_limit_val logical_scalar_stage_limit1) 1) true "make_scalar_partition_stage preserves explicit LIMIT 1")
	(assert (equal? (stage_offset_val logical_scalar_stage_limit1) 4) true "make_scalar_partition_stage preserves explicit OFFSET")
	(assert (equal? (stage_once_limit logical_scalar_stage_limit1) 1) true "make_scalar_partition_stage maps explicit LIMIT 1 to once_limit 1")
