	...
	(match logical_term
		'(select_core_term schema tables fields condition groups schemas replace_find_column init) (begin
			...
			(define explain_rows (list
				...
				(list "stage" "reorder" "kind" "tables" "value" (serialize (nth jr_result 1)))
...
(define explain_queryplan_reorder (lambda (query) (begin
	(if (or (nil? query) (< (count query) 1) (not (string? (nth query 0))))
		...
		nil)
	(define uq_result (apply untangle_query (merge query (list nil))))
	(if (not (equal? (count uq_result) 8))
		(error (concat "EXPLAIN_REORDER_UQ_RESULT " (serialize uq_result)))
		nil)
	(define uq_7tuple (list (nth uq_result 0) (nth uq_result 1) (nth uq_result 2) (nth uq_result 3) (nth uq_result 4) (nth uq_result 5) (nth uq_result 6)))
	(define jr_result (apply join_reorder uq_7tuple))
	...
	(explain_emit_rows (merge
		(table_rows_for_stage "untangle" (nth uq_result 1))
		(table_rows_for_stage "reorder" (nth jr_result 1))
	))
)))
