	...
	(define sql_alter_table (parser '(
		...
		(define alters (+ (or
			...
			(parser '((atom "MODIFY" true) (?(atom "COLUMN" true))
				...
				(define typeparams sql_column_attributes)
			) (lambda (id) (cons '!begin (merge
				(list '((quote altercolumn) '((quote table) schema id) col "type" type)
					'((quote altercolumn) '((quote table) schema id) col "dimensions" dimensions))
				(if (get_assoc typeparams "primary")
					(list '((quote createkey) '((quote table) schema id) "PRIMARY" true (list 'list col)))
					'())
				(list 1)))))
