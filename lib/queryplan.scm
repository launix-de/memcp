...
(define planner_debug_get_scalar_events (lambda ()
	(coalesceNil (planner_debug_scalar_events "rows") '())))
(define explain_scalar_debug_rows (lambda () (begin
	(define scalar_events (planner_debug_get_scalar_events))
	(if (equal? scalar_events '()) '()
		(list (list "stage" "planner" "kind" "scalar-events" "value" (serialize scalar_events)))))))
						...
						(match tables
							(cons '(tblvar schema tbl isOuter joinexpr) tables) (begin /* outer scan */
								...
								(match (split_scan_condition isOuter (replace_find_column (coalesceNil joinexpr true)) scan_condition tables) '(now_condition later_condition) (begin
									...
									(define _ord_once_name (scan_once_limit_promise_name tblvar condition joinexpr tbl tbl_once_limit))
									...
									(match (split_scan_condition isOuter (replace_find_column (coalesceNil joinexpr true)) scan_condition tables) '(now_condition later_condition) (begin
										...
										(if (and (equal? base_tbl ".(1)") (equal? cols (list)) (equal? filtercols (list)))
											...
											(begin
												...
												(define _scan_once_name (scan_once_limit_promise_name tblvar condition joinexpr tbl tbl_once_limit))
