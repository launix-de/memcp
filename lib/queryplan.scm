...
(define split_condition (lambda (expr tables) (match expr
	...
	'((symbol outer) outer_ref) /* outer cross-join column reference: (outer tb.col) generated by correlated subquery */
	/* Extract table alias from "tblvar.col" symbol, defer if that table is still pending */
	(begin
		(define ref_tbl (car (split (string outer_ref) ".")))
		(define is_later (reduce tables (lambda (found entry) (or found (equal? ref_tbl (car entry)))) false))
		(if is_later '(true expr) '(expr true)))
...
/* extract all (outer sym) references from an expression as a flat list of symbols.
Used during Neumann-style unnesting to identify free variables (D columns). */
(define extract_outer_sym_refs (lambda (expr) (match expr
	(cons sym args) (if (or (equal? sym (quote outer)) (equal? sym '(quote outer)) (equal? sym '(symbol outer)))
		(match args '(symname) (list symname) _ '())
		(merge (map args extract_outer_sym_refs))
	)
	'()
)))
...
(define untangle_query (lambda (schema tables fields condition group having order limit offset outer_schemas_chain) (begin
	...
	(define make_replace_find_column_subselect (lambda (schemas2 outer_schemas) (begin
		...
		(define column_exists_in_schema (lambda (schemas alias_name table_insensitive column_name column_insensitive) (begin
			/* Avoid nested closure scoping bug: define col_cmp and has_col at this scope level
			so the optimizer doesn't produce deep (outer (var N)) chains that resolve wrong vars */
			(define col_cmp (if column_insensitive equal?? equal?))
			(define tbl_cmp (if table_insensitive equal?? equal?))
			(define has_col (lambda (cols) (reduce cols (lambda (found coldef) (or found (col_cmp (coldef "Field") column_name))) false)))
			(define matches (reduce_assoc schemas (lambda (acc alias cols)
				(if (and (or (nil? alias_name) (tbl_cmp alias_name alias))
					(has_col cols))
					(cons alias acc)
					acc)
			) '()))
	...
	/* Accumulators for Neumann-style unnesting: scalar subqueries whose tables can be merged
	into the outer flat FROM list instead of embedding nested scan plans in the condition.
	Enabled only after the outer FROM list is assembled (main path, not nil-tables path). */
	(define _unnest_tables_acc (newsession))
	(define _unnest_conds_acc (newsession))
	(define _unnest_groups_acc (newsession))
	(define _unnest_schemas_acc (newsession))
	(define _unnesting_enabled (newsession))
	(_unnest_tables_acc "data" '())
	(_unnest_conds_acc "data" '())
	(_unnest_groups_acc "data" '())
	(_unnest_schemas_acc "data" '())
	(_unnesting_enabled "val" false)

	/* Nested-scan fallback for scalar subqueries that cannot be unnested (aggregates, complex cases).
	Extracted as a named function to avoid compounding AST nesting depth with build_scalar_subselect. */
	(define _build_scalar_nested_scan (lambda (outer_schemas schema2 tables2 fields2 condition2 groups2 schemas2 raw_group raw_having raw_order raw_limit raw_offset) (begin
	...
	(define build_scalar_subselect (lambda (subquery outer_schemas) (begin
		...
		(if (not (nil? union_parts))
			...
			(begin
				...
				(match (untangle_query (nth subquery 0) (nth subquery 1) (nth subquery 2) (nth subquery 3) (nth subquery 4) (nth subquery 5) (nth subquery 6) (nth subquery 7) (nth subquery 8) outer_schemas)
					'(schema2 tables2 fields2 condition2 groups2 schemas2 replace_find_column2)
					(begin
						(define groups2 (coalesceNil groups2 '()))
						/* Neumann-style unnesting: check if this scalar subquery can be flattened into the
						outer FROM list. Eligible when: unnesting is enabled (main path only), single scalar
						field with no aggregate, and subquery has at least one table to merge. */
						(define _unnest_field_exprs (extract_assoc fields2 (lambda (k v) v)))
						(define _unnest_field_expr (if (equal? (count _unnest_field_exprs) 1) (car _unnest_field_exprs) nil))
						(define _unnest_has_agg (if _unnest_field_expr (not (equal? (extract_aggregates _unnest_field_expr) '())) true))
						(define _can_unnest (and
							(_unnesting_enabled "val")
							(not (nil? _unnest_field_expr))
							(not _unnest_has_agg)
							(not (nil? tables2))
							(not (equal? tables2 '()))
						))
						(if _can_unnest
							/* Unnesting path: merge subquery tables/condition/groups into outer flat FROM list.
							D columns (free vars from outer context) are added as group keys so build_queryplan
							can partition LIMIT/ORDER correctly per outer row (Neumann D ▶ T2 materialization). */
							(begin
								(define _d_syms (merge_unique (merge
									(extract_outer_sym_refs (coalesceNil condition2 true))
									(extract_outer_sym_refs _unnest_field_expr)
								)))
								(define _d_keys (filter
									(map _d_syms (lambda (sym) (begin
										(define parts (split (string sym) "."))
										(match parts
											(list tbl col) (list (quote get_column) (symbol tbl) false col false)
											_ nil
										)
									)))
									(lambda (x) (not (nil? x)))
								))
								/* use groups2 from inner untangle (already encodes its LIMIT/ORDER);
								only create a new group stage from raw values if groups2 is empty */
								(define _groups_with_raw (if (and (or (nil? groups2) (equal? groups2 '())) (or raw_group raw_having raw_order raw_limit raw_offset))
									(list (make_group_stage raw_group raw_having raw_order raw_limit raw_offset))
									groups2
								))
								/* augment each group stage with D keys so partitioning is per outer row */
								(define _augmented_groups (if (equal? _d_keys '())
									_groups_with_raw
									(map _groups_with_raw (lambda (g) (make_group_stage
										(merge_unique (merge _d_keys (coalesceNil (stage_group_cols g) '())))
										(stage_having_expr g)
										(stage_order_list g)
										(stage_limit_val g)
										(stage_offset_val g)
								...
								(_unnest_tables_acc "data" (merge (_unnest_tables_acc "data") tables2))
								(_unnest_conds_acc "data" (cons (coalesceNil condition2 true) (_unnest_conds_acc "data")))
								(_unnest_groups_acc "data" (merge (_unnest_groups_acc "data") _augmented_groups))
								(_unnest_schemas_acc "data" (merge_assoc (_unnest_schemas_acc "data") schemas2))
								/* return the scalar field expression (already column-resolved by inner untangle) */
								_unnest_field_expr
							)
							/* Nested-scan fallback: delegate to extracted helper to keep AST depth bounded */
							(_build_scalar_nested_scan outer_schemas schema2 tables2 fields2 condition2 groups2 schemas2 raw_group raw_having raw_order raw_limit raw_offset)
						) /* close if _can_unnest */
					)
				)
			)
	))))
	...
	(if (or (nil? tables) (equal? tables '()))
		...
		(begin
			...
			/* enable Neumann unnesting only at top level — when called from build_scalar_subselect,
			outer_schemas_chain is non-empty (contains the caller's schemas), which means we are
			inside a subquery context and must NOT enable inner unnesting. This prevents doubly-nested
			subqueries from generating multiple group stages that build_queryplan cannot handle. */
			(_unnesting_enabled "val" (nil? outer_schemas_chain))
			...
			/* merge unnested subquery tables and schemas into the flat FROM list */
			(define _unnested_tables (_unnest_tables_acc "data"))
			(define _unnested_conds (_unnest_conds_acc "data"))
			(define _unnested_groups (_unnest_groups_acc "data"))
			(define _unnested_schemas (_unnest_schemas_acc "data"))
			(set tables (merge tables _unnested_tables))
			(set schemas (merge_assoc schemas _unnested_schemas))
			...
			/* include unnested subquery conditions in the combined WHERE expression */
			(set conditionAll (cons 'and (filter
				(merge (list (replace_rename condition)) conditionList _unnested_conds)
			...
			/* unnested group stages come first (inner partitioning), then this query's own group stage */
			(define groups (if (coalesce _cd_distinct_exprs false)
				...
				/* normal: unnested groups prepended, then own group stage */
				(merge _unnested_groups (if (or group having order limit offset) (list (make_group_stage group having order limit offset)) '()))))
