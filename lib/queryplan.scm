...
(define group_any_value_reduce (lambda (acc val)
	(if (nil? val) acc
		(if (nil? acc) val acc))
))
...
(define strip_logical_case_flags (lambda (expr) (match expr
	'((symbol get_column) alias_ _ col _)
	(list (quote get_column) alias_ false col false)
	'((quote get_column) alias_ _ col _)
	(list (quote get_column) alias_ false col false)
	(cons sym args) (if (is_opaque_scope_sym sym)
		expr
		(cons (strip_logical_case_flags sym)
			(map args strip_logical_case_flags)))
	expr
)))
/* Naming contract:
...
/* Planner-debug + EXPLAIN helpers live in lib/queryplan-debug.scm
...
(define canonical_expr_name (lambda (expr columns params alias_map)
	(serialize_canonical_expr (canonicalize_expr (strip_logical_case_flags expr) alias_map))
))
...
(define rewrite_materialized_source_columns (lambda (tbl tblvar expr)
	(begin
		(define normalized_tbl (normalize-materialized-subquery-source tbl))
		(define source_alias_map (prejoin_canonical_sources normalized_tbl))
		(if (or (nil? source_alias_map) (not (list? source_alias_map)))
			...
			(match expr
				...
				(begin
					(coalesce (source_alias_map col) expr))
				(cons sym args)
				(cons sym (map args (lambda (arg) (rewrite_materialized_source_columns normalized_tbl tblvar arg))))
				expr)))))
...
raw unn_* occurrence aliases into physical temp column names. */
(define lower_materialized_source_expr (lambda (tbl tblvar expr)
	(begin
		(define normalized_tbl (normalize-materialized-subquery-source tbl))
		(define expr_lookup (materialized_source_expr_lookup normalized_tbl))
		(define source_alias_map (prejoin_canonical_sources normalized_tbl))
		(define planned_cols (coalesceNil (planned_materialized_fields normalized_tbl) '()))
		(define lower_node (lambda (node) (begin
			...
			(define node_refs_current_source (match node
				'((symbol get_column) alias_ _ _ _)
				(has? (alias_lookup_variants tblvar) alias_)
				'((quote get_column) alias_ _ _ _)
				(has? (alias_lookup_variants tblvar) alias_)
				false))
			(define planned_source_expr
				(if node_refs_current_source
					(match node
						...
						(if (not (nil? found))
							...
							(begin
								...
								(if (and (not (nil? source_expr))
									(equal? (normalize_canonical_aliases source_expr) normalized_node))
									(coldef "Field")
									nil))))
						nil)
					nil))
			...
			(define direct_field
				(coalesce planned_field
					(if (or (nil? expr_lookup) (not node_refs_current_source))
						nil
			...
			(if (not (nil? direct_source_expr))
				...
				(if (not (nil? direct_field))
					...
					(match node
						...
						_
						(rewrite_materialized_source_columns normalized_tbl tblvar node)))))))
		(lower_node expr)
)))
	...
	(begin
		...
		(define preserve_node (lambda (node) (match node
			...
			(if (nil? (find_materialized_field_by_name planned_cols col))
				(lower_materialized_source_expr normalized_tbl tblvar node)
			...
			(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
				node
				(cons sym (map args preserve_node)))
			_ node)))
		(if (materialized-source? normalized_tbl)
			(preserve_node expr)
			expr)
)))
...
(define extract_tblvars (lambda (expr)
	(match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			'()
			(merge_unique (map args extract_tblvars)))
		_ (match expr
			(concat tbl "." _col) (if (equal? tbl "__unresolved__") '() (list tbl))
			'())
	)
))
...
(define extract_outer_tblvars (lambda (expr)
	(match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			'()
			(merge_unique (map args extract_outer_tblvars)))
		'()
	)
))

/* rewrite_outer_refs_as_columns: after a dependent helper has been lifted into
the same logical table scope as its domain source, `(outer alias.col)` is no
longer an outer-scope capture. Keep `outer` for dependency discovery before
join_reorder, but lower it to a normal get_column ref when emitting the
decorrelated join expression. */
(define rewrite_outer_refs_as_columns (lambda (expr)
	(match expr
		'((symbol outer) outer_sym) (match (split (string outer_sym) ".")
			(list outer_tbl outer_col)
			(list (quote get_column) outer_tbl false outer_col false)
			_ expr)
		'((quote outer) outer_sym) (match (split (string outer_sym) ".")
			(list outer_tbl outer_col)
			(list (quote get_column) outer_tbl false outer_col false)
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			expr
			(cons sym (map args rewrite_outer_refs_as_columns)))
		expr)))
...
(define extract_columns_for_tblvar (lambda (tblvar expr)
	(match expr
		...
		(cons sym args) /* function call */ (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			'()
			(merge_unique (map args (lambda (arg) (extract_columns_for_tblvar tblvar arg)))))
		'()
	)
))
...
(define extract_unqualified_columns (lambda (expr)
	(match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			'()
			(merge_unique (map args extract_unqualified_columns)))
		'()
	)
))
...
(define expr_has_tblvar_wildcard_ref (lambda (tblvar expr)
	(match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			false
			(reduce args (lambda (found arg) (or found (expr_has_tblvar_wildcard_ref tblvar arg))) false))
		false
	)
))
...
(define expr_has_unqualified_wildcard_ref (lambda (expr)
	(match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			false
			(reduce args (lambda (found arg) (or found (expr_has_unqualified_wildcard_ref arg))) false))
		false
	)
))
...
(define replace_columns_from_expr (lambda (expr)
	(match expr
		...
		(cons sym args) /* function call */ (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			expr
			(cons sym (map args replace_columns_from_expr)))
		expr /* literals */
	)
))
...
(define scan_tagged_table_base (lambda (tbl) (match tbl
	...
	(make_unnest_helper_table schema_name (scan_tagged_table_base base) helper_kind)
	'(scan-tagged-table base _ _ _ _ _) (normalize-materialized-subquery-source base)
	'(scan-tagged-table base _ _ _ _ _ _) (normalize-materialized-subquery-source base)
	'((symbol scan-tagged-table) base _ _ _ _ _) (normalize-materialized-subquery-source base)
	'((symbol scan-tagged-table) base _ _ _ _ _ _) (normalize-materialized-subquery-source base)
	'((quote scan-tagged-table) base _ _ _ _ _) (normalize-materialized-subquery-source base)
	'((quote scan-tagged-table) base _ _ _ _ _ _) (normalize-materialized-subquery-source base)
	tbl
)))
...
(define scan_tagged_table_with_outer_sources (lambda (tbl outer_sources)
	(if (or (nil? outer_sources) (equal? outer_sources '()))
		tbl
		(match tbl
			'((symbol scan-tagged-table) base order limit offset partition_cols once_limit)
			(make_scan_tagged_table_parts base order limit offset partition_cols once_limit outer_sources)
			'((quote scan-tagged-table) base order limit offset partition_cols once_limit)
			(make_scan_tagged_table_parts base order limit offset partition_cols once_limit outer_sources)
			'((symbol scan-tagged-table) base order limit offset partition_cols once_limit old_outer_sources)
			(make_scan_tagged_table_parts base order limit offset partition_cols once_limit
				(merge_unique (coalesceNil old_outer_sources '()) outer_sources))
			'((quote scan-tagged-table) base order limit offset partition_cols once_limit old_outer_sources)
			(make_scan_tagged_table_parts base order limit offset partition_cols once_limit
				(merge_unique (coalesceNil old_outer_sources '()) outer_sources))
			'(unnest_helper_table schema_name base helper_kind)
...
(define scalar_subselect_rewrite_prefixed_expr (lambda (expr lookup_alias) (match expr
	...
	(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
		expr
		(cons (scalar_subselect_rewrite_prefixed_expr sym lookup_alias) (map args (lambda (arg)
...
(define scalar_subselect_resolved_schema (lambda (schema_cols tschema ttbl)
	(begin
		(define physical_schema (scalar_subselect_physical_schema tschema ttbl))
		(define planned_schema (materialized_source_physical_schema tschema ttbl nil '()))
		(if (and (list? schema_cols) (not (equal? schema_cols '())))
			schema_cols
			(if (and (list? physical_schema) (not (equal? physical_schema '())))
				physical_schema
				(if (and (list? planned_schema) (not (equal? planned_schema '())))
					planned_schema
					nil))))
))
...
(define scalar_subselect_correlation_info (lambda (condition_expr inner_aliases rewrite_outer_expr) (begin
	(define ss_has_outer (lambda (expr) (unnest_expr_has_outer_ref expr inner_aliases)))
	(define ss_cond_parts (flatten_and_terms condition_expr))
	(define ss_inner_parts (filter ss_cond_parts (lambda (part) (not (ss_has_outer part)))))
	(define ss_outer_parts (filter ss_cond_parts (lambda (part) (ss_has_outer part))))
	(define ss_domain_cols (filter
		(map ss_outer_parts (lambda (part) (unnest_correlated_domain_col part ss_has_outer rewrite_outer_expr)))
		(lambda (x) (not (nil? x)))))
	(define ss_extra_inner_parts (filter ss_outer_parts (lambda (part)
		(unnest_correlated_residual_part? part ss_has_outer))))
	(define ss_inner_parts_combined (merge ss_inner_parts (map ss_extra_inner_parts rewrite_outer_expr)))
	(define ss_inner_cond_raw
		(if (equal? (count ss_inner_parts_combined) 0) nil
			(if (equal? (count ss_inner_parts_combined) 1) (car ss_inner_parts_combined)
				(cons (quote and) ss_inner_parts_combined))))
	(list ss_outer_parts ss_domain_cols ss_inner_cond_raw)
)))
...
(define wrap_once_limit_body (lambda (promise_name partition_key_exprs body)
	(if (nil? promise_name)
		body
		(if (or (nil? partition_key_exprs) (equal? partition_key_exprs '()))
			(list (quote begin)
				(list (symbol promise_name) "once" true "Subquery returns more than 1 row")
				body)
			(begin
				(define key_expr (list (quote serialize) (cons (quote list) partition_key_exprs)))
				(list (quote begin)
					(list (quote if)
						(list (symbol promise_name) key_expr)
						(list (quote error) "Subquery returns more than 1 row")
						(list (symbol promise_name) key_expr true))
					body))))
))
(define wrap_once_limit_scan (lambda (promise_name partition_key_exprs scan_expr)
	(if (nil? promise_name)
		scan_expr
		(list (quote begin)
			(list (quote set) (symbol promise_name)
				(if (or (nil? partition_key_exprs) (equal? partition_key_exprs '()))
...
(define scan-runtime-table (lambda (schema tbl)
	(planner_runtime_table_source schema (scan_tagged_table_base tbl))
))
/* scan-codegen-table: generates a table expression for codegen */
(define scan-codegen-table (lambda (schema tbl)
	(planner_codegen_table_source schema (scan_tagged_table_base tbl))
))
...
(define extract_aggregates (lambda (expr)
	(match expr
		...
		(cons (symbol window_func) _args) '()
		(cons (quote window_func) _args) '()
		(cons '(quote window_func) _args) '()
		(cons sym args) /* function call */ (if (list? args) (merge (map args extract_aggregates)) '())
		/* literal */ '()
	)
))
...
(define expr_uses_session_state (lambda (expr)
	(match expr
		...
		(cons sym args) (if (list? args) (reduce args (lambda (acc arg) (or acc (expr_uses_session_state arg))) false) false)
		false
	)
))
...
(define session_read_expr (lambda (expr)
	(match expr
		...
		(if (or
			...
			expr
			nil)
		(cons (cons (symbol context) '("session")) (cons _ '())) expr
		(cons (cons '(quote context) '("session")) (cons _ '())) expr
		_ nil)))
...
(define semijoin_outer_marker_alias (lambda (alias_)
	(if (nil? alias_)
		nil
		(if (equal?? alias_ "__semijoin_outer")
			""
			(match alias_
				(regex "__semijoin_outer:(.*)" _ inner_alias) inner_alias
				_ nil)))))
(define semijoin_unmark_outer_expr (lambda (expr) (match expr
	'((quote get_column) alias_ ti col ci) (begin
		(define inner_alias (semijoin_outer_marker_alias alias_))
		(if (nil? inner_alias)
			expr
			(list (quote get_column)
				(if (equal? inner_alias "") nil inner_alias)
				ti
				col
				ci)))
	'((symbol get_column) alias_ ti col ci) (begin
	...
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
		expr
		(cons (semijoin_unmark_outer_expr sym) (map args semijoin_unmark_outer_expr)))
	expr
)))
(define expr_contains_semijoin_outer_marker (lambda (expr) (match expr
	'((quote get_column) alias_ _ _ _) (not (nil? (semijoin_outer_marker_alias alias_)))
	'((symbol get_column) alias_ _ _ _) (not (nil? (semijoin_outer_marker_alias alias_)))
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
		false
		(or (expr_contains_semijoin_outer_marker sym)
			(reduce args (lambda (found arg) (or found (expr_contains_semijoin_outer_marker arg))) false)))
	false
)))
(define extract_semijoin_outer_target_exprs (lambda (expr) (match expr
	(cons eq_sym (cons left (cons right '())))
	(if (or
		(equal?? eq_sym (quote equal??))
		(equal?? eq_sym (quote equal?))
		(equal?? eq_sym (symbol equal??))
		(equal?? eq_sym (symbol equal?)))
		(if (and (expr_contains_semijoin_outer_marker left) (not (expr_contains_semijoin_outer_marker right)))
			(list (semijoin_unmark_outer_expr left))
			(if (and (expr_contains_semijoin_outer_marker right) (not (expr_contains_semijoin_outer_marker left)))
				(list (semijoin_unmark_outer_expr right))
				(merge_unique
					(extract_semijoin_outer_target_exprs left)
					(extract_semijoin_outer_target_exprs right))))
		(if (is_opaque_scope_sym eq_sym)
			'()
			(merge_unique
				(extract_semijoin_outer_target_exprs eq_sym)
				(merge_unique (map (list left right) extract_semijoin_outer_target_exprs)))))
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
		'()
		(merge_unique
			(extract_semijoin_outer_target_exprs sym)
			(merge_unique (map args extract_semijoin_outer_target_exprs))))
	'()
)))
...
(define extract_window_funcs (lambda (expr)
	(match expr
		...
		(cons '(quote window_func) rest) (list rest)
		(cons sym args) /* function call */ (if (list? args) (merge (map args extract_window_funcs)) '())
		/* literal */ '()
	)
))
...
(define extract_all_get_columns (lambda (expr)
	(match expr
		...
		(cons sym args) (if (list? args) (merge (map args extract_all_get_columns)) '())
		'()
	)
))
...
(define extract_all_table_aliases (lambda (expr)
	(match expr
		...
		(cons sym args) (if (list? args) (merge (map args extract_all_table_aliases)) '())
		_ (match expr
			(concat tbl "." _col) (if (equal? tbl "__unresolved__") '() (list tbl))
			'())
	)
...
(define extract_scanned_tables (lambda (expr)
	(match expr
		...
		(cons sym args) (if (list? args) (merge (map args extract_scanned_tables)) '())
		'()
	)
))
...
(define expr_has_scan (lambda (expr)
	(match expr
		(cons head rest) (match (string head)
			...
			_ (if (list? rest) (reduce rest (lambda (acc child) (or acc (expr_has_scan child))) false) false))
		false)))
...
(define parallelize_resultrows (lambda (ast)
	...
	/* Prejoin materialization guards can recur many times while compiling deeply
	nested top-down helper plans. Keep the runtime semantics lazy by generating
one thunk per exact guard/materialize body and replacing repeated occurrences
...
(define planner_prejoin_init_current_scope (lambda ()
	(match (coalesceNil (planner_prejoin_init_scope_stack "stack") '())
		(cons scope _) scope
		_ nil)))
(define planner_prejoin_init_scope_defs (lambda () (begin
	(define scope (planner_prejoin_init_current_scope))
	(if (nil? scope)
		'()
		(coalesceNil (scope "defs") '()))
)))
(define planner_compile_scope_depth (lambda () (begin
	(define scope (planner_prejoin_init_current_scope))
	(if (nil? scope)
		0
		(coalesceNil (scope "compile_depth") 0))
)))
(define planner_prejoin_init_register (lambda (key init_body) (begin
	(define scope (planner_prejoin_init_current_scope))
	/* The thunk hoist path is intentionally disabled until all callers emit
	definitions in the same runtime scope as their calls. Returning init_body
	keeps materialization semantics inline and avoids dangling __prejoin_init
	references in derived/correlated subquery plans. */
	(if true
		init_body
		(begin
			(define name_key (concat "name:" key))
			(define existing_name (scope name_key))
			(if (not (nil? existing_name))
				(list existing_name)
				(begin
					(define idx (coalesceNil (scope "idx") 0))
					(define init_name (symbol (concat "__prejoin_init:" idx)))
					(scope "idx" (+ idx 1))
					(scope name_key init_name)
					(scope "defs" (merge
						(coalesceNil (scope "defs") '())
						(list (list (quote define) init_name
							(list (quote lambda) '() init_body)))))
					(list init_name))))))))
(define planner_init_first_createtable_key (lambda (expr) (match expr
	(cons head args)
	(begin
		(define head_str (string head))
		(if (and (equal? head_str "createtable") (list? args) (>= (count args) 2))
			(concat "table:" (serialize (nth args 0)) ":" (serialize (nth args 1)))
			(reduce args (lambda (found arg)
				(if (not (nil? found)) found
					(planner_init_first_createtable_key arg)))
				nil)))
	nil)))
(define planner_init_register (lambda (init_body)
	(planner_prejoin_init_register
		(coalesce
			(planner_init_first_createtable_key init_body)
			(concat "expr:" (fnv_hash (serialize init_body))))
		init_body)))
(define planner_init_register_list (lambda (init_list)
	(map (coalesceNil init_list '()) planner_init_register)))
(define planner_with_prejoin_init_scope (lambda (thunk) (!begin
	(define saved_prejoin_stack (coalesceNil (planner_prejoin_init_scope_stack "stack") '()))
	(define prejoin_scope (newsession))
	(prejoin_scope "compile_depth" 0)
	(planner_prejoin_init_scope_stack "stack" (cons prejoin_scope saved_prejoin_stack))
	(define scoped_result (thunk))
	(planner_prejoin_init_scope_stack "stack" saved_prejoin_stack)
	scoped_result
)))

(define planner_analysis_cache_scope_stack (newsession))
(planner_analysis_cache_scope_stack "stack" '())
(define planner_analysis_cache_current (lambda ()
	(match (coalesceNil (planner_analysis_cache_scope_stack "stack") '())
		(cons scope _) scope
		_ nil)))
(define planner_with_analysis_cache_scope (lambda (thunk) (!begin
	(define old_stack (coalesceNil (planner_analysis_cache_scope_stack "stack") '()))
	(planner_analysis_cache_scope_stack "stack" (cons (newsession) old_stack))
	(define result (thunk))
	(planner_analysis_cache_scope_stack "stack" old_stack)
	result
)))
(define planner_neumann_lowered_scope_stack (newsession))
(planner_neumann_lowered_scope_stack "stack" '())
(define planner_neumann_lowered_current (lambda ()
	(match (coalesceNil (planner_neumann_lowered_scope_stack "stack") '())
		(cons current _) current
		_ false)))
(define planner_with_neumann_lowered_scope (lambda (thunk) (!begin
	(define old_stack (coalesceNil (planner_neumann_lowered_scope_stack "stack") '()))
	(planner_neumann_lowered_scope_stack "stack" (cons true old_stack))
	(define result (thunk))
	(planner_neumann_lowered_scope_stack "stack" old_stack)
	result
)))
(define planner_compile_scope_active? (lambda ()
	(and
		(not (nil? (planner_analysis_cache_current)))
		(not (nil? (planner_prejoin_init_current_scope))))))
(define planner_with_compile_scope (lambda (thunk)
	(if (planner_compile_scope_active?)
		(!begin
			(define scope (planner_prejoin_init_current_scope))
			(define old_depth (planner_compile_scope_depth))
			(scope "compile_depth" (+ old_depth 1))
			(define result (thunk))
			(scope "compile_depth" old_depth)
			result)
		(planner_with_analysis_cache_scope (lambda ()
			(planner_with_prejoin_init_scope thunk))))))

(define planner_dependent_group_suffix (lambda (collect_condition) (begin
	(define scope (planner_analysis_cache_current))
	(define key (concat "dep-group-suffix:" (sha1 (serialize collect_condition))))
	(if (nil? scope)
		(concat "|dep:" (sha1 (uuid)))
		(begin
			(define existing (scope key))
			(if (not (nil? existing))
				existing
				(begin
					(define suffix (concat "|dep:" (sha1 (uuid))))
					(scope key suffix)
					suffix)))))
)))
...
(define split_condition (lambda (expr tables) (match expr
	...
	(cons sym args) /* non-splittable function call */ (if (is_opaque_scope_sym sym)
...
(define combine_and_terms (lambda (parts) (begin
	(define _parts_raw (filter parts (lambda (x) (and (not (nil? x)) (not (equal? x true))))))
	(define _deduped (reduce _parts_raw (lambda (state part)
		(match state
			'(kept keys)
			(begin
				(define key (serialize (normalize_canonical_aliases part)))
				(if (has? keys key)
					state
					(list (merge kept (list part)) (merge keys (list key)))))
			state))
		(list '() '())))
	(define _parts (car _deduped))
...
(define collect_all_column_refs (lambda (expr) (match expr
	...
	'((quote get_column) tblvar _ col _) (list (list tblvar col))
	(cons sym args) (if (list? args) (merge_unique (map args collect_all_column_refs)) '())
	'()
)))
(define collect_all_outer_column_refs (lambda (expr) (match expr
	'((symbol outer) outer_sym) (match (split (string outer_sym) ".")
		(list tbl col) (list (list tbl col))
		_ '())
	'((quote outer) outer_sym) (match (split (string outer_sym) ".")
		(list tbl col) (list (list tbl col))
		_ '())
	(cons sym args) (if (or (is_quote_scope_sym sym) (not (list? args)))
		'()
		(merge_unique (map args collect_all_outer_column_refs)))
	'()
)))
(define collect_all_outer_ref_exprs (lambda (expr)
	(map (collect_all_outer_column_refs expr) (lambda (ref_pair) (match ref_pair
		'(tbl col) (list (quote get_column) tbl false col false)
		_ nil)))))
(define normalize_outer_column_ref_expr (lambda (expr) (match expr
	'((symbol outer) outer_sym) (match (split (string outer_sym) ".")
...
(define scan_alias_leaf (lambda (alias_)
	(begin
		(define parts (split (string alias_) "\0"))
		(if (> (count parts) 1) (nth parts (- (count parts) 1)) (string alias_)))))
(define flattened_scalar_helper_alias? (lambda (alias_)
	(begin
		(define alias_s (string alias_))
		(define parts (split alias_s "\0"))
		(and
			(> (count parts) 1)
			(>= (strlen alias_s) 3)
			(equal? (substr alias_s 0 3) "sq_")))))
(define scalar_once_helper_scan? (lambda (schema tbl tblvar schemas)
	(or
		(and
			(qpp-tuple? tbl)
			(reduce (qpp-fields-to-pairs (coalesceNil (qpp-tuple-fields tbl) '()))
				(lambda (found pair)
					(or found (equal? (nth pair 0) "__qpu_scalar_once_limit")))
				false))
		(and
			(materialized-source? tbl)
			(reduce (materialized_source_physical_schema schema tbl tblvar schemas)
				(lambda (found coldef)
					(or found (equal? (coldef "Field") "__qpu_scalar_once_limit")))
				false)))))
(define synthetic_unnest_alias_matches_scan_alias? (lambda (stage_alias tblvar)
	(and
		(string? stage_alias)
		(>= (strlen stage_alias) 4)
		(equal? (substr stage_alias 0 4) "unn_")
		(strlike stage_alias (concat "unn_" (scan_alias_leaf tblvar) "%")))))
(define synthetic_unnest_alias_matches_table? (lambda (stage_alias tbl)
	(begin
		(define base_tbl (planner_table_source_base tbl))
		(and
			(string? stage_alias)
			(string? base_tbl)
			(>= (strlen stage_alias) (+ (strlen base_tbl) 5))
			(equal? (substr stage_alias 0 (+ (strlen base_tbl) 5))
				(concat "unn_" base_tbl "_"))))))
(define synthetic_unnest_alias_matches_scan? (lambda (stage_alias tblvar tbl)
	(or
		(synthetic_unnest_alias_matches_scan_alias? stage_alias tblvar)
		(synthetic_unnest_alias_matches_table? stage_alias tbl))))
(define rewrite_partition_stage_for_scan_alias (lambda (stage tblvar tbl)
	(begin
		(define rewrite_stage_scan_expr (lambda (expr) (match expr
			'((symbol get_column) alias_ ti col ci)
			(if (synthetic_unnest_alias_matches_scan? alias_ tblvar tbl)
				(list (quote get_column) tblvar ti col ci)
				expr)
			'((quote get_column) alias_ ti col ci)
			(if (synthetic_unnest_alias_matches_scan? alias_ tblvar tbl)
				(list (quote get_column) tblvar ti col ci)
			...
			(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
				expr
				(cons (rewrite_stage_scan_expr sym) (map args rewrite_stage_scan_expr)))
			expr)))
		(rewrite_stage_for_flattened_aliases
			stage
			rewrite_stage_scan_expr
			(lambda (alias_)
				(if (synthetic_unnest_alias_matches_scan? alias_ tblvar tbl) tblvar alias_))))))
(define partition_stage_matches_scan_alias? (lambda (stage tblvar tbl)
	(reduce (coalesceNil (stage_partition_aliases stage) '()) (lambda (matched alias_)
		(or matched
			(equal? alias_ tblvar)
			(synthetic_unnest_alias_matches_scan? alias_ tblvar tbl)))
		false)))
(define find_partition_stage_for_alias (lambda (stages tblvar tbl)
	(reduce (coalesceNil stages '()) (lambda (found stage)
		(if (nil? found)
			(if (partition_stage_matches_scan_alias? stage tblvar tbl)
				(rewrite_partition_stage_for_scan_alias stage tblvar tbl)
				nil)
			found))
		nil)
))

(define extract_scan_order_terms_for_tblvar (lambda (order_items tblvar pick_col)
...
(define extract_scan_order_cols_for_tblvar (lambda (order_items tblvar)
	(extract_scan_order_terms_for_tblvar order_items tblvar true)
))

(define extract_scan_order_dirs_for_tblvar (lambda (order_items tblvar)
	(extract_scan_order_terms_for_tblvar order_items tblvar false)
))

/* symbols that canonicalize_columns must NOT recurse into — they have their own scope */
(define is_opaque_scope_sym (lambda (sym) (match sym
	...
	(symbol scan_batch) true '(quote scan_batch) true 'scan_batch true
	...
	/* Compile-only scalar markers are resolved by
	resolve_dependent_scalar_compile_markers; generic tree walkers must not
	treat their symbol head as a call form. */
	(symbol dependent_scalar_compile) true
	'(quote dependent_scalar_compile) true
	'dependent_scalar_compile true
	_ false)))
...
(define extract_runtime_cache_terms (lambda (expr) (match expr
	_ (if (or
		(not (nil? (session_read_expr expr)))
		(not (nil? (materialized_context_runtime_key expr))))
		(list expr)
		(match expr
			(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
				'()
...
(define materialized_context_runtime_key (lambda (_expr) nil))
(define planner_eval_runtime_term (lambda (expr) (begin
	(define mat_key (materialized_context_runtime_key expr))
	(if (not (nil? mat_key))
		mat_key
		(begin
			(define bind_context_session (lambda (node) (match node
				(symbol session) '(context "session")
				(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
					node
					(cons (bind_context_session sym) (map args bind_context_session)))
				node)))
			(eval (bind_context_session expr))))
)))
...
(define planner_current_session_snapshot_suffix (lambda () (begin
	(try
		(lambda () (begin
			(define sess (context "session"))
			(define keys (sess))
			(if (equal? keys '())
				""
				(concat "|sess:" (fnv_hash (serialize (map keys (lambda (key)
					(list key (sess key))))))))))
		(lambda (err) "")))))
(define planner_current_user_session_snapshot_suffix (lambda () (begin
	(try
		(lambda () (begin
			(define sess (context "session"))
			(define keys (filter (sess) (lambda (key)
				(not (strlike (string key) "__%")))))
			(if (equal? keys '())
				""
				(concat "|usersess:" (fnv_hash (serialize (map keys (lambda (key)
					(list key (sess key))))))))))
		(lambda (err) "")))))
(define user_session_runtime_cache_suffix_from_exprs (lambda (exprs) (begin
	(define user_session_cache_key? (lambda (key)
		(and
			(string? key)
			(not (strlike key "__%")))))
	(define user_session_term? (lambda (term) (match term
		(cons sym (cons key '()))
		(and
			(or
				(equal?? sym (symbol session))
				(equal?? sym (quote session))
				(equal?? sym (quote (quote session))))
			(user_session_cache_key? key))
		(cons (cons (symbol context) '("session")) (cons key '()))
		(user_session_cache_key? key)
		(cons (cons '(quote context) '("session")) (cons key '()))
		(user_session_cache_key? key)
		false)))
	(define extract_user_session_terms (lambda (expr) (match expr
		_ (if (user_session_term? expr)
			(list expr)
			(match expr
				(cons sym args)
				(if (or (is_opaque_scope_sym sym) (not (list? args)))
					...
					(merge_unique
						(merge
							(extract_user_session_terms sym)
							(merge_unique (map args extract_user_session_terms)))))
				'())))))
	(define terms (merge_unique (map exprs extract_user_session_terms)))
	(if (equal? terms '())
		""
		(concat "|rt:"
			(serialize_canonical_expr
...
(define aggregate_count_literal_expr? (lambda (expr)
	(or
		(equal? expr 1)
		(equal? (serialize expr) "(if (nil? 1) 0 1)")
)))
(define normalize_count_literal_aggregate_args (lambda (agg_args)
	(match agg_args
		'(agg_expr + 0)
		(if (aggregate_count_literal_expr? agg_expr)
			aggregate_count_descriptor
			agg_args)
		agg_args
)))
(define normalize_count_literal_aggregates (lambda (expr)
	(match expr
		(cons (symbol aggregate) agg_args)
		(match (normalize_count_literal_aggregate_args agg_args)
			'(agg_expr agg_reduce agg_neutral)
			(list (quote aggregate)
				(normalize_count_literal_aggregates agg_expr)
				agg_reduce
				agg_neutral)
			expr)
		...
		(if (or (is_opaque_scope_sym sym) (not (list? args)))
			expr
			(cons sym (map args normalize_count_literal_aggregates)))
		expr
)))
...
(define aggregate_cache_col_name_with_suffix (lambda (expr_name ag suffix)
	(if (equal? ag aggregate_count_descriptor)
		(concat "COUNT(*)|" suffix)
		(concat "AGG|" (fnv_hash (expr_name ag)) "|" suffix))
))
...
(define schema_alias_matches (lambda (query_alias schema_alias ti)
	/* Parser table aliases for unaliased INFORMATION_SCHEMA tables can keep the
	user's lower-case spelling while metadata exposes canonical upper-case table
	names. Bind to the provider alias case-insensitively; column casing is still
	resolved separately via the column ci flag. */
	(equal?? query_alias schema_alias)
))
...
(define collect_schema_column_matches_scoped (lambda (local_schemas visible_schemas helper_aliases alias_ ti col ci) (begin
	(define collect_matches (lambda (schemas alias_pred)
		(reduce_assoc schemas (lambda (acc alias cols)
			(begin
				(define norm_cols (schema_assoc_cols schemas alias))
				(if (and (alias_pred alias) (schema_has_column? norm_cols col ci))
					(merge acc (list (list alias (canonical_schema_column_name norm_cols col ci))))
					acc)))
			'())))
	(if (nil? alias_)
		(begin
...
(define canonicalize_columns_scoped (lambda (expr local_schemas visible_schemas helper_aliases) (match expr
	'((symbol get_column) alias_ ti col ci) (if (or ti ci (not (nil? alias_)))
		(begin
	...
	'((quote get_column) alias_ ti col ci) (if (or ti ci (not (nil? alias_)))
		(begin
			(define resolved (resolve_schema_column_expr_scoped local_schemas visible_schemas helper_aliases alias_ ti col ci))
			(if (nil? resolved)
				expr /* leave unresolved — replace_find_column will handle or error */
	...
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args))) expr
...
(define semijoin_outer_marker_fallback_expr (lambda (expr outer_schemas marker_alias ti col ci) (begin
	(define aliases (reduce_assoc outer_schemas (lambda (acc alias _cols)
		(merge acc (list alias))) '()))
	(define fallback_alias
		(if (equal? marker_alias "")
			(if (equal? (count aliases) 1) (nth aliases 0) nil)
			marker_alias))
	(if (nil? fallback_alias)
		expr
		(list (quote get_column) fallback_alias false col false)))))

(define rewrite_semijoin_outer_markers_scoped (lambda (expr outer_schemas helper_aliases) (begin
	(match expr
		'((quote get_column) alias_ ti col ci) (begin
			(define marker_alias (semijoin_outer_marker_alias alias_))
			(if (nil? marker_alias)
				expr
				(begin
					(define resolved
						(resolve_schema_column_expr_scoped
							outer_schemas
							outer_schemas
							helper_aliases
							(if (equal? marker_alias "") nil marker_alias)
							true
							col
							ci))
					(if (nil? resolved)
						(semijoin_outer_marker_fallback_expr expr outer_schemas marker_alias ti col ci)
						resolved))))
		'((symbol get_column) alias_ ti col ci) (begin
			(define marker_alias (semijoin_outer_marker_alias alias_))
			(if (nil? marker_alias)
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
			expr
			(cons (rewrite_semijoin_outer_markers_scoped sym outer_schemas helper_aliases)
				(map args (lambda (arg)
					(rewrite_semijoin_outer_markers_scoped arg outer_schemas helper_aliases)))))
		expr
))))
(define flatten_plus_terms (lambda (expr) (match expr
	(cons add_sym add_args) (if (or
		(equal?? add_sym (quote +))
		(equal?? add_sym (symbol +)))
		(merge (map add_args flatten_plus_terms))
		(list expr))
	_ (list expr)
)))
(define parser_semijoin_count_term_subquery (lambda (term) (match term
	(cons inner_sym (cons inner_subquery '()))
	(if (or
		(equal?? inner_sym (quote inner_select))
		(equal?? inner_sym (quote (quote inner_select)))
		(equal?? inner_sym (symbol inner_select)))
		inner_subquery
		nil)
	_ nil
)))
(define parser_semijoin_count_eq_info (lambda (cond_terms outer_schemas) (begin
	(define find_eq (lambda (term) (match term
		(cons eq_sym (cons left (cons right '())))
		(if (or
			(equal?? eq_sym (quote equal??))
			(equal?? eq_sym (quote equal?))
			...
			(equal?? eq_sym (symbol equal?)))
			(if (and (expr_contains_semijoin_outer_marker left) (not (expr_contains_semijoin_outer_marker right)))
				(list term right (rewrite_semijoin_outer_markers_scoped left outer_schemas '()))
				(if (and (expr_contains_semijoin_outer_marker right) (not (expr_contains_semijoin_outer_marker left)))
					(list term left (rewrite_semijoin_outer_markers_scoped right outer_schemas '()))
					nil))
			nil)
		_ nil)))
	(reduce cond_terms (lambda (found term)
		(if (not (nil? found))
			found
			(find_eq term)))
		nil)
)))
(define parser_semijoin_count_branch_info (lambda (subquery outer_schemas) (match subquery
	'(sub_schema sub_tables sub_fields sub_condition sub_group sub_having sub_order sub_limit sub_offset)
	(if (and
		(equal? (count sub_fields) 2)
		(equal? (car sub_fields) "__cnt")
		(reduce sub_tables (lambda (ok td) (and ok (match td
			'(_ tschema ttbl _ _)
			(and
				(string? ttbl)
				(not (materialized-source? ttbl))
				(or (equal? (strlen ttbl) 0) (not (equal? (substr ttbl 0 1) ".")))
				(not (nil? (try (lambda () (get_schema tschema ttbl)) (lambda (e) nil)))))
			false)))
			true)
		(or (nil? sub_order) (equal? sub_order '()))
		(nil? sub_limit)
		(or (nil? sub_offset) (equal? sub_offset 0)))
		(begin
			(define cond_terms (flatten_and_terms (coalesceNil sub_condition true)))
			(define eq_info (parser_semijoin_count_eq_info cond_terms outer_schemas))
			(match eq_info
				'(matched_term field_expr target_expr)
				(list
					target_expr
					(list sub_schema
						sub_tables
						(list "__semijoin_value" field_expr)
						(combine_and_terms (filter cond_terms (lambda (term) (not (equal? term matched_term)))))
						sub_group
						sub_having
						sub_order
						sub_limit
...
(define rewrite_dependent_parser_not_in_count_expr (lambda (expr outer_schemas) (begin
	(define positive_count_subquery (lambda (count_expr) (match count_expr
		(cons cmp_sym (cons inner_expr (cons zero_expr '())))
		(if (and
			(equal? zero_expr 0)
			(or (equal?? cmp_sym (quote >)) (equal?? cmp_sym (symbol >))))
			(parser_semijoin_count_term_subquery inner_expr)
			nil)
		_ nil)))
	(match expr
		(cons if_sym (cons nil_check (cons nil_branch (cons match_if '()))))
		(if (and
			(or (equal?? if_sym (quote if)) (equal?? if_sym (symbol if)))
			(nil? nil_branch))
			(match nil_check
				(cons nil_sym (cons _target_expr '()))
				(if (or (equal?? nil_sym (quote nil?)) (equal?? nil_sym (symbol nil?)))
					(match match_if
						(cons match_if_sym (cons match_expr (cons false_branch (cons null_if '()))))
						(if (and
							(or (equal?? match_if_sym (quote if)) (equal?? match_if_sym (symbol if)))
							(equal? false_branch false))
							(begin
								(define count_subquery (positive_count_subquery match_expr))
								(match (parser_semijoin_count_branch_info count_subquery outer_schemas)
									'(recovered_target recovered_subquery)
									(if (dependent_marker_required_subquery recovered_subquery outer_schemas)
										(list (quote if) nil_check nil
											(list (quote if)
												(replace_inner_selects
													(list (quote inner_select_in) recovered_target recovered_subquery)
													outer_schemas)
												false
												true))
										nil)
...
(define rewrite_parser_semijoin_union_count_expr (lambda (expr outer_schemas) (match expr
	(cons cmp_sym (cons count_expr (cons zero_expr '())))
	(if (equal? zero_expr 0)
		(begin
			(define negated (or
				(equal?? cmp_sym (quote equal?))
				(equal?? cmp_sym (quote equal??))
				(equal?? cmp_sym (symbol equal?))
				(equal?? cmp_sym (symbol equal??))))
			(define positive (or
				(equal?? cmp_sym (quote >))
				(equal?? cmp_sym (symbol >))))
			(if (or negated positive)
				(begin
					(define raw_terms (filter (flatten_plus_terms count_expr) (lambda (term) (not (equal? term 0)))))
					(define branch_infos (filter
						(map raw_terms (lambda (term)
							(parser_semijoin_count_branch_info
								(parser_semijoin_count_term_subquery term)
								outer_schemas)))
						(lambda (info) (not (nil? info)))))
					(if (or
						(< (count branch_infos) 2)
						(not (equal? (count branch_infos) (count raw_terms)))
						(not (reduce branch_infos (lambda (ok info)
							(and ok (equal? (serialize (car info)) (serialize (car (car branch_infos))))))
							true)))
						nil
						(cons (if negated (quote and) (quote or))
							(map branch_infos (lambda (info) (match info
								'(target_expr branch_subquery)
								(if negated
									(list (quote not) (list (quote inner_select_in) target_expr branch_subquery))
									(list (quote inner_select_in) target_expr branch_subquery))
								_ nil))))))
				nil))
		nil)
	_ nil
...
(define stage_rewrite_outer_sources (lambda (stage rewrite_expr rewrite_alias) (begin
	(define sos_sources (coalesceNil (stage_outer_sources stage) '()))
	(if (equal? sos_sources '())
		stage
		(stage_with_outer_sources
			stage
			(map sos_sources (lambda (src) (match src
...
(define unnest_rewrite_inner_aliases (lambda (expr alias_lookup) (match expr
	...
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
		expr
		(cons (unnest_rewrite_inner_aliases sym alias_lookup) (map args (lambda (arg)
...
(define rewrite_stage_for_flattened_aliases (lambda (stage rewrite_expr rewrite_alias) (begin
	...
	(define fin_init (if (or (nil? init) (equal? init '()))
		init
		(map init rewrite_expr)))
...
(define query_union_distinct_parts (lambda (query) (match query
...
(define query_union_parts (lambda (query) (begin
	...
	(if (not (nil? all_parts))
		(merge all_parts (list (equal? ((context "session") "__plain_union_distinct") true)))
		(begin
			(define distinct_parts (query_union_distinct_parts query))
			(if (not (nil? distinct_parts))
				(merge distinct_parts (list true))
				nil))))))
(define query_union_membership_parts (lambda (query) (begin
	(define union_parts (query_union_parts query))
	(if (nil? union_parts)
		nil
		(match union_parts '(branches order limit offset _distinct)
			(list branches order limit offset))))))
(define query_is_select_core (lambda (query) (match query
	'((string? _schema) tables fields _condition _group _having _order _limit _offset)
	(and (list? tables) (list? fields))
	false)))
...
(define expr_has_any_wildcard_ref (lambda (expr) (match expr
	...
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
		false
		(or (expr_has_any_wildcard_ref sym) (reduce args (lambda (found arg) (or found (expr_has_any_wildcard_ref arg))) false)))
	false
)))
...
(define rewrite_union_wrapper_expr (lambda (expr wrapper_alias branch_fields) (match expr
	...
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
		expr
		(cons (rewrite_union_wrapper_expr sym wrapper_alias branch_fields)
...
(define rewrite_query_term (lambda (query) (begin
	...
	(if (not (nil? top_union_parts))
		(match top_union_parts '(branches order limit offset distinct)
			(list (if distinct (quote union_distinct) (quote union_all))
				(map branches rewrite_query_term) order limit offset))
...
(define untangle_query_term (lambda (query outer_schemas) (begin
	(if neumann_pipeline_trace (print "[neumann] term: before rewrite_query_term") nil)
	(define rewritten_raw (rewrite_query_term query))
	(define rewritten_query (if (and (nil? rewritten_raw) (query_is_select_core query))
		query
		rewritten_raw))
	(if neumann_pipeline_trace (print "[neumann] term: after rewrite_query_term") nil)
	(define union_parts (query_union_parts rewritten_query))
	(if neumann_pipeline_trace (print "[neumann] term: after union check") nil)
	(if (nil? union_parts)
		(if (query_is_select_core rewritten_query)
			(begin
				(if (or (nil? rewritten_query) (< (count rewritten_query) 1) (not (string? (nth rewritten_query 0))))
					(error (concat "UNTANGLE_QUERY_TERM_NONSTRING_SCHEMA " (serialize rewritten_query)))
					nil)
				(define uq_result (apply untangle_query (merge rewritten_query (list outer_schemas))))
				(if neumann_pipeline_trace (print "[neumann] term: after apply untangle_query") nil)
				(if (not (equal? (count uq_result) 8))
					(error (concat "UNTANGLE_QUERY_TERM_UQ_RESULT " (serialize uq_result)))
					nil)
				(define select_term (make_select_core_term uq_result))
				(if neumann_pipeline_trace (print "[neumann] term: after make_select_core_term") nil)
				select_term)
...
(define make_keytable_schema (lambda (schema tbl keys tblvar) (begin
	(define keytable_source_name_raw (planner-temp-source-name tbl tblvar))
	(define keytable_source_name
		(coalesce (prejoin_keytable_domain_sources (normalize-materialized-subquery-source tbl))
			keytable_source_name_raw))
	...
	(define key_pairs_sorted (sort (map keys (lambda (k)
		(begin
			(define key_name (sanitize_temp_name
	...
	(define key_pairs (reduce key_pairs_sorted (lambda (acc pair)
		(if (reduce acc (lambda (found existing_pair)
			(or found (equal?? (nth existing_pair 0) (nth pair 0))))
			false)
			acc
			(merge acc (list pair))))
		'()))
	(define key_names (map key_pairs car))
	(define keytable_name (concat ".keytable:" (fnv_hash (concat keytable_source_name ":" key_names))))
...
(define make_keytable (lambda (schema tbl keys tblvar condition_suffix) (begin
	...
	(define has_condition_suffix
		(and (not (nil? condition_suffix)) (not (equal? condition_suffix true))))
	(define keytable_source_name_raw (planner-temp-source-name tbl tblvar))
	(define keytable_source_name
		(coalesce (prejoin_keytable_domain_sources (normalize-materialized-subquery-source tbl))
			keytable_source_name_raw))
	...
	(define fk_result (if (and physical_tbl (not has_condition_suffix) (equal? 1 (count keys)))
		(match (car keys)
	...
	(if (not (nil? fk_result))
		...
		(begin
			...
			(define key_pairs (reduce key_pairs_sorted (lambda (acc pair)
				...
				'()))
			(define key_names (map key_pairs car))
			(define key_exprs (map key_pairs cadr))
			(define condition_name (if (not has_condition_suffix) nil
				(fnv_hash (concat
					...
					(user_session_runtime_cache_suffix_from_exprs (list condition_suffix))))))
			(define key_name_at (lambda (i) (nth key_names i)))
			(define key_at (lambda (i) (nth key_exprs i)))
			(define keytable_name (concat ".keytable:"
					(fnv_hash
					(if (not has_condition_suffix)
						(concat keytable_source_name ":" key_names)
						(concat keytable_source_name ":" key_names "|" condition_name)))))
			(define keytable_dependency_tables
				(if (materialized-source? tbl)
					(coalesceNil (materialized_source_dependency_tables (normalize-materialized-subquery-source tbl)) '())
					(if physical_tbl
						(list (list schema tbl))
						'())))
			(materialized_source_dependency_tables keytable_name keytable_dependency_tables)
...
(define keytable_inject_oninit (lambda (expr oninit_thunk) (match expr
	...
	(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
		expr
		(cons (keytable_inject_oninit sym oninit_thunk) (map args (lambda (a) (keytable_inject_oninit a oninit_thunk)))))
	expr)))
	...
	(match kt_result '(grouptbl kt_schema_def keytable_init fk_pk_col) (begin
		...
		(define lower_window_runtime_expr (lambda (expr) (match expr
			(cons (symbol aggregate) agg_args) (begin
				...
				(define agg_prefix (if (equal? agg_args aggregate_count_descriptor) "COUNT(*)" agg_name))
				(define match_col (if materialized_source
					(reduce materialized_cols (lambda (found coldef)
						(if (not (nil? found)) found
							(begin
								(define field_name (coldef "Field"))
								(if (and (>= (strlen field_name) (+ (strlen agg_prefix) 1))
									(equal? (substr field_name 0 (strlen agg_prefix)) agg_prefix)
									(equal? (substr field_name (strlen agg_prefix) 1) "|"))
									field_name
									nil))))
						nil)
					nil))
			...
			(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
				expr
				(cons sym (map args lower_window_runtime_expr)))
			expr)))
			...
			(cons (symbol window_func) wf_rest) (begin
				...
				(if has_partition (begin
					...
					(list 'scan '(session "__memcp_tx") (list 'table schema grouptbl) '(list) '('lambda '() true)
						...
						'('lambda '('__v) '__v)
						'('lambda '('__a '__b) '__b) nil nil false)))
			(cons sym args_) (if (or (is_opaque_scope_sym sym) (not (list? args_)))
				expr
				(cons sym (map args_ replace_wf_with_fetch)))
			expr)))
...
(define make_col_replacer (lambda (grouptbl condition is_dedup expr_name src_tblvar agg_col_name) (begin
	...
	(define replacer (lambda (expr) (match expr
		(cons (symbol aggregate) rest) (if is_dedup
			expr
			'('get_column grouptbl false (agg_col_name (normalize_count_literal_aggregate_args rest)) false))
...
(define rewrite_for_prejoin (lambda (pjvar alias_map expr)
	(match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
			expr
			(cons sym (map args (lambda (a) (rewrite_for_prejoin pjvar alias_map a)))))
		expr
	)
))
...
(define replace_tblvar_with_dict (lambda (tv dict_sym expr)
	(match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
			expr
			(cons sym (map args (lambda (a) (replace_tblvar_with_dict tv dict_sym a)))))
		expr
	)
))
...
(define untangle_query (lambda (schema tables fields condition group having order limit offset outer_schemas_param) (begin
	...
	(sq_cache "init" '())
	(sq_cache "init_keys" '())
	(sq_cache "parallel_inits" '())
	(sq_cache "direct_parallel_inits" '())
	(define sq_cache_add_init_once (lambda (source init_code) (begin
		(if (or (nil? init_code) (equal? init_code '()))
			nil
			(begin
				(define init_key (concat "init:" (fnv_hash (serialize (coalesce source init_code)))))
				(define init_keys (coalesceNil (sq_cache "init_keys") '()))
				(if (has? init_keys init_key)
					nil
					(begin
						(sq_cache "init_keys" (merge init_keys (list init_key)))
						(sq_cache "init" (merge (coalesceNil (sq_cache "init") '()) (list init_code))))))))))
	(define sq_cache_add_init_list (lambda (init_codes)
		(reduce (coalesceNil init_codes '()) (lambda (acc init_code) (begin
			(sq_cache_add_init_once init_code init_code)
			acc)) nil)))
	(define qpu_low_initial_extra_inits (qpu-low-drain-extra-inits))
	(sq_cache_add_init_list qpu_low_initial_extra_inits)
	(define dep_scalar_cache (newsession))
	(define analysis_cache (coalesce (planner_analysis_cache_current) (newsession)))
	(define analysis_cache_key (lambda (kind query outer_schemas)
		(concat kind ":" (fnv_hash (serialize (list query outer_schemas))))))
	(define analysis_cache_get_or_compute (lambda (kind query outer_schemas compute_thunk)
		(begin
			(define key (analysis_cache_key kind query outer_schemas))
			(match (analysis_cache key)
				'(cached value) value
				_ (begin
					(define computed_value (compute_thunk))
					(analysis_cache key (list (quote cached) computed_value))
					computed_value)))))
	(define scalar_inner_select_true_literal? (lambda (expr) (match expr
		true true
		(symbol true) true
		'(quote true) true
		_ false)))
	(define scalar_inner_select_is_presence_query (lambda (subquery) (match subquery
		'(_ _ fields _condition group having _order limit offset)
		(begin
			(define first_field_expr (match fields
				(cons _ (cons v _)) v
				nil))
			(and
				(scalar_inner_select_true_literal? first_field_expr)
				(or (nil? group) (equal? group '()))
	...
	(define cd_is_subquery (lambda (sym) (match sym
		...
		_ false)))
	(define cd_find (lambda (expr) (match expr
		'((symbol count_distinct) _) true
		'((quote count_distinct) _) true
		(cons sym args) (if (or (cd_is_subquery sym) (is_quote_scope_sym sym) (not (list? args)))
			false
			(reduce args (lambda (a b) (or a (cd_find b))) false))
		false)))
	(define cd_extract (lambda (expr) (match expr
		'((symbol count_distinct) e) (list e)
		'((quote count_distinct) e) (list e)
		(cons sym args) (if (or (cd_is_subquery sym) (is_quote_scope_sym sym) (not (list? args)))
			'()
			(merge (map args cd_extract)))
		'())))
	(define cd_replace (lambda (expr) (match expr
		'((symbol count_distinct) e) '((quote aggregate) 1 (quote +) 0)
		'((quote count_distinct) e) '((quote aggregate) 1 (quote +) 0)
		(cons sym args) (if (or (cd_is_subquery sym) (is_quote_scope_sym sym) (not (list? args)))
			expr
			(cons sym (map args cd_replace)))
		expr)))
	(define cd_has (reduce_assoc fields (lambda (a k v) (or a (cd_find v))) false))
	...
	(define cd_distinct_exprs (if cd_has (reduce_assoc fields (lambda (a k v) (merge a (cd_extract v))) '()) nil))
	(define cd_having (if cd_has having nil))
	(define cd_order (if cd_has order nil))
	(define cd_limit (if cd_has limit nil))
	(define cd_offset (if cd_has offset nil))
	(define cd_user_group group)
	(define fields (if cd_has (map_assoc fields (lambda (k v) (cd_replace v))) fields))
	(define having (if cd_has nil having))
	(define order (if cd_has nil order))
	(define limit (if cd_has nil limit))
	(define offset (if cd_has nil offset))
	(define fields (map_assoc fields (lambda (k v) (normalize_count_literal_aggregates v))))
	(define tables (map tables (lambda (td) (match td
		'(tv tschema ttbl toisOuter tje)
		(list tv tschema ttbl toisOuter
			(if (nil? tje) nil (normalize_count_literal_aggregates tje)))
		_ td))))
	(define condition (normalize_count_literal_aggregates condition))
	(define group (map (coalesceNil group '()) normalize_count_literal_aggregates))
	(define having (normalize_count_literal_aggregates having))
	(define order (map (coalesceNil order '()) (lambda (o) (match o
		'(col dir) (list (normalize_count_literal_aggregates col) dir)
		_ o))))
	...
	(define make_replace_find_column_subselect (lambda (schemas2 outer_schemas preserve_grouped_outer_domain) (begin
			...
			(define matches (reduce_assoc schemas (lambda (acc alias cols)
				(begin
					...
					(if (and (or (nil? alias_name) ((if table_insensitive equal?? equal?) alias_name alias))
						(reduce norm_cols (lambda (found coldef) (or found ((if column_insensitive equal?? equal?) (coldef "Field") column_name))) false))
						(cons alias acc)
						acc))
			) '()))
			(match matches
				...
				(begin
					...
					(if (not (nil? acc)) acc
						(if (or (nil? alias_name) ((if table_insensitive equal?? equal?) alias_name alias))
							(reduce norm_cols (lambda (found coldef)
								(if (not (nil? found)) found
		...
		(define outer_alias_requires_domain_preservation (lambda (outer_alias) (begin
			(define outer_cols (schema_assoc_cols _o outer_alias))
		...
		(define wrap_outer_leaves (lambda (expr) (match expr
			(cons sym args) (if (is_get_column_sym sym)
				...
				(if (or (is_quote_scope_sym sym) (not (list? args)))
					expr
					(cons (wrap_outer_leaves sym) (map args wrap_outer_leaves)))
			)
			expr
						...
						(if (nil? outer_alias)
							...
							(begin
								...
								(if (or
									...
									domain; unqualified visible fields must also stay on the visible
									outer binding instead of collapsing to base-table refs */
		...
		(define replace_find_column_subselect (lambda (expr) (match expr
			(cons sym args) (if (is_get_column_sym sym)
				(match args
					...
					_ (if (list? args) (cons sym (map args replace_find_column_subselect)) expr)
				)
	...
	(define raw_query_local_aliases (lambda (query)
		(analysis_cache_get_or_compute "raw-local-aliases" query '()
			(lambda () (match query
	...
	(define expr_uses_any_alias (lambda (expr aliases) (match expr
		'((symbol get_column) alias_ _ _ _)
		(and (not (nil? alias_)) (alias_in_list aliases alias_))
		'((quote get_column) alias_ _ _ _)
		(and (not (nil? alias_)) (alias_in_list aliases alias_))
		'((symbol outer) outer_sym)
		(match (split (string outer_sym) ".")
			(list alias_ _col) (alias_in_list aliases alias_)
			_ false)
		'((quote outer) outer_sym)
		(match (split (string outer_sym) ".")
			(list alias_ _col) (alias_in_list aliases alias_)
			_ false)
		(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
			false
			(reduce args (lambda (acc arg) (or acc (expr_uses_any_alias arg aliases))) false))
		false)))
	(define raw_query_nested_outer_refs_for_domain (lambda (query current_aliases visible_schemas)
		(analysis_cache_get_or_compute "raw-nested-outer-domain-refs" query (list current_aliases visible_schemas)
			(lambda () (match query
				'(_ _ raw_fields raw_condition raw_group raw_having raw_order _ _) (begin
					(define local_aliases (raw_query_local_aliases query))
					(define nested_current_aliases (append_unique current_aliases local_aliases))
					(define visible_ref (lambda (alias_ col)
						(if (or
							(nil? alias_)
							(alias_in_list nested_current_aliases alias_)
							(nil? (schema_assoc_cols visible_schemas alias_)))
							nil
							(concat alias_ "." col))))
					(define raw_expr_outer_refs (lambda (expr) (match expr
						'((symbol get_column) alias_ _ col _)
						(match (visible_ref alias_ col)
							nil '()
							ref (list ref))
						'((quote get_column) alias_ _ col _)
						(match (visible_ref alias_ col)
						...
						(match (split (string outer_sym) ".")
							...
							_ '())
						(cons sym args) (begin
							(define kind (subquery_marker_kind sym))
							(define nested_subquery (if (nil? kind) nil
								(match kind
									...
									(quote inner_select_exists) (match args
										...
										nil)
									nil)))
							(if (not (nil? nested_subquery))
								(raw_query_nested_outer_refs_for_domain nested_subquery nested_current_aliases visible_schemas)
								(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
									'()
									(merge_unique (map args raw_expr_outer_refs)))))
						_ '())))
					(merge_unique (list
						(merge (extract_assoc raw_fields (lambda (_k v) (raw_expr_outer_refs v))))
						(raw_expr_outer_refs (coalesceNil raw_condition true))
						(merge (map (coalesceNil raw_group '()) raw_expr_outer_refs))
						(raw_expr_outer_refs (coalesceNil raw_having true))
						(merge (map (coalesceNil raw_order '()) (lambda (order_item)
							(match order_item
								'(col _dir) (raw_expr_outer_refs col)
								_ '())))))))
				_ '())))))
	(define scalar_independent_limited_subquery_expr (lambda (subquery outer_schemas)
		(match subquery
			'(q_schema q_tables q_fields q_condition q_group q_having _q_order q_limit q_offset)
			(begin
				(define local_aliases (raw_query_local_aliases subquery))
				(define expr_has_direct_aggregate (lambda (expr) (match expr
					'((symbol aggregate) _ _ _) true
					'((quote aggregate) _ _ _) true
					(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
						false
						(reduce args (lambda (found arg)
							(or found (expr_has_direct_aggregate arg)))
							false))
					false)))
				(define expr_has_direct_unqualified_column (lambda (expr) (match expr
					'((symbol get_column) nil _ col _) (not (equal? col "*"))
					'((quote get_column) nil _ col _) (not (equal? col "*"))
					(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
						false
						(reduce args (lambda (found arg)
							(or found (expr_has_direct_unqualified_column arg)))
							false))
					false)))
				(define value_expr (match q_fields
					(cons _ (cons v _)) v
					nil))
				(if (and
					(not (nil? value_expr))
					(not (expr_has_direct_aggregate value_expr))
					(not (expr_has_direct_unqualified_column value_expr))
					(not (expr_uses_any_alias value_expr local_aliases))
					(not (contains_inner_select_marker q_condition))
					(not (expr_uses_any_alias q_group local_aliases))
					(or (nil? q_group) (equal? q_group '()))
					(or (nil? q_having) (equal? q_having true))
					(equal? q_limit 1)
					(or (nil? q_offset) (equal? q_offset 0)))
					(begin
						(define presence_query
							(list q_schema q_tables
								(list "__exists" true)
								q_condition
								nil nil nil 1 nil))
						(list (quote if)
							(replace_inner_selects
								(list (quote inner_select_exists) presence_query)
								outer_schemas)
							(replace_inner_selects value_expr outer_schemas)
							nil))
					nil))
			_ nil)))
	(define raw_query_uses_alias_outside_current (lambda (query current_aliases)
		(analysis_cache_get_or_compute "raw-alias-outside" query current_aliases
			(lambda () (match query
						...
						(cons sym args) (begin
							...
							(define nested_subquery (if (nil? kind) nil
								(match kind
									...
									nil)))
							(if (not (nil? nested_subquery))
								(raw_query_uses_alias_outside_current
									nested_subquery
									(merge_unique current_aliases nested_local_aliases))
								(if (or (is_opaque_scope_sym sym) (not (list? args)))
									false
									(reduce args (lambda (acc arg) (or acc (raw_expr_uses_alias_outside_current arg))) false))))
						false)))
					(or
						...
						(reduce (coalesceNil raw_order '()) (lambda (acc order_item)
							...
							false)))
				false)))))
	(define raw_query_contains_skip_level_nested_outer_ref (lambda (query current_aliases)
		(analysis_cache_get_or_compute "raw-skip-level" query current_aliases
			(lambda () (match query
				'(_ _ raw_fields raw_condition raw_group raw_having raw_order _ _) (begin
					...
					(or
						...
						(reduce (coalesceNil raw_order '()) (lambda (acc order_item)
							...
							false)))
				false)))))
	(define raw_query_inner_selects_use_aliases (lambda (query aliases)
		(analysis_cache_get_or_compute "raw-inner-selects-use-aliases" query aliases
			(lambda () (match query
				'(_ _ raw_fields raw_condition raw_group raw_having raw_order _ _) (begin
					(define local_aliases (raw_query_local_aliases query))
					(define raw_expr_uses_alias (lambda (expr) (match expr
						'((symbol get_column) alias_ _ _ _) (and
							(not (nil? alias_))
							(alias_in_list aliases alias_)
							(not (alias_in_list local_aliases alias_)))
						...
						(cons sym args) (begin
							...
							(define nested_subquery (if (nil? kind) nil
								(match kind
									...
									nil)))
							(if (not (nil? nested_subquery))
								(raw_query_inner_selects_use_aliases nested_subquery aliases)
								(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
									false
									(reduce args (lambda (acc arg) (or acc (raw_expr_uses_alias arg))) false))))
						false)))
					(or
						(reduce_assoc raw_fields (lambda (acc _k v) (or acc (raw_expr_uses_alias v))) false)
						(raw_expr_uses_alias (coalesceNil raw_condition true))
						(reduce (coalesceNil raw_group '()) (lambda (acc gexpr) (or acc (raw_expr_uses_alias gexpr))) false)
						(raw_expr_uses_alias (coalesceNil raw_having true))
						(reduce (coalesceNil raw_order '()) (lambda (acc order_item)
							(or acc (match order_item
								'(col _dir) (raw_expr_uses_alias col)
								false)))
							false)))
				false)))))
	(define scalar_subselect_shape_facts (lambda (subquery outer_schemas)
		(analysis_cache_get_or_compute "scalar-shape" subquery outer_schemas
			(lambda () (match subquery
	...
	(define scalar_subselect_limitk_trigger (lambda (subquery outer_schemas) (match subquery
		'(_ _ _ _ _ _ _order limit offset) (and
			...
			(or
				(and (not (nil? limit)) (> limit 1))
				(and (not (nil? offset)) (> offset 0))))
		false)))
	(define scalar_subselect_limitk_default_order (lambda (subquery) (match subquery
		'(_ _ flds _ _ _ _ _ _) (match flds
			(cons _ (cons value_expr _)) (list (list value_expr '<))
			'())
		'())))
	(define scalar_subselect_limitk_effective_order (lambda (subquery outer_schemas raw_order)
		(if (and (scalar_subselect_limitk_trigger subquery outer_schemas)
			(or (nil? raw_order) (equal? raw_order '())))
			(scalar_subselect_limitk_default_order subquery)
			raw_order)))
	(define untangle_scalar_subquery_scope (lambda (subquery outer_schemas raw_group raw_having raw_order raw_limit raw_offset) (begin
		...
		(define scope_order (scalar_subselect_limitk_effective_order subquery outer_schemas raw_order))
		(define scope_subquery (match subquery
			'(schema tables fields condition group having _order limit offset)
			(list schema tables fields condition group having scope_order limit offset)
			_ subquery))
		(if (or (nil? subquery) (< (count subquery) 1) (not (string? (nth subquery 0))))
			(error (concat "UNTANGLE_SCALAR_SCOPE_NONSTRING_SCHEMA " (serialize subquery)))
			nil)
		(match (apply untangle_query (merge scope_subquery (list outer_schemas)))
			...
			(begin
				...
				(define groups2 (if (or (nil? groups2) (equal? groups2 '()))
					(begin
						(define scope_fields_have_window
							(reduce_assoc fields2 (lambda (found _k v)
								(or found (not (equal? (extract_window_funcs v) '()))))
								false))
						(define scope_having_for_stage
							(if (and scope_fields_have_window (equal? raw_having true))
								nil
								raw_having))
						(if (group_stage_requested raw_group scope_having_for_stage scope_order raw_limit raw_offset)
							(list (make_group_stage raw_group scope_having_for_stage scope_order raw_limit raw_offset nil nil))
							groups2))
					groups2))
	...
	(define direct_scalar_scan_source? (lambda (tschema ttbl)
		(or
			(materialized-subquery-source? (normalize-materialized-subquery-source ttbl))
			(and
				(string? ttbl)
				(or (equal? (strlen ttbl) 0) (not (equal? (substr ttbl 0 1) ".")))
				(not (nil? (try (lambda () (get_schema tschema ttbl)) (lambda (e) nil))))))))
	(define build_simple_in_scalar_scan (lambda (target_expr subquery) (begin (match subquery
		'(sq_schema sq_tables sq_fields sq_condition sq_group sq_having sq_order sq_limit sq_offset)
		(if (nil? target_expr)
			nil
			(if (and
				(equal? (count sq_tables) 1)
				(or (nil? sq_group) (equal? sq_group false) (equal? sq_group '()))
				(or (nil? sq_having) (equal? sq_having false) (equal? sq_having true))
				(or (nil? sq_order) (equal? sq_order false) (equal? sq_order '()))
				(or (nil? sq_limit) (equal? sq_limit false))
				(or (nil? sq_offset) (equal? sq_offset false) (equal? sq_offset 0)))
				(match (car sq_tables)
					'(tv tschema ttbl _isOuter tjoinexpr)
					(if (not (direct_scalar_scan_source? tschema ttbl))
						nil
						(begin
							(define first_field_expr (match sq_fields
								(cons _ (cons v _)) v
								nil))
							(define filter_expr (combine_and_terms
								(filter
									(list
										(coalesceNil tjoinexpr true)
										(coalesceNil sq_condition true)
										(if (nil? first_field_expr)
											nil
											(list (quote equal??) first_field_expr target_expr)))
									(lambda (term) (and (not (nil? term)) (not (equal? term true)))))))
							(if (or
								(nil? first_field_expr)
								(contains_inner_select_marker filter_expr)
								(not (equal? (extract_aggregates first_field_expr) '())))
								nil
								(begin
									(define filter_cols
										(reduce (extract_columns_for_tblvar tv filter_expr)
											(lambda (acc col) (if (has? acc col) acc (merge acc (list col))))
											'()))
									(define filter_params (map filter_cols (lambda (col) (symbol (concat tv "." col)))))
									(define lower_filter_expr (lambda (expr) (match expr
										'((symbol get_column) alias_ _ col _) (if (equal?? alias_ tv)
											(symbol (concat tv "." col))
											(list (quote outer) (symbol (concat alias_ "." col))))
										'((quote get_column) alias_ _ col _) (if (equal?? alias_ tv)
											(symbol (concat tv "." col))
											(list (quote outer) (symbol (concat alias_ "." col))))
										(cons fsym fargs) (if (is_opaque_scope_sym fsym)
											expr
											(cons (lower_filter_expr fsym) (map fargs lower_filter_expr)))
										_ expr)))
									(define in_scan
										(list (quote scalar_scan)
											...
											(list (quote lambda) filter_params (lower_filter_expr filter_expr))
											(list (quote list))
											...
											nil))
									(list (quote if)
										(list (quote nil?) target_expr)
										nil
										(list (quote not) (list (quote nil?) in_scan)))))))
					_ nil)
				nil)))
		_ nil)))
	(define build_simple_exists_scalar_scan (lambda (subquery negated) (match subquery
		...
		(if (and
			...
			(or (nil? sq_limit) (equal? sq_limit 1))
			(or (nil? sq_offset) (equal? sq_offset 0)))
			(match (car sq_tables)
				'(tv tschema ttbl _isOuter tjoinexpr)
				(if (or
					(not (string? ttbl))
					(materialized-source? ttbl)
					(and (> (strlen ttbl) 0) (equal? (substr ttbl 0 1) "."))
					(nil? (try (lambda () (get_schema tschema ttbl)) (lambda (e) nil))))
					nil
					(begin
						(define filter_expr (combine_and_terms
							(filter
								(list
									...
									(coalesceNil sq_condition true))
								(lambda (term) (and (not (nil? term)) (not (equal? term true)))))))
						(if (contains_inner_select_marker filter_expr)
							nil
							(begin
								(define filter_cols
									(reduce (extract_columns_for_tblvar tv filter_expr)
								...
								(define exists_scan
									(list (quote scalar_scan)
										...
										nil))
								(if negated
									(list (quote nil?) exists_scan)
									(list (quote not) (list (quote nil?) exists_scan)))))))
				_ nil)
			nil)
		_ nil)))
	...
	(define unnest_subselect (lambda (subquery outer_schemas) (begin
		...
		(if (or
			(nil? subquery)
			(< (count subquery) 1)
			(not (string? (nth subquery 0)))
			(not (nil? union_parts_us)))
			nil /* Non-flat fragments and UNION ALL are handled by fallback paths. */
			(begin
				...
				(match (untangle_scalar_subquery_scope
					...
					'(schema2_us tables2_us fields2_us condition2_us groups2_us schemas2_us rfcol2_us init2_us) (begin
						(if (and (not (nil? init2_us)) (not (equal? init2_us '())))
							(sq_cache_add_init_list init2_us))
						...
						(match (unnest_operator_select_rule)
							'(us_select_rule_result us_select_info)
							(if (not (nil? us_select_rule_result))
								us_select_rule_result
								(begin
									...
									(define nt_virtual_init
										(list (quote createtable) schema2_us "(1)"
											...
											(list (quote lambda) '()
												(list (quote insert) (list (quote table) schema2_us "(1)")
													(list (quote quote) (list "1"))
													(list (quote quote) (list (list 1)))
									...
									(define groups2_us (if (or (nil? groups2_us) (equal? groups2_us '()))
										(begin
											(define us_fields_have_window
												(reduce_assoc fields2_us (lambda (found _k v)
													(or found (not (equal? (extract_window_funcs v) '()))))
													false))
											(define us_having_for_stage
												(if (and us_fields_have_window (equal? raw_having_us true))
													nil
													raw_having_us))
											(if (group_stage_requested raw_group_us us_having_for_stage raw_order_us raw_limit_us raw_offset_us)
												(list (make_group_stage raw_group_us us_having_for_stage raw_order_us raw_limit_us raw_offset_us nil nt_virtual_init))
												groups2_us))
										groups2_us))
									(define us_has_field_aggregate (reduce_assoc fields2_us (lambda (found _k v)
									...
									(define rfcs_us (make_replace_find_column_subselect schemas2_us outer_schemas
										(or us_has_field_aggregate raw_group_us raw_having_us raw_order_us)))
									...
									(define us_wrap (lambda (e) (match e
										'((symbol get_column) alias_ ti col ci) (if (and (not (nil? alias_)) (or ti ci))
											(list (quote outer) (symbol (concat alias_ "." col)))
											e)
										(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
											e
											(cons (us_wrap sym) (map args us_wrap)))
										e)))
									(set fields2_us (map_assoc fields2_us (lambda (k v) (us_wrap v))))
									(set condition2_us (us_wrap condition2_us))
									...
									must be defined BEFORE _us_inner_aliases which depends on us_inner_stages */
									(define us_own_stages (filter (coalesceNil groups2_us '()) (lambda (s) (nil? (stage_partition_aliases s)))))
									(define us_inner_stages (filter (coalesceNil groups2_us '()) (lambda (s) (not (nil? (stage_partition_aliases s))))))
									/* count only OWN tables (not inner scoped ones from nested decorrelation) */
									(define _us_inner_aliases (merge (map us_inner_stages (lambda (s) (coalesceNil (stage_partition_aliases s) '())))))
									(define us_own_tables (filter tables2_us (lambda (t) (match t '(a _ _ _ _) (not (has? _us_inner_aliases a)) true))))
									(define us_base_tables (filter us_own_tables (lambda (t) (match t
										'(_ _ ttbl _ _) (not (unnest_helper_table? ttbl))
										true))))
									(define us_nested_direct_tbls (filter us_own_tables (lambda (t) (match t
										'(_ _ ttbl _ _) (unnest_helper_table? ttbl)
										false))))
									(define us_base_aliases (map us_base_tables (lambda (td) (match td '(a _ _ _ _) a nil))))
									(define us_single_tbl (and (list? us_base_tables) (equal? (count us_base_tables) 1)))
									/* check for aggregates in fields */
									(define us_agg (lambda (expr) (match expr
										'((symbol aggregate) _ _ _) true
										(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
											false
											(reduce args (lambda (a b) (or a (us_agg b))) false))
										false)))
									(define us_has_agg (reduce_assoc fields2_us (lambda (a k v) (or a (us_agg v))) false))
									/* check for GROUP/HAVING in OWN stages only */
									(define us_has_stages (not (equal? us_own_stages '())))
									(define us_has_grp (if us_has_stages
										(reduce us_own_stages (lambda (acc stage) (or acc
											(begin
									...
									(define us_has_limit (if us_has_stages
										...
										false))
									(define us_simple_agg_stages (or (equal? us_own_stages '())
										(and (equal? (count us_own_stages) 1)
											(not (stage_is_dedup (car us_own_stages))))))
									...
									(if us_outer_in_fields nil /* outer refs in fields: not handled yet */
										(begin
											...
											(define us_own_tblname (match (car us_base_tables) '(_ _ t _ _) (if (string? t) t "x") "x"))
											(define us_sq_prefix (concat "unn_" us_own_tblname "_" us_sq_idx))
											/* build alias rename map: only OWN tables get prefixed.
											Inner-scoped tables (from nested decorrelation) keep their alias. */
											(define us_alias_map (scalar_subselect_alias_map us_base_tables us_single_tbl us_sq_prefix))
											(define us_lookup (lambda (a) (scalar_subselect_lookup_alias us_alias_map a)))
											...
											(define us_ror unnest_runtime_outer_ref_expr)
											(define us_ria (lambda (expr) (unnest_rewrite_inner_aliases expr us_lookup)))
											(define unnest_operator_join_rule (lambda ()
												(unnest_join_rule annotated_tree condition2_us us_inner_aliases us_ror)))
												...
												(begin
													...
													(if us_has_window
														...
														(if (or us_has_agg us_has_grp)
															(if (or
																(not us_simple_agg_stages)
																(not (equal? us_inner_stages '())))
																nil
	...
	(define subquery_marker_kind (lambda (sym) (begin
		(define s (serialize sym))
		(define ss (string sym))
		(if (or (equal? s "inner_select") (equal? s "(symbol inner_select)") (equal? s "(quote inner_select)") (equal?? ss "inner_select"))
			(quote inner_select)
			(if (or (equal? s "inner_select_in") (equal? s "(symbol inner_select_in)") (equal? s "(quote inner_select_in)") (equal?? ss "inner_select_in"))
				(quote inner_select_in)
				(if (or (equal? s "inner_select_exists") (equal? s "(symbol inner_select_exists)") (equal? s "(quote inner_select_exists)") (equal?? ss "inner_select_exists"))
					(quote inner_select_exists)
					(inner_select_kind sym)))))))
	(define subquery_marker_head? (lambda (sym) (begin
		(define s (serialize sym))
		(or
			(equal? s "inner_select")
			(equal? s "(symbol inner_select)")
			(equal? s "(quote inner_select)")
			(equal? s "inner_select_in")
			(equal? s "(symbol inner_select_in)")
			(equal? s "(quote inner_select_in)")
			(equal? s "inner_select_exists")
			(equal? s "(symbol inner_select_exists)")
			(equal? s "(quote inner_select_exists)")))))
	(define dependent_scalar_compile_marker (lambda (idx)
		(list (quote dependent_scalar_compile) idx)))
	(define dependent_scalar_compile_marker_id (lambda (expr)
		(match expr
			(cons sym (cons idx '()))
			(if (or
				(equal?? sym (quote dependent_scalar_compile))
				(equal?? sym (quote (quote dependent_scalar_compile)))
				(equal?? sym (symbol dependent_scalar_compile)))
				idx
				nil)
			_ nil)))
	(define build_dependent_scalar_via_marker (lambda (kind subquery target_expr outer_schemas)
		(begin
			(define dep_id (coalesceNil (dep_scalar_cache "idx") 1))
			(dep_scalar_cache "idx" (+ dep_id 1))
			(dep_scalar_cache dep_id (list kind subquery target_expr))
			(resolve_dependent_scalar_compile_markers
				(dependent_scalar_compile_marker dep_id)
				outer_schemas))))
	(define build_dependent_negated_scalar_via_marker (lambda (kind subquery target_expr outer_schemas)
		(begin
			(define dep_id (coalesceNil (dep_scalar_cache "idx") 1))
			(dep_scalar_cache "idx" (+ dep_id 1))
			(dep_scalar_cache dep_id (list kind subquery target_expr true))
			(resolve_dependent_scalar_compile_markers
				(dependent_scalar_compile_marker dep_id)
				outer_schemas))))
	(define dependent_marker_required_subquery (lambda (subquery outer_schemas)
		(analysis_cache_get_or_compute "dependent-marker-required" subquery outer_schemas
			(lambda () (or
				(subquery_has_outer_refs subquery outer_schemas)
				(raw_query_uses_alias_outside_current subquery (raw_query_local_aliases subquery))
				(raw_query_contains_skip_level_nested_outer_ref subquery (raw_query_local_aliases subquery)))))))
	...
	(define subquery_resolved_scope (lambda (query outer_schemas)
		(analysis_cache_get_or_compute "resolved-scope" query outer_schemas
			(lambda () (begin
				(if (or (nil? query) (< (count query) 1) (not (string? (nth query 0))))
					(error (concat "SUBQUERY_OUTER_REFS_NONSTRING_SCHEMA " (serialize query)))
					nil)
				(apply untangle_query (merge query (list outer_schemas))))))))
	(define subquery_outer_refs (lambda (query outer_schemas)
		(analysis_cache_get_or_compute "outer-refs" query outer_schemas
			(lambda () (if (or (nil? query) (< (count query) 1) (not (string? (nth query 0))))
				'()
				(match (try
					(lambda () (subquery_resolved_scope query outer_schemas))
					(lambda (e) (list (quote __resolve_failed))))
					'(__resolve_failed)
					(raw_query_nested_outer_refs_for_domain query '() outer_schemas)
					'(_ tables2 fields2 condition2 _groups2 schemas2 rfcol2 _init2) (begin
						(define _inner_aliases (map tables2 (lambda (td) (match td '(a _ _ _ _) a ""))))
						(define eor (lambda (expr) (match expr
							(cons sym args) (if (or (equal? sym (quote outer)) (equal? sym '(quote outer)))
								...
								(if (or (equal? sym (quote get_column)) (equal? sym '(quote get_column)) (equal? sym '(symbol get_column)))
									(match args '(alias_ _ col _)
										...
										'())
									(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args))) '()
										(merge_unique (map args eor)))))
							'())))
						(merge_unique
							(merge (extract_assoc fields2 (lambda (_k v) (eor v))))
							(eor (coalesceNil condition2 true))))
					'()))))))
	(define make_visible_outer_binding (lambda (ref source_expr direct domain_safe)
		(list ref source_expr direct domain_safe)))
	(define visible_outer_binding_ref (lambda (binding) (match binding
		'(ref _source_expr _direct _domain_safe) ref
		_ nil)))
	(define visible_outer_binding_source_expr (lambda (binding) (match binding
		'(_ref source_expr _direct _domain_safe) source_expr
		_ nil)))
	(define visible_outer_binding_direct (lambda (binding) (match binding
		'(_ref _source_expr direct _domain_safe) direct
		_ false)))
	(define visible_outer_binding_domain_safe (lambda (binding) (match binding
		'(_ref _source_expr _direct domain_safe) domain_safe
		_ false)))
	(define outer_ref_visible_binding (lambda (outer_schemas ref) (match (split ref ".")
		(list alias col) (begin
			(define cols (schema_assoc_cols outer_schemas alias))
			...
			(if (nil? coldef)
				nil
				(begin
					(define canonical_col (coldef "Field"))
					(define expr (coalesceNil (coldef "Expr") nil))
					(define source_expr (match expr
						nil (list (quote get_column) alias false canonical_col false)
						'((quote get_column) _ _ _ _) expr
						'((symbol get_column) _ _ _ _) expr
						_ (list (quote get_column) alias false canonical_col false)))
					(make_visible_outer_binding
						(concat alias "." canonical_col)
						source_expr
						(match expr
							nil true
		...
		_ nil)))
	(define subquery_outer_bindings (lambda (query outer_schemas)
		(analysis_cache_get_or_compute "outer-bindings" query outer_schemas
			(lambda ()
				(filter
					(map (subquery_outer_refs query outer_schemas) (lambda (ref)
						(outer_ref_visible_binding outer_schemas ref)))
					(lambda (binding) (not (nil? binding))))))))
	(define rewrite_dependent_scalar_visible_outer_refs (lambda (query outer_schemas) (match query
		'(q_schema q_tables q_fields q_condition q_group q_having q_order q_limit q_offset)
		(begin
			(define bindings (subquery_outer_bindings query outer_schemas))
			(define local_schemas (match (subquery_resolved_scope query outer_schemas)
				'(_ _ _ _ _ resolved_schemas _ _) resolved_schemas
				_ nil))
			(define visible_outer_match_for_column (lambda (col ci)
				(if (not (nil? (resolve_schema_column_ref_scoped local_schemas local_schemas '() nil false col ci)))
					nil
					(match (filter
						(map bindings (lambda (binding)
							(match (split (visible_outer_binding_ref binding) ".")
								(list visible_alias visible_col)
								(if ((if ci equal?? equal?) visible_col col)
									(list visible_alias visible_col)
									nil)
								_ nil)))
						(lambda (x) (not (nil? x))))
						(cons only '()) only
						_ nil))))
			(define rewrite_expr (lambda (expr) (match expr
				'((quote get_column) nil _ col ci)
				(match (visible_outer_match_for_column col ci)
					'(visible_alias visible_col)
					(list (quote get_column) visible_alias false visible_col false)
					_ expr)
			...
			(list
				q_schema
				q_tables
				(map_assoc q_fields (lambda (k v) (rewrite_expr v)))
				(rewrite_expr q_condition)
				(map (coalesceNil q_group '()) rewrite_expr)
				(rewrite_expr q_having)
				(map (coalesceNil q_order '()) (lambda (oi) (match oi
					'(col dir) (list (rewrite_expr col) dir)
					_ oi)))
				q_limit
				q_offset))
	...
	(define outer_ref_is_domain_column (lambda (outer_schemas ref) (match (split ref ".")
		(list _alias _col) (begin
			(define binding (outer_ref_visible_binding outer_schemas ref))
			(and (not (nil? binding))
				(visible_outer_binding_domain_safe binding)))
		_ false)))
	(define outer_ref_is_direct_column (lambda (outer_schemas ref) (match (split ref ".")
		(list _alias _col) (begin
			(define binding (outer_ref_visible_binding outer_schemas ref))
			(and (not (nil? binding))
				(visible_outer_binding_direct binding)))
		_ false)))
	(define subquery_outer_refs_are_direct_columns (lambda (query outer_schemas)
		(begin
			(define refs (subquery_outer_refs query outer_schemas))
			(define bindings (subquery_outer_bindings query outer_schemas))
			(and
				(equal? (count refs) (count bindings))
				(reduce bindings (lambda (all_ok binding)
					(and all_ok (visible_outer_binding_direct binding)))
					true)))))
	(define subquery_outer_refs_are_domain_columns (lambda (query outer_schemas)
		(begin
			(define refs (subquery_outer_refs query outer_schemas))
			(define bindings (subquery_outer_bindings query outer_schemas))
			(and
				(equal? (count refs) (count bindings))
				(reduce bindings (lambda (all_ok binding)
					(and all_ok (visible_outer_binding_domain_safe binding)))
					true)))))
	(define subquery_outer_refs_need_domain_preservation (lambda (query outer_schemas)
		(and
			(not (subquery_outer_refs_are_direct_columns query outer_schemas))
			(subquery_outer_refs_are_domain_columns query outer_schemas))))
	(define raw_subquery_has_non_equality_outer_condition (lambda (query outer_schemas)
		(analysis_cache_get_or_compute "raw-non-eq-outer-condition" query outer_schemas
			(lambda () (match query
	...
	(define unnest_scalar_subselect (lambda (subquery outer_schemas) (begin
		...
		(match (unnest_subselect subquery outer_schemas)
			'(subst tbls) (begin
				(define sq_tables_after (coalesceNil (sq_cache "tables") '()))
				(define captured_scalar_tbls (filter sq_tables_after (lambda (td)
					(not (reduce sq_tables_before (lambda (found prev_td)
						(or found (equal? (serialize prev_td) (serialize td))))
						false)))))
				...
				(if (not (equal? captured_scalar_tbls '()))
					(sq_cache "tables" sq_tables_before))
				(sq_cache "scalar_tables" (merge tbls captured_scalar_tbls (coalesceNil (sq_cache "scalar_tables") '())))
				subst)
			nil)
	)))
	(define materialize_uncorrelated_promise_value (lambda (cache_prefix subquery value_expr error_message) (begin
		...
		(if (not (nil? cached_promise_name))
			...
			(begin
				...
				(sq_cache "parallel_inits" (merge
					(coalesceNil (sq_cache "parallel_inits") '())
					(list
	...
	(define materialize_uncorrelated_direct_promise_value (lambda (cache_prefix subquery direct_expr error_message) (begin
		(define uncorr_promise_cache (coalesceNil (sq_cache "uncorr_promise_bindings") '()))
		(define uncorr_promise_key (concat cache_prefix ":" (serialize subquery)))
		(define cached_promise_name (coalesce (uncorr_promise_cache uncorr_promise_key) nil))
		(if (not (nil? cached_promise_name))
			...
			(begin
				...
				(sq_cache "init" (merge (coalesceNil (sq_cache "init") '())
					(list
						(list (quote set) promise_sym (list (quote newpromise))))))
				(sq_cache "direct_parallel_inits" (merge
					(coalesceNil (sq_cache "direct_parallel_inits") '())
					(list (list promise_sym "once" direct_expr error_message))))
				(sq_cache "uncorr_promise_bindings"
					(set_assoc uncorr_promise_cache uncorr_promise_key promise_name))
				(list promise_sym "value"))))))
	(define materialize_uncorrelated_membership_set_expr (lambda (subquery target_expr) (begin
		(define membership_cache (coalesceNil (sq_cache "uncorr_membership_bindings") '()))
		(define membership_key (concat "membership:" (serialize subquery)))
		(define cached_membership_name (coalesce (membership_cache membership_key) nil))
		(define membership_name (if (not (nil? cached_membership_name))
			cached_membership_name
			(begin
				(define membership_idx (coalesceNil (sq_cache "idx") 0))
				(sq_cache "idx" (+ membership_idx 1))
				(define new_membership_name (concat "__uncorr_membership_" membership_idx))
				(define membership_sym (symbol new_membership_name))
				(define membership_sink_sym (symbol (concat "__uncorr_membership_sink:" membership_idx)))
				(sq_cache "init" (merge (coalesceNil (sq_cache "init") '())
					(list
						(list (quote set) membership_sym (list (quote newsession)))
						(list (quote set) membership_sink_sym
							(list (quote lambda) (list (symbol "row"))
								(list membership_sym
									(list (quote serialize) (list (quote nth) (symbol "row") 1))
									true))))))
				(sq_cache "parallel_inits" (merge
					(coalesceNil (sq_cache "parallel_inits") '())
					(list
						(build_queryplan_term_with_sink subquery
							(list (quote callback) membership_sink_sym)))))
				(sq_cache "uncorr_membership_bindings"
					(set_assoc membership_cache membership_key new_membership_name))
				new_membership_name)))
		(list (quote if)
			(list (quote nil?) target_expr)
			nil
			(list (symbol membership_name) (list (quote serialize) target_expr))))))
	(define uncorrelated_session_aggregate_direct_expr (lambda (subquery value_expr) (match subquery
		'(sub_schema sub_tables _sub_fields sub_condition sub_group sub_having sub_order sub_limit sub_offset)
		(begin
			(define cache_policy (count_subquery_cache_policy subquery value_expr))
			(if (and
				(equal? (count sub_tables) 1)
				(or (nil? sub_group) (equal? sub_group '()) (equal? sub_group '(1))))
				...
				nil))
		_ nil)))
	(define uncorrelated_literal_sum_count_value_expr (lambda (field_name field_expr)
		(if (literal_sum_count_aggregate_expr? field_expr)
			field_expr
			(if (or
				(equal?? field_name "SUM(1)")
				(equal?? field_name "COUNT(1)")
				(equal?? field_name "COUNT(*)"))
	...
	(define materialize_uncorrelated_scalar_subselect (lambda (subquery outer_schemas) (begin
		...
		(match shape_facts
			'(g h _o _l _off _value_expr has_outer outer_refs_are_direct contains_inner_select has_value_agg session_sensitive skip_level)
			(begin
				(define local_aliases (raw_query_local_aliases subquery))
				(define direct_outer_schemas
					(if contains_inner_select
						(match (subquery_resolved_scope subquery outer_schemas)
							'(_ _ _ _ _ resolved_schemas _ _) resolved_schemas
							_ outer_schemas)
						outer_schemas))
				(define replace_independent_inner_selects (and
					has_outer
					contains_inner_select
					(not skip_level)
					(not (raw_query_uses_alias_outside_current subquery local_aliases))
					(not (raw_query_inner_selects_use_aliases subquery local_aliases))))
				(if (or
					(and has_outer (not replace_independent_inner_selects))
					...
					(match subquery
						'(sub_schema _sub_tables sub_fields _sub_condition _sub_group _sub_having _sub_order _sub_limit _sub_offset) (begin
							...
							(if (nil? first_field_name)
								nil
								(begin
									(define direct_subquery (if contains_inner_select
										(match subquery
											'(direct_schema direct_tables direct_fields direct_condition direct_group direct_having direct_order direct_limit direct_offset)
											(list direct_schema
												direct_tables
												(map_assoc direct_fields (lambda (direct_name direct_expr)
													(replace_inner_selects direct_expr direct_outer_schemas)))
												(replace_inner_selects direct_condition direct_outer_schemas)
												(map (coalesceNil direct_group '()) (lambda (direct_group_expr)
													(replace_inner_selects direct_group_expr direct_outer_schemas)))
												(replace_inner_selects direct_having direct_outer_schemas)
												(map (coalesceNil direct_order '()) (lambda (direct_order_item) (match direct_order_item
													'(direct_order_expr direct_order_dir)
													(list (replace_inner_selects direct_order_expr direct_outer_schemas) direct_order_dir)
													_ direct_order_item)))
												direct_limit
												direct_offset)
											_ subquery)
										subquery))
									(define direct_fields (match direct_subquery
										'(_ _ direct_fields_list _ _ _ _ _ _) direct_fields_list
										_ sub_fields))
									(define first_field_expr (uncorrelated_literal_sum_count_value_expr first_field_name (nth direct_fields 1)))
									(define direct_expr (uncorrelated_session_aggregate_direct_expr direct_subquery first_field_expr))
									(if (nil? direct_expr)
										...
										(materialize_uncorrelated_direct_promise_value
											"scalar-direct"
											subquery
											direct_expr
											"scalar subselect returned more than one row")))))
						_ nil))
				_ nil)))))
	(define try_build_scalar_subselect_with_strategy (lambda (subquery outer_schemas) (begin
		(if (or (nil? subquery) (< (count subquery) 1) (not (string? (nth subquery 0))))
			nil
			(begin
	...
	(define build_scalar_subselect_with_strategy (lambda (subquery outer_schemas) (begin
		(if (or (nil? subquery) (< (count subquery) 1) (not (string? (nth subquery 0))))
			nil
			(begin
				(define lowered (try_build_scalar_subselect_with_strategy subquery outer_schemas))
				(if (nil? lowered)
	...
	(define unnest_count_subselect (lambda (subquery outer_schemas target_expr comparison) (begin
		(define resolve_outer (lambda (expr) (match expr
			'((symbol get_column) nil ti col ci) (begin
				(define _resolved (reduce_assoc outer_schemas (lambda (a alias cols)
					(begin
						(define norm_cols (schema_assoc_cols outer_schemas alias))
						(if (reduce norm_cols (lambda (a coldef) (or a ((if ci equal?? equal?) (coldef "Field") col))) false) alias a))) nil))
				(if (nil? _resolved) expr
					(list (quote get_column) _resolved false col false)))
			(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
				expr
				(cons (resolve_outer sym) (map args resolve_outer)))
			expr)))
		(define resolved_target_expr (if (nil? target_expr) nil (resolve_outer target_expr)))
		(define uncorrelated_union_membership_expr (lambda (branches) (begin
			(if (and
				(not (nil? resolved_target_expr))
				(equal?? comparison (quote >))
				(not (subquery_has_outer_refs subquery outer_schemas)))
				(begin
					(define first_cols (match branches
					...
					(if (not (equal? (count first_cols) 1))
						...
						nil)
					(materialize_uncorrelated_membership_set_expr subquery resolved_target_expr))
				nil))))
		/* UNION inside membership predicates is duplicate-insensitive: recurse into
		each branch and skip the top-level UNION DISTINCT materialization step. */
		(define _union_parts (query_union_membership_parts subquery))
		(if (not (nil? _union_parts))
			(match _union_parts '(branches order limit offset)
				(begin
					(define uncorr_union_membership (uncorrelated_union_membership_expr branches))
					(if (not (nil? uncorr_union_membership))
						uncorr_union_membership
						(begin
							(define branch_simple_for_count_membership? (lambda (branch) (match branch
								'(_ b_tables _ _ _ _ b_order b_limit b_offset)
								(and
									(or (nil? b_order) (equal? b_order '()))
									(nil? b_limit)
									(or (nil? b_offset) (equal? b_offset 0))
									(reduce b_tables (lambda (ok td) (and ok (match td
										'(_ tschema ttbl _ _)
										(and
											(string? ttbl)
											(not (materialized-source? ttbl))
							...
							(if (or
								...
								(not (nil? limit))
								(not (nil? offset))
								(not (reduce branches (lambda (ok branch)
									(and ok (branch_simple_for_count_membership? branch)))
									true)))
								(if (and
									(not (nil? resolved_target_expr))
									(equal?? comparison (quote >)))
								...
								(begin
									...
									(if (not (nil? resolved_target_expr))
										...
										(begin
											(define branch_membership_info (lambda (branch) (match branch
												'(s t f c g h o l off)
												(begin
													(define projected_expr (match f
														(cons _ (cons value_expr _)) value_expr
														nil))
													(if (nil? projected_expr)
														nil
														(begin
															(define cond_terms (flatten_and_terms (coalesceNil c true)))
															(define projected_key (serialize projected_expr))
															(define eq_info (reduce cond_terms (lambda (found term)
																(if (not (nil? found))
																	...
																	(match term
																		...
																		(if (or
																			...
																			(equal?? eq_sym (quote equal?))
																			(equal?? eq_sym (symbol equal?)))
																			(if (equal? (serialize lhs) projected_key)
																				(list term rhs)
																				(if (equal? (serialize rhs) projected_key)
																					(list term lhs)
																					nil))
																			nil)
																		_ nil)))
																nil))
															(match eq_info
																'(matched_term target_from_eq)
																(list
																	target_from_eq
																	(list s t f
																		(combine_and_terms (filter cond_terms (lambda (term)
																			(not (equal? term matched_term)))))
																		g h o l off))
																_ nil))))
												_ nil)))
											(define branch_memberships (filter (map branches branch_membership_info)
												(lambda (info) (not (nil? info)))))
											(define use_membership_union (and
												(not (equal? branch_memberships '()))
												(equal? (count branch_memberships) (count branches))
												(reduce branch_memberships (lambda (ok info)
													(and ok (equal? (serialize (car info)) (serialize (car (car branch_memberships))))))
													true)))
											(define branch_exists_expr (lambda (branch) (match branch
												...
												nil)))
											(define branch_results (filter (map branches branch_exists_expr)
												(lambda (r) (not (nil? r)))))
											(if use_membership_union
												(begin
													(define membership_target (car (car branch_memberships)))
													(define membership_results (filter
														(map branch_memberships (lambda (info) (match info
															'(_ branch_subquery)
															(unnest_count_subselect
																branch_subquery
																outer_schemas
															...
															_ nil)))
														(lambda (r) (not (nil? r)))))
													(if (or (equal? membership_results '()) (not (equal? (count membership_results) (count branch_memberships))))
														nil
														(if (equal? 1 (count membership_results)) (car membership_results)
															(cons (if (equal?? comparison (quote >)) (quote or) (quote and)) membership_results))))
												(if (or (equal? branch_results '()) (not (equal? (count branch_results) (count branches))))
													nil
													(if (equal? 1 (count branch_results)) (car branch_results)
														(cons (if (equal?? comparison (quote >)) (quote or) (quote and)) branch_results))))))))))))
			...
			(begin
				...
				(define simple_count_membership_subquery? (lambda (sq) (match sq
					'(_ sq_tables _ _ _ _ sq_order sq_limit sq_offset)
					(and
						(or (nil? sq_order) (equal? sq_order '()))
						(nil? sq_limit)
						(or (nil? sq_offset) (equal? sq_offset 0))
						(reduce sq_tables (lambda (ok td) (and ok (match td
							'(_ tschema ttbl _ _)
							(and
								(string? ttbl)
								(not (materialized-source? ttbl))
				...
				(if (and (nil? target_expr) (not (subquery_has_outer_refs subquery outer_schemas)))
					(begin
						(define count_sq (match subquery
					...
					(if (and (not (nil? target_expr)) (nil? _first_field))
						...
						(if (and
							...
							(equal?? comparison (quote >))
							(not (subquery_has_outer_refs subquery outer_schemas)))
							(materialize_uncorrelated_membership_set_expr subquery target_expr)
							(if (and
								(not (nil? target_expr))
								(equal?? comparison (quote >))
								(not (simple_count_membership_subquery? subquery))
								(equal? (raw_query_nested_outer_refs_for_domain subquery '() outer_schemas) '()))
								(materialize_uncorrelated_membership_set_expr subquery target_expr)
								(begin
									(if (and
										(not (nil? target_expr))
										...
										(begin
											...
											(define count_sq (match subquery
												'(s t f c g h o l off) (list s t
													(list "__cnt" (list (quote aggregate) (count-map-expr-for match_condition) (symbol "+") 0))
													true
											...
											(if (nil? count_sq)
												nil
												(begin
													(define sq_tables_before_count (coalesceNil (sq_cache "tables") '()))
													(define _result (unnest_subselect count_sq outer_schemas))
													(if (nil? _result)
														nil
														(match _result '(_subst _tbls) (begin
															(define sq_tables_after_count (coalesceNil (sq_cache "tables") '()))
															(define captured_count_tbls (filter sq_tables_after_count (lambda (td)
																(not (reduce sq_tables_before_count (lambda (found prev_td)
																	(or found (equal? (serialize prev_td) (serialize td))))
																	false)))))
															(define scalar_count_tbls (merge _tbls captured_count_tbls))
															(define scalar_count_tbl_keys (map scalar_count_tbls serialize))
															(define sq_tables_without_count (filter sq_tables_after_count (lambda (td)
																(not (has? scalar_count_tbl_keys (serialize td))))))
															(if (not (equal? sq_tables_without_count sq_tables_after_count))
																(sq_cache "tables" sq_tables_without_count))
															(define scalar_count_sources_direct (reduce scalar_count_tbls (lambda (ok td)
																(and ok (match td
																	'(_tv tschema ttbl _isOuter _tje)
																	(direct_scalar_scan_source? tschema ttbl)
																	false)))
																true))
															(define scalar_subst_has_nondirect_scan (lambda (scan_expr) (match scan_expr
																'(scalar_scan scan_schema scan_tbl _ _ _ _ _ _ _)
																(not (direct_scalar_scan_source? scan_schema scan_tbl))
																'((symbol scalar_scan) scan_schema scan_tbl _ _ _ _ _ _ _)
																(not (direct_scalar_scan_source? scan_schema scan_tbl))
																'((quote scalar_scan) scan_schema scan_tbl _ _ _ _ _ _ _)
																(not (direct_scalar_scan_source? scan_schema scan_tbl))
																(cons scan_sym scan_args) (if (or (is_quote_scope_sym scan_sym) (not (list? scan_args)))
																	false
																	(or
																		(scalar_subst_has_nondirect_scan scan_sym)
																		(reduce scan_args (lambda (found scan_arg)
																			(or found (scalar_subst_has_nondirect_scan scan_arg)))
																			false)))
																false)))
															(if (and
																(not (nil? target_expr))
																(equal?? comparison (quote >))
																(or
																	(not scalar_count_sources_direct)
																	(scalar_subst_has_nondirect_scan _subst)))
																(materialize_uncorrelated_membership_set_expr subquery target_expr)
																(begin
																	/* COUNT-based EXISTS/IN lowering is an expression:
																	missing helper rows must survive so COALESCE(count,0)
																	can implement per-domain false/anti semantics. */
																	(sq_cache "scalar_tables" (merge
																		(map scalar_count_tbls (lambda (td) (match td
																			'(tv tschema ttbl _isOuter tje)
																			(list tv tschema ttbl true tje)
																			_ td)))
	...
	(define replace_inner_selects (lambda (expr outer_schemas) (match expr
		(cons sym args) (begin
			(define kind (match sym
				...
				'(quote inner_select_exists) (quote inner_select_exists)
				_ nil))
			(define union_exists_expr (lambda (subquery negated) nil))
			(define union_in_expr (lambda (target_expr subquery negated) nil))
			(define union_in_expr_disabled (lambda (target_expr subquery negated) (begin
				(define union_parts (query_union_membership_parts subquery))
				(if (nil? union_parts)
					nil
					(match union_parts '(branches union_order union_limit union_offset)
						(if (or (not (nil? union_order)) (not (nil? union_limit)) (not (nil? union_offset)))
							...
							(begin
								(if (not (reduce branches (lambda (ok branch)
									...
									(error "UNION ALL subquery must project exactly one column for IN")
									nil)
								(define union_branch_simple_for_membership? (lambda (branch) (match branch
									'(_ b_tables _ _ _ _ b_order b_limit b_offset)
									(and
										(or (nil? b_order) (equal? b_order '()))
										(nil? b_limit)
								...
								(define normalize_union_in_branch (lambda (branch)
									(match branch
										...
										branch)))
								(if (not (reduce branches (lambda (ok branch)
									(and ok (union_branch_simple_for_membership? branch)))
									true))
									nil
									(begin
			...
			(define not_expr (if (or
				(equal?? sym (symbol not))
				(equal?? sym (quote not))
				(equal?? sym (quote (quote not))))
				(match args
					(cons inner_expr '()) (match inner_expr
						(cons inner_sym inner_args) (begin
							(define inner_kind (match inner_sym
								(symbol inner_select) (quote inner_select)
								'inner_select (quote inner_select)
								'(quote inner_select) (quote inner_select)
								(symbol inner_select_in) (quote inner_select_in)
							...
							(if (equal?? inner_kind (quote inner_select_in))
								...
								(if (equal?? inner_kind (quote inner_select_exists))
									(match inner_args
										(cons subquery '()) (begin
											(define visible_subquery
												(rewrite_dependent_scalar_visible_outer_refs subquery outer_schemas))
											(coalesce
												(union_exists_expr subquery true)
												(unnest_count_subselect visible_subquery outer_schemas nil (quote equal?))
												(if (dependent_marker_required_subquery subquery outer_schemas)
													(begin
														(define dep_expr
															(build_dependent_scalar_via_marker
					...
					_ nil)
				nil))
			(define parser_semijoin_if_union_expr (lambda (candidate) (match candidate
				(cons or_sym or_args)
				(if (or (equal?? or_sym (quote or)) (equal?? or_sym (symbol or)))
					(begin
						(define positive_count_subquery (lambda (count_expr) (match count_expr
							(cons coalesce_sym (cons inner_expr (cons zero_default '())))
							(if (and
								(equal? zero_default 0)
								(or (equal?? coalesce_sym (quote coalesceNil)) (equal?? coalesce_sym (symbol coalesceNil))))
								(parser_semijoin_count_term_subquery inner_expr)
								nil)
							_ (parser_semijoin_count_term_subquery count_expr))))
						(define positive_count_branch_info (lambda (branch_expr) (match branch_expr
							(cons cmp_sym (cons count_expr (cons zero_expr '())))
							(if (and
								(equal? zero_expr 0)
								(or (equal?? cmp_sym (quote >)) (equal?? cmp_sym (symbol >))))
								(parser_semijoin_count_branch_info
									(positive_count_subquery count_expr)
									outer_schemas)
								nil)
							_ nil)))
						(define branch_info (lambda (branch_expr) (match branch_expr
							(cons outer_if_sym (cons nil_check (cons nil_branch (cons match_if '()))))
							(if (and
								(or (equal?? outer_if_sym (quote if)) (equal?? outer_if_sym (symbol if)))
								(nil? nil_branch))
								(match nil_check
									(cons nil_sym (cons target_expr '()))
									(if (or (equal?? nil_sym (quote nil?)) (equal?? nil_sym (symbol nil?)))
										(match match_if
											(cons inner_if_sym (cons match_expr (cons true_branch _rest)))
											(if (and
												(or (equal?? inner_if_sym (quote if)) (equal?? inner_if_sym (symbol if)))
												(equal? true_branch true))
												(match (parser_semijoin_count_branch_info
													(parser_semijoin_count_term_subquery match_expr)
													outer_schemas)
													'(recovered_target recovered_subquery)
													(if (equal? (serialize recovered_target) (serialize target_expr))
														(list recovered_target recovered_subquery)
														nil)
													_ nil)
												nil)
											_ nil)
									...
									_ nil)
								nil)
							(cons _cmp_sym _cmp_args)
							(positive_count_branch_info branch_expr)
							_ nil)))
						(define infos (filter (map or_args branch_info) (lambda (info) (not (nil? info)))))
						(if (and
							(>= (count infos) 2)
							(equal? (count infos) (count or_args))
							(reduce infos (lambda (ok info)
								(and ok (equal? (serialize (car info)) (serialize (car (car infos))))))
								true))
							(cons (quote or)
								(map infos (lambda (info) (match info
									'(target_expr branch_subquery)
									(list (quote inner_select_in) target_expr branch_subquery)
									_ nil))))
							nil))
					nil)
				_ nil)))
			(define semijoin_if_union_expr
				(parser_semijoin_if_union_expr expr))
			(define semijoin_union_expr
				(rewrite_parser_semijoin_union_count_expr expr outer_schemas))
			(if (not (nil? semijoin_if_union_expr))
				(replace_inner_selects semijoin_if_union_expr outer_schemas)
				(if (not (nil? semijoin_union_expr))
					(replace_inner_selects semijoin_union_expr outer_schemas)
					(begin
						(define dependent_not_in_expr
							(rewrite_dependent_parser_not_in_count_expr expr outer_schemas))
						(if (not (nil? dependent_not_in_expr))
							dependent_not_in_expr
							(if (nil? not_expr)
								(match kind
									(quote inner_select) (match args
										(cons subquery '())
										(begin
											...
											(define replace_independent_inner_selects (and
												(contains_inner_select_marker subquery)
												(subquery_has_outer_refs subquery outer_schemas)
												(not (raw_query_contains_skip_level_nested_outer_ref subquery local_aliases))
												(not (raw_query_uses_alias_outside_current subquery local_aliases))
												(not (raw_query_inner_selects_use_aliases subquery local_aliases))))
											(coalesce
												(if (scalar_inner_select_is_presence_query subquery)
													(replace_inner_selects
														(list (quote inner_select_exists) subquery)
														outer_schemas)
													nil)
												(scalar_independent_limited_subquery_expr subquery outer_schemas)
												(if (or
													(dependent_marker_required_subquery subquery outer_schemas)
													(and
														(contains_inner_select_marker subquery)
														(subquery_has_outer_refs subquery outer_schemas)
														(not replace_independent_inner_selects)))
													(build_dependent_scalar_via_marker
														(quote inner_select)
														subquery
														nil
									...
									(quote inner_select_in) (match args
										...
										(coalesce
											...
											(if (and
												(not (nil? (query_union_membership_parts subquery)))
												(not (subquery_has_outer_refs subquery outer_schemas)))
									...
									(quote inner_select_exists) (match args
										...
										(coalesce
											...
											(build_simple_exists_scalar_scan subquery false)
											(unnest_count_subselect subquery outer_schemas nil (quote >))
	...
	(define nil_test_of_inner_select (lambda (expr) (match expr
		...
		(and
			...
			(match inner_expr
				...
				_ false))
		_ false)))
	(define collect_dependent_scalar_compile_markers (lambda (expr outer_schemas) (begin
		(define semijoin_union_expr
			(rewrite_parser_semijoin_union_count_expr expr outer_schemas))
		(if (not (nil? semijoin_union_expr))
			(collect_dependent_scalar_compile_markers semijoin_union_expr outer_schemas)
					...
					(cons sym args) (begin
						...
						(if (equal?? kind (quote inner_select))
							...
							(if (nil? kind)
								...
								(replace_inner_selects expr outer_schemas))))
					_ expr))))))
	(define collect_row_domain_subselects (lambda (expr outer_schemas)
		(match expr
			(cons sym args) (begin
				(define kind (subquery_marker_kind sym))
				(if (nil? kind)
					(if (or (is_opaque_scope_sym sym) (not (list? args)))
						expr
						(cons sym (map args (lambda (arg)
							(collect_row_domain_subselects arg outer_schemas)))))
					(replace_inner_selects expr outer_schemas)))
			_ expr)))
	(define collect_dependent_field_compile_markers (lambda (expr outer_schemas)
		(collect_dependent_scalar_compile_markers expr outer_schemas)))
		...
		(match expr
			(cons sym args) (begin
				...
				(if (nil? dep_id)
					...
					(begin
						(define dep_entry (dep_scalar_cache dep_id))
						(define dep_kind (match dep_entry
							'(kind _subquery _target_expr) kind
							'(kind _subquery _target_expr _negated) kind
							_ (quote inner_select)))
						(define dep_target_expr (match dep_entry
							'(_kind _subquery target_expr) target_expr
							'(_kind _subquery target_expr _negated) target_expr
							_ nil))
						(define dep_negated (match dep_entry
							'(_kind _subquery _target_expr negated) negated
							_ false))
						(define subquery (match dep_entry
							'(_kind dep_subquery _target_expr) dep_subquery
							'(_kind dep_subquery _target_expr _negated) dep_subquery
							_ dep_entry))
						(if (nil? subquery)
							(error (concat "DEPENDENT_SCALAR_NIL_SUBQUERY "
								(serialize (list dep_id expr outer_schemas))))
							nil)
						...
						(define dep_nested_outer_refs
							(raw_query_nested_outer_refs_for_domain
								subquery
								(raw_query_local_aliases subquery)
								dep_binding_schemas))
						(define dep_outer_binding_refs (merge_unique
							dep_outer_refs
							dep_nested_outer_refs
							(map (collect_all_outer_column_refs normalized_subquery) (lambda (ref_pair) (match ref_pair
								'(alias col) (concat alias "." col)
								_ nil)))
						...
						(define dep_outer_bindings (filter
							(map dep_outer_binding_refs (lambda (ref)
								(outer_ref_visible_binding dep_binding_schemas ref)))
							(lambda (binding) (not (nil? binding)))))
						(define dep_domain_outer_refs dep_outer_binding_refs)
						...
						(define dep_info (make_dependent_expr_compile_info
							expr
							bound_subquery
							dep_domain_outer_refs
							dep_outer_bindings
							dep_kind
							dep_negated
							dep_target_expr
							(and
								(equal?? dep_kind (quote inner_select))
								(subquery_outer_refs_are_direct_columns normalized_subquery outer_schemas)
								(not (raw_subquery_has_non_equality_outer_condition normalized_subquery outer_schemas)))
							(query_union_membership_parts normalized_subquery)
							(expr_uses_session_state normalized_subquery)))
						(define dep_alias (concat
							"dep_scalar_"
							dep_id
							"_"
							(substr
								(fnv_hash (serialize (list dep_kind bound_subquery dep_outer_binding_refs dep_target_expr dep_negated)))
								0
								8)))
						(define dep_result_col "__result")
						(define dep_domain_cols (dependent_join_helper_normalize_domain_cols
							(dependent_join_helper_domain_cols_from_info dep_info dep_alias)))
						(define dep_unnesting_info (make_dependent_expr_unnesting_info_with_domain_cols dep_info dep_domain_cols))
						(if (and
							(equal?? dep_kind (quote inner_select))
							(equal? dep_outer_refs '())
							(not (contains_inner_select_marker normalized_subquery)))
							(coalesce
								(build_scalar_subselect normalized_subquery outer_schemas)
								(replace_inner_selects (list (quote inner_select) normalized_subquery) outer_schemas)
								expr)
							(begin
								(define dep_shape_facts
									(if (equal?? dep_kind (quote inner_select))
										(scalar_subselect_shape_facts normalized_subquery outer_schemas)
										nil))
								(define dep_has_value_agg (match dep_shape_facts
									'(_ _ _ _ _ _ _ _ _ has_value_agg _ _) has_value_agg
									_ false))
								(define dep_contains_inner_select (match dep_shape_facts
									'(_ _ _ _ _ _ _ _ contains_inner_select _ _ _) contains_inner_select
									_ false))
								...
								(define dep_unnest_result
									(if (or
										...
										(equal? dep_outer_refs '())
										dep_has_value_agg
										dep_contains_inner_select
										(not (dependent_expr_compile_info_simple_candidate dep_info)))
										nil
									...
									_ (begin
										(define dep_simple_source_query
											(if (equal?? dep_kind (quote inner_select_exists))
												(dependent_join_helper_build_simple_exists_source_query
										...
										(if (and (nil? dep_simple_source_query) (nil? dep_materialized_source_query))
											...
											(begin
													...
													(if (or (equal?? dep_kind (quote inner_select_exists))
														...
														(if dep_negated
															...
															(list
																...
																(list (quote get_column) dep_alias false dep_result_col false)))
														(begin
															(define dep_base_result_expr
																(list (quote get_column) dep_alias false dep_result_col false))
															(if (dependent_join_helper_scalar_presence_value? dep_info)
												...
												dep_result_expr)))))))))
			_ expr)))
	(define make_dependent_expr_compile_info (lambda (expr subquery outer_refs outer_bindings kind negated target_expr simple_candidate union_parts uses_session_state)
		(list expr subquery outer_refs outer_bindings kind negated target_expr simple_candidate union_parts uses_session_state)))
	(define dependent_expr_compile_info_expr (lambda (info)
		(match info
			'(expr _subquery _outer_refs _outer_bindings _kind _negated _target_expr _simple_candidate _union_parts _uses_session_state) expr
			_ nil)))
	(define dependent_expr_compile_info_subquery (lambda (info)
		(match info
			'(_expr subquery _outer_refs _outer_bindings _kind _negated _target_expr _simple_candidate _union_parts _uses_session_state) subquery
			_ nil)))
	(define dependent_expr_compile_info_outer_refs (lambda (info)
		(match info
			'(_expr _subquery outer_refs _outer_bindings _kind _negated _target_expr _simple_candidate _union_parts _uses_session_state) outer_refs
			_ '())))
	(define dependent_expr_compile_info_outer_bindings (lambda (info)
		(match info
			'(_expr _subquery _outer_refs outer_bindings _kind _negated _target_expr _simple_candidate _union_parts _uses_session_state) outer_bindings
			_ '())))
	(define dependent_expr_compile_info_kind (lambda (info)
		(match info
			'(_expr _subquery _outer_refs _outer_bindings kind _negated _target_expr _simple_candidate _union_parts _uses_session_state) kind
			_ nil)))
	(define dependent_expr_compile_info_negated (lambda (info)
		(match info
			'(_expr _subquery _outer_refs _outer_bindings _kind negated _target_expr _simple_candidate _union_parts _uses_session_state) negated
			_ false)))
	(define dependent_expr_compile_info_target_expr (lambda (info)
		(match info
			'(_expr _subquery _outer_refs _outer_bindings _kind _negated target_expr _simple_candidate _union_parts _uses_session_state) target_expr
			_ '())))
	(define dependent_expr_compile_info_simple_candidate (lambda (info)
		(match info
			'(_expr _subquery _outer_refs _outer_bindings _kind _negated _target_expr simple_candidate _union_parts _uses_session_state) simple_candidate
			_ false)))
	(define dependent_expr_compile_info_union_parts (lambda (info)
		(match info
			'(_expr _subquery _outer_refs _outer_bindings _kind _negated _target_expr _simple_candidate union_parts _uses_session_state) union_parts
			_ nil)))
	(define dependent_expr_compile_info_uses_session_state (lambda (info)
		(match info
			'(_expr _subquery _outer_refs _outer_bindings _kind _negated _target_expr _simple_candidate _union_parts uses_session_state) uses_session_state
			_ false)))
	...
	(define dependent_join_helper_domain_col_name (lambda (idx namespace)
		/* Domain columns are visible while nested dependent helpers are flattened.
		Keep them helper-scoped so two nested no-FROM scalar helpers do not both
		export an unqualified __dep_d0 and trip ambiguous column resolution. */
		(if (or (nil? namespace) (equal? namespace ""))
			(concat "__dep_d" idx)
			(concat "__dep_d_" (fnv_hash namespace) "_" idx))))
	(define dependent_join_helper_domain_col_name? (lambda (col)
		(and (string? col)
			(>= (strlen col) (strlen "__dep_d"))
			(equal? (substr col 0 (strlen "__dep_d")) "__dep_d"))))
	...
	(define dependent_join_helper_domain_ref_to_expr (lambda (ref)
		(if (not (nil? (session_read_expr ref)))
			ref
			(match (split ref ".")
				(list alias col) (if (not (nil? (semijoin_outer_marker_alias alias)))
					nil
					(list (quote get_column) alias false col false))
				_ nil))))
	(define dependent_join_helper_domain_expr_needed? (lambda (expr)
		(or
			(not (equal? (extract_tblvars expr) '()))
			(expr_uses_session_state expr))))
	(define dependent_join_helper_domain_cols_from_info (lambda (dep_info namespace) (begin
		(define dep_domain_exprs
			(filter
				(merge_unique
					(map (dependent_expr_compile_info_outer_bindings dep_info) visible_outer_binding_source_expr)
					(filter
						(map (dependent_expr_compile_info_outer_refs dep_info)
							dependent_join_helper_domain_ref_to_expr)
						(lambda (expr) (not (nil? expr))))
					(extract_session_read_refs (dependent_expr_compile_info_expr dep_info))
					(extract_session_read_refs (dependent_expr_compile_info_subquery dep_info))
					(if (nil? (dependent_expr_compile_info_target_expr dep_info))
						'()
						(list (dependent_expr_compile_info_target_expr dep_info)))
					(collect_all_outer_ref_exprs (dependent_expr_compile_info_expr dep_info))
					(collect_all_outer_ref_exprs (dependent_expr_compile_info_subquery dep_info))
					(if (nil? (dependent_expr_compile_info_target_expr dep_info))
						'()
						(collect_all_outer_ref_exprs
							(dependent_expr_compile_info_target_expr dep_info)))
					(extract_semijoin_outer_target_exprs (dependent_expr_compile_info_expr dep_info))
					(extract_semijoin_outer_target_exprs (dependent_expr_compile_info_subquery dep_info)))
				dependent_join_helper_domain_expr_needed?))
		(match (reduce dep_domain_exprs (lambda (state ref_expr) (match state
			'(idx acc)
			(list
				(+ idx 1)
				(merge acc
					(list (list
						(dependent_join_helper_domain_col_name idx namespace)
						ref_expr))))
			_ state))
			(list 0 '()))
			'(_ acc) acc
			_ '()))))
	(define dependent_join_helper_normalize_domain_cols (lambda (domain_cols)
		(filter (coalesceNil domain_cols '()) (lambda (dc) (match dc
			'(dom_col dom_expr) (not (nil? dom_expr))
			_ false)))))
	(define dependent_join_helper_domain_field_assoc (lambda (domain_cols source_alias)
		(reduce (dependent_join_helper_normalize_domain_cols domain_cols) (lambda (acc dc) (match dc
			'(dom_col _dom_expr)
			(merge acc
				(list
					dom_col
	...
	(define dependent_join_helper_domain_group_exprs (lambda (domain_cols source_alias)
		(reduce (dependent_join_helper_normalize_domain_cols domain_cols) (lambda (acc dc) (match dc
			'(dom_col _dom_expr)
			(merge acc
				(list (list (quote get_column) source_alias false dom_col false)))
			_ acc))
			'())))
	...
	(define make_dependent_expr_unnesting_info_with_domain_cols (lambda (dep_info domain_cols) (begin
		...
		(list
			...
			norm_domain_cols
			nil
			(filter (map norm_domain_cols (lambda (dc) (match dc
				'(dom_col dom_expr)
				(list dom_expr
	...
	(define dependent_join_helper_state_from_source_query (lambda (source_query unnesting_info) (match source_query
		...
		_ unnesting_info)))
	(define dependent_join_helper_outer_join_operand (lambda (expr) (match expr
		'((quote get_column) alias_ _ col _)
		(if (nil? alias_)
			expr
			(if (and (string? alias_) (strlike alias_ "%\0%"))
				(list (quote get_column) alias_ false col false)
				(list (quote outer) (symbol (concat alias_ "." col)))))
		'((symbol get_column) alias_ _ col _)
		(if (nil? alias_)
			...
			(map domain_cols (lambda (dc) (match dc
				...
				(if (nil? dom_expr)
					...
					(dependent_join_helper_null_safe_equal_expr
						...
						(dependent_join_helper_outer_join_operand dom_expr)))
				_ nil)))
			(lambda (x) (not (nil? x)))))
		(if (equal? join_parts '())
			true
	...
	(define dependent_join_helper_domain_self_joinexpr (lambda (helper_alias domain_cols) (begin
		(define domain_alias (dependent_join_helper_domain_source_alias))
		(define join_parts (filter
			(map domain_cols (lambda (dc) (match dc
				'(dom_col _dom_expr)
		...
		(if (equal? join_parts '())
			...
			(if (equal? (count join_parts) 1)
				(car join_parts)
				(cons (quote and) join_parts))))))
	(define dependent_join_helper_rewrite_expr_for_domain_source (lambda (dep_info expr domain_cols) (begin
		(define domain_alias (dependent_join_helper_domain_source_alias))
		(define dep_nested_outer_schemas
			(match (subquery_resolved_scope
				(dependent_expr_compile_info_subquery dep_info)
				outer_schemas_chain)
				'(_ _ _ _ _ resolved_schemas _ _) resolved_schemas
				_ outer_schemas_chain))
		(define rewrite_independent_inner_selects (lambda (node) (match node
			(cons sym args)
			(begin
				(define kind (inner_select_kind sym))
				(define subquery_has_local_ordering (lambda (query) (match query
					'(_ _ _ _ _ _ q_order q_limit q_offset)
					(or
						(not (equal? (coalesceNil q_order '()) '()))
						(not (nil? q_limit))
						(not (nil? q_offset)))
					false)))
				(if (nil? kind)
					(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						node
						(cons (rewrite_independent_inner_selects sym)
							(map args rewrite_independent_inner_selects)))
					(match kind
						(quote inner_select)
						(match args
							(cons subquery '())
							(if (dependent_marker_required_subquery subquery dep_nested_outer_schemas)
								(list sym
									(dependent_join_helper_rewrite_query_for_domain_source
										dep_info
										subquery
										domain_cols))
								(replace_inner_selects node dep_nested_outer_schemas))
							_ node)
						(quote inner_select_exists)
						(match args
							(cons subquery '())
						...
						(match args
							...
							(if (dependent_marker_required_subquery subquery dep_nested_outer_schemas)
								(list sym
									(rewrite_independent_inner_selects target_expr)
									(dependent_join_helper_rewrite_query_for_domain_source
										dep_info
										subquery
										domain_cols))
								(replace_inner_selects
									(list sym (rewrite_independent_inner_selects target_expr) subquery)
									dep_nested_outer_schemas))
							_ node)
						node)))
			_ node)))
		(define ref_to_domain_col (lambda (ref_expr)
			(begin
				(define normalized_ref_expr (normalize_outer_column_ref_expr ref_expr))
				(if (not (dependent_join_helper_domain_expr_needed? normalized_ref_expr))
					nil
					(reduce domain_cols (lambda (found dc) (match dc
						'(dom_col dom_expr) (if (not (nil? found))
							found
							(begin
								(define normalized_dom_expr (normalize_outer_column_ref_expr dom_expr))
								(if (and
									(dependent_join_helper_domain_expr_needed? normalized_dom_expr)
									(or
										(equal? (semijoin_unmark_outer_expr normalized_dom_expr) (semijoin_unmark_outer_expr normalized_ref_expr))
										(match normalized_ref_expr
											'((quote get_column) alias_ _ col _)
											(and
												(not (nil? alias_))
												(string? col)
											...
											_ false)))
									dom_col
									(match normalized_dom_expr
										'((quote get_column) alias_ _ col _) (match normalized_ref_expr
											...
											_ nil)
										'((symbol get_column) alias_ _ col _) (match normalized_ref_expr
											'((quote get_column) ref_alias _ ref_col _) (if (and (equal? alias_ ref_alias) (equal? col ref_col)) dom_col nil)
											'((symbol get_column) ref_alias _ ref_col _) (if (and (equal? alias_ ref_alias) (equal? col ref_col)) dom_col nil)
											_ nil)
										_ nil))))
						_ found))
						nil)))))
		(define visible_ref_to_domain_col (lambda (alias col)
			(reduce (dependent_expr_compile_info_outer_bindings dep_info) (lambda (found binding)
				(if (not (nil? found))
					found
					(match (split (visible_outer_binding_ref binding) ".")
						(list visible_alias visible_col)
						(if (and (equal?? visible_alias alias) (equal?? visible_col col))
							(ref_to_domain_col (visible_outer_binding_source_expr binding))
							nil)
						_ nil)))
				nil)))
		(define rewrite_expr (lambda (e) (match e
			'((quote get_column) alias_ ti col ci) (begin
				(define mapped_col (if (nil? alias_)
					nil
					(coalesce
						(ref_to_domain_col e)
						(visible_ref_to_domain_col alias_ col))))
				(if (nil? mapped_col)
					e
			...
			(cons sym (cons _ '())) (if (not (nil? (session_read_expr e)))
				(begin
					...
					(if (nil? mapped_col)
						e
						(list (quote get_column) domain_alias false mapped_col false)))
				(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym))
					e
			...
			(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
				...
				(cons (rewrite_expr sym) (map args rewrite_expr)))
			_ e)))
		(rewrite_expr (rewrite_independent_inner_selects expr)))))
	(define dependent_join_helper_build_source_query (lambda (dep_info result_col domain_cols source_result_expr) (begin
		...
		(match dep_subquery
			...
			(begin
				(define pulled_semijoin
					(dependent_join_helper_pull_simple_semijoins dep_tables dep_condition))
				(define effective_dep_tables (match pulled_semijoin
					'(tables_acc _terms_acc) tables_acc
					_ dep_tables))
				(define effective_dep_condition (match pulled_semijoin
					'(_tables_acc terms_acc) (combine_and_terms terms_acc)
					_ dep_condition))
				(list dep_schema
					(merge
						(list (list domain_alias dep_schema (make_dependent_domain_table_spec norm_domain_cols) false nil))
						(map effective_dep_tables (lambda (td) (match td
							'(tv tschema ttbl toisOuter tje)
							(list tv tschema ttbl toisOuter
								(if (nil? tje) nil
					...
					(dependent_join_helper_rewrite_expr_for_domain_source dep_info effective_dep_condition domain_cols)
					'()
					nil
					'()
					nil
	...
	(define dependent_join_helper_build_source_query_with_extra_condition (lambda (dep_info result_col domain_cols source_result_expr extra_condition) (begin
		...
		(match dep_subquery
			'(dep_schema dep_tables dep_fields dep_condition dep_group dep_having dep_order dep_limit dep_offset)
			(begin
				(define raw_combined_condition
					(combine_and_terms (filter
						(list dep_condition extra_condition)
						(lambda (term) (and (not (nil? term)) (not (equal? term true)))))))
				(define pulled_semijoin
					(dependent_join_helper_pull_simple_semijoins dep_tables raw_combined_condition))
				(define effective_dep_condition (match pulled_semijoin
					'(_tables_acc terms_acc) (combine_and_terms terms_acc)
					_ raw_combined_condition))
				(define rewritten_condition
					(dependent_join_helper_rewrite_expr_for_domain_source dep_info effective_dep_condition domain_cols))
				(define effective_dep_tables (match pulled_semijoin
					'(tables_acc _terms_acc) tables_acc
				...
				(list dep_schema
					...
					(map (coalesceNil dep_group '()) (lambda (expr)
						(dependent_join_helper_rewrite_expr_for_domain_source dep_info expr domain_cols)))
					(dependent_join_helper_rewrite_expr_for_domain_source dep_info dep_having domain_cols)
					(map (coalesceNil dep_order '()) (lambda (oi) (match oi
						'(col dir) (list (dependent_join_helper_rewrite_expr_for_domain_source dep_info col domain_cols) dir)
						_ oi)))
					dep_limit dep_offset))
			_ nil))))
	(define dependent_join_helper_match_source_alias (lambda (dep_info)
		(concat "__dep_match_" (fnv_hash (serialize (list
			(dependent_expr_compile_info_kind dep_info)
			(dependent_expr_compile_info_target_expr dep_info)
			(dependent_expr_compile_info_subquery dep_info)))))))
	(define dependent_join_helper_rewrite_query_for_domain_source (lambda (dep_info query domain_cols) (match query
		'(q_schema q_tables q_fields q_condition q_group q_having q_order q_limit q_offset)
		(list q_schema
			(map q_tables (lambda (td) (match td
				'(tv tschema ttbl toisOuter tje)
				(list tv tschema ttbl toisOuter
					(if (nil? tje)
						nil
						(dependent_join_helper_rewrite_expr_for_domain_source dep_info tje domain_cols)))
				_ td)))
			(map_assoc q_fields (lambda (_k v)
				(dependent_join_helper_rewrite_expr_for_domain_source dep_info v domain_cols)))
			(dependent_join_helper_rewrite_expr_for_domain_source dep_info q_condition domain_cols)
			(map (coalesceNil q_group '()) (lambda (expr)
				(dependent_join_helper_rewrite_expr_for_domain_source dep_info expr domain_cols)))
			(dependent_join_helper_rewrite_expr_for_domain_source dep_info q_having domain_cols)
			(map (coalesceNil q_order '()) (lambda (oi) (match oi
				'(col dir) (list (dependent_join_helper_rewrite_expr_for_domain_source dep_info col domain_cols) dir)
				_ oi)))
			q_limit
			q_offset)
		_ query)))
	(define dependent_join_helper_build_direct_exists_source_query (lambda (dep_info result_col domain_cols) (begin
		(define dep_subquery (dependent_expr_compile_info_subquery dep_info))
		(define domain_alias (dependent_join_helper_domain_source_alias))
		(define norm_domain_cols (dependent_join_helper_normalize_domain_cols domain_cols))
		(define rewritten_subquery
			(dependent_join_helper_rewrite_query_for_domain_source dep_info dep_subquery domain_cols))
		(define exists_expr
			(build_simple_exists_scalar_scan
				rewritten_subquery
				(dependent_expr_compile_info_negated dep_info)))
		(if (nil? exists_expr)
			nil
			(match dep_subquery
				'(dep_schema _ _ _ _ _ _ _ _)
				(list dep_schema
					(list (list domain_alias dep_schema (make_dependent_domain_table_spec norm_domain_cols) false nil))
					(merge
						(dependent_join_helper_domain_field_assoc norm_domain_cols domain_alias)
						(list result_col exists_expr))
					true
					'()
					nil
					'()
					...
					nil)
				_ nil)))))
	(define dependent_join_helper_wrap_grouped_presence_source_query (lambda (source_alias source_query result_col domain_cols) (begin
		(define norm_domain_cols (dependent_join_helper_normalize_domain_cols domain_cols))
		(define presence_result_expr
			(list (quote get_column) source_alias false result_col false))
		(match source_query
			...
			(list src_schema
				...
				(merge
					...
					(list result_col presence_result_expr))
				true
				'()
				nil
				'()
				...
				nil)
			_ source_query))))
	(define dependent_join_helper_wrap_full_presence_source_query (lambda (source_alias source_query result_col domain_cols) (begin
		(define domain_alias (dependent_join_helper_domain_source_alias))
		(define norm_domain_cols (dependent_join_helper_normalize_domain_cols domain_cols))
		(match source_query
			'(src_schema _ _ _ _ _ _ _ _)
			(list src_schema
				(list
					(list domain_alias src_schema (make_dependent_domain_table_spec norm_domain_cols) false nil)
					(list source_alias src_schema source_query true
						(dependent_join_helper_domain_self_joinexpr source_alias norm_domain_cols)))
				(merge
					(dependent_join_helper_domain_field_assoc norm_domain_cols domain_alias)
					(list result_col
						(list (quote if)
	...
	(define dependent_join_helper_build_simple_exists_source_query (lambda (dep_info result_col domain_cols)
		(begin
			(define source_alias (dependent_join_helper_match_source_alias dep_info))
			(define match_alias (concat source_alias "_match"))
			(define exists_source_query
				(dependent_join_helper_build_source_query dep_info result_col domain_cols 1))
			(dependent_join_helper_wrap_full_presence_source_query
				source_alias
	...
	(define dependent_join_helper_build_simple_in_source_query (lambda (dep_info result_col domain_cols) (begin
		...
		(define rewritten_target_expr
			(dependent_join_helper_rewrite_expr_for_domain_source dep_info target_expr domain_cols))
		(match dep_subquery
			'(_ _ dep_fields _ _ _ _ _ _)
			(begin
					...
					(dependent_join_helper_wrap_full_presence_source_query
						...
						(dependent_join_helper_wrap_grouped_presence_source_query
							...
							(dependent_join_helper_build_source_query_with_extra_condition
								dep_info result_col domain_cols 1
								(list (quote equal??)
						...
						domain_cols)))
			_ nil))))
	(define dependent_join_helper_table_aliases (lambda (tables_)
		(filter (map (coalesceNil tables_ '()) (lambda (td) (match td
			'(alias _ _ _ _) alias
			_ nil)))
			(lambda (alias) (not (nil? alias))))))
	(define dependent_join_helper_aliases_disjoint (lambda (left_aliases right_aliases)
		(reduce right_aliases (lambda (ok alias_)
			(and ok (not (alias_in_list left_aliases alias_))))
			true)))
	(define dependent_join_helper_positive_count_subquery (lambda (expr) (match expr
		(cons cmp_sym (cons count_expr (cons zero_expr '())))
		(if (and
			(equal? zero_expr 0)
			(or (equal?? cmp_sym (quote >)) (equal?? cmp_sym (symbol >))))
			(match count_expr
				(cons coalesce_sym (cons inner_count_expr (cons zero_default '())))
				(if (and
					(equal? zero_default 0)
					(or (equal?? coalesce_sym (quote coalesceNil)) (equal?? coalesce_sym (symbol coalesceNil))))
					(parser_semijoin_count_term_subquery inner_count_expr)
					(parser_semijoin_count_term_subquery count_expr))
				_ (parser_semijoin_count_term_subquery count_expr))
			nil)
		_ nil)))
	(define dependent_join_helper_parser_in_match_subquery (lambda (expr) (match expr
		(cons if_sym (cons nil_check (cons nil_branch (cons match_branch '()))))
		(if (and
			(or (equal?? if_sym (quote if)) (equal?? if_sym (symbol if)))
			(nil? nil_branch)
			(match nil_check
				(cons nil_sym (cons _target '()))
				(or (equal?? nil_sym (quote nil?)) (equal?? nil_sym (symbol nil?)))
				_ false))
			(match match_branch
				(cons inner_if_sym (cons match_expr (cons true_branch _rest)))
				(if (and
					(or (equal?? inner_if_sym (quote if)) (equal?? inner_if_sym (symbol if)))
					(equal? true_branch true))
					(dependent_join_helper_positive_count_subquery match_expr)
					nil)
				_ nil)
			nil)
		_ nil)))
	(define dependent_join_helper_count_subquery_join_parts (lambda (count_subquery current_tables) (match count_subquery
		'(sj_schema sj_tables _sj_fields sj_condition sj_group sj_having sj_order sj_limit sj_offset)
		(begin
			(define current_aliases (dependent_join_helper_table_aliases current_tables))
			(define sj_aliases (dependent_join_helper_table_aliases sj_tables))
			(define simple_visible_aliases (merge_unique current_aliases sj_aliases))
			(define sj_condition_scope_local
				(reduce (qpir-expr-column-refs (coalesceNil sj_condition true))
					(lambda (ok ref) (match ref
						'(tv _col) (and ok
							(or (nil? tv) (has? simple_visible_aliases tv)))
						ok))
					true))
			(if (and
				sj_condition_scope_local
				(not (equal? sj_tables '()))
				(dependent_join_helper_aliases_disjoint current_aliases sj_aliases)
				(or (nil? sj_group) (equal? sj_group '()))
				(or (nil? sj_having) (equal? sj_having true))
				(or (nil? sj_order) (equal? sj_order '()))
				(nil? sj_limit)
				(or (nil? sj_offset) (equal? sj_offset 0)))
				(list sj_tables (coalesceNil sj_condition true))
				nil))
		_ nil)))
	(define dependent_join_helper_simple_semijoin_join_parts (lambda (term current_tables) (match term
		(cons sym args)
		(begin
			(define kind (inner_select_kind sym))
			(if (or (equal?? kind (quote inner_select_in)) (equal?? kind (quote inner_select_exists)))
				(begin
					(define sj_target (if (equal?? kind (quote inner_select_in))
						(match args (cons target_expr (cons _subquery '())) target_expr nil)
						nil))
					(define sj_subquery (if (equal?? kind (quote inner_select_in))
						(match args (cons _target_expr (cons subquery '())) subquery nil)
						(match args (cons subquery '()) subquery nil)))
					(match sj_subquery
						'(sj_schema sj_tables sj_fields sj_condition sj_group sj_having sj_order sj_limit sj_offset)
						(begin
							(define first_field_expr (match sj_fields
								(cons _ (cons v _)) v
								nil))
							(define current_aliases (dependent_join_helper_table_aliases current_tables))
							(define sj_aliases (dependent_join_helper_table_aliases sj_tables))
							...
							(if (and
								...
								(or
									(nil? sj_limit)
									(and (equal?? kind (quote inner_select_exists))
										(<= sj_limit 1)))
								(or (nil? sj_offset) (equal? sj_offset 0))
								(or (equal?? kind (quote inner_select_exists)) (not (nil? first_field_expr))))
								(list
									sj_tables
									(if (equal?? kind (quote inner_select_in))
										(combine_and_terms (list
											(coalesceNil sj_condition true)
											(list (quote equal??) first_field_expr sj_target)))
										(coalesceNil sj_condition true)))
								nil))
						_ nil))
				(dependent_join_helper_count_subquery_join_parts
					(coalesce
						(dependent_join_helper_parser_in_match_subquery term)
						(dependent_join_helper_positive_count_subquery term))
					current_tables)))
		_ nil)))
	(define dependent_join_helper_localize_outer_expr (lambda (expr local_aliases) (match expr
		'((quote outer) outer_sym) (match (split (string outer_sym) ".")
			(list alias_ col) (if (alias_in_list local_aliases alias_)
				(list (quote get_column) alias_ false col false)
				expr)
			_ expr)
		'((symbol outer) outer_sym) (match (split (string outer_sym) ".")
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			expr
			(cons (dependent_join_helper_localize_outer_expr sym local_aliases)
				(map args (lambda (arg)
					(dependent_join_helper_localize_outer_expr arg local_aliases)))))
		_ expr)))
	(define dependent_join_helper_nil_test_expr (lambda (expr) (match expr
		(cons sym (cons inner_expr '()))
		(if (or
			(equal?? sym (quote nil?))
			(equal?? sym (symbol nil?))
			(equal?? sym (quote (quote nil?))))
			inner_expr
			nil)
		_ nil)))
	(define dependent_join_helper_not_nil_test_expr (lambda (expr) (match expr
		(cons sym (cons inner_expr '()))
		(if (or
			(equal?? sym (quote not))
			(equal?? sym (symbol not))
			(equal?? sym (quote (quote not))))
			(dependent_join_helper_nil_test_expr inner_expr)
			nil)
		_ nil)))
	(define dependent_join_helper_simple_scalar_bool_join_parts (lambda (term current_tables) (begin
		(define scalar_expr (match term
			(cons sym args) (if (equal?? (inner_select_kind sym) (quote inner_select))
				term
				(dependent_join_helper_not_nil_test_expr term))
			_ nil))
		(if (nil? scalar_expr)
			nil
			(match scalar_expr
				(cons scalar_sym scalar_args)
				(if (equal?? (inner_select_kind scalar_sym) (quote inner_select))
					(match scalar_args
						...
						(match scalar_subquery
							...
							(begin
								...
								(define current_aliases (dependent_join_helper_table_aliases current_tables))
								(define sj_aliases (dependent_join_helper_table_aliases sj_tables))
								(define local_aliases (merge_unique current_aliases sj_aliases))
								(define is_not_nil_context (not (equal? scalar_expr term)))
								(if (and
									(not (nil? first_field_expr))
									(not (equal? sj_tables '()))
									(dependent_join_helper_aliases_disjoint current_aliases sj_aliases)
									(or (nil? sj_group) (equal? sj_group '()))
									(or (nil? sj_having) (equal? sj_having true))
									(or (nil? sj_order) (equal? sj_order '()))
									(or (nil? sj_limit) (equal? sj_limit 1))
									(or (nil? sj_offset) (equal? sj_offset 0))
									(equal? (extract_aggregates first_field_expr) '())
									(not (contains_inner_select_marker first_field_expr))
									(not (contains_inner_select_marker sj_condition)))
									(begin
										(define localized_field
											(dependent_join_helper_localize_outer_expr first_field_expr local_aliases))
										(define localized_condition
											(dependent_join_helper_localize_outer_expr
												(coalesceNil sj_condition true)
												local_aliases))
										(list sj_tables
											(combine_and_terms (list
												localized_condition
												(if is_not_nil_context
													(list (quote not) (list (quote nil?) localized_field))
													localized_field)))))
									nil))
							_ nil)
						_ nil)
					nil)
				_ nil)))))
	(define dependent_join_helper_simple_scalar_equality_join_parts (lambda (term current_tables) (begin
		(define eq_parts (match term
			'((quote equal??) left right) (list left right)
			'((symbol equal??) left right) (list left right)
			'((quote =) left right) (list left right)
			'((symbol =) left right) (list left right)
			'((quote sql_truthy) inner) (dependent_join_helper_simple_scalar_equality_join_parts inner current_tables)
			'((symbol sql_truthy) inner) (dependent_join_helper_simple_scalar_equality_join_parts inner current_tables)
			_ nil))
		(if (nil? eq_parts)
			nil
			(begin
				(define left_expr (nth eq_parts 0))
				(define right_expr (nth eq_parts 1))
				(define scalar_side (lambda (expr other_expr)
					(match expr
						(cons scalar_sym scalar_args)
						(if (equal?? (inner_select_kind scalar_sym) (quote inner_select))
							(match scalar_args
								(cons scalar_subquery '())
								(list scalar_subquery other_expr)
								_ nil)
							nil)
						_ nil)))
				(match (coalesce
					(scalar_side left_expr right_expr)
					(scalar_side right_expr left_expr))
					'(scalar_subquery target_expr)
					(match scalar_subquery
						'(sj_schema sj_tables sj_fields sj_condition sj_group sj_having sj_order sj_limit sj_offset)
						(begin
							(define first_field_expr (match sj_fields
							...
							(if (and
								...
								(begin
									(define localized_field
										(dependent_join_helper_localize_outer_expr first_field_expr local_aliases))
									(define localized_target
										(dependent_join_helper_localize_outer_expr target_expr local_aliases))
									(define localized_condition
										(dependent_join_helper_localize_outer_expr
											(coalesceNil sj_condition true)
											local_aliases))
									(list sj_tables
										(combine_and_terms (list
											localized_condition
											(list (quote equal??) localized_field localized_target)))))
								nil))
						_ nil)
					_ nil))))))
	(define dependent_join_helper_pull_simple_semijoins (lambda (dep_tables dep_condition)
		(reduce (flatten_and_terms (coalesceNil dep_condition true)) (lambda (state term)
			(match state
				'(tables_acc terms_acc)
				(match (coalesce
					(dependent_join_helper_simple_semijoin_join_parts term tables_acc)
					(dependent_join_helper_simple_scalar_bool_join_parts term tables_acc)
					(dependent_join_helper_simple_scalar_equality_join_parts term tables_acc))
					'(sj_tables sj_condition)
					(list
						(merge tables_acc sj_tables)
						(merge terms_acc (flatten_and_terms (coalesceNil sj_condition true))))
					_ (list tables_acc (merge terms_acc (list term))))
				_ state))
			(list dep_tables '()))))
	...
	(define dependent_join_helper_scalar_count_aggregate?
		(lambda (dep_info)
			(match (normalize_count_literal_aggregates
				...
				(equal? (list agg_expr agg_reduce agg_neutral) aggregate_count_descriptor)
				_ false)))
	(define dependent_join_helper_scalar_presence_value?
		(lambda (dep_info)
			(match (dependent_join_helper_scalar_aggregate_expr dep_info)
				'((quote >) ((quote coalesceNil) _count_expr 0) 0) true
				'((symbol >) ((quote coalesceNil) _count_expr 0) 0) true
				'((quote >) ((symbol coalesceNil) _count_expr 0) 0) true
				'((symbol >) ((symbol coalesceNil) _count_expr 0) 0) true
				_ false)))
	(define dependent_join_helper_scalar_presence_subquery_field?
		(lambda (dep_info)
			(match (dependent_join_helper_scalar_aggregate_expr dep_info)
				(cons sym _args)
				(or
					(equal?? (inner_select_kind sym) (quote inner_select_exists))
					(equal?? (inner_select_kind sym) (quote inner_select_in)))
				_ false)))
	(define dependent_join_helper_positive_count_presence_expr?
		(lambda (expr)
			(match expr
				(cons cmp_sym (cons count_expr (cons zero_expr '())))
				(and
					(equal? zero_expr 0)
					(or (equal?? cmp_sym (quote >)) (equal?? cmp_sym (symbol >)))
					(match (parser_semijoin_count_term_subquery count_expr)
						'(_ _ sub_fields _ _ _ _ _ _)
						(match (normalize_count_literal_aggregates (match sub_fields
							(cons _ (cons v _)) v
							nil))
							'((quote aggregate) agg_expr agg_reduce agg_neutral)
							(equal? (list agg_expr agg_reduce agg_neutral) aggregate_count_descriptor)
						...
						_ false))
				_ false)))
	(define dependent_join_helper_expr_contains_presence_subquery?
		(lambda (expr)
			(match expr
				(cons sym args)
				(or
					(dependent_join_helper_positive_count_presence_expr? expr)
					(equal?? (inner_select_kind sym) (quote inner_select_exists))
					(equal?? (inner_select_kind sym) (quote inner_select_in))
					(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						false
						(reduce args (lambda (found arg)
							(or found (dependent_join_helper_expr_contains_presence_subquery? arg)))
							false)))
				false)))
					...
					(begin
						...
						(define raw_group_simple
							(or (nil? dep_group)
								(equal? dep_group '())
								(equal? dep_group '(1))))
						(define trivial_scalar_limit
							(or (nil? dep_limit) (equal? dep_limit 1)))
						(define trivial_scalar_offset
							(or (nil? dep_offset) (equal? dep_offset 0)))
						...
						(if (and
							(or
								(dependent_join_helper_scalar_presence_subquery_field? dep_info)
								(dependent_join_helper_scalar_presence_value? dep_info)
								(dependent_join_helper_expr_contains_presence_subquery? scalar_agg_field_expr))
							raw_group_simple
							(or (nil? dep_having) (equal? dep_having true))
							(or (nil? dep_order) (equal? dep_order '()))
							trivial_scalar_limit
							trivial_scalar_offset)
							(begin
								(define source_alias (dependent_join_helper_match_source_alias dep_info))
								(define match_alias (concat source_alias "_match"))
								(dependent_join_helper_wrap_full_presence_source_query
									...
									(dependent_join_helper_wrap_grouped_presence_source_query
										match_alias
										(dependent_join_helper_build_source_query_with_extra_condition
											dep_info result_col domain_cols 1 scalar_agg_field_expr)
										result_col
										domain_cols)
									result_col
									domain_cols))
							(if (or
								(nil? scalar_agg_field_expr)
								(equal? (extract_aggregates scalar_agg_field_expr) '())
								(not raw_group_simple)
								(not (or (nil? dep_having) (equal? dep_having true)))
								(not (or (nil? dep_order) (equal? dep_order '())))
								(not trivial_scalar_limit)
								(not trivial_scalar_offset))
								nil
								(list dep_schema
									(merge
										(list (list domain_alias dep_schema (make_dependent_domain_table_spec norm_domain_cols) false nil))
									...
									(cons
										(match oi
											'(col dir) (list (concat "__dep_o" idx)
												(dependent_join_helper_rewrite_expr_for_domain_source dep_info col domain_cols)
												dir)
											_ nil)
										(build_dep_order_cols rest (+ idx 1))))))
								(define dep_order_cols (filter
									(build_dep_order_cols dep_order_items 0)
									(lambda (oc) (not (nil? oc)))))
								(define pulled_semijoin
									(dependent_join_helper_pull_simple_semijoins dep_tables dep_condition))
								(define effective_dep_tables (match pulled_semijoin
									'(tables_acc _terms_acc) tables_acc
								...
								(list dep_schema
									...
									(merge
										...
										(list
											result_col
											(dependent_join_helper_rewrite_expr_for_domain_source dep_info first_field_expr domain_cols))
										(reduce dep_order_cols (lambda (acc oc) (match oc
											'(ord_col ord_expr _dir) (merge acc (list ord_col ord_expr))
											_ acc))
											'()))
									(dependent_join_helper_rewrite_expr_for_domain_source dep_info effective_dep_condition domain_cols)
									'()
					...
					(begin
						...
						(define dep_inner_query
							(list dep_schema
								...
								(merge
									(dependent_join_helper_domain_field_assoc norm_domain_cols domain_alias)
									(list result_col (dependent_join_helper_rewrite_expr_for_domain_source dep_info first_field_expr domain_cols))
	...
	(define dependent_join_helper_domain_query (lambda (domain_cols outer_tables outer_condition outer_group outer_having) (begin
		...
		(if (equal? domain_fields '())
			...
			(list schema
				...
				(dedupe_canonical_exprs domain_group_exprs)
				outer_having
				'()
				nil
				nil)))))
	...
	(define dependent_join_helper_rewrite_source_table (lambda (td rewrite_expr recurse_query current_unnesting_info) (match td
		...
		(begin
			(define rewritten_tbl (match ttbl
				...
				(begin
					(define nested_domain_cols
						(merge_unique
							domain_cols
							(dependent_expr_unnesting_info_domain current_unnesting_info)))
				...
				(match source_query
					...
					(begin
						...
						(list dep_schema
							...
							dep_limit dep_offset))
					_ source_query))))
	(define dependent_join_helper_active_domain_cols (lambda (_source_query domain_cols)
		(dependent_join_helper_normalize_domain_cols domain_cols)))
	(define dependent_join_helper_finalize_source_query (lambda (source_query domain_cols outer_tables outer_condition outer_group outer_having unnesting_info) (begin
		...
		(define outer_table_aliases
			(filter (map outer_tables (lambda (td) (match td
				'(tv _ _ _ _) tv
				_ nil)))
				(lambda (tv) (not (nil? tv)))))
		(define domain_col_refs_only_outer_tables? (lambda (dc) (match dc
			'(_dom_col dom_expr)
			(and
				(not (equal? (extract_tblvars dom_expr) '()))
				(reduce (extract_tblvars dom_expr) (lambda (ok tv)
					(and ok (has? outer_table_aliases tv)))
					true))
			false)))
		(define collect_nested_outer_domain_cols_from_query (lambda (query) (match query
			'(q_schema q_tables _q_fields _q_condition _q_group _q_having _q_order _q_limit _q_offset)
			(merge_unique (map q_tables (lambda (td) (match td
				'(_ _ ttbl _ _)
				(match ttbl
					'((quote dependent_join_helper) _ _ nested_domain_cols _ nested_source_query _)
					(merge_unique
						(filter nested_domain_cols domain_col_refs_only_outer_tables?)
						(collect_nested_outer_domain_cols_from_query nested_source_query))
					'(dependent_join_helper _ _ nested_domain_cols _ nested_source_query _)
					(merge_unique
						(filter nested_domain_cols domain_col_refs_only_outer_tables?)
						(collect_nested_outer_domain_cols_from_query nested_source_query))
					'(sub_schema sub_tables sub_fields sub_condition sub_group sub_having sub_order sub_limit sub_offset)
					(collect_nested_outer_domain_cols_from_query ttbl)
					_ '())
				_ '()))))
			_ '())))
		(define active_domain_cols
			(merge_unique
				(dependent_join_helper_active_domain_cols source_query domain_cols)
				(collect_nested_outer_domain_cols_from_query source_query)
				(if (nil? (dependent_expr_unnesting_info_parent unnesting_info))
					'()
					(dependent_expr_unnesting_info_domain
						(dependent_expr_unnesting_info_parent unnesting_info)))))
		(define rewrite_source_domain_expr (lambda (expr) (begin
			(define domain_expr_matches? (lambda (candidate domain_expr) (or
				(equal? candidate domain_expr)
				(match domain_expr
					'((quote get_column) alias_ _ col _)
					(equal?? candidate (symbol (concat alias_ "." col)))
					'((symbol get_column) alias_ _ col _)
					(equal?? candidate (symbol (concat alias_ "." col)))
					_ false))))
			(define mapped_domain_col
				(reduce active_domain_cols (lambda (found dc) (match dc
					'(dom_col dom_expr)
					(if (or (not (nil? found)) (not (domain_expr_matches? expr dom_expr)))
						found
						(symbol (concat domain_alias "." dom_col)))
					_ found))
					nil))
			(if (not (nil? mapped_domain_col))
				mapped_domain_col
				(match expr
					(cons sym args)
					(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						expr
						(cons (rewrite_source_domain_expr sym)
							(map args rewrite_source_domain_expr)))
					_ expr)))))
		...
		(match source_query
			...
			(list dep_schema
				(map dep_tables (lambda (td) (match td
					'(tv tschema ttbl toisOuter tje)
					(begin
						(define finalized_ttbl (match ttbl
							'(sub_schema sub_tables sub_fields sub_condition sub_group sub_having sub_order sub_limit sub_offset)
							...
							(if (and (equal? tv domain_alias) (is_dependent_domain_table_spec ttbl))
								domain_query
								ttbl)))
						(list tv tschema finalized_ttbl toisOuter (if (nil? tje) nil (rewrite_source_domain_expr tje))))
					_ td)))
				(map_assoc dep_fields (lambda (k v) (rewrite_source_domain_expr v)))
				(rewrite_source_domain_expr dep_condition)
				(map (coalesceNil dep_group '()) rewrite_source_domain_expr)
				(rewrite_source_domain_expr dep_having)
				(map (coalesceNil dep_order '()) (lambda (oi) (match oi
					'(col dir) (list (rewrite_source_domain_expr col) dir)
					_ oi)))
				dep_limit dep_offset)
			_ source_query))))
	(define dependent_join_helper_source_preserves_full_domain (lambda (source_query result_col) (match source_query
		'(_src_schema src_tables src_fields src_condition src_group src_having src_order src_limit src_offset)
		(and
			(equal? (count src_tables) 1)
			(match (car src_tables)
				'(tv _ _ _ tje)
				(and
					(equal?? tv (dependent_join_helper_domain_source_alias))
					(or (nil? tje) (equal? tje true)))
				_ false)
			(has_assoc? src_fields result_col)
			(or (nil? src_condition) (equal? src_condition true))
			(or (nil? src_group) (equal? src_group '()))
			(or (nil? src_having) (equal? src_having true))
			(or (nil? src_order) (equal? src_order '()))
			(nil? src_limit)
			(or (nil? src_offset) (equal? src_offset 0)))
		_ false)))
	...
	(set tables (if (or (nil? tables) (equal? tables '()))
		(begin
			(if (not (string? schema))
				(error (concat "UNTANGLE_NOFROM_NONSTRING_SCHEMA "
					(serialize (list schema tables fields condition group having order schemas))))
				nil)
			...
			(list (list ".(1)" schema ".(1)" false nil)))
		tables))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: before zipped inputs") nil)
	(set zippedInputs (map tables (lambda (tbldesc) (begin
		(match tbldesc
			'(_ _ table_source _ _)
			(begin
				(define embedded_init (materialized-subquery-source-init-code table_source))
				(if (or (nil? embedded_init) (equal? embedded_init '()))
					nil
					(sq_cache_add_init_once table_source embedded_init)))
			_ nil)
		(if neumann_pipeline_trace (print (concat "[neumann] legacy untangle: tabledesc " (serialize tbldesc))) nil)
		(match tbldesc
			'(alias schema (string? tbl) _ _) (list (list tbldesc) '() true (list alias (get_schema schema tbl))) /* leave primary tables as is and load their schema definition */
			'(alias schema '(scan-tagged-table (string? base_tbl) _ _ _ _ _) _ _) (list (list tbldesc) '() true (list alias (get_schema schema base_tbl)))
			'(alias schema '((quote scan-tagged-table) (string? base_tbl) _ _ _ _ _) _ _) (list (list tbldesc) '() true (list alias (get_schema schema base_tbl)))
			'(alias schema '((symbol scan-tagged-table) (string? base_tbl) _ _ _ _ _) _ _) (list (list tbldesc) '() true (list alias (get_schema schema base_tbl)))
			...
			'(alias schema '((quote scan-tagged-table) base_tbl _ _ _ _ _) _ _) (if (materialized-subquery-source? base_tbl)
				(list (list tbldesc) '() true (list alias (materialized_source_schema schema base_tbl alias schemas)))
				(error (concat "ZIP_TABLEDESC_NONSTRING_SUBQUERY_SCHEMA " (serialize (nth tbldesc 2)))))
			'(alias schema '((symbol scan-tagged-table) base_tbl _ _ _ _ _) _ _) (if (materialized-subquery-source? base_tbl)
				(list (list tbldesc) '() true (list alias (materialized_source_schema schema base_tbl alias schemas)))
				(error (concat "ZIP_TABLEDESC_NONSTRING_SUBQUERY_SCHEMA " (serialize (nth tbldesc 2)))))
			'(alias schema '((quote scan-tagged-table) base_tbl _ _ _ _ _ _) _ _) (if (materialized-subquery-source? base_tbl)
				(list (list tbldesc) '() true (list alias (materialized_source_schema schema base_tbl alias schemas)))
				(error (concat "ZIP_TABLEDESC_NONSTRING_SUBQUERY_SCHEMA " (serialize (nth tbldesc 2)))))
			'(alias schema '((symbol scan-tagged-table) base_tbl _ _ _ _ _ _) _ _) (if (materialized-subquery-source? base_tbl)
				(list (list tbldesc) '() true (list alias (materialized_source_schema schema base_tbl alias schemas)))
				(error (concat "ZIP_TABLEDESC_NONSTRING_SUBQUERY_SCHEMA " (serialize (nth tbldesc 2)))))
				...
				(sq_cache "init" (merge (coalesceNil (sq_cache "init") '())
					(list (list (quote begin)
						...
						(list (quote if)
							...
							(list (quote insert) (list (quote table) schema ".(1)")
								(list (quote quote) (list "1"))
								(list (quote quote) (list (list 1)))
								'() (list (quote lambda) '() true) true)
							nil)))))
			...
			'(id schemax subquery isOuter joinexpr) (begin
				(define union_parts_from (query_union_parts subquery))
				(if (not (nil? union_parts_from))
					(match union_parts_from '(branches union_order union_limit union_offset _distinct) (begin
						...
						(sq_cache_add_init_once mat_source mat_init)
						(list
					...
					(begin
						(if (or (nil? subquery) (< (count subquery) 9) (not (string? (nth subquery 0))))
							(error (concat "ZIP_TABLEDESC_NONSTRING_SUBQUERY_SCHEMA "
								(serialize subquery)))
							nil)
						(define sub_schema (car subquery))
						(define sub_rest1 (cdr subquery))
						(define sub_tables (car sub_rest1))
						(define sub_rest2 (cdr sub_rest1))
						(define sub_fields (car sub_rest2))
						(define sub_rest3 (cdr sub_rest2))
						(define sub_condition (car sub_rest3))
						(define sub_rest4 (cdr sub_rest3))
						(define sub_group (car sub_rest4))
						(define sub_rest5 (cdr sub_rest4))
						(define sub_having (car sub_rest5))
						(define sub_rest6 (cdr sub_rest5))
						(define sub_order (car sub_rest6))
						(define sub_rest7 (cdr sub_rest6))
						(define sub_limit (car sub_rest7))
						(define sub_rest8 (cdr sub_rest7))
						(define sub_offset (car sub_rest8))
						(define subquery_untangled
							(untangle_query
								sub_schema
								sub_tables
								sub_fields
								...
								sub_offset
								outer_schemas_chain))
						(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after subquery untangled")) nil)
						(define derived_zip_result (match subquery_untangled '(schema2 tables2 fields2 condition2 groups2 schemas2 replace_find_column2 _init2) (begin
							/* Do not hoist subquery init code before deciding whether this
							derived table is materialized. Materialized derived tables compile
							the subquery again inside mat_init with their own resultrow sink;
							hoisting _init2 as well creates duplicate scalar-helper caches that
							share inner session keys but write to different temp tables. */
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after init defer")) nil)
							(define tables2_aliases (map tables2 (lambda (td) (match td
								'(alias _ _ _ _) alias
								_ nil))))
							(define tables2_has_alias? (lambda (alias_)
								(reduce tables2_aliases (lambda (found table_alias)
									(or found (equal?? table_alias alias_)))
									false)))
							(define derived_schema_field_expr (lambda (alias_ col ci)
								(if (or (nil? alias_) (not (has_assoc? schemas2 alias_)))
									nil
									(reduce (schema_assoc_cols schemas2 alias_) (lambda (found coldef)
										(if (not (nil? found))
											found
											(if ((if ci equal?? equal?) (coldef "Field") col)
												(coalesceNil (coldef "Expr") nil)
												nil)))
										nil))))
							(define derived_missing_alias_field_expr (lambda (alias_ col ci ti)
								(if (or
									(nil? alias_)
									(not (nil? (schemas2 alias_)))
									(not (nil? (outer_schemas_chain alias_))))
									nil
									(begin
										(define matches (reduce_assoc schemas2 (lambda (acc schema_alias cols)
											(begin
												(define norm_cols (schema_assoc_cols schemas2 schema_alias))
												(if (and
													(equal? (replace schema_alias "\0" "") schema_alias)
													(reduce norm_cols (lambda (found coldef)
														(or found ((if ci equal?? equal?) (coldef "Field") col)))
														false))
													(cons schema_alias acc)
													acc)))
											'()))
										(define matches (reduce matches (lambda (acc schema_alias)
											(append_unique acc schema_alias))
											'()))
										(match matches
											(cons only '())
											(list (quote get_column)
												(if (and (string? only) (strlike only "dep_scalar_%"))
													only
													(concat id "\0" only))
												ti col ci)
											_ nil)))))
							...
							(define replace_column_alias (lambda (expr) (match expr
								'((symbol get_column) nil ti col ci) (begin
									...
									(if (not (nil? (fields2 col)))
										(replace_column_alias (fields2 col))
										(begin
											(define matches (reduce_assoc schemas2 (lambda (acc alias cols)
												(begin
													(define norm_cols (schema_assoc_cols schemas2 alias))
													(if (and (equal? (replace alias "\0" "") alias)
														(reduce norm_cols (lambda (a coldef) (or a ((if ci equal?? equal?) (coldef "Field") col))) false))
														(cons alias acc)
														acc))) '()))
								...
								'((symbol get_column) alias_ ti col ci) (if (not (nil? (schemas2 alias_)))
									(begin
										(define field_expr (derived_schema_field_expr alias_ col ci))
										(if (and (not (nil? field_expr)) (not (equal? field_expr expr)))
											(replace_column_alias field_expr)
											'('get_column (if (and (string? alias_) (strlike alias_ "dep_scalar_%")) alias_ (concat id "\0" alias_)) ti col ci)))
									(coalesce
										(derived_missing_alias_field_expr alias_ col ci ti)
										expr)) /* alias not in schemas2 → inner subselect scope, leave as-is unless a flattened helper uniquely provides the field */
								'((quote get_column) alias_ ti col ci) (if (not (nil? (schemas2 alias_)))
									(begin
										(define field_expr (derived_schema_field_expr alias_ col ci))
										(if (and (not (nil? field_expr)) (not (equal? field_expr expr)))
											(replace_column_alias field_expr)
								...
								(cons sym args) /* function call */ (if (or
									(is_quote_scope_sym sym)
									(not (list? args)))
									expr /* inner subselects resolved later by replace_inner_selects */
									(begin
										(define rewrite_subquery_aliases (lambda (subquery) (match subquery
											'(sub_schema sub_tables sub_fields sub_condition sub_group sub_having sub_order sub_limit sub_offset)
											(list
												sub_schema
												sub_tables
												(map_assoc sub_fields (lambda (field_name field_expr)
													(replace_column_alias field_expr)))
												(replace_column_alias sub_condition)
												(map (coalesceNil sub_group '()) replace_column_alias)
												(if (nil? sub_having) nil (replace_column_alias sub_having))
												(map (coalesceNil sub_order '()) (lambda (order_item) (match order_item
													'(order_expr order_dir) (list (replace_column_alias order_expr) order_dir)
													order_item)))
												sub_limit
												sub_offset)
											_ subquery)))
										(if (or
											...
											(equal?? sym (symbol inner_select_exists)))
											(match args
												(cons subquery '()) (list sym (rewrite_subquery_aliases subquery))
												_ expr)
											(if (or
												(equal?? sym (quote inner_select_in))
												(equal?? sym (quote (quote inner_select_in)))
												...
												(match args
													(cons target_expr (cons subquery '()))
													(list sym (replace_column_alias target_expr) (rewrite_subquery_aliases subquery))
													_ expr)
							...
							(set tablesPrefixed (map tables2 (lambda (x) (match x '(alias schema tbl a innerJoinexpr)
								(list (if (and (string? alias) (strlike alias "dep_scalar_%")) alias (concat id "\0" alias)) schema (replace_column_alias_table_ref tbl) a
									(if (nil? innerJoinexpr) nil (replace_column_alias innerJoinexpr)))))))
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after tables prefixed")) nil)
							(define derived_field_expr_lookup (lambda (field_list field_name)
								(coalesce
									(get_assoc field_list field_name)
									(reduce (qpp-fields-to-pairs field_list) (lambda (found pair)
										(match pair
											'(name expr) (if (and (nil? found) (equal? name field_name)) expr found)
											found))
										nil))))
							(define derived_boundary_domain_ref (lambda (col) (begin
								(define boundary-col? (and (string? col) (>= (strlen col) 5)
									(equal? (substr col 0 5) "__kt_")))
								(define col-field-expr (if boundary-col?
									(coalesce
										(derived_field_expr_lookup fields2 col)
										(get_assoc fields2 col))
									nil))
								(define domain-ref? (lambda (expr) (match expr
									'((symbol get_column) alias_ _ _ _)
									(and (string? alias_) (strlike alias_ "dom_%"))
									'((quote get_column) alias_ _ _ _)
									(and (string? alias_) (strlike alias_ "dom_%"))
									false)))
								(define find-domain-eq (lambda (lhs rhs)
									(if (and (not (nil? col-field-expr)) (equal?? lhs col-field-expr) (domain-ref? rhs))
										rhs
										(if (and (not (nil? col-field-expr)) (equal?? rhs col-field-expr) (domain-ref? lhs))
											lhs
											nil))))
								(define condition-domain-ref
									(if boundary-col?
										(reduce (flatten_and_terms (coalesceNil condition2 true)) (lambda (found term)
											(if (not (nil? found))
												found
												(match term
													'((symbol sql_truthy) inner) (match inner
														'((symbol equal??) lhs rhs) (find-domain-eq lhs rhs)
														'((quote equal??) lhs rhs) (find-domain-eq lhs rhs)
														'((symbol =) lhs rhs) (find-domain-eq lhs rhs)
														'((quote =) lhs rhs) (find-domain-eq lhs rhs)
														nil)
											...
											nil)
										nil))
								(define local_col (if (and (string? col) (>= (strlen col) 5)
									(equal? (substr col 0 5) "__kt_"))
									(substr col 5 (- (strlen col) 5))
									col))
								(coalesce condition-domain-ref
									(if boundary-col?
										(reduce tables2 (lambda (found td) (match td
											'(alias _ tbl _ _)
											(if (not (nil? found))
												found
												(if (and (string? alias)
													(>= (strlen alias) 4)
													(equal? (substr alias 0 4) "dom_")
													(qpp-tuple? tbl)
													(not (nil? (derived_field_expr_lookup
														(qpp-tuple-fields tbl)
														local_col))))
													(list (quote get_column) alias false local_col false)
													nil))
											_ found))
											nil)
										nil)))))
							(define derived_boundary_outer_ref (lambda (col outer_expr) (begin
								(define outer_col (match outer_expr
									'((symbol get_column) _ _ ocol _) ocol
									'((quote get_column) _ _ ocol _) ocol
									nil))
								(if (or
									(nil? outer_col)
									(not (string? col))
									(< (strlen col) 5)
									(not (equal? (substr col 0 5) "__kt_")))
									nil
									(reduce tables2 (lambda (found td) (match td
										'(alias _ tbl _ _)
										(if (not (nil? found))
											...
											(if (and
												...
												(not (nil? (derived_field_expr_lookup
													(qpp-tuple-fields tbl)
													outer_col))))
												(list (quote get_column) alias false outer_col false)
												nil))
										_ found))
										nil)))))
							(define transform_boundary_equal (lambda (sym lhs rhs)
								(begin
									(define lhs-boundary (match lhs
										'((symbol get_column) alias_ _ col _) (if (equal?? alias_ id)
											(derived_boundary_outer_ref col rhs)
											nil)
										'((quote get_column) alias_ _ col _) (if (equal?? alias_ id)
											(derived_boundary_outer_ref col rhs)
											nil)
										nil))
									(define rhs-boundary (match rhs
										'((symbol get_column) alias_ _ col _) (if (equal?? alias_ id)
											(derived_boundary_outer_ref col lhs)
											nil)
										'((quote get_column) alias_ _ col _) (if (equal?? alias_ id)
											(derived_boundary_outer_ref col lhs)
											nil)
										nil))
									(if (not (nil? lhs-boundary))
										(list sym lhs-boundary (transform_joinexpr rhs))
										(if (not (nil? rhs-boundary))
											(list sym (transform_joinexpr lhs) rhs-boundary)
											nil)))))
							...
							(define transform_joinexpr (lambda (expr) (match expr
								'((symbol equal??) lhs rhs) (coalesce
									(transform_boundary_equal (quote equal??) lhs rhs)
									(list (quote equal??) (transform_joinexpr lhs) (transform_joinexpr rhs)))
								'((quote equal??) lhs rhs) (coalesce
									(transform_boundary_equal (quote equal??) lhs rhs)
									(list (quote equal??) (transform_joinexpr lhs) (transform_joinexpr rhs)))
								'((symbol =) lhs rhs) (coalesce
									(transform_boundary_equal (quote =) lhs rhs)
									(list (quote =) (transform_joinexpr lhs) (transform_joinexpr rhs)))
								'((quote =) lhs rhs) (coalesce
									(transform_boundary_equal (quote =) lhs rhs)
									(list (quote =) (transform_joinexpr lhs) (transform_joinexpr rhs)))
								...
								(cons sym args) /* function call */ (if (or
									...
									(is_opaque_scope_sym sym)
									(is_quote_scope_sym sym)
									(not (list? args))) expr /* inner subselects have their own scope */ (cons sym (map args transform_joinexpr)))
								expr
							)))
							...
							(define prefix_joinexpr_inner_aliases (lambda (expr) (match expr
								'((symbol get_column) alias_ ti col ci)
								(if (and (not (nil? alias_))
									(or
										(not (nil? (schemas2 alias_)))
										(not (nil? (schemas2 (string alias_))))
										(not (nil? (schemas2 (symbol (string alias_)))))))
									(list (quote get_column) (concat id "\0" alias_) ti col ci)
									expr)
								'((quote get_column) alias_ ti col ci)
								(if (and (not (nil? alias_))
									(or
								...
								(cons sym args) (if (or
									...
									(not (list? args)))
									expr
									(cons (prefix_joinexpr_inner_aliases sym)
										(map args prefix_joinexpr_inner_aliases)))
								expr)))
							(set joinexpr2 (if (nil? joinexpr2) nil
								(prefix_joinexpr_inner_aliases joinexpr2)))
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after joinexpr transform")) nil)
							...
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after condition transform")) nil)
							(set joinexpr2 (if isOuter
								/* merge condition2 into joinexpr for outer joins.
								Flatten + de-duplicate conjuncts so nested scalar/semijoin
								flattening does not re-append the same row-domain terms on
								each outer wrapper layer. */
								(if (nil? joinexpr2)
									condition2_transformed
									(if (or (nil? condition2_transformed) (equal? condition2_transformed true))
										joinexpr2
										(combine_and_terms (merge_unique
											(flatten_and_terms joinexpr2)
											(flatten_and_terms condition2_transformed)))))
								joinexpr2))
							(if (and (not (nil? joinexpr2)) (or (nil? tablesPrefixed) (equal? tablesPrefixed '())))
								(error (concat "FLATTEN_EMPTY_TABLES_WITH_JOINEXPR "
									(serialize (list id subquery tables2 fields2 condition2 groups2 schemas2 joinexpr joinexpr2))))
								nil)
							(set tablesPrefixed
								(if (and (not (nil? joinexpr2)) (not (nil? tablesPrefixed)))
									(begin
										(define prefixed_aliases (map tablesPrefixed (lambda (td) (match td
											'(a _ _ _ _) a
											_ nil))))
										(define join_term_scope_valid? (lambda (term)
											(begin
												(define refs (extract_tblvars term))
												(define has_prefixed_ref
													(reduce refs (lambda (found ref_alias)
														(or found (has? prefixed_aliases ref_alias)))
														false))
												(or
													has_prefixed_ref
													(reduce refs (lambda (ok ref_alias)
														(and ok (has? dep_outer_visible_aliases (string ref_alias))))
														true)))))
										(define join_term_owner (lambda (term) (begin
											(define refs (filter (extract_tblvars term) (lambda (ref_alias)
												(has? prefixed_aliases ref_alias))))
											(if (equal? refs '())
												(car prefixed_aliases)
												(reduce prefixed_aliases (lambda (owner alias_name)
													(if (has? refs alias_name) alias_name owner))
													(car prefixed_aliases))))))
										(define join_terms_by_alias
											(reduce (flatten_and_terms joinexpr2) (lambda (acc term) (begin
												(if (join_term_scope_valid? term)
													(begin
														(define owner (join_term_owner term))
														(set_assoc acc owner (merge (coalesceNil (get_assoc acc owner) '()) (list term))))
													acc)))
												'()))
										(define join_term_has_null_default (lambda (expr) (match expr
											(cons sym args)
											(if (or
												(equal?? sym (quote coalesce))
												(equal?? sym (quote (quote coalesce)))
												(equal?? sym (symbol coalesce))
												(equal?? sym (quote coalesceNil))
												(equal?? sym (quote (quote coalesceNil)))
												(equal?? sym (symbol coalesceNil)))
												true
												(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
													false
													(reduce args (lambda (found arg)
														(or found (join_term_has_null_default arg)))
														false)))
											false)))
										(map tablesPrefixed (lambda (td) (match td
											'(a s t io je)
											(begin
												(define assigned_terms_raw (coalesceNil (get_assoc join_terms_by_alias a) '()))
												(define assigned_terms (if io
													(filter assigned_terms_raw (lambda (term)
														(not (join_term_has_null_default term))))
													assigned_terms_raw))
												(define propagate_derived_outer
													(and isOuter
														(or
															(equal?? a (car prefixed_aliases))
															(not (equal? assigned_terms_raw '())))))
												/* The derived LEFT-join ON predicate belongs to the inner alias that can
												evaluate each conjunct. Keeping all conjuncts on the first table lets
												join_reorder move scalar helpers ahead of their domain filter, producing
												duplicate NULL rows for nested EXISTS/scalar helpers. */
												(list a s t
													(or io propagate_derived_outer)
													(combine_and_terms (merge (flatten_and_terms (coalesceNil je true)) assigned_terms))))
											td))))
									tablesPrefixed))
							(define outer_has_group_stage (or
								(not (nil? group))
								(not (nil? having))
								(reduce (coalesceNil groups '()) (lambda (acc stage)
									(or acc (stage_has_group_boundary stage)))
									false)
								(reduce_assoc fields (lambda (acc _k v) (or acc (not (equal? (extract_aggregates v) '())))) false)))
							(define flatten_referenced_cols (merge_unique (list
								/* Parent output aliases are required columns too. Row-domain
								subselect rewrites can hide a pass-through field behind helper
								markers before expression-based extraction sees it. Keep matching
								derived fields conservatively instead of dropping user-visible
								columns during flattening. */
								(extract_assoc fields (lambda (k _v) k))
								(extract_columns_for_tblvar id fields)
								(extract_columns_for_tblvar id condition)
								(extract_columns_for_tblvar id joinexpr)
								(extract_columns_for_tblvar id joinexpr2)
								...
								(merge (map tables (lambda (td) (match td
									...
									'())))))))
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after flatten referenced cols")) nil)
							...
							(define pruned_fields2_base (if flatten_uses_subquery_wildcard
								fields2
								(filter_assoc fields2 (lambda (k v)
									(or
										(and (string? k) (>= (strlen k) 5)
											(equal? (substr k 0 5) "__kt_"))
										(reduce flatten_referenced_cols (lambda (keep refcol)
											(or keep (equal?? refcol k)))
											false))))))
							(define pruned_fields2 pruned_fields2_base)
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after pruned fields")) nil)
							...
							(define flatten_helper_projection_aliases (map (filter tables2 (lambda (td) (match td
								'(alias _ ttbl _ _) (or
									(scan_tagged_table_needs_scan_order ttbl)
									(unnest_helper_table? ttbl)
									(materialized-source? ttbl)
									(and (string? ttbl)
										(or
											(strlike ttbl ".mat:%")
											(strlike ttbl ".prejoin:%")
											(strlike ttbl ".keytable:%"))))
								false))) (lambda (td) (match td '(alias _ _ _ _) alias ""))))
							(define flatten_nested_derived_aliases (map (filter tables2 (lambda (td) (match td
								'(alias _ ttbl _ _) (query_is_select_core ttbl)
								false))) (lambda (td) (match td '(alias _ _ _ _) alias ""))))
							(define expr_contains_scan_tagged_helper (lambda (expr)
								(reduce (extract_tblvars expr) (lambda (acc alias_)
									...
									false)))
							(define expr_contains_nested_derived_ref (lambda (expr)
								(reduce (extract_tblvars expr) (lambda (acc alias_)
									(or acc (has? flatten_nested_derived_aliases alias_)))
									false)))
							(define flatten_has_helper_backed_projection
								(reduce_assoc pruned_fields2 (lambda (acc _k v)
									(or acc
										...
										(expr_contains_nested_derived_ref v)))
									false))
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after helper projection check")) nil)
							...
							(define correlated_inner_select_refs_subquery_alias (lambda (expr)
								(and
									(contains_inner_select_marker expr)
									(reduce (extract_tblvars expr) (lambda (found tv)
										(or found (equal?? tv id)))
										false))))
							(define expr_refs_subquery_alias (lambda (expr)
								(reduce (extract_tblvars expr) (lambda (found tv)
									(or found (equal?? tv id)))
									false)))
							(define expr_refs_subquery_alias_exact (lambda (expr)
								(reduce (extract_tblvars expr) (lambda (found tv)
									(or found (equal? tv id)))
									false)))
							(define outer_refs_subquery_alias (or
								(reduce_assoc fields (lambda (acc _k v)
									(or acc (expr_refs_subquery_alias v))) false)
								(expr_refs_subquery_alias condition)
								(expr_refs_subquery_alias (coalesceNil having true))
								(reduce (coalesceNil group '()) (lambda (acc gexpr)
									(or acc (expr_refs_subquery_alias gexpr))) false)
								(reduce (coalesceNil order '()) (lambda (acc o) (or acc (match o
									'(col _dir) (expr_refs_subquery_alias col)
									false))) false)))
							(define derived_trivial_terms? (lambda (expr)
								(reduce (flatten_and_terms (coalesceNil expr true)) (lambda (ok term)
									(and ok (or (nil? term) (equal? term true))))
									true)))
							(define outer_plain_projection_only (and
								(derived_trivial_terms? condition)
								(or (nil? group) (equal? group '()))
								(derived_trivial_terms? having)
								(reduce_assoc fields (lambda (ok _k v)
									(and ok
										(or
											(not (expr_refs_subquery_alias_exact v))
											(match v
												'((symbol get_column) alias_ _ _ _) (equal? alias_ id)
												'((quote get_column) alias_ _ _ _) (equal? alias_ id)
												_ false))))
									true)))
							...
							(define outer_uses_subquery_group_boundary (or
								(and grouped_or_helper_boundary outer_refs_subquery_alias)
							...
							(define derived_plain_projection_can_flatten_group_boundary
								(and
									...
									(not flatten_has_helper_backed_projection)))
							...
							(define unsupported_groups (if groups2_present
								...
								(define outer_has_non_group_stage (or
									...
									(not (nil? limit))
									(not (nil? offset))))
								(define flatten_has_order_limit_boundary
									(reduce flatten_groups2 (lambda (found stage)
										(or found
											(not (equal? (coalesceNil (stage_order_list stage) '()) '()))
											(not (nil? (stage_limit_val stage)))
											(not (nil? (stage_offset_val stage)))))
										false))
								(define outer_nullable_group_boundary
									(and
										isOuter
										groups2_present
										derived_plain_projection_can_flatten_group_boundary
							...
							(define condition2_has_null_default (lambda (expr) (match expr
								...
								(if (or
									...
									(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
										false
										(reduce args (lambda (found arg)
											(or found (condition2_has_null_default arg)))
											false)))
								false)))
							(define outer_post_filter_needs_boundary
								(and isOuter (condition2_has_null_default condition2_transformed)))
							(define outer_filter_projection_chain_only (and
								(or (nil? group) (equal? group '()))
								(derived_trivial_terms? having)
								(not (aggregate_refs_subquery_alias (coalesceNil condition true)))
								(not (correlated_inner_select_refs_subquery_alias (coalesceNil condition true)))
								(not (aggregate_refs_subquery_alias (coalesceNil having true)))
								(not (correlated_inner_select_refs_subquery_alias (coalesceNil having true)))
								(reduce_assoc fields (lambda (ok _k v)
									(and ok
										(or
											(not (expr_refs_subquery_alias_exact v))
											(match v
												...
												_ false))))
									true)
								(reduce (coalesceNil order '()) (lambda (ok o)
									(and ok (match o
										'(col _dir) (and
											(not (aggregate_refs_subquery_alias col))
											(not (correlated_inner_select_refs_subquery_alias col)))
										_ true)))
									true)))
								(define use_materialize (or
									...
									(and (not derived_plain_projection_can_flatten_group_boundary) (not (equal? flatten_groups2 '())) outer_has_non_group_stage)
								(and (not derived_plain_projection_can_flatten_group_boundary) (not (equal? flatten_groups2 '())) outer_has_group_stage)
								(and (not derived_plain_projection_can_flatten_group_boundary) (not (equal? flatten_groups2 '())) (not (equal? (coalesceNil outer_schemas '()) '())))))
							(if (and materialize_reasons use_materialize)
								(print (concat "[materialize] derived " id " reasons="
									(serialize
										(list
											(list "window" subquery_has_window)
											(list "outer-nullable-group-boundary" outer_nullable_group_boundary)
											(list "unsupported-groups" unsupported_groups)
											(list "outer-post-filter" outer_post_filter_needs_boundary)
											(list "flatten-groups" (not (equal? flatten_groups2 '())))
											(list "outer-non-group-stage" outer_has_non_group_stage)
											(list "outer-group-stage" outer_has_group_stage)
											(list "outer-schemas" (not (equal? (coalesceNil outer_schemas '()) '())))
											(list "helper-backed-projection" flatten_has_helper_backed_projection)
											(list "outer-uses-boundary" outer_uses_subquery_group_boundary)
											(list "outer-plain-projection-only" outer_plain_projection_only)))))
								nil)
								(if (and
									(not use_materialize)
									(not (nil? _init2))
								...
								(sq_cache "init" (merge
									...
									(map _init2 replace_column_alias)))
									nil)
								(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after use materialize")) nil)
							/* Window-function LIMIT pushdown */
							(define mat_limit nil)
							(if subquery_has_window (begin
								(define check_wf_limit (lambda (cond) (match cond
									...
									'('and a b) (begin (check_wf_limit a) (check_wf_limit b))
									nil)))
								(check_wf_limit condition)
							))
							(define flatten_stage_table_aliases (map tables2 (lambda (td) (match td '(a _ _ _ _) a nil))))
							(define flatten_stage_single_alias (if (equal? (count flatten_stage_table_aliases) 1)
								(car flatten_stage_table_aliases)
								nil))
							(define flatten_stage_synthetic_unnest_alias? (lambda (alias)
								(and
									(not (nil? flatten_stage_single_alias))
									(string? alias)
									(>= (strlen alias) 4)
									(equal? (substr alias 0 4) "unn_"))))
							(define flatten_stage_alias (lambda (alias)
								(if (nil? alias) nil
									(if (flatten_stage_synthetic_unnest_alias? alias)
										(concat id "\0" flatten_stage_single_alias)
										(if (or (not (nil? (schemas2 alias)))
											(not (nil? (schemas2 (string alias))))
											(and (string? alias) (not (nil? (schemas2 (symbol alias))))))
											(concat id "\0" alias)
											alias)))))
							(define replace_stage_column_alias (lambda (expr) (match expr
								'((symbol get_column) alias_ ti col ci)
								(if (flatten_stage_synthetic_unnest_alias? alias_)
									(list (quote get_column) (concat id "\0" flatten_stage_single_alias) ti col ci)
									(replace_column_alias expr))
								'((quote get_column) alias_ ti col ci)
								(if (flatten_stage_synthetic_unnest_alias? alias_)
									(list (quote get_column) (concat id "\0" flatten_stage_single_alias) ti col ci)
									(replace_column_alias expr))
								(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
									expr
									(cons (replace_stage_column_alias sym) (map args replace_stage_column_alias)))
								expr)))
								...
								(if (and groups2_present (not use_materialize) (not (equal? flatten_groups2 '())))
									(sq_cache "groups" (merge
										(map flatten_groups2 (lambda (stage)
											(stage_set
												(rewrite_stage_for_flattened_aliases stage replace_stage_column_alias flatten_stage_alias)
												(quote flattened-derived)
												true)))
										(coalesceNil (sq_cache "groups") '()))))
							(define derived_result
								(if use_materialize
									(begin
										(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": materialize begin")) nil)
										(define materialized_needs_boundary_keys
											(reduce
												(merge
													(extract_columns_for_tblvar id joinexpr)
													(extract_columns_for_tblvar id joinexpr2))
												(lambda (found col)
													(or found
														(and (string? col) (>= (strlen col) 5)
															(equal? (substr col 0 5) "__kt_")
															(nil? (fields2 col))
															(not (nil? (pruned_fields2 col))))))
												false))
										(define materialized_subquery
											(if materialized_needs_boundary_keys
												(list (quote select_core_term)
													schema2 tables2 pruned_fields2 condition2
													nil schemas2 replace_find_column2 _init2)
												subquery))
										(define output_cols_sub (extract_assoc pruned_fields2 (lambda (k v) k)))
										(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": materialize output cols")) nil)
										(define rows_sym (symbol (concat "__fsr:" id)))
										(define row_sink_sym (symbol (concat "__fss:" id)))
										(define cnt_sym (symbol (concat "__fsc:" id)))
										(define mat_runtime_id
											(concat id
												(runtime_cache_suffix_from_exprs
													(list materialized_subquery))
												(user_session_runtime_cache_suffix_from_exprs
													(list materialized_subquery))))
										(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": materialize after runtime id")) nil)
										(define materialized_binding
											(legacy_materialized_query_term_binding_ast
												mat_runtime_id materialized_subquery rows_sym row_sink_sym mat_limit cnt_sym))
										(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": materialize after binding")) nil)
										(define mat_source (nth materialized_binding 0))
										(define mat_init (nth materialized_binding 1))
										(sq_cache_add_init_once mat_source mat_init)
										(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": materialize after init cache")) nil)
										(define mat_schema_def
											(register_materialized_subquery_metadata mat_source pruned_fields2 outer_uses_subquery_group_boundary))
										(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": materialize after metadata")) nil)
										(list
											(list (list id schemax mat_source isOuter joinexpr))
											'()
											true
									...
									(begin
										/* For LEFT JOIN, most condition2 terms were integrated into joinexpr
										to preserve ON semantics. Terms with NULL-default wrappers (COALESCE/
										COALESCE_NIL) are deliberately not attached to the nullable helper's
										ON clause above; they are post-join filters such as EXISTS/NOT EXISTS
										tests over a NULL-extended aggregate helper. Keep those terms here
										instead of dropping them via globalFilter=true. */
										(define condition_term_has_null_default (lambda (expr) (match expr
											(cons sym args)
											(if (or
												(equal?? sym (quote coalesce))
												(equal?? sym (quote (quote coalesce)))
												...
												(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
													false
													(reduce args (lambda (found arg)
														(or found (condition_term_has_null_default arg)))
														false)))
											false)))
										(define outer_post_filter
											(combine_and_terms (filter
												(flatten_and_terms (coalesceNil condition2_transformed true))
												condition_term_has_null_default)))
										/* for INNER JOIN: condition2 becomes global filter (can be reordered) */
										(set globalFilter (if isOuter outer_post_filter (replace_column_alias condition2)))
										(define resolvedGlobalFilter (replace_column_alias globalFilter))
										(if (not (equal? (extract_aggregates (coalesceNil resolvedGlobalFilter true)) '()))
											(begin
												(define attach_having_to_last_stage (lambda (stage_list) (match stage_list
													(cons only_stage '())
													(list (stage_set only_stage (quote having)
														(combine_and_terms (list
															(stage_having_expr only_stage)
															resolvedGlobalFilter))))
													(cons head_stage rest_stages)
													(cons head_stage (attach_having_to_last_stage rest_stages))
													_ stage_list)))
												(define existing_flatten_groups (coalesceNil (sq_cache "groups") '()))
												(if (not (equal? existing_flatten_groups '()))
													(begin
														(sq_cache "groups" (attach_having_to_last_stage existing_flatten_groups))
														(set globalFilter true))
													nil))
											nil)
										(define projection_prefixed_aliases (map tablesPrefixed (lambda (td) (match td
											'(a _ _ _ _) a
											_ nil))))
										(define projection_alias_rank (lambda (alias_name)
											(match (reduce projection_prefixed_aliases (lambda (state current_alias) (match state
												'(idx found)
												(list (+ idx 1)
													(if (or (not (nil? found)) (not (equal?? current_alias alias_name)))
														found
														idx))
												_ state))
												(list 0 nil))
												'(_ found) found
												_ nil)))
										(define projection_expr_rank (lambda (expr)
											(reduce (extract_tblvars expr) (lambda (rank alias_name)
												(begin
													(define alias_rank (projection_alias_rank alias_name))
													(if (nil? alias_rank)
														rank
														(if (> alias_rank rank) alias_rank rank))))
												-1)))
										(define check_inner_select (lambda (expr) (match expr
											(cons sym args) (if (or
												(equal?? sym (quote inner_select))
												(equal?? sym (quote (quote inner_select)))
												(equal?? sym (symbol inner_select))
												...
												(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
													false
													(reduce args (lambda (a b) (or a (check_inner_select b))) false)))
											false)))
										(define projection_joinexpr_unqualified_cols (extract_unqualified_columns joinexpr2))
										(define projection_joinexpr_resolvable (reduce projection_joinexpr_unqualified_cols (lambda (ok col)
											(and ok
												(reduce_assoc schemas2 (lambda (found _alias cols)
													(or found
														(reduce cols (lambda (col_found coldef)
															(or col_found (equal?? (coldef "Field") col)))
															false)))
													false)))
											true))
										(define projection_joinexpr (if (and (not (nil? joinexpr2)) projection_joinexpr_resolvable)
											(replace_column_alias joinexpr2)
											joinexpr2))
										(define projection_joinexpr_resolved (and projection_joinexpr_resolvable (equal? (extract_unqualified_columns projection_joinexpr) '())))
										(define projection_expr_has_null_default (lambda (expr) (match expr
											(cons sym args)
											(if (or
												(equal?? sym (quote coalesce))
												(equal?? sym (quote (quote coalesce)))
												...
												(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
													false
													(reduce args (lambda (found arg)
														(or found (projection_expr_has_null_default arg)))
														false)))
											false)))
										(define projection_guard_for_expr (lambda (expr)
											(begin
												(define expr_rank (projection_expr_rank expr))
												(define effective_rank (if (< expr_rank 0)
													(- (count projection_prefixed_aliases) 1)
													expr_rank))
												(combine_and_terms (filter (flatten_and_terms (coalesceNil projection_joinexpr true)) (lambda (term)
													(begin
														(define term_rank (projection_expr_rank term))
														(or
															(< term_rank 0)
															(<= term_rank effective_rank)
															(projection_expr_has_null_default term)))))))))
										(define wrap_outer_join_projection (lambda (expr)
											(begin
												(define projection_guard (projection_guard_for_expr expr))
												(if (and
													isOuter
													(not (equal? joinexpr true))
													projection_joinexpr_resolved
													(not (nil? projection_guard))
													(not (equal? projection_guard true))
													(not (check_inner_select projection_guard))
													(not (projection_expr_has_null_default expr))
													/* Column-backed projections are already NULL-extended by the
													LEFT scan itself. Guard only closed expressions such as constants;
													otherwise sibling scalar helpers in JOIN ON can leak as dangling
													alias refs into the projection guard. */
													(equal? (extract_tblvars expr) '()))
													(list (quote if) projection_guard expr nil)
													expr))))
										(define flattened_result
											(list
												tablesPrefixed
												(list id (map_assoc pruned_fields2 (lambda (k v) (wrap_outer_join_projection (replace_column_alias v)))))
												globalFilter
												(merge
													(list id (extract_assoc pruned_fields2 (lambda (k v) (list "Field" k "Type" "any" "Expr" (replace_column_alias v)))))
													(merge (extract_assoc schemas2 (lambda (k v) (list (concat id "\0" k) v)))))))
										(if (< (count flattened_result) 4)
											(error (concat "DERIVED_FLATTEN_RESULT_SHORT "
												(serialize (list id tablesPrefixed pruned_fields2 globalFilter schemas2))
												" => "
												(serialize flattened_result)))
											nil)
										flattened_result
									)
							))
							(if (< (count derived_result) 4)
								(error (concat "DERIVED_RESULT_SHORT "
									(serialize (list id subquery subquery_untangled))
									" => "
									(serialize derived_result)))
								nil)
							(if neumann_pipeline_trace (print (concat "[neumann] legacy derived " id ": after derived result")) nil)
							derived_result
						) (error "non matching return value for untangle_query")))
						(if (< (count derived_zip_result) 4)
							(error (concat "DERIVED_MATCH_RESULT_SHORT "
								(serialize (list id subquery subquery_untangled))
								" => "
	...
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after zipped inputs") nil)
	(set zipped (zip zippedInputs))
	(if (or (nil? zipped) (equal? zipped '()))
		(error (concat "UNTANGLE_ZIPPED_EMPTY "
			(serialize (list schema tables fields condition group having order schemas))
			" :: "
			(serialize zippedInputs)))
		nil)
	(if (< (count zipped) 4)
		(error (concat "UNTANGLE_ZIPPED_SHORT "
			(serialize (list schema tables fields condition group having order schemas))
			" tables_count="
			(string (count tables))
			" table_shape="
			(serialize (map tables (lambda (td)
				(if (list? td)
					(count td)
					td))))
			" :: "
			" zipped_inputs_shape="
			(serialize (map zippedInputs (lambda (zi)
				(if (list? zi)
					(count zi)
					zi))))
			" "
	...
	(define schema_alias_matches (lambda (query_alias schema_alias ti)
		(reduce (schema_alias_variants schema_alias) (lambda (matched alias_v)
			...
			false)))
	(define planner_fields_have_pending_subqueries
		(reduce_assoc fields (lambda (found _col field_expr)
			(or found (contains_inner_select_marker field_expr)))
			false))
	(define replace_find_column (lambda (expr) (match expr
			...
			(define find_column_collation (lambda (tblalias colname) (begin
				...
				(define alias_lookup
					(coalesce
						...
						nil))
				(define cols (if (nil? alias_lookup) nil (schema_assoc_cols schemas alias_lookup)))
		...
		'((symbol get_column) nil _ col ci) (begin
			...
			(define is_main_alias (lambda (alias) (begin
				(define s (string alias))
				(and (not (strlike s "%:%"))
					(not (strlike s "%\0%"))
			...
			(define main_match (reduce_assoc schemas (lambda (a alias cols)
				(begin
					(define norm_cols (schema_assoc_cols schemas alias))
					(if (and (is_main_alias alias) (reduce norm_cols (lambda (a coldef) (or a ((if ci equal?? equal?) (coldef "Field") col))) false))
						alias a))) nil))
			...
			(define any_match (if (nil? main_match)
				(reduce_assoc schemas (lambda (a alias cols)
					(begin
						(define norm_cols (schema_assoc_cols schemas alias))
						(if (reduce norm_cols (lambda (a coldef) (or a ((if ci equal?? equal?) (coldef "Field") col))) false)
							alias a))) nil)
				main_match))
			(begin
				(define outer_matches (if (nil? any_match)
					(reduce_assoc outer_schemas_chain (lambda (acc outer_alias outer_cols)
						(begin
							(define norm_outer_cols (schema_assoc_cols outer_schemas_chain outer_alias))
							(if (reduce norm_outer_cols (lambda (a coldef)
				...
				(define outer_matches (if (nil? any_match)
					(reduce outer_matches (lambda (acc outer_alias)
						...
						'())
					'()))
				(define resolved_alias (if (nil? any_match)
					(match outer_matches
						(cons outer_only '()) outer_only
						'() (if planner_fields_have_pending_subqueries
							nil
							(error (concat "column " col " does not exist in tables")))
						(cons _ _) (error (concat "ambiguous outer column " col)))
					any_match))
				(if (nil? resolved_alias)
					expr
					(begin
						(define resolved_schema_space (if (has_assoc? schemas resolved_alias) schemas outer_schemas_chain))
						(define canonical_col (if ci (coalesce (reduce (schema_assoc_cols resolved_schema_space resolved_alias) (lambda (a coldef) (if (not (nil? a)) a (if (equal?? (coldef "Field") col) (coldef "Field") nil))) nil) col) col))
			...
			(if (nil? resolved_alias)
				...
				(begin
					(define canonical_col (if ci
						(coalesce (reduce (schema_assoc_cols schemas resolved_alias) (lambda (a coldef) (if (not (nil? a)) a (if (equal?? (coldef "Field") col) (coldef "Field") nil))) nil) col)
						col))
					'((quote get_column) resolved_alias false canonical_col false))))
			...
			(if (nil? resolved_alias)
				...
				(begin
					(define canonical_col (if ci
						...
						col))
					'((quote get_column) resolved_alias false canonical_col false))))
		(cons sym args) /* function call */ (if (or (is_opaque_scope_sym sym) (not (list? args)))
			expr
			(if (subquery_marker_head? sym)
				expr
	...
	(define ris_schemas (merge schemas outer_schemas_chain))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: before collect markers") nil)
	(if neumann_pipeline_trace (print (concat "[neumann] legacy untangle: neumann_lowered=" (string (planner_neumann_lowered_current)))) nil)
	(define table_joinexpr_has_inner_select
		(reduce tables (lambda (found td) (or found (match td
			'(_tv _tschema _ttbl _toisOuter tje)
			(contains_inner_select_marker (coalesceNil tje true))
			false)))
			false))
	(define fields_have_inner_select
		(reduce_assoc fields (lambda (found _k v)
			(or found (contains_inner_select_marker v)))
			false))
	(if (or
		(not (planner_neumann_lowered_current))
		table_joinexpr_has_inner_select
		fields_have_inner_select)
		(begin
			(set tables (map tables (lambda (td) (match td
				'(tv tschema ttbl toisOuter tje)
				(list tv tschema ttbl toisOuter
					(if (nil? tje) nil (collect_row_domain_subselects tje ris_schemas)))
				td))))
			(if (or (not (planner_neumann_lowered_current)) fields_have_inner_select)
				(set fields (map_assoc fields (lambda (k v) (collect_dependent_scalar_compile_markers v ris_schemas))))
				nil))
		nil)
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after field marker collect") nil)
	...
	(if (or
		(not (planner_neumann_lowered_current))
		(contains_inner_select_marker condition))
		(begin
			(set condition (collect_dependent_scalar_compile_markers condition ris_schemas))
			(set group (map group (lambda (g) (collect_row_domain_subselects g ris_schemas)))))
		nil)
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after condition/group marker collect") nil)
	(if (not (planner_neumann_lowered_current))
		(begin
			(set having (begin
				(define hv_resolved (replace_inner_selects having ris_schemas))
				/* check if any inner_select nodes remain — HAVING with subqueries
				requires post-group processing which is not yet implemented */
				(define hv_check (lambda (expr) (match expr
					(cons sym args) (if (or
						(equal?? sym (quote inner_select))
						(equal?? sym (quote (quote inner_select)))
						(equal?? sym (symbol inner_select))
						...
						true
						(if (list? args)
							(reduce args (lambda (a b) (or a (hv_check b))) false)
							false))
					false)))
				(if (and (not (nil? hv_resolved)) (hv_check hv_resolved))
					(error "HAVING with subqueries not yet supported")
					hv_resolved)))
			(set order (map order (lambda (o) (match o '(col dir) (list (collect_row_domain_subselects col ris_schemas) dir))))))
		nil)
	...
	(set condition (freeze_visible_field_refs condition))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after freeze visible refs") nil)
	(set fields (map_assoc fields (lambda (k v)
		(resolve_dependent_scalar_compile_markers v ris_schemas))))
	(set condition (resolve_dependent_scalar_compile_markers condition ris_schemas))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after resolve scalar markers") nil)
	...
	(define sq_tbls (coalesceNil (sq_cache "tables") '()))
	(define sq_helper_specs (filter sq_tbls (lambda (td) (match td
		'(_ _ ttbl _ _) (is_dependent_join_helper_spec ttbl)
		_ false))))
	(define sq_helper_aliases (map sq_helper_specs (lambda (td) (match td
		'(tv _ _ _ _) tv
		_ nil))))
	(define rewrite_helper_logical_expr (lambda (expr) (begin
		(define rewritten
			(reduce sq_helper_specs (lambda (current td) (match td
				'(tv _ ttbl _ _)
				(match current
					'((quote get_column) alias_ _ col _) (if (and
						(equal?? alias_ tv)
						(equal? col (dependent_join_helper_spec_result_col ttbl)))
						(begin
							(define helper_legacy_expr (dependent_join_helper_spec_legacy_expr ttbl))
							(if (equal? (extract_aggregates helper_legacy_expr) '())
								helper_legacy_expr
								current))
						current)
					'((symbol get_column) alias_ _ col _) (if (and
						(equal?? alias_ tv)
						(equal? col (dependent_join_helper_spec_result_col ttbl)))
						(begin
							(define helper_legacy_expr (dependent_join_helper_spec_legacy_expr ttbl))
				...
				_ current))
				expr))
		(if (equal? rewritten expr)
			(match expr
				(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
					expr
					(cons (rewrite_helper_logical_expr sym) (map args rewrite_helper_logical_expr)))
				_ expr)
			rewritten))))
	(set condition (rewrite_helper_logical_expr condition))
	(set having (rewrite_helper_logical_expr having))
	(define having_refs_helper (reduce (extract_tblvars (coalesceNil having true)) (lambda (found alias_)
		(or found (has? sq_helper_aliases (string alias_))))
		false))
	(if (and having_refs_helper (equal? (extract_aggregates (coalesceNil having true)) '()))
		(begin
			(set condition (list (quote and) condition having))
			(set having nil))
		nil)
	(define dep_join_needs_dual_init (reduce sq_tbls (lambda (found td) (or found (match td
		'(_ _ ttbl _ _) (is_dependent_join_helper_spec ttbl)
		false))) false))
	(if dep_join_needs_dual_init
		(sq_cache "init" (merge (coalesceNil (sq_cache "init") '())
			(list (list (quote begin)
				(list (quote createtable) schema ".(1)"
					(list (quote quote) (list (list "unique" "group" (list "1")) (list "column" "1" "any" '() '())))
				...
				(list (quote if)
					...
					nil)))))
		nil)
	(define sq_dep_renames (reduce sq_tbls (lambda (acc td) (match td
	...
	(define dep_rewrite_domain_expr (lambda (unnesting_info expr) (begin
		(define parent_info (dependent_expr_unnesting_info_parent unnesting_info))
		(define normalize_bare_column_symbol (lambda (node) (begin
			(if (equal? node (symbol (string node)))
				(match (split (string node) ".")
					(list alias_ col) (list (quote get_column) alias_ false col false)
					_ node)
				node))))
		(define parent_domain_ref (lambda (node)
			(if (nil? parent_info)
				nil
				(reduce (dependent_expr_unnesting_info_domain parent_info) (lambda (found dc) (match dc
					'(dom_col dom_expr)
					(if (or
						(not (nil? found))
						(not (equal? (semijoin_unmark_outer_expr dom_expr) (semijoin_unmark_outer_expr node))))
						found
						(list (quote get_column) domain_alias false dom_col false))
					_ found))
					nil))))
		(define rewrite_parent_repr (lambda (node) (begin
			(define normalized_node (normalize_bare_column_symbol node))
			(define parent_repr
				(coalesce
					(parent_domain_ref normalized_node)
					(if (nil? parent_info)
						nil
						(dependent_join_helper_repr_lookup parent_info normalized_node))))
			(if (and
				(not (nil? parent_repr))
				(list? parent_repr))
				parent_repr
				(match normalized_node
					(cons sym args) (if (or
						(is_opaque_scope_sym sym)
						(is_quote_scope_sym sym)
						(not (list? args)))
						normalized_node
						(cons sym (map args rewrite_parent_repr)))
					_ normalized_node)))))
		(define rewritten_expr (rewrite_parent_repr expr))
		(match rewritten_expr
			'((quote get_column) alias_ _ col _)
			(if (nil? alias_)
				rewritten_expr
				(coalesce (dep_lookup_renamed_column alias_ col) rewritten_expr))
			'((symbol get_column) alias_ _ col _)
			(if (nil? alias_)
				rewritten_expr
				(coalesce (dep_lookup_renamed_column alias_ col) rewritten_expr))
			_ rewritten_expr))))
	(define dep_rewrite_domain_cols (lambda (domain_cols unnesting_info)
		(map domain_cols (lambda (dc) (match dc
			'(dom_col dom_expr)
			(begin
				(define rewritten (dep_rewrite_domain_expr unnesting_info dom_expr))
		...
		(if (is_dependent_join_helper_spec ttbl)
			(begin
				...
				(define dep_spec_domain_cols
					(begin
						(define dep_spec_unnesting_info
							(dependent_join_helper_spec_unnesting_info ttbl))
						(define dep_parent_unnesting_info
							(dependent_expr_unnesting_info_parent dep_spec_unnesting_info))
						(merge_unique
							(dep_rewrite_domain_cols
								(dependent_join_helper_spec_domain_cols ttbl)
								dep_spec_unnesting_info)
							(if (nil? dep_parent_unnesting_info)
								'()
								(dep_rewrite_domain_cols
									(dependent_expr_unnesting_info_domain dep_parent_unnesting_info)
									dep_spec_unnesting_info)))))
				...
				(define dep_active_domain_cols
					(dependent_join_helper_active_domain_cols
						dep_source_query
						dep_spec_domain_cols))
				(define dep_join_domain_cols
					(filter dep_active_domain_cols (lambda (dc) (match dc
						'(dom_col dom_expr)
						(dep_expr_only_visible_outer_refs dom_expr)
						false))))
				(define dep_result_col
					(dependent_join_helper_spec_result_col ttbl))
				(define dep_info
					(dependent_join_helper_spec_dep_info ttbl))
				(define dep_source_preserves_full_domain
					(dependent_join_helper_source_preserves_full_domain dep_source_query dep_result_col))
				(define dep_preserve_full_domain
					(and
						(not dep_source_preserves_full_domain)
				...
				(define dep_materialize_source_query
					(if dep_preserve_full_domain
						(begin
							(define dep_domain_runtime_suffix
								(runtime_cache_suffix_from_exprs dep_active_domain_cols))
							(define dep_domain_alias
								(if (equal? dep_domain_runtime_suffix "")
									"__dep_full_domain"
									(concat "__dep_full_domain_" (fnv_hash dep_domain_runtime_suffix))))
							(define dep_match_alias "__dep_full_match")
							(define dep_subquery_having_trivial
								(match (dependent_expr_compile_info_subquery dep_info)
									'(_ _ _ _ _ dep_having _ _ _)
									(or (nil? dep_having) (equal? dep_having true))
									_ true))
							(define dep_empty_result
								(if (and
									...
									(if (or
										...
										false
										nil)))
							(define dep_full_domain_flattened_expr (lambda (expr) (match expr
								'((quote get_column) alias_ ti col ci)
								(if (nil? alias_)
									expr
									(list (quote get_column) (concat dep_domain_alias "\0" alias_) ti col ci))
								'((symbol get_column) alias_ ti col ci)
								(if (nil? alias_)
									expr
									(list (quote get_column) (concat dep_domain_alias "\0" alias_) ti col ci))
								...
								(if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
									expr
									(cons (dep_full_domain_flattened_expr sym)
										(map args dep_full_domain_flattened_expr)))
								_ expr)))
							(define dep_full_domain_cols
								(map dep_active_domain_cols (lambda (dc) (match dc
									'(dom_col dom_expr)
									(list dom_col (dep_full_domain_flattened_expr dom_expr))
									_ dc))))
							(define dep_full_domain_query
								(dependent_join_helper_domain_query
									dep_active_domain_cols
									tables
									(dep_rewrite_domain_expr
										(dependent_join_helper_spec_unnesting_info ttbl)
										dep_outer_condition)
									group
									having))
							(match dep_source_query
								'(dep_schema _ _ _ _ _ _ _ _)
								(list dep_schema
									(list
										(list dep_domain_alias dep_schema dep_full_domain_query false nil)
										(list dep_match_alias dep_schema dep_source_query true
											(dependent_join_helper_domain_joinexpr dep_match_alias dep_full_domain_cols)))
									(merge
										(dependent_join_helper_domain_field_assoc dep_full_domain_cols dep_domain_alias)
										(list dep_result_col
											(list
												(quote if)
												(list
				...
				(define dep_domain_joinexpr
					(dependent_join_helper_domain_joinexpr tv dep_join_domain_cols))
				(define dep_same_scope_joinexpr
					(rewrite_outer_refs_as_columns dep_domain_joinexpr))
				(define dep_rows_sym (symbol (concat "__dhr:" tv)))
				(define dep_sink_sym (symbol (concat "__dhs:" tv)))
				...
				(define dep_helper_structural_key
					(list
						dep_materialize_source_query
						dep_active_domain_cols
						(dependent_join_helper_spec_domain_cols ttbl)
						(dependent_expr_unnesting_info_parent
							(dependent_join_helper_spec_unnesting_info ttbl))))
				(define dep_helper_cache_name
					(concat tv ":dep_helper:"
						(fnv_hash (serialize dep_helper_structural_key))
						(if (dependent_expr_compile_info_uses_session_state dep_info)
							(planner_current_session_snapshot_suffix)
							"")))
				(define dep_mat_binding
					(coalesce
						(dep_helper_keytable_binding
							dep_helper_cache_name
							dep_materialize_source_query
							dep_rows_sym
							dep_sink_sym
							nil
				...
				(define dep_user_offset
					(match dep_subquery_for_contract
						...
						_ nil))
				(define dep_user_order
					(match dep_subquery_for_contract
						'(_ _ _ _ _ _ user_order _ _) (begin
							(define build_user_order (lambda (items idx) (match items
								nil '()
								(cons oi rest)
								(cons
									(match oi
										'(_col dir) (list (list (quote get_column) tv false (concat "__dep_o" idx) false) dir)
										_ nil)
									(build_user_order rest (+ idx 1))))))
							(filter (build_user_order (coalesceNil user_order '()) 0)
				...
				(list tv tschema dep_helper_source true
					(if (or (nil? dep_same_scope_joinexpr) (equal? dep_same_scope_joinexpr true))
						nil
						dep_same_scope_joinexpr)))
			td)
		_ td))))
	(define sq_tbl_alias (lambda (td) (match td
		'(tv _ ttbl _ _) (if (nil? tv) ttbl tv)
		nil)))
	(define sq_tbl_dependency_order (lambda (helper_tables) (begin
		(define helper_aliases (map helper_tables sq_tbl_alias))
		...
		(define order_step (lambda (ordered remaining) (begin
			(define ordered_aliases (map ordered sq_tbl_alias))
			...
			(if (equal? ready '())
				...
				(order_step
					...
					(filter remaining (lambda (td)
						(not (has? (map ready sq_tbl_alias) (sq_tbl_alias td))))))))))
		(order_step '() helper_tables))))
	(define sq_tbls (sq_tbl_dependency_order sq_tbls))
	(define sq_scalar_tbls (coalesceNil (sq_cache "scalar_tables") '()))
	...
	recognize that two unn_ aliases describe the same LEFT JOIN relation once
	...
	(define scalar_left_join_dedup_key (lambda (td) (match td
		'(tv tschema ttbl isOuter joinexpr) (begin
			(define dedup_alias_map
				(reduce (alias_lookup_variants tv) (lambda (acc alias_v)
					(set_assoc acc (string alias_v) "__scalar_left_join__"))
					'()))
			(serialize (list
				tschema
				(rewrite_source_aliases dedup_alias_map ttbl)
				isOuter
				(rewrite_source_aliases dedup_alias_map (coalesceNil joinexpr true)))))
		nil)))
	(define sq_scalar_dedup_state (if scalar_query_has_opaque_runtime
		(list sq_scalar_tbls '() '())
		(reduce sq_scalar_tbls (lambda (state td) (match state
			'(kept key_map alias_map) (match td
				'(tv _ _ _ _) (begin
					(define dedup_key (scalar_left_join_dedup_key td))
					(define canonical_alias (get_assoc key_map dedup_key))
					(if (nil? canonical_alias)
						(list
							(merge kept (list td))
							(set_assoc key_map dedup_key tv)
							alias_map)
						(list
							kept
							key_map
							(reduce (alias_lookup_variants tv) (lambda (acc alias_v)
								(set_assoc acc (string alias_v) canonical_alias))
								alias_map))))
				_ td)
			_ state))
			(list '() '() '()))))
	(define sq_scalar_tbls (nth sq_scalar_dedup_state 0))
	(define sq_scalar_alias_map (nth sq_scalar_dedup_state 2))
	(define rewrite_scalar_left_join_alias (lambda (alias_)
		(coalesceNil (get_assoc sq_scalar_alias_map (string alias_)) alias_)))
	(define rewrite_scalar_left_join_aliases (lambda (expr)
		(if (equal? sq_scalar_alias_map '())
			expr
			(rewrite_source_aliases sq_scalar_alias_map expr))))
	(define dedupe_logical_stages (lambda (stages)
		(match (reduce (coalesceNil stages '()) (lambda (state stage) (match state
			...
			'(out _) out
			_ stages)))
	(if (not (equal? sq_scalar_alias_map '()))
		(begin
			(set tables (map tables (lambda (td) (match td
				'(tv tschema ttbl toisOuter tje)
				(list tv tschema ttbl toisOuter
	...
	(define sq_scalar_condition_tbls (filter sq_scalar_tbls (lambda (t) (match t
		'(tv _ ttbl _ _)
		(has? condition_ref_aliases (if (nil? tv) ttbl tv))
		false))))
	(define sq_scalar_projection_tbls (filter sq_scalar_tbls (lambda (t)
	...
	(define sq_scalar_helper_depends_on_alias? (lambda (helper_td alias_name) (match helper_td
		'(_ _ _ _ helper_joinexpr)
		(and
			(not (nil? helper_joinexpr))
			(has? (extract_tblvars helper_joinexpr) alias_name))
		_ false)))
	(define tables_with_joinexpr_scalar_helpers
		(match (reduce tables (lambda (state td) (match state
			'(out remaining) (begin
				(define td_alias (match td '(tv _ _ _ _) tv nil))
				...
				(define emit_helpers (filter remaining (lambda (ht)
					(and
						...
						(not (sq_scalar_helper_depends_on_alias? ht td_alias))))))
			...
			tables))
	(set tables (merge tables_with_joinexpr_scalar_helpers sq_scalar_condition_tbls sq_tbls sq_scalar_tail_projection_tbls))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after sq table merge") nil)
	(define sq_helper_output_fields (lambda (alias)
		(reduce sq_helper_specs (lambda (found td) (match td
			'(tv _ ttbl _ _)
			(if (or (not (nil? found)) (not (equal? tv alias)))
				found
				(match (dependent_join_helper_spec_source_query ttbl)
					'(_ _ dep_fields _ _ _ _ _ _)
			...
			nil)))
	(define sq_schs
		(map_assoc (coalesceNil (sq_cache "schemas") '()) (lambda (alias schema_def)
			(begin
				(define active_fields (sq_helper_output_fields alias))
				(if (nil? active_fields)
					schema_def
					(filter schema_def (lambda (coldef)
						(has? active_fields (coldef "Field")))))))))
	(if (not (equal? sq_schs '())) (set schemas (merge schemas sq_schs)))
	(if (not (equal? sq_dep_renames '()))
		(set renamelist (merge renamelist sq_dep_renames)))
	...
	(define sq_local_scalar_stage_aliases (merge_unique (map (coalesceNil groups '()) (lambda (stage)
		(if (and
			(not (nil? (stage_partition_aliases stage)))
			(not (nil? (stage_limit_val stage)))
			...
			'())))))
	(define sq_global_joinexpr_tbls (filter sq_tbls (lambda (t) (match t
		'(tv _ _ _ _) (and
			(not (has? sq_local_scalar_stage_aliases tv))
			(not (has? sq_helper_aliases tv)))
		true))))
	(define sq_jes (filter (map sq_global_joinexpr_tbls (lambda (t) (match t '(_ _ _ _ je) je nil))) (lambda (x) (not (nil? x)))))
	(set condition (if (equal? sq_jes '()) condition (cons (quote and) (cons condition sq_jes))))
	(define sq_local_helper_joinexprs (filter (map tables (lambda (t) (match t
		'(_ _ _ isOuter je) (if isOuter je nil)
		nil))) (lambda (x) (not (nil? x)))))
	(set conditionList (filter conditionList (lambda (part)
		(not (reduce sq_local_helper_joinexprs (lambda (found helper_joinexpr)
			(or found (equal? helper_joinexpr part))) false)))))
		(define sq_prop_groups (dedupe_logical_stages
			(if (equal? sq_scalar_alias_map '())
				(coalesceNil (sq_cache "groups") '())
				(map (coalesceNil (sq_cache "groups") '()) (lambda (stage)
					(rewrite_stage_for_flattened_aliases
						stage
						rewrite_scalar_left_join_aliases
						rewrite_scalar_left_join_alias))))))
		(define sq_prop_groups_from_flattened_derived
			(reduce sq_prop_groups (lambda (found stage)
				(or found (equal? (stage_get stage (quote flattened-derived) false) true)))
				false))
		(set groups (if (equal? sq_prop_groups '()) groups (merge sq_prop_groups (coalesceNil groups '()))))
	(if (not (equal? (coalesceNil groups '()) '()))
		(begin
			(define condition_terms_for_group (flatten_and_terms (coalesceNil condition true)))
			(define condition_aggregate_terms (filter condition_terms_for_group (lambda (term)
				(not (equal? (extract_aggregates term) '())))))
			(if (not (equal? condition_aggregate_terms '()))
				(begin
					(define condition_plain_terms (filter condition_terms_for_group (lambda (term)
						(equal? (extract_aggregates term) '()))))
					(define append_having_to_first_stage (lambda (stage_list) (match stage_list
						(cons first_stage rest_stages)
						(cons
							(stage_set first_stage (quote having)
								(combine_and_terms (merge
									(flatten_and_terms (coalesceNil (stage_having_expr first_stage) true))
									condition_aggregate_terms)))
							rest_stages)
						_ stage_list)))
					(set condition (combine_and_terms condition_plain_terms))
					(set groups (append_having_to_first_stage groups)))
				nil))
		nil)
	...
	(define canonicalize_for_rename (lambda (expr) (match expr
		'((symbol get_column) alias_ ti col ci) (if (and ci (not (nil? alias_)))
			(if (has_assoc? renamelist (string alias_))
				(begin
					(define alias_cols (schema_assoc_cols schemas (string alias_)))
		...
		(cons sym args) (if (is_opaque_scope_sym sym)
			expr
			(if (list? args) (cons sym (map args canonicalize_for_rename)) expr))
		expr
	)))
	...
	(define replace_rename (lambda (expr) (match expr
		...
		(cons sym args) /* function call */ (if (is_opaque_scope_sym sym)
			expr
			(if (list? args) (cons sym (map args replace_rename)) expr))
		expr
	)))
	...
	(define finalize_visible_expr (lambda (expr)
		(begin
			(define resolved_expr
				(resolve_dependent_scalar_compile_markers expr planner_visible_schemas))
			(define rewrite_helper_visible_expr (lambda (e) (begin
				(define rewritten
					(reduce sq_helper_specs (lambda (current td) (match td
						'(tv _ ttbl _ _)
						(match current
						...
						_ current))
						e))
				(if (equal? rewritten e)
					(match e
						(cons sym args) (if (is_opaque_scope_sym sym)
							e
							(if (list? args)
								(cons (rewrite_helper_visible_expr sym) (map args rewrite_helper_visible_expr))
								e))
						_ e)
					rewritten))))
			(define visible_expr
				(rewrite_helper_visible_expr resolved_expr))
	...
	(set fields (map_assoc (expand_star_fields_with_schemas fields schemas) (lambda (col expr)
		(finalize_visible_expr
			(if (contains_inner_select_marker expr)
				(resolve_dependent_scalar_compile_markers
					(collect_dependent_scalar_compile_markers expr ris_schemas)
					ris_schemas)
				(replace_find_column expr))))))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after fields finalize") nil)
	(if (not (equal? sq_scalar_alias_map '()))
		(set fields (map_assoc fields (lambda (k v) (rewrite_scalar_left_join_aliases v)))))
	...
	(set conditionAll (cons 'and (filter
		...
		(lambda (x) (not (nil? x))))))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after condition finalize") nil)
	(set tables (map tables (lambda (td) (match td
		'(tv tschema ttbl toisOuter tje)
		(list tv tschema (finalize_visible_table_ref ttbl) toisOuter
			(if (nil? tje) nil (finalize_visible_expr tje)))
		td))))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after tables finalize") nil)
	...
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after group/order/having finalize") nil)
	(define having_helper_group_exprs
		(filter (map (extract_all_get_columns having) (lambda (ref) (match ref
			'(ref_key ref_expr)
			(match (split ref_key ".")
				(list ref_alias _ref_col)
				(if (has? sq_helper_aliases ref_alias) ref_expr nil)
				_ nil)
			_ nil)))
			(lambda (expr) (not (nil? expr)))))
	(set group (dedupe_canonical_exprs (merge group having_helper_group_exprs)))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after helper group merge") nil)
	...
	(define stage_live_aliases (lambda (stage) (merge_unique (list
		...
		(extract_all_table_aliases (coalesceNil (stage_condition stage) true))
		(merge (map (coalesceNil (stage_order_list stage) '()) (lambda (o) (match o
			'(col _dir) (extract_all_table_aliases col)
			_ '()))))))))
	(define cd_live_source_aliases
		(if cd_has
			(merge
				(map cd_distinct_exprs extract_all_table_aliases)
				(merge (map tables (lambda (td) (match td
					'(alias _ _ true _)
					(if (reduce (schema_assoc_cols schemas alias) (lambda (found coldef)
						(or found (cd_find (coalesceNil (coldef "Expr") nil))))
						false)
						(list (string alias))
						'())
					_ '())))))
			'()))
	(define all_referenced_aliases (merge_unique (list
		(extract_all_table_aliases fields)
		cd_live_source_aliases
		(merge (map (coalesceNil sq_prop_groups '()) stage_live_aliases))
	...
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after referenced alias collect") nil)
	(set tables (filter tables (lambda (td) (match td
		'(alias _ _ isOuter _) (or (not isOuter) (has? all_referenced_aliases (string alias)))
		true))))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after left join prune 1") nil)

	(define groups (merge
		(coalesceNil sq_prop_groups '())
		(if (coalesce cd_distinct_exprs false)
			/* COUNT(DISTINCT): two group stages - first dedup, then aggregate */
			(list
				(make_dedup_stage
					(merge
						(map (coalesce cd_user_group '()) finalize_visible_expr)
						(map cd_distinct_exprs (lambda (e) (replace_find_column (finalize_visible_expr e)))))
					nil)
				(make_group_stage
					(if (or (nil? cd_user_group) (equal? cd_user_group '()))
						'(1)
						(map cd_user_group (lambda (e) (replace_find_column (finalize_visible_expr e)))))
					(cd_replace (finalize_visible_expr cd_having))
					(map (coalesce cd_order '()) (lambda (o) (match o '(col dir) (list (cd_replace (finalize_visible_expr col)) dir))))
					cd_limit cd_offset nil nil))
			/* normal: single group stage */
			(begin
				(define fields_have_aggregate
					(reduce_assoc fields (lambda (found _k v)
						(or found (not (equal? (extract_aggregates v) '()))))
						false))
				(define fields_have_window
					(reduce_assoc fields (lambda (found _k v)
						(or found (not (equal? (extract_window_funcs v) '()))))
						false))
					(define stage_having
						(if (and fields_have_window (equal? having true))
							nil
							having))
					/* Flattened grouped derived tables expose aggregate results as their
					output relation. A parent that only projects/orders that relation must
					not synthesize a new GROUP BY 1 just because those output expressions
					still contain aggregate markers after alias rewriting. */
					(define flattened_group_supplies_aggregates
						(and
							fields_have_aggregate
							sq_prop_groups_from_flattened_derived
							(or (nil? group) (equal? group '()))
							(or (nil? stage_having) (equal? stage_having true))))
					(define aggregate_stage_needed
						(and fields_have_aggregate (not flattened_group_supplies_aggregates)))
					(if (or aggregate_stage_needed (group_stage_requested group stage_having order limit offset))
						(list (make_group_stage
							(if (and aggregate_stage_needed (or (nil? group) (equal? group '())))
								'(1)
								group)
							stage_having order limit offset nil nil))
						'())))))
	...
	(define canon_fields fields)
	(define canon_condition conditionAll)
	(define canon_groups (map (coalesceNil groups '()) (lambda (stage)
	...
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after canon groups") nil)
	...
	(define unnested_aliases (map sq_tbls (lambda (t) (match t '(alias _ _ _ _) alias _ nil))))
	(define joinexpr_dependency_aliases (merge_unique (map tables (lambda (td) (match td
		'(alias _ _ _ je) (filter (if (nil? je) '() (extract_tblvars je)) (lambda (ref_alias)
			(not (equal?? ref_alias alias))))
		'())))))
	(define used_tvs (merge_unique
		unnested_aliases
		joinexpr_dependency_aliases
		(merge (extract_assoc canon_fields (lambda (k v) (extract_tblvars v))))
		(extract_tblvars canon_condition)
		(merge (map canon_groups (lambda (stage)
			(merge_unique
				...
				(coalesceNil (stage_partition_aliases stage) '())))))))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after used tv collect") nil)
	...
	(define has_non_dual (reduce tables (lambda (a t) (or a (match t '(_ _ tbl _ _) (not (equal? tbl ".(1)")) true))) false))
	(define table_alias_used? (lambda (alias_)
		(reduce (alias_lookup_variants alias_) (lambda (found alias_v)
			(or found (has? used_tvs alias_v)))
			false)))
	(define pruned_tables (filter tables (lambda (t) (match t
		'(alias _ tbl isOuter _) (if isOuter (table_alias_used? alias)
			(if (and has_non_dual (equal? tbl ".(1)")) (table_alias_used? alias) true))
		true))))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: after final table prune") nil)
	/* rebuild condition: drop AND-parts that reference ONLY eliminated aliases */
	(define elim_aliases (filter (map tables (lambda (t) (match t
		'(alias _ _ true _) (if (table_alias_used? alias) nil alias)
		nil))) (lambda (x) (not (nil? x)))))
	(define canon_condition (if (equal? (count pruned_tables) (count tables)) canon_condition
		(begin
			/* flatten nested (and ...) to get individual condition parts */
			(define flatten_and (lambda (expr)
				(match expr (cons (symbol and) parts) (if (list? parts) (merge (map parts flatten_and)) (list expr))
					(list expr))))
			(define cond_parts (flatten_and canon_condition))
			/* drop condition parts that reference ANY eliminated alias */
			(define _kept_parts (filter cond_parts (lambda (part)
				(not (reduce (extract_tblvars part) (lambda (acc tv) (or acc (has? elim_aliases tv))) false)))))
			(if (equal? 0 (count _kept_parts)) true
				(if (equal? 1 (count _kept_parts)) (car _kept_parts)
					(cons 'and _kept_parts))))))
	(if neumann_pipeline_trace (print "[neumann] legacy untangle: return select core") nil)
	(define parallel_inits (merge
		(coalesceNil (sq_cache "parallel_inits") '())
		(coalesceNil (sq_cache "direct_parallel_inits") '())))
	(define parallel_init_code
		(if (equal? parallel_inits '())
			'()
...
(define jqr_estimate_rows (lambda (td condition schemas) (begin
	...
	(define cols (schema_assoc_cols schemas alias))
...
(define join_reorder (lambda (schema tables fields condition groups schemas replace_find_column) (begin
	...
	(define jqr_constant_scalar_aliases (reduce (coalesceNil groups '()) (lambda (acc stage)
		(begin
			...
			(define spc (coalesceNil (stage_limit_partition_cols stage) 0))
			(define sos (coalesceNil (stage_outer_sources stage) '()))
			(if (or
				(nil? _spa)
				(not (equal? spc 0))
				(not (equal? sos '()))
...
(define iap_find_td (lambda (iap_tables iap_alias)
	...
	(define build_queryplan_term_from_logical_with_sink (lambda (logical_term sink_mode) (begin
		...
		(define queryplan_internal_result_field? (lambda (name)
			(and
				(string? name)
				(or
					(and (>= (strlen name) 5) (equal? (substr name 0 5) "__kt_"))
					(equal? name "__rn")))))
		(define queryplan_visible_result_fields_for_sink (lambda (fields)
			(match sink_mode
				'(callback _sink_fn) fields
				_ (merge (extract_assoc fields (lambda (field_name field_expr)
					(if (queryplan_internal_result_field? field_name)
						'()
		...
		(if (logical_query_term_is_select_core logical_term)
			(match logical_term '(select_core_term schema tables fields condition groups schemas replace_find_column init) (begin
				(define visible_fields (queryplan_visible_result_fields_for_sink fields))
				(define uq_7tuple (list schema tables visible_fields condition groups schemas replace_find_column))
			(define _reorder (apply join_reorder uq_7tuple))
			(if neumann_pipeline_trace (print "[neumann] after join_reorder") nil)
			(define _plan (apply build_queryplan (merge _reorder (list nil))))
			(if neumann_pipeline_trace (print "[neumann] after build_queryplan") nil)
			(define _plan (match _reorder
				'(r_schema r_tables r_fields r_condition r_groups r_schemas r_rfcol) (begin
					(define r_markers (iap_collect_markers r_groups))
					(if (equal? r_markers '()) _plan
						(begin
							(define anti_plans (map r_markers (lambda (ap_m) (begin
								(define ap_helper_tv (nth ap_m 0))
								(define ap_helper_td (iap_find_td r_tables ap_helper_tv))
								(if (nil? ap_helper_td) nil
									(begin
										(define ap_helper_schema (nth ap_helper_td 1))
										(define ap_helper_tbl (nth ap_helper_td 2))
										(define ap_helper_je (nth ap_helper_td 4))
										(define ap_antifilter (iap_build_antifilter ap_helper_schema ap_helper_tbl ap_helper_je ap_helper_tv))
										(define ap_tables (filter r_tables (lambda (ap_td) (not (equal?? (nth ap_td 0) ap_helper_tv)))))
										(define ap_fields (map_assoc r_fields (lambda (ap_k ap_v) (iap_nullify_helper_refs ap_v ap_helper_tv))))
										(define ap_groups (filter (coalesceNil r_groups '()) (lambda (ap_s) (nil? (stage_anti_pass_marker ap_s)))))
										(define ap_condition_raw (iap_nullify_helper_refs (coalesceNil r_condition true) ap_helper_tv))
										(define ap_condition (if (or (nil? ap_condition_raw) (equal? ap_condition_raw true))
											ap_antifilter
											(list (quote and) ap_condition_raw ap_antifilter)))
										(apply build_queryplan (list r_schema ap_tables ap_fields ap_condition ap_groups r_schemas r_rfcol nil))))))))
							(define valid_anti_plans (filter anti_plans (lambda (ap_p) (not (nil? ap_p)))))
							(if (equal? valid_anti_plans '()) _plan
								(cons (quote begin) (cons _plan valid_anti_plans))))))
				_ _plan))
			(define registered_init (planner_init_register_list init))
			(define plan_with_init (if (equal? registered_init '()) _plan (cons (quote begin) (merge registered_init (list _plan)))))
			(define prejoin_init_defs (if (equal? (planner_compile_scope_depth) 0)
				(planner_prejoin_init_scope_defs)
				'()))
			(define full_plan (parallelize_resultrows
				(if (equal? prejoin_init_defs '())
					plan_with_init
					(cons (quote begin) (merge prejoin_init_defs (list plan_with_init))))))
			(match sink_mode
					'(callback sink_fn) (list (quote begin)
						(list (quote set) (symbol "__tpr") (symbol "resultrow"))
						(list (quote set) (symbol "resultrow")
							(list (quote lambda) (list (symbol "item"))
								(list sink_fn (symbol "item"))))
						full_plan
						(list (quote set) (symbol "resultrow") (symbol "__tpr")))
					_ full_plan)))
							...
							(begin
								...
								(define resolved_branches (map branches (lambda (branch) (begin
									...
									(match branch '(select_core_term schema2 tables2 fields2 condition2 groups2 schemas2 replace_find_column2 init2) (begin
										...
										(define uq7 (list schema2 tables2 fields2 condition2 groups2 schemas2 replace_find_column2))
										(define jr (apply join_reorder uq7))
										(define jr_tables (nth jr 1))
										...
										(define jr_fields (nth jr 2))
										(define jr_condition ((nth jr 6) (coalesceNil (nth jr 3) true)))
...
(define build_queryplan_term_with_sink (lambda (query sink_mode)
	(planner_with_compile_scope (lambda () (begin
		(define logical_term (untangle_query_term query nil))
		(if neumann_pipeline_trace (print "[neumann] after legacy untangle term") nil)
		(build_queryplan_term_from_logical_with_sink logical_term sink_mode))
))))
(define build_queryplan_term_with_sink_neumann_lowered (lambda (query sink_mode)
	(planner_with_compile_scope (lambda () (begin
		...
		(if neumann_pipeline_trace (print (concat "[neumann] sink: flag after set="
			(string (planner_neumann_lowered_current)))) nil)
		(define logical_term (untangle_query_term query nil))
		(planner_neumann_lowered_scope_stack "stack" neumann_old_stack)
		(if neumann_pipeline_trace (print "[neumann] after legacy untangle term") nil)
		(build_queryplan_term_from_logical_with_sink logical_term sink_mode))
))))
(define neumann_exact_replace_find_column (lambda (expr) (match expr
	'((symbol get_column) alias_ _ col _) (if (nil? alias_)
		(symbol col)
		(symbol (concat alias_ "." col)))
	'((quote get_column) alias_ _ col _) (if (nil? alias_)
		(symbol col)
		(symbol (concat alias_ "." col)))
	(cons sym args) (if (list? args)
		(cons sym (map args neumann_exact_replace_find_column))
		expr)
	expr)))

(define neumann_select_core_term_from_tuple (lambda (query) (begin
	(define tuple_fields_have_window
		(not (equal? (extract_window_funcs (qpp-tuple-fields query)) '())))
	(define tuple_having_for_stage
		(if (and tuple_fields_have_window (equal? (qpp-tuple-having query) true))
			nil
	...
	(list
		...
		(qpp-tuple-fields query)
		(coalesceNil (qpp-tuple-condition query) true)
		(if (group_stage_requested
			(qpp-tuple-group query)
			tuple_having_for_stage
			...
			(list (make_group_stage
				...
				nil nil))
			'())
		(qpp-schemas-from-tables (qpp-tuple-tables query))
		neumann_exact_replace_find_column
		'()))))
(define build_queryplan_term_from_logical (lambda (logical_term)
	(planner_with_compile_scope (lambda ()
		(build_queryplan_term_from_logical_with_sink logical_term '(resultrow))))
))
(define build_queryplan_term (lambda (query) (begin
	/* BTW2025 top-down pipeline is mandatory for SELECT tuples on this branch:
	inner_select markers are lifted to dependent joins, decorrelated top-down,
	and only then handed to the legacy physical lowering. Do not bypass this
	with scalar fallback paths; performance regressions must surface as compiler
	...
	(if (qpp-tuple? query)
		(begin
			...
			(define neumann_old_stack (coalesceNil (planner_neumann_lowered_scope_stack "stack") '()))
			(planner_neumann_lowered_scope_stack "stack" (cons true neumann_old_stack))
			(if neumann_pipeline_trace (print (concat "[neumann] build_term: flag after set="
				(string (planner_neumann_lowered_current)))) nil)
			(define neumann_plan (build_queryplan_term_with_sink_neumann_lowered neu '(resultrow)))
			(planner_neumann_lowered_scope_stack "stack" neumann_old_stack)
			neumann_plan)
...
(define build_dml_plan (lambda (schema target_tbl target_alias all_defs cols condition order limit_val offset_val) (begin
	...
	(if (or (nil? synthetic_select) (< (count synthetic_select) 1) (not (string? (nth synthetic_select 0))))
		(error (concat "DML_SYNTHETIC_SELECT_NONSTRING_SCHEMA " (serialize synthetic_select)))
		nil)
	(define _uq_result (apply untangle_query (merge synthetic_select (list nil))))
	(define uq_init (if (>= (count _uq_result) 8) (nth _uq_result 7) '()))
	(define uq_7tuple (list (nth _uq_result 0) (nth _uq_result 1) (nth _uq_result 2) (nth _uq_result 3) (nth _uq_result 4) (nth _uq_result 5) (nth _uq_result 6)))
	(define pipeline_result (apply join_reorder uq_7tuple))
...
(define build_queryplan_inner (lambda (schema tables fields condition groups schemas replace_find_column update_target) (begin
	(if neumann_pipeline_trace (print (concat "[neumann] build_inner: begin tables=" (string (count tables)) " fields=" (string (count fields)))) nil)

	/* TODO: order tables: outer joins behind */
	(set groups (coalesceNil groups '()))
	(if neumann_pipeline_trace (print (concat "[neumann] build_inner: after groups coalesce " (string (count groups)))) nil)
	/* separate partition stages (have partition-aliases) from regular stages */
	/* Constant scalar helper stages carry partition-alias metadata only so
	build_scan can honor once-limit/order contracts on the helper table itself.
	They are not real later stages at all and must stay only in the tagged table
	metadata, otherwise mixed predicates like
	base_col = scalar_helper.value get deferred out of the row-domain scan. */
	(define stage_is_constant_scalar_partition (lambda (s) (begin
		(define _spa (stage_partition_aliases s))
		(define _sg (stage_group_cols s))
		(define spc (coalesceNil (stage_limit_partition_cols s) 0))
		(define sos (coalesceNil (stage_outer_sources s) '()))
		(and
			(not (nil? _spa))
			...
			(reduce tables (lambda (ok td) (if (not ok)
				...
				(match td
					...
					(if (has? _spa alias)
						(reduce (extract_tblvars (coalesceNil je true)) (lambda (refs_ok ref_alias)
							(and refs_ok (has? _spa ref_alias)))
							true)
						true)
					_ true)))
				true)))))
	...
	(define partition_stages (filter groups (lambda (s) (begin
		...
		(and
			(not (nil? _spa))
			(or (nil? _sg) (equal? _sg '()))
			(not (stage_is_constant_scalar_partition s)))))))
	(if neumann_pipeline_trace (print (concat "[neumann] build_inner: after partition stages " (string (count partition_stages)))) nil)
	...
	(define stage_initial (if groups_present (car groups) nil))
	(define rest_groups_initial (coalesceNil (if groups_present (cdr groups) nil) '()))
	(define stage_empty_group? (lambda (s)
		(or
			(nil? (stage_group_cols s))
			(equal? (stage_group_cols s) '())
			(and (list? (stage_group_cols s)) (equal? (count (stage_group_cols s)) 0)))))
	(define stage_noop_having_true? (lambda (s)
		(and
			(not (nil? s))
			(stage_empty_group? s)
			(equal? (stage_having_expr s) true)
			(equal? (coalesceNil (stage_order_list s) '()) '())
			(nil? (stage_limit_val s))
			(nil? (stage_offset_val s))
			(or (nil? (stage_partition_aliases s)) (equal? (stage_partition_aliases s) '()))
			(nil? (stage_init_code s))
			(nil? (stage_condition s))
			(nil? (stage_once_limit s)))))
	(define table_joinexpr_constrains_primary? (lambda (td) (match td
		'(alias tschema (string? ttbl) _ joinexpr)
		(begin
			(define primary_cols (filter
				(coalesceNil (try (lambda () (get_schema tschema ttbl)) (lambda (e) '())) '())
				(lambda (coldef) (equal? (coalesceNil (coldef "Key") "") "PRI"))))
			(if (nil? primary_cols)
				false
				(begin
					(define primary_names (map primary_cols (lambda (coldef) (coldef "Field"))))
					(define primary_ref? (lambda (expr) (match expr
						'((symbol get_column) (eval alias) _ col _) (has? primary_names col)
						'((quote get_column) (eval alias) _ col _) (has? primary_names col)
						false)))
					(define expr_constrains_primary? (lambda (expr) (match expr
						'((symbol equal??) left right) (or
							(primary_ref? left)
							(primary_ref? right))
						'((quote equal??) left right) (or
							(primary_ref? left)
							(primary_ref? right))
						(cons sym args) (if (or (is_opaque_scope_sym sym) (not (list? args)))
							false
							(reduce args (lambda (found arg)
								(or found (expr_constrains_primary? arg)))
								false))
						false)))
					(expr_constrains_primary? (coalesceNil joinexpr true)))))
		_ false)))
	(define scalar_flattened_inner_unique? (lambda ()
		(begin
			(define checked_tables (filter tables (lambda (td) (match td
				'(_ _ (string? _ttbl) true _joinexpr) true
				false))))
			(and
				(not (equal? checked_tables '()))
				(reduce checked_tables (lambda (ok td)
					(and ok (table_joinexpr_constrains_primary? td)))
					true)))))
	(define drop_unique_scalar_noop_stage (and
		(stage_noop_having_true? stage_initial)
		(not (equal? rest_groups_initial '()))
		(scalar_flattened_inner_unique?)))
	(define stage (if drop_unique_scalar_noop_stage (car rest_groups_initial) stage_initial))
	(define rest_groups (if drop_unique_scalar_noop_stage (cdr rest_groups_initial) rest_groups_initial))
	(define stage_group_raw (if stage (stage_group_cols stage) nil))
	(define stage_group (if (or
		(nil? stage_group_raw)
		(equal? stage_group_raw '())
		(and (list? stage_group_raw) (equal? (count stage_group_raw) 0)))
		nil
		stage_group_raw))
	...
	(if neumann_pipeline_trace (print (concat "[neumann] build_inner: after stage setup has_stage=" (string (not (nil? stage))))) nil)
	...
	(if neumann_pipeline_trace (print (concat "[neumann] build_inner: after window detect has_window=" (string has_window))) nil)
	...
	that actually reads that temp source. Lower them exactly once here. */
		(define lower_materialized_scan_expr (lambda (scan_schema scan_tbl scan_tblvar scan_expr agg_name_context) (begin
			...
			(define planner_materialized_scan_table
				(and (string? scan_tbl)
					(or
						(strlike scan_tbl ".keytable:%")
						(strlike scan_tbl ".prejoin:%")
						(strlike scan_tbl ".mat:%"))))
			(define scan_has_planned_fields
				(not (equal? (coalesceNil (planned_materialized_fields scan_tbl) '()) '())))
			(if (and (not materialized_source) (not planner_materialized_scan_table) (not scan_has_planned_fields))
				scan_expr
				(begin
								...
								(if (not (nil? found))
									...
									(begin
										...
										(if (and (not (nil? source_expr))
											...
											nil))))
								nil))))))
				(define current_scan_agg_field (lambda (expr agg_args) (begin
								...
								(begin
									...
									(match source_expr
										...
										(if (or (equal? source_col target_col)
											(and (>= (strlen source_col) (+ (strlen agg_prefix) 1))
												(equal? (substr source_col 0 (strlen agg_prefix)) agg_prefix)
												(equal? (substr source_col (strlen agg_prefix) 1) "|")))
											(coldef "Field")
											nil)
	...
	(if neumann_pipeline_trace (print "[neumann] build_inner: after window condition check") nil)
	...
	(define wg_store (newsession))
	(wg_store "fields" nil)
	(define grouped_laglead_window
		(and has_window stage_group
			(reduce window_funcs_all (lambda (found wf)
				(or found (match wf
					'(fn _args _over)
					(or (equal? fn "LAG") (equal? fn "LEAD"))
					false)))
				false)))
	(if grouped_laglead_window
		(error "LAG/LEAD window functions over GROUP BY are not yet supported")
		nil)
	(define strip_window_inner (lambda (expr) (match expr
		...
		(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
			expr
			(cons sym (map args strip_window_inner)))
		_ expr)))
	(define strip_window_group_stage (and has_window stage_group))
	(if strip_window_group_stage
		(wg_store "fields" fields)
		nil) /* save original fields with window_func */
	(set fields (if strip_window_group_stage
		(map_assoc fields (lambda (k v) (strip_window_inner v)))
		fields))
	(set has_window (if strip_window_group_stage false has_window))
	(if neumann_pipeline_trace (print (concat "[neumann] build_inner: after window group strip has_window=" (string has_window))) nil)
	...
	(if stage_group (begin
		...
		temp names. */
		(define group_live_aliases (reduce
			(map tables (lambda (td) (match td
				'(tv _ _ _ _) tv
				nil)))
			(lambda (acc alias_) (append_unique acc alias_))
			'()))
		(define group_schema_expr_for_ref (lambda (alias_ col ci)
			(begin
				(define lookup_expr (lambda (schema_alias)
					(if (or (nil? schema_alias) (not (has_assoc? schemas schema_alias)))
						nil
						(begin
							(define coldef (schema_column_def (schema_assoc_cols schemas schema_alias) col ci))
							(if (or (nil? coldef) (not (has_assoc? coldef "Expr")))
								nil
								(coldef "Expr"))))))
				(coalesce
					(lookup_expr alias_)
					(lookup_expr (visible_occurrence_alias alias_))))))
		(define group_alias_parent_prefix (lambda (alias_)
			(if (string? alias_)
				(begin
					(define parts (split alias_ "\0"))
					(if (> (count parts) 1)
						(concat
							(reduce (produceN (- (count parts) 1)) (lambda (prefix i)
								(if (equal? i 0)
									(nth parts 0)
									(concat prefix "\0" (nth parts i))))
								"")
							"\0")
						""))
				"")))
		(define group_alias_has_prefix? (lambda (alias_ prefix)
			(and
				(string? alias_)
				(> (strlen prefix) 0)
				(>= (strlen alias_) (strlen prefix))
				(equal? (substr alias_ 0 (strlen prefix)) prefix))))
		(define group_rebase_schema_expr (lambda (schema_alias expr)
			(begin
				(define parent_prefix (group_alias_parent_prefix schema_alias))
				(define rebase_node (lambda (node) (match node
					'((symbol get_column) src_alias ti col ci)
					(if (and
						(> (strlen parent_prefix) 0)
						(string? src_alias)
						(not (group_alias_has_prefix? src_alias parent_prefix))
						(not (has? group_live_aliases src_alias))
						(not (strlike src_alias "dep_scalar_%")))
						(list (quote get_column) (concat parent_prefix src_alias) ti col ci)
						node)
					'((quote get_column) src_alias ti col ci)
					(if (and
						(> (strlen parent_prefix) 0)
						(string? src_alias)
						(not (group_alias_has_prefix? src_alias parent_prefix))
					...
					(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						node
						(cons sym (map args rebase_node)))
					node)))
				(rebase_node expr))))
		(define expand_group_schema_expr_refs (lambda (expr) (match expr
			'((symbol get_column) alias_ ti col ci)
			(begin
				(define field_expr (group_schema_expr_for_ref alias_ col ci))
				(if (and
					(not (has? group_live_aliases alias_))
					(not (nil? field_expr))
					(not (equal? field_expr expr)))
					(expand_group_schema_expr_refs (group_rebase_schema_expr alias_ field_expr))
					expr))
			'((quote get_column) alias_ ti col ci)
			(begin
				(define field_expr (group_schema_expr_for_ref alias_ col ci))
			...
			(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
				expr
				(cons sym (map args expand_group_schema_expr_refs)))
			expr)))
		(define fields (map_assoc fields (lambda (k v) (expand_group_schema_expr_refs v))))
		(define stage_group (map stage_group expand_group_schema_expr_refs))
		(define stage_having (expand_group_schema_expr_refs stage_having))
		(define stage_order (map stage_order (lambda (o) (match o
			'(col dir) (list (expand_group_schema_expr_refs col) dir)
			o))))
		...
		(define stage_group (map stage_group replace_find_column))
		(define stage_having (replace_find_column stage_having))
		(define stage_order (map stage_order (lambda (o) (match o '(col dir) (list (replace_find_column col) dir)))))
		(define is_dedup (stage_is_dedup stage))
		(define scoped_stage (not (nil? (stage_partition_aliases stage))))
		(define field_agg_has_nested_agg (lambda (args)
			(if (list? args)
				(reduce args (lambda (acc arg)
					(or acc (not (equal? (extract_aggregates arg) '()))))
					false)
				false)))
		(define needs_outer_group_expr (lambda (expr) (match expr
			(cons (symbol aggregate) args)
			(and (equal? (extract_tblvars expr) '()) (field_agg_has_nested_agg args))
			...
			(cons sym args) (if (or
				...
				(not (list? args)))
				false
				(reduce args (lambda (acc arg) (or acc (needs_outer_group_expr arg))) false))
			false)))
		(define has_existing_later_group_stage (reduce rest_groups (lambda (acc s)
			(or acc (begin
				(define later_sg (stage_group_cols s))
				(and (not (nil? later_sg)) (not (equal? later_sg '()))))))
			false))
		(define needs_synthetic_outer_group (and scoped_stage
			(not has_existing_later_group_stage)
			(or
				(reduce_assoc fields (lambda (acc _k expr) (or acc (needs_outer_group_expr expr))) false)
				(needs_outer_group_expr (coalesce stage_having true))
				(reduce (coalesce stage_order '()) (lambda (acc o)
					(or acc (match o '(col _dir) (needs_outer_group_expr col) false))) false))))
		(define has_later_group_stage (or has_existing_later_group_stage needs_synthetic_outer_group))
		(define defer_field_agg (lambda (expr args)
			(and (equal? (extract_tblvars expr) '())
				scoped_stage
				has_later_group_stage
				(field_agg_has_nested_agg args))))
		(define extract_stage_field_aggregates (lambda (expr deferred_outer) (match expr
			(cons (symbol aggregate) args)
			(if (and (not deferred_outer) (defer_field_agg expr args))
			...
			(cons sym args) (if (or
				...
				(not (list? args)))
				'()
				(merge (map args (lambda (arg) (extract_stage_field_aggregates arg deferred_outer)))))
			'())))
		...
		(define stage_scope (stage_partition_aliases stage))
		(define grp_tables_raw (if (not (nil? stage_scope))
			/* scoped GROUP: only the tables listed in the stage's aliases */
			(filter tables (lambda (t) (match t '(tv _ _ _ _) (has? stage_scope tv) false)))
			/* global GROUP: all tables except partition-staged */
			(filter tables (lambda (t) (match t '(tv _ _ _ _) (not (has? _grp_ps_aliases tv)) true)))))
		(define grp_ps_tables_seed (filter tables (lambda (t) (match t '(tv _ _ _ _)
			(and (not (has? (coalesceNil stage_scope '()) tv))
				(or (has? _grp_ps_aliases tv) (not (nil? stage_scope))))
			false))))
		(define grp_table_alias (lambda (td) (match td
			'(tv _ ttbl _ _) (if (nil? tv) ttbl tv)
			nil)))
		(define grp_tblvar_owns_ref? (lambda (tblvar ref) (begin
			(define tbl_aliases (alias_lookup_variants tblvar))
			(or
				(has? tbl_aliases ref)
				(if (string? ref)
					(begin
						(define parts (split ref "\0"))
						(and (> (count parts) 1)
							(has? tbl_aliases (car parts))))
					false)))))
		(define grp_table_depends_on_alias (lambda (td alias_name) (match td
			'(_ _ _ _ je) (and
				(not (nil? je))
				(has? (extract_tblvars je) alias_name))
			false)))
		(define grp_expand_ps_tables (lambda (cur) (begin
			(define cur_aliases (map cur grp_table_alias))
			(define next (merge cur (filter tables (lambda (td) (begin
				(define td_alias (grp_table_alias td))
				(and
					(not (has? cur_aliases td_alias))
					(reduce cur_aliases (lambda (found alias_name)
						(or found (grp_table_depends_on_alias td alias_name)))
						false)))))))
			(if (equal? (count next) (count cur))
				cur
				(grp_expand_ps_tables next)))))
		(define grp_ps_tables_raw (grp_expand_ps_tables grp_ps_tables_seed))
		(define grp_ps_helper_tables_raw (filter grp_ps_tables_raw (lambda (td) (match td
			'(_ _ ttbl _ _) (unnest_helper_table? ttbl)
			false))))
		(define grp_ps_outer_tables_raw (filter grp_ps_tables_raw (lambda (td) (match td
			'(_ _ ttbl _ _) (not (unnest_helper_table? ttbl))
			true))))
		(define grp_ps_visible_aliases (merge_unique (map grp_ps_outer_tables_raw (lambda (td) (match td
			'(tv tschema ttbl _ _)
			(merge
				(alias_lookup_variants tv)
				(if (equal? (visible_occurrence_alias tv) ttbl) (list (concat tschema "." ttbl)) '()))
			'())))))
		(define expr_refs_grp_ps_table (lambda (expr)
			(reduce (extract_all_get_columns expr) (lambda (acc mc)
				(or acc (match mc
					'(name '((symbol get_column) alias_ _ _ _))
					(or (has? grp_ps_visible_aliases alias_)
						(has? grp_ps_visible_aliases (visible_occurrence_alias alias_)))
					'(name '((quote get_column) alias_ _ _ _))
					(or (has? grp_ps_visible_aliases alias_)
						(has? grp_ps_visible_aliases (visible_occurrence_alias alias_)))
					false)))
				false)))
		(define resolved_expr_refs_grp_ps_table (lambda (expr)
			(expr_refs_grp_ps_table (replace_find_column expr))))
		(define must_prejoin_outer_group_tables (and
			(not (equal? grp_ps_outer_tables_raw '()))
			(or
				(and (not (equal? ags '()))
					(nil? stage_scope)
					(reduce_assoc fields (lambda (acc _k expr) (or acc (resolved_expr_refs_grp_ps_table expr))) false))
				(reduce stage_group (lambda (acc expr) (or acc (resolved_expr_refs_grp_ps_table expr))) false)
				(reduce ags (lambda (acc ag) (or acc (resolved_expr_refs_grp_ps_table ag))) false)
				(resolved_expr_refs_grp_ps_table (coalesce stage_having true))
				(reduce (coalesce stage_order '()) (lambda (acc o) (or acc (match o '(col _dir) (resolved_expr_refs_grp_ps_table col) false))) false))))
		(define grp_tables (if must_prejoin_outer_group_tables
			(merge grp_tables_raw grp_ps_helper_tables_raw grp_ps_outer_tables_raw)
			(merge grp_tables_raw grp_ps_helper_tables_raw)))
		...
		participating tables (LEFT JOIN ON clauses). Without extracting them
		here the current stage would never materialize the corresponding
		keytable column and the (aggregate ...) marker would survive into
		runtime code as "Unknown function: aggregate". An aggregate is owned
		by this stage when its input expression only references tables that
		are in `grp_tables`. We walk BOTH `grp_tables` and `grp_ps_tables`
		joinexprs because the multi-table prejoin path may recursively
		re-enter this stage compiler with the relevant tables in either bucket. */
		...
		(define agg_input_owned_by_current_stage (lambda (agg_args) (match agg_args
			...
			(begin
				...
				(or (equal? refs '())
					(reduce refs (lambda (acc tv) (and acc
						(reduce grp_table_aliases_for_owned_agg (lambda (found grp_alias)
							(or found (grp_tblvar_owns_ref? grp_alias tv)))
							false))) true)))
			false)))
		(define joinexpr_owned_aggs (merge (map (merge grp_ps_tables grp_tables) (lambda (td) (match td
			'(_ _ _ _ tje)
			(if (nil? tje) '()
		...
		(match grp_tables
			...
			'('(tblvar schema tbl isOuter _)) (begin
				(define ags (filter ags (lambda (ag) (match ag
					'(agg_expr _ _)
					(begin
						(define ownership_expr (if (materialized-source? tbl)
							(lower_materialized_source_expr tbl tblvar agg_expr)
							agg_expr))
						(define refs (extract_tblvars ownership_expr))
						(or (equal? refs '())
							(reduce refs (lambda (acc tv)
								(and acc (grp_tblvar_owns_ref? tblvar tv)))
								true)))
					false))))
					...
					'((symbol get_column) _ _ _ _) (begin
						...
						(define visible_field_expr (match expr
							...
							(if (or (nil? alias_) (not (has_assoc? schemas alias_))) nil
								(reduce (schema_assoc_cols schemas alias_) (lambda (found coldef)
									(if (not (nil? found))
										found
						...
						(define source_alias_field (if (nil? source_alias_map) nil
							(reduce source_alias_map (lambda (found pair)
								(if (not (nil? found))
									found
									(match pair
										'(key source_expr)
										(if (and
											(not (nil? (find_materialized_field_by_name materialized_cols key)))
											(equal? (normalize_canonical_aliases source_expr) normalized_expr))
											key
											nil)
										nil)))
								nil)))
						(define exact_field (reduce materialized_cols (lambda (found coldef)
							(if (not (nil? found))
								found
								(begin
									(define source_expr (coalesceNil (coldef "Expr") nil))
						...
						(define direct_field (if (nil? expr_lookup) nil
							(reduce (materialized_source_expr_keys expr) (lambda (found key)
								(if (not (nil? found)) found
									(coalesce (expr_lookup key) nil)))
								nil)))
						(define direct_field (coalesce exact_field
				...
				(define lower_visible_materialized_aggs_single (lambda (expr) (match expr
					...
					expr)))
				/* Preserve GROUP BY key cardinality here. append_unique/equal? can
				collapse distinct SQL builtin proc applications such as YEAR(dt) and
				MONTH(dt), which then creates a keytable missing later ORDER BY keys. */
				(define resolved_stage_group_base (if materialized_source
					(map stage_group rewrite_materialized_source_cols_single)
					stage_group))
				(define scoped_constant_group? (and scoped_stage (equal? resolved_stage_group_base '(1))))
				(define scoped_stage_domain_group (lambda () (begin
					(define cols_from_schema (schema_assoc_cols schemas tblvar))
					(define cols_from_table (if (and (nil? cols_from_schema) (string? schema) (string? tbl) (not (materialized-source? tbl)))
						(try (lambda () (get_schema schema tbl)) (lambda (e) '()))
						'()))
					(define cols (coalesceNil (if (nil? cols_from_schema) cols_from_table cols_from_schema) '()))
					(define primary_cols (filter cols (lambda (coldef) (equal? (coalesceNil (coldef "Key") "") "PRI"))))
					(define domain_cols (if (nil? primary_cols) cols primary_cols))
					(map domain_cols (lambda (coldef)
						(list (quote get_column) tblvar false (coldef "Field") false))))))
				(define resolved_stage_group (if scoped_constant_group?
					(begin
						(define domain_group (scoped_stage_domain_group))
						(if (nil? domain_group)
							resolved_stage_group_base
							domain_group))
					resolved_stage_group_base))
				...
				(define group_any_reduce (quote group_any_value_reduce))
				(define group_value_reduce group_any_reduce)
				(define group_value_ag (lambda (expr)
					(list expr group_value_reduce nil)))
				(define group_value_ag_expr (lambda (expr)
					(list (quote aggregate) expr group_value_reduce nil)))
				(define outer_stage_aliases (map grp_ps_tables (lambda (td) (match td
					'(tv _ ttbl _ _) (if (nil? tv) ttbl tv)
					""))))
				(define refs_only_outer_stage (lambda (expr)
					(begin
						(define _refs (extract_tblvars expr))
						(and (not (equal? _refs '()))
							(reduce _refs (lambda (acc tv) (and acc (has? outer_stage_aliases tv))) true)))))
				(define matches_group_expr (lambda (expr)
					(or
						...
						(reduce resolved_stage_group (lambda (acc group_expr)
							...
							false))))
				(define field_has_agg_expr (lambda (expr) (match expr
					(cons (symbol aggregate) _) true
					(cons '(quote aggregate) _) true
					(cons sym args) (reduce args (lambda (a b) (or a (field_has_agg_expr b))) false)
					false)))
				(define expr_has_non_group_column_refs (lambda (expr)
					(reduce (extract_all_get_columns expr) (lambda (acc mc)
						(or acc
							(not (matches_group_expr (cadr mc)))))
						false)))
				(define field_needs_group_value_agg (lambda (expr)
					(and (not (field_has_agg_expr expr))
						(not (matches_group_expr expr))
						(not (and (not (nil? stage_scope)) (refs_only_outer_stage expr)))
						(expr_has_non_group_column_refs expr))))
				...
				(define group_value_local_col_name (lambda (expr)
					(begin
						...
						(concat ".gv|" (fnv_hash (concat head_key "|" ref_key))))))
				...
				(define group_value_local_expr (lambda (expr)
					(if (and group_value_local_materializable (field_needs_group_value_agg expr))
						(begin
				...
				(define collect_group_value_local_exprs (lambda (expr) (match expr
					(cons (symbol aggregate) agg_rest) '()
					(cons '(quote aggregate) agg_rest) '()
					(cons sym args)
					(if (field_needs_group_value_agg expr)
						(list expr)
						(merge (map args collect_group_value_local_exprs)))
					_ '())))
				...
				(define flatten_and_parts (lambda (expr) (match expr
					(cons sym parts) (if (or (equal? sym (quote and)) (equal? sym '(quote and)) (equal? sym 'and))
						(merge (map parts flatten_and_parts))
						(list expr))
					(list expr))))
				(define condition_parts0 (flatten_and_parts condition))
				...
				(define has_agg_expr (lambda (expr) (match expr
					(cons (symbol aggregate) _) true
					(cons '(quote aggregate) _) true
					(cons sym args) (reduce args (lambda (a b) (or a (has_agg_expr b))) false)
					false)))
				(define cond_refs_only_current_tblvar (lambda (expr)
					(reduce (extract_tblvars expr) (lambda (acc tv)
						(and acc (equal?? tv tblvar)))
						true)))
				(define is_post_group_agg_part (lambda (expr)
					/* Any condition term containing an aggregate must be evaluated after
					the keytable/group stage. Restricting this to obvious current-tblvar
					refs leaves nested EXISTS/COUNT markers stranded in row-level bool
					callbacks, where raw `(aggregate ...)` nodes leak into runtime code. */
					(has_agg_expr expr)))
				(define cond_parts (flatten_and_parts condition))
				(define cond_agg_parts (filter cond_parts is_post_group_agg_part))
				(define cond_non_agg (filter cond_parts (lambda (p) (not (is_post_group_agg_part p)))))
				(define ags (if is_dedup ags
					(merge_unique ags (merge (map cond_agg_parts extract_aggregates)))))
				(define group_value_refs_only_current_tblvar (lambda (expr)
					(begin
						(define refs (extract_tblvars expr))
						(and (not (equal? refs '()))
							(reduce refs (lambda (acc tv) (and acc (equal?? tv tblvar))) true)))))
				(define group_value_local_required_exprs (merge_unique (merge
					...
					such as `(lambda () if)`. */
					(extract_assoc fields (lambda (_key expr)
						(collect_group_value_local_exprs expr)))
					(map cond_agg_parts (lambda (part)
						(filter (collect_group_value_local_exprs part)
							group_value_refs_only_current_tblvar)))
					(if (nil? stage_having) '()
						(collect_group_value_local_exprs stage_having))
					(if (nil? stage_order) '()
						(merge (map stage_order (lambda (o) (match o
							'(col _dir)
							(collect_group_value_local_exprs col)
							'()))))))))
				(define group_value_local_fields (if group_value_local_materializable
					group_value_local_required_exprs
					'()))
				(if group_value_local_materializable
					(map group_value_local_required_exprs (lambda (expr) (begin
						(define col_name (group_value_local_col_name expr))
						(define key_expr (group_value_local_key_expr expr))
						(define logical_expr (if group_value_local_materializable
							(lower_materialized_source_expr tbl tblvar expr)
				...
				(define synthetic_field_ags (if is_dedup '()
					(map group_value_local_required_exprs (lambda (expr)
						(group_value_ag (group_value_local_expr expr))))))
				(define ags (if is_dedup ags (merge_unique ags synthetic_field_ags)))
				(define grp_refs_src_tbl (lambda (expr)
				...
				(define ags (if materialized_source
					...
					ags))
				(define grp_has_explicit_outer (lambda (expr) (match expr
					(cons sym args) (if (or (equal? sym (quote outer)) (equal? sym '(quote outer)))
						true
						(reduce args (lambda (acc arg) (or acc (grp_has_explicit_outer arg))) false))
					false)))
				...
				(define grp_key_corr_part (lambda (part) (match part
					...
					'((quote equal??) left right) (if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr left))) false)
						...
						(if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr right))) false)
							...
							false))
					false)))
				(define cond_key_corr (if (nil? stage_scope) '()
					(filter cond_non_agg grp_key_corr_part)))
				(define cond_non_agg_effective (if (nil? stage_scope) cond_non_agg
					(filter cond_non_agg (lambda (p) (not (grp_key_corr_part p))))))
				/* non-aggregate condition = keytable scan filter */
				(set condition (if (equal? 0 (count cond_non_agg_effective)) true
					(if (equal? 1 (count cond_non_agg_effective)) (car cond_non_agg_effective)
						(cons (quote and) cond_non_agg_effective))))
				/* split non-aggregate condition: parts referencing partition-staged tables go to grouped_plan */
				(define grp_cond_split (split_condition condition grp_ps_tables))
				(define grp_ps_condition (match grp_cond_split '(_ later) later))
				(set condition (match grp_cond_split '(now _) now))
				...
				(define grp_ps_aliases (if (nil? grp_ps_tables) '()
					(map grp_ps_tables (lambda (td) (match td
						'(tv _ ttbl _ _)
						(if (nil? tv) ttbl tv)
						"")))))
				(define grp_ps_runtime_aliases (filter grp_ps_aliases (lambda (alias_name)
					(not (has? planner_helper_aliases alias_name)))))
				(define grp_ps_condition_refs_stage (lambda (expr) (match expr
					'((symbol get_column) alias_ _ _ _) (and (not (nil? alias_)) (has? grp_ps_runtime_aliases alias_))
					'((quote get_column) alias_ _ _ _) (and (not (nil? alias_)) (has? grp_ps_runtime_aliases alias_))
					(cons sym args) (if (is_opaque_scope_sym sym)
						false
						(reduce args (lambda (found arg) (or found (grp_ps_condition_refs_stage arg))) false))
					false)))
				(if (and (not (nil? grp_ps_condition))
					(not (equal? grp_ps_condition true))
					(not (grp_ps_condition_refs_stage grp_ps_condition)))
					(begin
						(set condition (combine_and_terms (list condition grp_ps_condition)))
						(set grp_ps_condition true))
					nil)
				(define grp_outer_aliases (if (nil? grp_ps_tables) '()
					(map grp_ps_tables (lambda (td) (match td
						'(tv _ ttbl _ _) (if (nil? tv) ttbl tv)
						"")))))
				(define resolve_outer_group_field (lambda (expr) (match expr
					'((symbol get_column) alias_ ti col ci) (if (and (not (nil? alias_)) (has? grp_outer_aliases alias_))
						expr
						(if (nil? alias_)
							(begin
								(define matches (filter grp_ps_tables (lambda (td) (match td
									'(tv _ ttbl _ _)
									(begin
										(define lookup_alias (if (nil? tv) ttbl tv))
										(reduce (schema_assoc_cols schemas lookup_alias) (lambda (found coldef)
								...
								(if (equal? 1 (count matches))
									...
									nil))
							nil))
					'((quote get_column) alias_ ti col ci) (if (and (not (nil? alias_)) (has? grp_outer_aliases alias_))
						expr
						(if (nil? alias_)
							(begin
								(define matches (filter grp_ps_tables (lambda (td) (match td
				...
				(define collect_condition (if (and is_dedup materialized_source)
					(begin
						(define dedup_collect_parts (filter (flatten_and_parts condition) (lambda (part)
							(and (equal? (has_only_tblvar_refs part tblvar) true)
								(not (has_agg_expr part))))))
						(if (equal? 0 (count dedup_collect_parts)) true
							(if (equal? 1 (count dedup_collect_parts)) (car dedup_collect_parts)
								(cons (quote and) dedup_collect_parts))))
					condition))
				(define stage_join_outer_expr (lambda (expr) (match expr
					(cons sym args) (if (or (equal? sym (quote outer)) (equal? sym '(quote outer)))
						expr
						(cons sym (map args stage_join_outer_expr)))
					_ (begin
						(define _parts (split (string expr) "."))
						(match _parts
							(list _tbl _col) (list (quote get_column) _tbl false _col false)
							_ expr)))))
				(define dedup_join_term (lambda (part) (match part
					'((symbol equal??) left right) (if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr left))) false)
						(if (grp_refs_src_tbl right) nil
							(list (quote equal??) (replace_col_for_dedup left) (stage_join_outer_expr right)))
						(if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr right))) false)
							(if (grp_refs_src_tbl left) nil
								(list (quote equal??) (stage_join_outer_expr left) (replace_col_for_dedup right)))
							nil))
					'((quote equal??) left right) (if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr left))) false)
						(if (grp_refs_src_tbl right) nil
							(list (quote equal??) (replace_col_for_dedup left) (stage_join_outer_expr right)))
				...
				(define expr_uses_dependent_helper_result (lambda (expr) (match expr
					'((quote get_column) alias_ _ _ _) (and (string? alias_) (strlike alias_ "dep_scalar_%"))
					'((symbol get_column) alias_ _ _ _) (and (string? alias_) (strlike alias_ "dep_scalar_%"))
					(cons sym args) (or (expr_uses_dependent_helper_result sym)
						(reduce args (lambda (found arg) (or found (expr_uses_dependent_helper_result arg))) false))
					false)))
				(define session_sensitive_group_domain
					(or (expr_uses_session_state collect_condition)
						(expr_uses_dependent_helper_result collect_condition)))
				(define group_runtime_suffix (if session_sensitive_group_domain
					(if (expr_uses_session_state collect_condition)
						(concat
							(runtime_cache_suffix_from_exprs (list collect_condition))
							(user_session_runtime_cache_suffix_from_exprs (list collect_condition)))
						(planner_dependent_group_suffix collect_condition))
					nil))
			/* Session-sensitive groups use a GLOBAL keytable domain (no condition suffix)
				...
				(define keytable_condition_suffix
					(if is_dedup
						(if (or (nil? collect_condition) (equal? collect_condition true))
							(list (quote dedup_domain))
							(list (quote dedup_domain) collect_condition))
						(if (or session_sensitive_group_domain (expr_uses_session_state condition))
							(if (or (nil? collect_condition) (equal? collect_condition true))
								condition
								collect_condition)
							(if (and scoped_stage (not (or (nil? collect_condition) (equal? collect_condition true))))
								collect_condition
								nil))))
				(define kt_result (make_keytable schema tbl resolved_stage_group tblvar
					keytable_condition_suffix))
							...
							(begin
								...
								(scan_wrapper 'scan schema tbl
									...
									(list (quote lambda)
										...
										(cons (quote list) (map resolved_stage_group (lambda (expr) (replace_columns_from_expr expr))))) /* build records (list k1 k2 ...) at runtime */
				...
				(if is_dedup (begin
					...
					(define dedup_refs_current (lambda (e)
						(reduce (extract_tblvars e) (lambda (acc tv) (or acc (equal?? tv tblvar))) false)))
					(define dedup_resolve (lambda (e)
						(if (dedup_refs_current e)
							(replace_col_for_dedup (replace_find_column e))
							(coalesce (resolve_outer_group_field e) e))))
					(define dedup_kt_is_outer (not (nil? stage_scope)))
					(define dedup_kt_je (if dedup_kt_is_outer
						(begin
							(define dedup_terms (filter (map cond_key_corr dedup_join_term) (lambda (x) (not (nil? x)))))
							(if (equal? dedup_terms '()) nil
								(if (equal? 1 (count dedup_terms)) (car dedup_terms)
									(cons (quote and) dedup_terms))))
						nil))
					(define transformed_rest_groups (map rest_groups (lambda (s)
						(stage_rebuild_with_meta s (make_group_stage
							(map (stage_group_cols s) dedup_resolve)
							(dedup_resolve (stage_having_expr s))
							(map (coalesce (stage_order_list s) '()) (lambda (o) (match o '(col dir) (list (dedup_resolve col) dir))))
							(stage_limit_val s)
							(stage_offset_val s)
					...
					(define dedup_output_fields
						(map_assoc fields (lambda (k v) (dedup_resolve v))))
					(define dedup_visible_schema
						(extract_assoc dedup_output_fields (lambda (k v)
							(list "Field" k "Type" "any" "Expr" v))))
					(if neumann_pipeline_trace (print "[neumann] build_inner: group before dedup grouped_plan") nil)
					(define grouped_plan (build_queryplan schema
						(if dedup_kt_is_outer
							(merge grp_ps_tables (list (list grouptbl schema grouptbl true dedup_kt_je)))
						...
						replace_find_column
						update_target))
					(if neumann_pipeline_trace (print "[neumann] build_inner: group after dedup grouped_plan") nil)
					...
					(cons 'begin (merge
						...
						(define stage_has_agg_descriptor (lambda (agg_desc)
							(reduce ags (lambda (acc ag)
								(or acc (equal? (agg_col_name ag) (agg_col_name agg_desc))))
								false)))
						(define stage_fetch_owned_agg (lambda (expr agg_desc)
							(if (stage_has_agg_descriptor agg_desc)
								(replace_group_key_or_fetch expr)
								nil)))
						(define replace_count_agg_with_fetch (lambda (agg_desc)
							(if (equal? (normalize_count_literal_aggregate_args agg_desc) aggregate_count_descriptor)
								(list (quote get_column) grouptbl false (agg_col_name aggregate_count_descriptor) false)
								nil)))
						(define replace_group_agg_input_expr (lambda (expr)
							(if (and (not (nil? stage_scope)) (refs_only_outer_stage expr) (not (field_has_agg_expr expr)))
								expr
								(match expr
									'((symbol get_column) _ _ _ _)
									(if (matches_group_expr expr)
									...
									(match agg_rest
										'(agg_expr agg_reduce agg_neutral)
										(coalesce
											(replace_count_agg_with_fetch agg_rest)
											(stage_fetch_owned_agg expr agg_rest)
									...
									(if (is_opaque_scope_sym sym)
										expr
										(cons sym (map args replace_group_agg_input_expr)))
									expr))))
						...
						(define replace_group_field_expr (lambda (expr)
							...
							(if (and (not (nil? stage_scope)) (refs_only_outer_stage expr) (not (field_has_agg_expr expr)))
								expr
								(match expr
									'((symbol get_column) _ _ _ _) (if (field_needs_group_value_agg expr)
										(replace_group_key_or_fetch (group_value_ag_expr (group_value_local_expr expr)))
										(replace_group_key_or_fetch (rewrite_materialized_source_cols_single expr)))
									'((quote get_column) _ _ _ _) (if (field_needs_group_value_agg expr)
										(replace_group_key_or_fetch (group_value_ag_expr (group_value_local_expr expr)))
										(replace_group_key_or_fetch (rewrite_materialized_source_cols_single expr)))
									(cons (symbol aggregate) agg_rest)
									(coalesce
										(replace_count_agg_with_fetch agg_rest)
										(if (or (and (not (nil? stage_scope)) has_later_group_stage (equal? (extract_tblvars expr) '()) (not (equal? agg_rest aggregate_count_descriptor)))
											(and (field_agg_has_nested_agg agg_rest) (equal? (extract_tblvars expr) '())))
											(match agg_rest
												'(agg_expr agg_reduce agg_neutral)
												(list (quote aggregate) (replace_group_agg_input_expr agg_expr) agg_reduce agg_neutral)
												_ expr)
						...
						(define replace_group_output_field_expr (lambda (expr) (match expr
							(cons (symbol aggregate) agg_rest)
							(if (and
								(not (nil? stage_scope))
								has_later_group_stage
								(equal? (extract_tblvars expr) '())
								(equal? agg_rest aggregate_count_descriptor))
								expr
							...
							(if (and
								...
								expr
								(replace_group_field_expr expr))
							_ (replace_group_field_expr expr))))
						...
						(define grp_join_outer_expr (lambda (expr) (match expr
							(cons sym args) (if (or (equal? sym (quote outer)) (equal? sym '(quote outer)))
								expr
								(cons sym (map args grp_join_outer_expr)))
							_ (begin
								(define _parts (split (string expr) "."))
								(match _parts
									(list _tbl _col) (list (quote get_column) _tbl false _col false)
									_ expr)))))
						(define grp_join_term (lambda (part) (match part
							'((symbol equal??) left right) (if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr left))) false)
								(if (grp_refs_src_tbl right) nil
									(list (quote equal??) (replace_group_key_or_fetch left) (grp_join_outer_expr right)))
								(if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr right))) false)
									(if (grp_refs_src_tbl left) nil
										(list (quote equal??) (grp_join_outer_expr left) (replace_group_key_or_fetch right)))
									nil))
							'((quote equal??) left right) (if (reduce resolved_stage_group (lambda (acc group_expr) (or acc (equal? group_expr left))) false)
								(if (grp_refs_src_tbl right) nil
									(list (quote equal??) (replace_group_key_or_fetch left) (grp_join_outer_expr right)))
						...
						(define real_outer_ps_tables (filter grp_ps_tables (lambda (td) (match td
							'(_ _ ttbl _ _) (not (equal? ttbl ".(1)"))
							true))))
						(define kt_is_outer (and (not (nil? stage_scope)) (not (equal? real_outer_ps_tables '()))))
						(define flatten_gp_part (lambda (expr) (match expr
							(cons sym parts) (if (or (equal? sym (quote and)) (equal? sym '(quote and)) (equal? sym 'and))
								(merge (map parts flatten_gp_part))
								(list expr))
							(list expr))))
						(define grp_ps_join_terms
							(merge (map grp_ps_tables (lambda (td) (match td
								'(_ _ _ _ je)
								(if (nil? je) '() (flatten_gp_part je))
								'())))))
						(define kt_terms (if kt_is_outer
							(filter (map (merge cond_non_agg grp_ps_join_terms) grp_join_term) (lambda (x) (not (nil? x))))
							'()))
						(define strip_kt_term (lambda (part)
							(not (reduce kt_terms (lambda (acc kt_part) (or acc (equal? part kt_part))) false))))
						...
						(define next_groups (merge
							...
							(if needs_synthetic_outer_group (list (make_group_stage '(1) nil nil nil nil nil nil)) '())
							rest_groups
						))
						...
						(define marker_in_fields (reduce_assoc fields (lambda (acc _k v) (or acc (has_agg_expr v))) false))
						(define marker_in_having (if (nil? stage_having) false (has_agg_expr stage_having)))
						(define marker_in_order (reduce (coalesce stage_order '()) (lambda (acc o) (or acc (match o '(col _dir) (has_agg_expr col) false))) false))
						(define needs_count (or
							(not (equal? resolved_stage_group '(1)))
							(not (equal? cond_agg_parts '()))
							marker_in_fields
							marker_in_having
							marker_in_order))
						...
						(define filter_empty_groups (and
							...
							(or
								(not (equal? resolved_stage_group '(1)))
								(and
									(not marker_in_fields)
									(not marker_in_having)
									(not marker_in_order)))))
						...
						(define replaced_agg_parts (map cond_agg_parts replace_group_field_expr))
						...
						(define keytable_field_names
							(map keytable_schema_def (lambda (coldef) (coldef "Field"))))
						(define post_group_outer_aliases
							(filter grp_outer_aliases (lambda (alias_name)
								(not (has? planner_helper_aliases alias_name)))))
						(define allowed_post_group_aliases
							(append_unique post_group_outer_aliases grouptbl))
						(define gp_part_refs_known_keytable_cols (lambda (expr) (match expr
							'((symbol get_column) alias_ _ col _)
							(if (equal?? alias_ grouptbl)
								(has? keytable_field_names col)
								(if (nil? alias_)
									true
									(has? allowed_post_group_aliases alias_)))
							'((quote get_column) alias_ _ col _)
							(if (equal?? alias_ grouptbl)
								(has? keytable_field_names col)
								(if (nil? alias_)
									...
									(has? allowed_post_group_aliases alias_)))
							(cons _ args)
							(reduce args (lambda (ok arg)
								(and ok (gp_part_refs_known_keytable_cols arg)))
								true)
							true)))
						(define gp_parts (filter (merge (map (merge
							(if (or (nil? effective_having) (equal? effective_having true)) '() (list effective_having))
							replaced_agg_parts
							(if (equal? grp_ps_condition true) '() (list (replace_group_key_or_fetch grp_ps_condition))))
							flatten_gp_part))
							(lambda (x) (and
								(not (nil? x))
								(not (equal? x true))
								(strip_kt_term x)
								(gp_part_refs_known_keytable_cols x)))))
						(define gp_condition (if (equal? 0 (count gp_parts)) nil
							(if (equal? 1 (count gp_parts)) (car gp_parts)
								(cons (quote and) gp_parts))))
						...
						(define remaining_pstages (filter partition_stages (lambda (ps)
							(not (reduce (coalesceNil (stage_partition_aliases ps) '()) (lambda (acc a)
								(or acc (has? (coalesceNil stage_scope '()) a))) false)))))
						...
						(define kt_je (if kt_is_outer
							(begin
								(if (equal? kt_terms '()) nil
									(if (equal? 1 (count kt_terms)) (car kt_terms)
										(cons (quote and) kt_terms))))
							nil))
						(define grouped_output_fields
							(map_assoc fields (lambda (k v) (replace_group_output_field_expr v))))
						(define grouped_visible_schema
							(extract_assoc grouped_output_fields (lambda (k v)
								(list "Field" k "Type" "any" "Expr" v))))
						(define grouped_keytable_schema
							(merge_schema_fields_unique (list keytable_schema_def grouped_visible_schema)))
						...
						/* Topologically sort the recursive call's tables so that LEFT-JOIN
						secondary tables whose joinexpr references the keytable see the
						keytable already scanned. Without this, the keytable lookup ends
						up nested INSIDE the secondary table's mapfn, and the aggregate
						predicate (now a keytable column reference) gets evaluated inside
						the keytable scan callback — when the predicate fails or the
						keytable has no row for the current outer binding, no resultrow is
						emitted, which incorrectly drops outer rows whose LEFT-JOIN
						secondary should just be NULL-extended. */
						(define recursive_tables_raw (if kt_is_outer
							(merge rewritten_grp_ps_tables (list (list grouptbl schema grouptbl true kt_je)))
							(list (list grouptbl schema grouptbl false nil))))
						(define recursive_tables (jqr_order_by_joinexpr_dependencies recursive_tables_raw))
						(if neumann_pipeline_trace (print "[neumann] build_inner: group before normal grouped_plan") nil)
						(define grouped_plan (build_queryplan schema
							recursive_tables
							grouped_output_fields
							gp_condition
							(merge rewritten_next_groups remaining_pstages)
							(merge schemas (list grouptbl grouped_keytable_schema))
							replace_find_column
							update_target))
						(if neumann_pipeline_trace (print "[neumann] build_inner: group after normal grouped_plan") nil)
						...
						(define match_runtime_materialized_agg_col (lambda (target_col agg_name)
							(define agg_prefix (if (equal? ag aggregate_count_descriptor) "COUNT(*)" agg_name))
							(if materialized_source
								(begin
									(define materialized_cols (materialized_source_physical_schema schema tbl tblvar schemas))
									(coalesce
										...
										(if (not (equal? visible_expr marker_expr))
											...
											(begin
												...
												(if (nil? match_col)
													...
													(list (quote get_column) tblvar false match_col false)))))))))))
						(define runtime_condition_part_local_to_tblvar? (lambda (part local_tblvar)
							(and
								(not (strlike (serialize part) "%__dep_domain%"))
								(reduce (extract_tblvars part) (lambda (ok tv)
									(and ok (equal?? tv local_tblvar)))
									true))))
						(define agg_plans (map ags (lambda (ag) (match ag '(expr reduce neutral) (begin
							(define runtime_expr
								(rewrite_materialized_source_cols_single
							...
							(define runtime_condition_expr_raw
								(rewrite_materialized_source_cols_single
									(rewrite_materialized_source_aggs_single
										(lower_runtime_materialized_aggs_single condition))))
							(define runtime_condition_expr (if materialized_source
								(begin
									(define local_runtime_condition_parts
										(filter (flatten_and_parts runtime_condition_expr_raw) (lambda (part)
											(runtime_condition_part_local_to_tblvar? part tblvar))))
									(if (equal? 0 (count local_runtime_condition_parts))
										true
										(if (equal? 1 (count local_runtime_condition_parts))
											(car local_runtime_condition_parts)
											(cons (quote and) local_runtime_condition_parts))))
								runtime_condition_expr_raw))
							(define runtime_group_filter_parts
								(if (equal? resolved_stage_group '(1))
									'()
									(map resolved_stage_group (lambda (col)
										(begin
											(define runtime_group_value
												(if materialized_source
													(match col
														'((quote get_column) (eval tblvar) _ local_col _) (symbol (concat tblvar "." local_col))
														'((symbol get_column) (eval tblvar) _ local_col _) (symbol (concat tblvar "." local_col))
														(replace_columns_from_expr col))
													(replace_columns_from_expr col)))
											'((quote equal?) runtime_group_value '((quote outer) (symbol (if is_fk_reuse fk_pk_col (expr_name col))))))))))
							(set cols (merge_unique (list
							...
							'((quote createcolumn) '('table schema grouptbl) (agg_col_name ag) "any" '(list) this_options
								...
								'((quote lambda) (map resolved_stage_group (lambda (col) (symbol (if is_fk_reuse fk_pk_col (expr_name col)))))
									(scan_wrapper 'scan schema tbl
										...
										'((quote lambda) (map (merge tblvar_cols filtercols) (lambda (col) (symbol (concat tblvar "." col)))) (optimize (cons (quote and) (merge (list (replace_columns_from_expr runtime_condition_expr)) runtime_group_filter_parts))))
										(cons list cols)
						...
						(cons 'begin (merge
							...
							(list
								...
								(if (nil? (wg_store "fields")) grouped_plan
									(begin
										(define wg_ctr (newsession)) (wg_ctr "n" 0)
										(define wg_nn (lambda () (begin (wg_ctr "n" (+ (wg_ctr "n") 1)) (concat "__wgp_" (wg_ctr "n")))))
										...
										(define wg_pl (newsession)) (wg_pl "l" '())
										(define wg_row_fields (newsession))
										(map_assoc fields (lambda (fk fv)
											(begin
												(map (materialized_source_expr_keys fv) (lambda (key)
													(wg_row_fields key fk)))
												nil)))
										(define wg_find_row_field (lambda (expr)
											(reduce (materialized_source_expr_keys expr) (lambda (found key)
												(if (not (nil? found))
													found
													(coalesce (wg_row_fields key) nil)))
												nil)))
										(define wg_promises (newsession))
										(define wg_window_value (lambda (expr) (begin
											(define existing (reduce (materialized_source_expr_keys expr) (lambda (found key)
												(if (not (nil? found))
													found
													(coalesce (wg_promises key) nil)))
												nil))
											(if (not (nil? existing))
												(symbol existing)
												(begin
													...
													(map (materialized_source_expr_keys expr) (lambda (key)
														(wg_promises key pn)))
													(wg_pl "l" (cons (list pn acn wfn) (wg_pl "l")))
													(symbol pn))))))
										(define wg_emit_window_expr (lambda (expr) (begin
											(define has_nested_window (not (equal? (extract_window_funcs expr) '())))
											(if has_nested_window
												(match expr
													(cons (symbol window_func) _) (wg_window_value expr)
													(cons (quote window_func) _) (wg_window_value expr)
													(cons '(quote window_func) _) (wg_window_value expr)
													(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
														expr
														(cons sym (map args wg_emit_window_expr)))
													expr)
												(begin
													(define row_field (wg_find_row_field expr))
													(if (not (nil? row_field))
														...
														(match expr
															(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
																...
																(cons sym (map args wg_emit_window_expr)))
															expr)))))))
										(define wg_out_fields (map_assoc (wg_store "fields") (lambda (k v)
											(if (equal? (extract_window_funcs v) '())
												(list (quote get_assoc) (symbol "__wgr") k)
												(wg_emit_window_expr v)))))
										/* scan keytable for each promise: aggregate the column globally */
										(define wg_scans (map (wg_pl "l") (lambda (pi) (match pi '(pn acn wfn)
											(begin
										...
										(define wg_rr_body (cons (quote list) (merge (extract_assoc wg_out_fields (lambda (k v)
											(list k v))))))
										(cons 'begin (merge wg_scans (list
											(list (quote set) (symbol "__wg_orig_rr") (symbol "resultrow"))
											(list (quote set) (symbol "resultrow")
												(list (quote lambda) (list (symbol "__wgr"))
													(list (symbol "__wg_orig_rr") wg_rr_body)))
											grouped_plan))))))))
				))
			)
			(begin /* multi-table GROUP BY via prejoin materialization */
				...
				(define grp_table_aliases (map grp_tables (lambda (t) (match t '(tv _ _ _ _) tv ""))))
				(define prejoin_joinexpr_has_aggregate (lambda (expr) (match expr
					(cons (symbol aggregate) _) true
					(cons '(quote aggregate) _) true
					(cons sym args) (reduce args (lambda (a b) (or a (prejoin_joinexpr_has_aggregate b))) false)
					false)))
				(define prejoin_local_joinexpr_part (lambda (part)
					(and
						(not (prejoin_joinexpr_has_aggregate part))
						(reduce (extract_tblvars part) (lambda (acc tv)
							(and acc (has? grp_table_aliases tv)))
							true))))
				(define split_prejoin_joinexpr (lambda (expr)
					(begin
						(define _parts (flatten_and_terms (coalesceNil expr true)))
						(list
							(combine_and_terms (filter _parts prejoin_local_joinexpr_part))
							(combine_and_terms (filter _parts (lambda (part) (not (prejoin_local_joinexpr_part part)))))))))
				(define prejoin_joinexpr_split (reduce grp_tables (lambda (acc td)
					(match acc '(tables_acc raw_deferred_acc deferred_acc)
						(match td '(tv tschema ttbl tisOuter tjoinexpr)
							(match (split_prejoin_joinexpr tjoinexpr) '(local_joinexpr deferred_joinexpr)
								(list
					...
					(list '() '() '())))
				(define prejoin_source_tables (car prejoin_joinexpr_split))
				(define prejoin_source_table_priority (lambda (td) (match td
					'(_ _ ttbl _ _)
					(begin
						...
						(if (match base_tbl
							...
							(if (materialized-source? base_tbl)
								...
								2)))
					_ 2)))
				(define prejoin_source_tables_have_outer (reduce prejoin_source_tables (lambda (found td) (or found (match td
					'(_ _ _ tisOuter _) tisOuter
					false)))
					false))
				(define prejoin_source_tables (if prejoin_source_tables_have_outer
					prejoin_source_tables
					(sort prejoin_source_tables (lambda (a b)
						(< (prejoin_source_table_priority a) (prejoin_source_table_priority b))))))
				(define deferred_prejoin_joinexpr_parts_raw (cadr prejoin_joinexpr_split))
				(define deferred_prejoin_joinexpr_parts (cadr (cdr prejoin_joinexpr_split)))
				(define prejoin_alias_map (build_occurrence_alias_map prejoin_source_tables))
				(define prejoin_alias_variants (lambda (tv tschema ttbl)
					(merge
						(alias_lookup_variants tv)
						(if (equal? (visible_occurrence_alias tv) ttbl) (list (concat tschema "." ttbl)) '()))))
				(define prejoin_exact_table_aliases (map prejoin_source_tables (lambda (t) (match t
					'(tv _ _ _ _) tv
					nil))))
				(define known_table_aliases (merge (map prejoin_source_tables (lambda (t) (match t
					'(tv tschema ttbl _ _) (prejoin_alias_variants tv tschema ttbl)
					'())))))
				(define prejoin_local_join_terms_raw
					(reduce prejoin_source_tables (lambda (acc td) (match td
						'(_ _ _ _ tjoinexpr)
						(reduce (flatten_and_terms (coalesceNil tjoinexpr true))
							(lambda (terms_acc part) (append_unique terms_acc part))
							acc)
						acc))
						'()))
				(define prejoin_local_join_terms
					(reduce prejoin_local_join_terms_raw (lambda (acc part)
						(append_unique acc (replace_find_column part)))
						'()))
				(define normalize_prejoin_condition (lambda (expr join_terms) (begin
					(define _terms
						(reduce (flatten_and_terms (coalesceNil expr true))
							(lambda (acc part) (append_unique acc part))
							'()))
					(define filtered_terms
						(filter _terms (lambda (part)
							(not (reduce join_terms (lambda (found known)
								(or found (equal? part known)))
								false)))))
				...
				(define rewrite_materialized_source_runtime_markers (lambda (expr) (begin
					(define match_materialized_runtime_agg (lambda (agg_args)
						(begin
							...
							(define agg_name (serialize_canonical_expr canonical_agg_args))
							(define agg_prefix (if (equal? agg_args aggregate_count_descriptor) "COUNT(*)" agg_name))
							(reduce prejoin_source_tables (lambda (acc td)
								(if (not (nil? acc))
									acc
									(match td '(tv tschema ttbl _ _)
										(begin
					...
					(match expr
						(cons (symbol aggregate) agg_args)
						(if (equal? (extract_tblvars expr) '())
							expr
							(begin
								(define match_col (match_materialized_runtime_agg agg_args))
								(if (nil? match_col)
									(match agg_args
										'(agg_expr agg_reduce agg_neutral)
										(list (quote aggregate) (rewrite_materialized_source_runtime_markers agg_expr) agg_reduce agg_neutral)
										_ expr)
									(list (quote get_column) (car match_col) false (cadr match_col) false))))
						(cons '(quote aggregate) agg_args)
						(if (equal? (extract_tblvars expr) '())
						...
						(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
							expr
							(cons sym (map args rewrite_materialized_source_runtime_markers)))
						expr))))
				...
				(define prejoin_condition_refs_only_sources (lambda (expr)
					(reduce (extract_all_get_columns expr) (lambda (ok mc) (and ok (match mc
						'(_ '((symbol get_column) alias_ _ _ _))
						(or (nil? alias_) (has? known_table_aliases alias_))
						'(_ '((quote get_column) alias_ _ _ _))
						(or (nil? alias_) (has? known_table_aliases alias_))
						true)))
						true)))
				(define split_prejoin_source_condition (lambda (expr)
					(begin
						(define expr_parts (flatten_and_terms (coalesceNil expr true)))
						(define local_parts (filter expr_parts prejoin_condition_refs_only_sources))
						(define deferred_parts (filter expr_parts (lambda (part)
							(not (prejoin_condition_refs_only_sources part)))))
						(list
							(combine_and_terms local_parts)
							(combine_and_terms deferred_parts)))))
				(define grp_condition_split (split_condition condition grp_ps_tables))
				(define prejoin_source_condition_split
					(split_prejoin_source_condition
						(match grp_condition_split '(prejoin_condition _) prejoin_condition)))
				(define condition (normalize_prejoin_condition
					(match prejoin_source_condition_split '(prejoin_condition _) prejoin_condition)
					prejoin_local_join_terms))
				(define post_group_condition (match grp_condition_split '(_ deferred_condition)
					(begin
						(define source_deferred_condition
							(match prejoin_source_condition_split '(_ source_deferred) source_deferred))
						(define combined_deferred_condition
							(combine_and_terms (merge
								(flatten_and_terms deferred_condition)
								(flatten_and_terms source_deferred_condition))))
						(if (equal? combined_deferred_condition true) post_group_condition
							(if (equal? post_group_condition true)
								combined_deferred_condition
								(cons (quote and) (cons post_group_condition (list combined_deferred_condition))))))))
				(define raw_grp_condition_split (split_condition raw_materialize_condition grp_ps_tables))
				(define raw_prejoin_source_condition_split
					(split_prejoin_source_condition
						(match raw_grp_condition_split '(raw_prejoin_condition _) raw_prejoin_condition)))
				(define raw_condition (normalize_prejoin_condition
					(match raw_prejoin_source_condition_split '(raw_prejoin_condition _) raw_prejoin_condition)
					prejoin_local_join_terms_raw))
				(define raw_post_group_condition (match raw_grp_condition_split '(_ raw_deferred_condition)
					(begin
						(define raw_source_deferred_condition
							(match raw_prejoin_source_condition_split '(_ raw_source_deferred) raw_source_deferred))
						(define raw_combined_deferred_condition
							(combine_and_terms (merge
								(flatten_and_terms raw_deferred_condition)
								(flatten_and_terms raw_source_deferred_condition))))
						(if (equal? raw_combined_deferred_condition true) raw_post_group_condition
							(if (equal? raw_post_group_condition true)
								raw_combined_deferred_condition
								(cons (quote and) (cons raw_post_group_condition (list raw_combined_deferred_condition))))))))
				/* Aggregate-bearing condition parts belong to the grouped stage, even for
				global multi-table prejoins. Without preserved outer tables there is no
				later keytable join to reintroduce them, so dropping them here turns
				scalar SUM/COUNT filters into unconditional group scans. */
				...
				/* Aggregate-bearing WHERE terms stay in post_group_condition. Duplicating
				them here as raw stage_having can strand nested COUNT(*) markers inside a
				group-stage bool callback before replace_group_field_expr has a chance to
				substitute them through the helper/keytable columns. */
				(define stage_having stage_having)
				...
				(define outer_visible_aliases (if (nil? grp_ps_tables) '()
					(map grp_ps_tables (lambda (td) (match td
						'(tv _ ttbl _ _) (if (nil? tv) ttbl (visible_occurrence_alias tv))
						"")))))
				(define keep_outer_field_early (lambda (expr)
					(if (nil? stage_scope)
						nil
						(match expr
							'((symbol get_column) alias_ _ _ _) (if (has? outer_visible_aliases alias_) expr nil)
							'((quote get_column) alias_ _ _ _) (if (has? outer_visible_aliases alias_) expr nil)
							_ nil))))
				...
				(define all_referenced_columns (merge
					...
					(merge (extract_assoc fields (lambda (k v) (extract_all_get_columns v))))
					(merge (extract_assoc resolved_fields (lambda (k v) (extract_all_get_columns v))))
					(merge (map ags (lambda (ag) (match ag
						'(agg_expr _ _) (extract_all_get_columns agg_expr)
						'()))))
				...
				(define lower_prejoin_lineage_expr (lambda (expr) (begin
					(define lower_once (lambda (cur)
						(reduce prejoin_source_tables (lambda (inner td) (match td
							...
							inner))
							cur)))
					(define expr2 (lower_once expr))
					(define expr3 (lower_once expr2))
					(define expr4 (lower_once expr3))
				...
				(define prejoin_canonical_column_expr (lambda (expr)
					(canonicalize_expr
						(normalize_canonical_aliases (canonicalize_prejoin_source_expr expr))
						prejoin_alias_map)))
				(define prejoin_physical_column_name (lambda (expr)
					(concat "p" (substr
						(fnv_hash (serialize_canonical_expr (prejoin_canonical_column_expr expr)))
						0 8))))
				(define prejoin_canonicalize_source_columns (lambda (expr) (match expr
					'((symbol get_column) alias_ ti col ci)
					(begin
						/* Virtual schema fields are aliases for expressions. Canonical
						prejoin names must be based on that source expression, not on the
						visible alias column, so sibling group caches can reuse the same
						physical payload column. */
						(define prejoin_direct_coldef (schema_column_def (schema_assoc_cols schemas alias_) col ci))
						(define prejoin_visible_coldef (schema_column_def (schema_assoc_cols schemas (visible_occurrence_alias alias_)) col ci))
						(define prejoin_schema_expr_entry (if (or (nil? alias_) (has? prejoin_exact_table_aliases alias_))
							nil
							(if (and (not (nil? prejoin_direct_coldef)) (has_assoc? prejoin_direct_coldef "Expr"))
								(list (quote found) (prejoin_direct_coldef "Expr"))
								(if (and (not (nil? prejoin_visible_coldef)) (has_assoc? prejoin_visible_coldef "Expr"))
									(list (quote found) (prejoin_visible_coldef "Expr"))
									nil))))
						(match prejoin_schema_expr_entry
							'(found prejoin_schema_expr)
							(prejoin_canonicalize_source_columns (group_rebase_schema_expr alias_ prejoin_schema_expr))
							_ (begin
								(define cols (coalesce
									(schema_assoc_cols schemas alias_)
									(reduce_assoc schemas (lambda (found schema_alias schema_cols)
										(if (not (nil? found)) found
											(if (equal?? schema_alias alias_) schema_cols nil)))
										nil)))
								(define canonical_col (coalesce
									(reduce cols (lambda (found coldef)
										...
										nil)
									col))
								(list (quote get_column) alias_ false canonical_col false))))
					'((quote get_column) alias_ ti col ci)
					(begin
						/* Keep the quoted matcher in sync with the symbol matcher above;
						both forms represent the same get_column IR in this dialect. */
						(define prejoin_direct_coldef (schema_column_def (schema_assoc_cols schemas alias_) col ci))
						(define prejoin_visible_coldef (schema_column_def (schema_assoc_cols schemas (visible_occurrence_alias alias_)) col ci))
						(define prejoin_schema_expr_entry (if (or (nil? alias_) (has? prejoin_exact_table_aliases alias_))
							nil
					...
					(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						expr
						(cons sym (map args prejoin_canonicalize_source_columns)))
					expr)))
				(define prejoin_row_independent_scalar? (lambda (expr)
					(and
						(equal? (extract_all_get_columns expr) '())
						(equal? (extract_aggregates expr) '())
						(equal? (extract_window_funcs expr) '()))))
				...
				(define prejoin_columns_base (reduce all_referenced_columns (lambda (acc mc)
					(begin
						(define prejoin_source_expr (prejoin_canonicalize_source_columns (replace_find_column (cadr mc))))
						(define canon_name (prejoin_physical_column_name prejoin_source_expr))
						/* Literal exports, e.g. SELECT true AS can_view, are row-independent.
						Do not materialize them as payload columns: true/1 equality would
						let them collide with count-like literals in grouped caches. Logical
						aggregate/window sentinels are not scalar literals and must stay materialized. */
						(if (or
							(prejoin_row_independent_scalar? prejoin_source_expr)
							(reduce acc (lambda (found mc2) (or found (equal? (car mc2) canon_name))) false))
							acc
							(merge acc (list (list canon_name prejoin_source_expr)))))) '()))
				...
				(define prejoin_columns_projected (reduce (extract_assoc resolved_fields (lambda (field_name field_expr)
					(match field_expr
						...
						(if (prejoin_materializable_projection? field_expr)
							(list field_name (prejoin_canonicalize_source_columns (replace_find_column field_expr)))
							nil))))
					(lambda (acc mc)
						(if (or (nil? mc)
				...
				(define prejoin_condition_name (serialize_canonical_expr
					(canonicalize_expr
						...
						prejoin_alias_map)))
				(define prejoin_session_sensitive
					(or
						(expr_uses_session_state prejoin_row_domain_raw)
						(expr_uses_session_state raw_condition)
						(expr_uses_session_state condition)
						(expr_uses_session_state prejoin_source_tables)))
				(define prejoin_runtime_suffix
					(concat
						(runtime_cache_suffix_from_exprs
							(list prejoin_row_domain_raw raw_condition condition prejoin_source_tables))
						(if prejoin_session_sensitive
							(planner_current_user_session_snapshot_suffix)
							"")))
				(define prejoin_domain_key (concat
					(map prejoin_source_tables (lambda (t) (match t '(_ tschema ttbl _ _) (concat tschema "." ttbl))))
					"|" prejoin_condition_name prejoin_runtime_suffix))
				(define prejoin_keytable_domain_name
					(concat ".prejoin-domain:" (fnv_hash prejoin_domain_key)))
				(define prejoin_key (concat
					(map prejoin_source_tables (lambda (t) (match t '(_ tschema ttbl _ _) (concat tschema "." ttbl))))
					":" prejoin_col_names "|" prejoin_condition_name prejoin_runtime_suffix))
				(define prejointbl (concat ".prejoin:" (fnv_hash prejoin_key)))
				(prejoin_keytable_domain_sources prejointbl prejoin_keytable_domain_name)
				...
				(define temp_source_table? (lambda (table_source)
					(materialized-source? (planner_table_source_base table_source))))
				...
				(define rewrite_as_prejoin_column (lambda (expr) (match expr
					'((symbol get_column) src_alias ti col ci) (begin
						...
						(define scope_match (or (has? known_table_aliases src_alias)
							(has? known_table_aliases _visible_alias)))
						(define expr_lookup (materialized_source_expr_lookup prejointbl))
						(define canonical_source_expr (prejoin_canonicalize_source_columns (replace_find_column expr)))
						(define rewritten_source_expr (rewrite_source_aliases prejoin_alias_map (normalize_visible_aliases expr)))
						(define rewritten_canonical_source_expr
							(rewrite_source_aliases prejoin_alias_map (normalize_visible_aliases canonical_source_expr)))
						(define logical_source_expr (lower_prejoin_lineage_expr rewritten_source_expr))
						(define logical_canonical_source_expr (lower_prejoin_lineage_expr rewritten_canonical_source_expr))
						(define constant_source_expr (and scope_match (prejoin_row_independent_scalar? canonical_source_expr)))
						(define lookup_exprs (list
							expr
							canonical_source_expr
							(normalize_visible_aliases expr)
							(normalize_visible_aliases canonical_source_expr)
							(normalize_canonical_aliases expr)
							(normalize_canonical_aliases canonical_source_expr)
							rewritten_source_expr
							rewritten_canonical_source_expr
							(normalize_canonical_aliases rewritten_source_expr)
							(normalize_canonical_aliases rewritten_canonical_source_expr)
							logical_source_expr
							logical_canonical_source_expr
							(normalize_canonical_aliases logical_source_expr)))
						(define direct_field (if (nil? expr_lookup) nil
							(reduce lookup_exprs (lambda (found lookup_expr)
								(if (not (nil? found))
									found
						...
						(define direct_field (coalesce direct_field
							(materialized_field_from_get_column_name
								...
								expr)))
						(define direct_field_source (if (nil? direct_field)
							nil
							(coalesce (get_assoc (prejoin_canonical_sources prejointbl) direct_field) nil)))
						(define direct_field_valid (or
							(nil? direct_field)
							(nil? direct_field_source)
							(equal? (normalize_canonical_aliases direct_field_source)
								(normalize_canonical_aliases canonical_source_expr))
							(equal? (normalize_canonical_aliases direct_field_source)
								(normalize_canonical_aliases logical_canonical_source_expr))))
						(if (not direct_field_valid)
							(set direct_field nil)
							nil)
						(if (not (nil? direct_field))
							(list (quote get_column) prejoin_alias false direct_field false)
							(if constant_source_expr
								canonical_source_expr
								(if scope_match
									(list (quote get_column) prejoin_alias false (prejoin_physical_column_name canonical_source_expr) false)
									expr))))
					...
					(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						expr
						(cons sym (map args rewrite_as_prejoin_column)))
					expr)))
				...
				(define prejoin_projected_field_ref (lambda (expr) (match expr
					'((symbol get_column) src_alias _ col _)
					(if (has? known_table_aliases src_alias)
						nil
						(reduce prejoin_columns (lambda (found mc)
							(if (not (nil? found))
								found
								(if (equal? (car mc) col)
									(list (quote get_column) prejoin_alias false col false)
									nil)))
							nil))
					'((quote get_column) src_alias _ col _)
					(if (has? known_table_aliases src_alias)
						nil
						(reduce prejoin_columns (lambda (found mc)
							(if (not (nil? found))
							...
							nil))
					nil)))
				(define lower_prejoin_direct_ref (lambda (expr) (begin
					(define rewritten (rewrite_as_prejoin_column expr))
					(if (equal? rewritten expr)
						(coalesce (prejoin_projected_field_ref expr) rewritten)
						rewritten))))
				(define lower_prejoin_group_expr (lambda (expr) (begin
					(match expr
						'((symbol get_column) _ _ _ _)
						(lower_prejoin_direct_ref expr)
						'((quote get_column) _ _ _ _)
						(lower_prejoin_direct_ref expr)
						(cons (symbol aggregate) agg_args)
						(match agg_args
						...
						_ (begin
							...
							(define canonical_lowered (prejoin_canonical_column_expr expr))
							(define projected_field
								(reduce prejoin_columns (lambda (found mc)
									(if (not (nil? found))
										found
										(if (equal? canonical_lowered (prejoin_canonical_column_expr (cadr mc)))
											(car mc)
											nil)))
									nil))
							(if (not (nil? projected_field))
								...
								(begin
									(define direct_prejoin_expr (rewrite_as_prejoin_column expr))
									(if (not (equal? direct_prejoin_expr expr))
										direct_prejoin_expr
										(match lowered
											(cons (symbol window_func) wf_rest)
											(match wf_rest
												'(fn wf_args over)
												(list (quote window_func)
													fn
													(map wf_args lower_prejoin_group_expr)
													(match over
														'(partition_exprs order_exprs)
														(list
															(map partition_exprs lower_prejoin_group_expr)
															(map order_exprs (lambda (o) (match o
																'(col dir) (list (lower_prejoin_group_expr col) dir)
																o))))
														over))
												_ expr)
											(cons '(quote window_func) wf_rest)
				...
				(define rewrite_group_key_to_group_alias (lambda (expr)
					(coalesce
						...
						(match expr
							(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
								expr
								(cons sym (map args rewrite_group_key_to_group_alias)))
							expr))))
				(define grouped_outer_condition_aliases (map grp_ps_tables (lambda (td) (match td
				...
				(define rewrite_local_prejoin_count_term (lambda (expr) (match expr
					...
					(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						expr
						(cons sym (map args rewrite_local_prejoin_count_term)))
					expr)))
				...
				(define grouped_having
					(if (or (nil? grouped_having_source_raw) (equal? grouped_having_source_raw true))
						...
						(rewrite_group_key_to_group_alias
							(lower_prejoin_group_expr
								(rewrite_materialized_source_runtime_markers grouped_having_source_raw)))))
				...
				(define lower_prejoin_agg_inputs (lambda (expr) (match expr
					...
					(match agg_args
						'(agg_expr agg_reduce agg_neutral)
						(if (equal? agg_reduce (quote group_any_value_reduce))
							expr
							(list (quote aggregate) (lower_prejoin_group_expr agg_expr) agg_reduce agg_neutral))
						_ expr)
					(cons '(quote aggregate) agg_args)
					...
					(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
						expr
						(cons sym (map args lower_prejoin_agg_inputs)))
					expr)))
				...
				(define grouped_outer_schema_bindings (merge (map grouped_outer_tables (lambda (td) (match td
					...
					'())))))
				(define grouped_visible_schemas (filter_assoc schemas (lambda (k _v)
					(not (has? grouped_outer_aliases k)))))
				...
				(define grouped_plan_condition_base (if (nil? grp_ps_tables)
					nil
					(begin
						...
						(if (or (nil? grouped_plan_condition_base_raw) (equal? grouped_plan_condition_base_raw true))
							nil
							(lower_prejoin_group_expr
								(rewrite_materialized_source_runtime_markers grouped_plan_condition_base_raw))))))
				(define recursive_replace_find_column (lambda (expr)
					(match expr
						...
						(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
							expr
							(cons sym (map args recursive_replace_find_column)))
						expr)))
				(define recursive_replace_find_column_condition (lambda (expr)
					(match expr
						...
						(cons sym args) (if (or (is_opaque_scope_sym sym) (is_quote_scope_sym sym) (not (list? args)))
							expr
							(cons sym (map args recursive_replace_find_column_condition)))
						expr)))
				(define grouped_having_for_recursive (if (nil? grouped_having) nil
					(if (nil? grp_ps_tables)
						grouped_having
				...
				(define grouped_stage (if is_dedup
					(make_dedup_stage grouped_keys
						(if (nil? stage_scope) nil (list prejoin_alias)))
				...
				(define grouped_expr_is_group_key (lambda (expr)
					(reduce grouped_keys (lambda (found gexpr)
						...
						false)))
				(define grouped_any_value_aggregate (lambda (expr)
					/* MySQL permits non-GROUP-BY projections in grouped queries. Once the
					prejoin row-domain is folded into a keytable, preserve those row-local
					values as an arbitrary group member instead of dropping them to NULL. */
					(list (quote aggregate) expr (quote group_any_value_reduce) nil)))
				(define grouped_expr_refs_only_prejoin? (lambda (expr)
					(reduce (extract_tblvars expr) (lambda (ok tv)
						(and ok (equal?? tv prejoin_alias)))
						true)))
				(define grouped_carry_row_value_if_needed (lambda (expr)
					(if (or
						(grouped_expr_is_group_key expr)
						(not (equal? (extract_aggregates expr) '()))
						(not (equal? (extract_window_funcs expr) '()))
						(not (grouped_expr_refs_only_prejoin? expr)))
						expr
				...
				(define no_outer_group_fields_for_recursive (reduce_assoc grouped_fields_for_recursive (lambda (acc k v)
					(if (or
						...
						(set_assoc acc k v)
						(set_assoc acc k (grouped_any_value_aggregate v))))
					'()))
				...
				(define remaining_partition_stages (filter partition_stages (lambda (ps)
					(not (reduce (coalesceNil (stage_partition_aliases ps) '()) (lambda (acc a)
						(or acc (has? known_table_aliases a))) false)))))
				(define grouped_result (if (nil? grp_ps_tables)
					(begin
						...
						(define no_outer_group_condition (if (or (nil? no_outer_group_condition_raw) (equal? no_outer_group_condition_raw true))
							nil
							(rewrite_materialized_source_runtime_markers no_outer_group_condition_raw)))
						...
						(define no_outer_group_stage_needed (or
						...
						(define no_outer_group_stage (if no_outer_group_stage_needed
							(if is_dedup
								nil
								(make_group_stage grouped_keys grouped_having grouped_order stage_limit stage_offset nil nil))
							nil))
					...
					(build_queryplan schema
						...
						(merge grouped_visible_schemas (list prejoin_alias prejoin_schema_def) grouped_outer_schema_bindings)
						recursive_replace_find_column
						update_target)))
				(if neumann_pipeline_trace (print "[neumann] build_inner: group after grouped_result") nil)
					...
					(reduce prejoin_trigger_source_tables (lambda (acc td) (match td
						...
						(begin
							...
							(if (materialized-source? normalized_ttbl)
								(reduce (coalesceNil (materialized_source_dependency_tables normalized_ttbl) '())
									append_dependency_table_unique
									acc)
								acc))
						_ acc))
						'()))
				(define prejoin_has_temp_source_table
					(reduce prejoin_trigger_source_tables (lambda (found td) (or found (match td
						'(_ _ ttbl _ _) (temp_source_table? ttbl)
						false)))
						false))
				/* Planner-owned prejoins are cache-engine tables. For ordinary
				persistent source tables, keep the cache hot and maintain it
								...
								(begin
									...
									(begin (seen_trigger_tables trigger_table_key true)
										(if prejoin_use_invalidation_triggers
											...
											(begin
												(define prejoin_incremental_guard_terms
													(filter (map prejoin_source_tables (lambda (td) (match td
														'(_ tschema ttbl _ _)
														(if (temp_source_table? ttbl)
															(list 'not (list 'nil? (planner_codegen_table_source tschema ttbl)))
															nil)
														_ nil)))
														(lambda (x) (not (nil? x)))))
												(define prejoin_incremental_guard_expr
													(if (equal? prejoin_incremental_guard_terms '())
														true
														(if (equal? 1 (count prejoin_incremental_guard_terms))
															(car prejoin_incremental_guard_terms)
															(cons 'and prejoin_incremental_guard_terms))))
												(define prejoin_invalidation_expr
			...
			(if has_window (begin
				...
				(if neumann_pipeline_trace (print "[neumann] build_inner: window begin") nil)
				/* Case 8: different OVER clauses */
				(define first_over (nth (car window_funcs_all) 2))
				(if neumann_pipeline_trace (print (concat "[neumann] build_inner: window funcs " (serialize window_funcs_all))) nil)
				(if neumann_pipeline_trace (print (concat "[neumann] build_inner: first over " (serialize first_over))) nil)
				(if neumann_pipeline_trace (print "[neumann] build_inner: window after first_over") nil)
				...
				(if neumann_pipeline_trace (print "[neumann] build_inner: window after over compare") nil)
				/* extract and resolve OVER info */
				(define over_partition (map (car first_over) replace_find_column))
				(if neumann_pipeline_trace (print "[neumann] build_inner: window after partition") nil)
				(define over_order (map (cadr first_over) (lambda (o) (match o '(col dir) (list (replace_find_column col) dir)))))
				(if neumann_pipeline_trace (print "[neumann] build_inner: window after order") nil)
				(define effective_sort (merge (map over_partition (lambda (pe) (list pe <))) over_order))
				(if neumann_pipeline_trace (print "[neumann] build_inner: window after effective sort") nil)
				(define stage_order_resolved (map (coalesce stage_order '()) (lambda (x) (match x '(col dir) (list (replace_find_column col) dir)))))
				(if neumann_pipeline_trace (print "[neumann] build_inner: window after stage order") nil)
			...
			/* orc_window_descriptor: fn × args × sort_col_names → (extra_mapcols mapfn reducefn reduceinit)
				...
				(define all_orc_window (reduce wf_resolved (lambda (acc wf) (and acc (or
					(and has_over_order (or (is_orc_window wf) (is_ordered_agg wf)))
					(match wf '("ROW_NUMBER" _ _) true _ false))))
					true))
				...
				(if all_orc_window
					(match tables
						...
						'('(tblvar schema tbl isOuter _)) (begin
							...
							/* ORC columns are physical cache columns. Include the full
							window signature so different PARTITION/ORDER specs never
							reuse a stale ROW_NUMBER/RANK value for the same table. */
							(define orc_signature
								(list wf_fn wf_args over_partition over_order tbl))
							(define orc_col_name
								(concat ".orc_" wf_fn "_" (sanitize_temp_name tbl) "_"
									(fnv_hash (string orc_signature))))
					...
					(define order_driver_aliases (reduce (coalesce stage_order '()) (lambda (acc order_item) (match order_item
						'(col _dir) (match col
						...
						acc))
						'()))
					(define all_inner_tables (reduce tables (lambda (acc td) (match td
						'(_ _ _ isOuter _) (and acc (not isOuter))
						acc))
						true))
					(define order_driver_alias (if (and all_inner_tables (equal? 1 (count order_driver_aliases)))
						(car order_driver_aliases)
						nil))
					(define ordered_tables (if (nil? order_driver_alias)
						tables
						(merge
							(filter tables (lambda (td) (match td '(tblvar _ _ _ _) (equal?? tblvar order_driver_alias) false)))
							(filter tables (lambda (td) (match td '(tblvar _ _ _ _) (not (equal?? tblvar order_driver_alias)) true))))))
						...
						(match tables
							(cons '(tblvar schema tbl isOuter joinexpr) tables) (begin /* outer scan */
								...
								(match (split_scan_condition isOuter (replace_find_column (coalesceNil joinexpr true)) scan_condition tables) '(now_condition later_condition) (begin
									...
									/* Outer scalar helper scans may need current filter columns again in
									the map body for NULL-extension guards. Keep those bindings available. */
									(set cols (merge_unique (list cols filtercols)))
									...
									(define ps_ord (if (not (nil? tbl_once_limit)) nil
										(find_partition_stage_for_alias partition_stages tblvar tbl)))
									(define ord_stage_cfg (make_ordered_scan_stage_config
										tbl ps_ord stage_order stage_limit stage_offset stage_partcols is_first))
									...
									(define eff_order_raw (ordered_scan_stage_config_order ord_stage_cfg))
									(define eff_order (map (coalesce eff_order_raw '()) (lambda (o) (match o
										'(col dir)
										(list (lower_materialized_emit_expr schema base_tbl tblvar col) dir)
										o))))
									/* extract order cols for this tblvar */
									(set ordercols (extract_scan_order_cols_for_tblvar eff_order tblvar))
									(set dirs (extract_scan_order_dirs_for_tblvar eff_order tblvar))

									/* offset/limit: tagged helper scans carry their own local limits. */
									(define ord_raw_scan_offset (ordered_scan_stage_config_offset ord_stage_cfg))
									(define ord_raw_scan_limit (ordered_scan_stage_config_limit ord_stage_cfg))
									(define ord_raw_scan_partcols (ordered_scan_stage_config_partcols ord_stage_cfg))
									(define ord_effective_once_limit (coalesce
										(ordered_scan_stage_config_once_limit ord_stage_cfg)
										(if (and isOuter (scalar_once_helper_scan? schema base_tbl tblvar schemas) (not (nil? joinexpr))) 2 nil)))
									...
									(define ord_once_key_exprs
										(if (and (not (nil? ord_once_contract)) (> ord_scan_partcols 0))
											(filter
												(map (produceN ord_scan_partcols) (lambda (idx)
													(if (< idx (count eff_order))
														(match (nth eff_order idx)
															'(col _dir) (replace_columns_from_expr col)
															_ nil)
														nil)))
												(lambda (expr) (not (nil? expr))))
											'()))
									(define ord_scan_body (wrap_once_limit_body ord_once_name ord_once_key_exprs ord_child_body))
									/* emit init code from partition stage if present */
									(define ps_init (ordered_scan_stage_config_init ord_stage_cfg))
									(define ord_scan_core (scan_wrapper 'scan_order schema base_tbl
										/* condition */
										(cons list filtercols)
									...
									(define ord_scan (wrap_once_limit_scan ord_once_name ord_once_key_exprs ord_scan_core))
									(if (nil? ps_init) ord_scan (list (quote begin) ps_init ord_scan))
								))
							)
									...
									(list
										...
										(list
											...
											(list
												(symbol "resultrow")
												(cons (symbol "list")
													(map_assoc emit_fields (lambda (k v) (replace_columns_from_expr v)))))
											0)
										0))
								/* DML mode: emit mutation payload; actual DELETE/UPDATE runs in build_dml_plan's resultrow wrapper */
								(begin (define ut_cols (nth update_target 1))
									(define ut_tag (nth update_target 2))
									(define emit_condition (if (nil? last_scan_ctx) condition
										(match last_scan_ctx
											'(scan_schema scan_tbl scan_tblvar) (lower_materialized_scan_condition scan_schema scan_tbl scan_tblvar condition)
											condition)))
									(define ut_cols (if (nil? last_scan_ctx) ut_cols
										(match last_scan_ctx
											'(scan_schema scan_tbl scan_tblvar) (lower_materialized_emit_assoc scan_schema scan_tbl scan_tblvar ut_cols)
											ut_cols)))
									(if (equal? ut_cols '())
										'('if (optimize (replace_columns_from_expr emit_condition))
											'((symbol "resultrow") (list (symbol "list") "__dml_tag" ut_tag "__update" '$update "__values" nil))
											0)
										'('if (optimize (replace_columns_from_expr emit_condition))
											'((symbol "resultrow") (list (symbol "list") "__dml_tag" ut_tag "__update" '$update "__values" (cons (symbol "list") (map_assoc ut_cols (lambda (k v) (replace_columns_from_expr v))))))
											0))))
						)
					))
						...
						(define build_scan (lambda (tables condition last_scan_ctx bound_update_expr)
							(match tables
								(cons '(tblvar schema tbl isOuter joinexpr) tables) (begin /* outer scan */
									...
									(define tbl_scan_partcols (scan_tagged_table_partition_cols tbl))
									(define tbl_once_limit (scan_tagged_table_once_limit tbl))
									(define scalar_once_outer_helper
										(and
											(not isOuter)
											(not (nil? joinexpr))
											(scalar_once_helper_scan? schema base_tbl tblvar schemas)))
									(define scan_isOuter (or isOuter scalar_once_outer_helper))
									(define scan_condition (lower_materialized_scan_condition schema base_tbl tblvar condition))
									(define visible_fields (lower_materialized_emit_assoc schema base_tbl tblvar fields))
									...
									(match (split_scan_condition scan_isOuter (replace_find_column (coalesceNil joinexpr true)) scan_condition tables) '(now_condition later_condition) (begin
										(define effective_later_condition later_condition)
										(set cols (extend_scan_cols_for_later_condition tblvar cols effective_later_condition))
										(set filtercols (merge_unique (list (extract_columns_for_tblvar tblvar now_condition) (extract_outer_columns_for_tblvar tblvar now_condition))))
										/* Outer scalar helper scans may need current filter columns again in
										the map body for NULL-extension guards. Keep those bindings available. */
										(set cols (merge_unique (list cols filtercols)))
										/* For UPDATE target: prepend $update to mapcols. Build this after
										all scan-body column extensions so the map lambda receives every
										...
										(if (and (equal? base_tbl ".(1)") (equal? cols (list)) (equal? filtercols (list)))
											...
											(begin
												...
												(define child_scan (build_scan tables effective_later_condition (list schema base_tbl tblvar) next_update_expr))
												...
												(define _ps (if (not (nil? tbl_once_limit))
													nil
													(find_partition_stage_for_alias partition_stages tblvar tbl)))
												(define scan_stage_cfg (make_local_scan_stage_config tbl _ps))
												(define _tagged_scan (local_scan_stage_config_tagged scan_stage_cfg))
												(define scan_raw_partcols (local_scan_stage_config_partcols scan_stage_cfg))
												(define scan_raw_limit (local_scan_stage_config_limit scan_stage_cfg))
												(define scan_raw_offset (local_scan_stage_config_offset scan_stage_cfg))
												(define scan_effective_once_limit (coalesce
													(local_scan_stage_config_once_limit scan_stage_cfg)
													(if (and scan_isOuter (scalar_once_helper_scan? schema base_tbl tblvar schemas) (not (nil? joinexpr))) 2 nil)))
												...
												(if (or _tagged_scan (not (nil? _ps)))
													...
													(begin
														(define ps_filtercols (merge_unique (list (extract_columns_for_tblvar tblvar now_condition) (extract_outer_columns_for_tblvar tblvar now_condition))))
														(define ps_order (local_scan_stage_config_order scan_stage_cfg))
														(define ps_partcols (if (nil? scan_once_contract)
															(local_scan_stage_config_partcols scan_stage_cfg)
															(once_limit_scan_contract_partition_cols scan_once_contract)))
														(define ps_limit (if (nil? scan_once_contract)
															(local_scan_stage_config_limit scan_stage_cfg)
															(once_limit_scan_contract_limit scan_once_contract)))
														(define ps_offset (if (nil? scan_once_contract)
															(local_scan_stage_config_offset scan_stage_cfg)
															(once_limit_scan_contract_offset scan_once_contract)))
														(define ps_ordercols (extract_scan_order_cols_for_tblvar ps_order tblvar))
														(define ps_dirs (extract_scan_order_dirs_for_tblvar ps_order tblvar))
														(define ps_once_key_exprs
															(if (and (not (nil? scan_once_contract)) (> ps_partcols 0))
																(filter
																	(map (produceN ps_partcols) (lambda (idx)
																		(if (< idx (count ps_order))
																			(match (nth ps_order idx)
																				'(col _dir) (replace_columns_from_expr col)
																				_ nil)
																			nil)))
																	(lambda (expr) (not (nil? expr))))
																'()))
														(define ps_scan_body (wrap_once_limit_body scan_once_name ps_once_key_exprs scan_body))
														/* emit init code from partition stage if present */
														(define ps_init2 (local_scan_stage_config_init scan_stage_cfg))
														(define ps_scan_core (scan_wrapper 'scan_order schema base_tbl
															(cons list (merge_unique ps_filtercols cols))
															'((quote lambda) (map (merge_unique ps_filtercols cols) (lambda(col) (symbol (concat tblvar "." col)))) (optimize (replace_columns_from_expr now_condition)))
															(cons list ps_ordercols)
															(cons list ps_dirs)
															ps_partcols ps_offset ps_limit
															scan_mapcols
															(list (symbol "lambda") scan_mapfn_params ps_scan_body)
															nil nil scan_isOuter))
														(define ps_scan (wrap_once_limit_scan scan_once_name ps_once_key_exprs ps_scan_core))
														(if (nil? ps_init2) ps_scan (list (quote begin) ps_init2 ps_scan)))
													/* === regular scan === */
													(begin
														(define regular_scan_body (wrap_once_limit_body scan_once_name '() scan_body))
														(wrap_once_limit_scan scan_once_name '()
															(scan_wrapper 'scan schema base_tbl
																(cons list filtercols)
																'((quote lambda) (map filtercols (lambda(col) (symbol (concat tblvar "." col)))) (optimize (replace_columns_from_expr now_condition)))
																scan_mapcols
																(list (symbol "lambda") scan_mapfn_params regular_scan_body)
								...
								'() /* final inner (=scalar) */ (if (nil? update_target)
									...
									(begin (define ut_cols (nth update_target 1))
										...
										(if (equal? ut_cols '())
											/* DELETE */
											(list (quote if) (list (quote optimize) (replace_columns_from_expr emit_condition))
												(list (symbol "resultrow") (list (symbol "list") "__dml_tag" ut_tag "__update" bound_update_expr "__values" nil))
												0)
											/* UPDATE */
											(list (quote if) (list (quote optimize) (replace_columns_from_expr emit_condition))
												(list (symbol "resultrow") (list (symbol "list") "__dml_tag" ut_tag "__update" bound_update_expr "__values" (cons (symbol "list") (map_assoc ut_cols (lambda (k v) (replace_columns_from_expr v))))))
												0))))
							)
						))
...
/* replace_table_with_sym: replaces (table schema name) AST nodes with tbl:schema:name symbols
for tables that have pre-resolved defines. tbl_map is an assoc list of (schema.name . symbol). */
(define replace_table_with_sym (lambda (expr tbl_map)
	(if (not (list? expr)) expr
		(if (and (equal? (count expr) 3) (equal? (car expr) 'table) (string? (nth expr 1)) (string? (nth expr 2)))
			(begin
				...
				(if (nil? sym) expr sym))
			(map expr (lambda (e) (replace_table_with_sym e tbl_map)))))))

/* build_queryplan: wraps build_queryplan_inner with table-pointer pre-resolution */
(define build_queryplan (lambda (schema tables fields condition groups schemas replace_find_column update_target) (begin
	(if neumann_pipeline_trace (print "[neumann] build_queryplan: begin") nil)
	(define build_queryplan_contains_dep_marker (lambda (expr) (match expr
		(cons sym args)
		(or
			(equal?? sym (quote dependent_scalar_compile))
			(equal?? sym (quote (quote dependent_scalar_compile)))
			(equal?? sym (symbol dependent_scalar_compile))
			(and (list? sym) (build_queryplan_contains_dep_marker sym))
			(and (list? args)
				(reduce args (lambda (found arg)
					(or found (build_queryplan_contains_dep_marker arg)))
					false)))
		false)))
	(if neumann_pipeline_trace (print (concat "[neumann] build_queryplan: input markers tables="
		(string (build_queryplan_contains_dep_marker tables))
		" fields=" (string (build_queryplan_contains_dep_marker fields))
		" condition=" (string (build_queryplan_contains_dep_marker condition))
		" groups=" (string (build_queryplan_contains_dep_marker groups)))) nil)
	/* Collect base tables that can be pre-resolved */
	(define tbl_entries_raw (filter (map tables (lambda (td) (match td
		'(_ tschema tname _ _) (begin
			(define base_tname (scan_tagged_table_base tname))
			(if (string? base_tname) (list tschema base_tname) nil))
		nil))) (lambda (d) (not (nil? d)))))
	(define tbl_entries (reduce tbl_entries_raw (lambda (acc entry)
		(if (has? acc entry) acc (merge acc (list entry)))) '()))
	(if neumann_pipeline_trace (print "[neumann] build_queryplan: after tbl entries") nil)
	(define tbl_defines (map tbl_entries (lambda (e) (tbl-define-code (car e) (car (cdr e))))))
	(if neumann_pipeline_trace (print "[neumann] build_queryplan: after tbl defines") nil)
	(define _tbl_map (merge (map tbl_entries (lambda (e)
		(list (concat (car e) ":" (car (cdr e))) (symbol (concat "tbl:" (car e) ":" (car (cdr e)))))))))
	(if neumann_pipeline_trace (print "[neumann] build_queryplan: before inner") nil)
	(define _plan (build_queryplan_inner schema tables fields condition groups schemas replace_find_column update_target))
	(if neumann_pipeline_trace (print "[neumann] build_queryplan: after inner") nil)
	/* Replace (table schema name) calls with pre-resolved symbols in the plan. */
	(define opt_plan (replace_table_with_sym _plan _tbl_map))
	(if neumann_pipeline_trace (print "[neumann] build_queryplan: after replace table sym") nil)
	(if neumann_pipeline_trace (print (concat "[neumann] build_queryplan: tbl_defines has marker=" (string (build_queryplan_contains_dep_marker tbl_defines)))) nil)
	(if neumann_pipeline_trace (print (concat "[neumann] build_queryplan: opt_plan has marker=" (string (build_queryplan_contains_dep_marker opt_plan)))) nil)
	(define _final_plan (if (equal? (count tbl_defines) 0)
		opt_plan
		(cons (quote begin) (merge tbl_defines (list opt_plan)))))
	(if neumann_pipeline_trace (print "[neumann] build_queryplan: return") nil)
	_final_plan)
))
