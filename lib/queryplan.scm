...
(define build_row_assoc_expr (lambda (fields)
	(build_row_assoc_expr_with_specs fields '())))

(define build_row_assoc_expr_with_specs (lambda (fields specs)
	(cons (quote list)
		(reduce_assoc (coalesceNil fields '()) (lambda (acc key expr)
			(merge acc (list key (lower_embedded_scalars_with_specs expr specs))))
			'()))))
...
(define indexed_expr_fields (lambda (prefix exprs)
	(merge (map (produceN (count exprs)) (lambda (i)
		(list (concat prefix (string i)) (nth exprs i)))))))

(define row_values_expr (lambda (row_sym prefix n)
	(cons (quote list) (map (produceN n) (lambda (i)
		(list (quote get_assoc) row_sym (concat prefix (string i))))))))

(define row_project_expr (lambda (fields row_sym)
	(cons (quote list)
		(reduce_assoc (coalesceNil fields '()) (lambda (acc key _expr)
			(merge acc (list key (list (quote get_assoc) row_sym key))))
			'()))))

(define order_exprs (lambda (order)
	(map (coalesceNil order '()) (lambda (item) (match item
		'(expr _dir) expr
		_ nil)))))

(define order_key_dirs (lambda (keys dirs)
	(match keys
		(cons key rest_keys) (match dirs
			(cons dir rest_dirs) (cons (list key dir) (order_key_dirs rest_keys rest_dirs))
			_ '())
		'() '())))
...
(define order_sort_rows_expr (lambda (state)
	(begin
		...
		(define key_dirs (nth state 1))
		(define order (nth state 2))
...
(define window_func_expr? (lambda (expr) (match expr
	'((symbol window_func) _fn _args _over) true
	'((quote window_func) _fn _args _over) true
	_ false)))

(define collect_window_funcs (lambda (expr) (match expr
	(cons sym args) (if (window_func_expr? expr)
		(list expr)
		(dedupe_list (merge (map args collect_window_funcs))))
	'())))

(define collect_field_windows (lambda (fields)
	(dedupe_list (merge (extract_assoc (coalesceNil fields '()) (lambda (_key expr)
		(collect_window_funcs expr)))))))

(define has_windows? (lambda (fields)
	(not (equal? (collect_field_windows fields) '()))))

(define window_fn (lambda (wf) (match wf
	'((symbol window_func) fn _args _over) (toUpper fn)
	'((quote window_func) fn _args _over) (toUpper fn)
	_ (neumann_fail "build_queryplan" "malformed window function"))))

(define window_args (lambda (wf) (match wf
	'((symbol window_func) _fn args _over) (coalesceNil args '())
	'((quote window_func) _fn args _over) (coalesceNil args '())
	_ (neumann_fail "build_queryplan" "malformed window function"))))

(define window_over (lambda (wf) (match wf
	'((symbol window_func) _fn _args over) over
	'((quote window_func) _fn _args over) over
	_ (neumann_fail "build_queryplan" "malformed window function"))))

(define window_partition (lambda (wf)
	(match (window_over wf)
		'(partition _order) (coalesceNil partition '())
		_ (neumann_fail "build_queryplan" "malformed window OVER clause"))))

(define window_order (lambda (wf)
	(match (window_over wf)
		'(_partition order) (coalesceNil order '())
		_ (neumann_fail "build_queryplan" "malformed window OVER clause"))))

(define offset_window_func? (lambda (wf)
	(or (equal?? (window_fn wf) "LAG")
		(equal?? (window_fn wf) "LEAD"))))

(define window_func_columns_for_scan (lambda (wf scan_node)
	(dedupe_list (merge (list
		(dedupe_list (merge (map (window_args wf) (lambda (expr)
			(scan_expr_columns expr scan_node)))))
		(dedupe_list (merge (map (window_partition wf) (lambda (expr)
			(scan_expr_columns expr scan_node)))))
		(scan_order_columns (window_order wf) scan_node))))))
...
(define window_key_names (lambda (prefix n)
	(map (produceN n) (lambda (i) (concat prefix (string i))))))

(define window_index_of (lambda (windows target idx)
	(match windows
		(cons wf rest) (if (equal? wf target)
			idx
			(window_index_of rest target (+ idx 1)))
		'() nil)))

(define replace_window_refs (lambda (expr windows row_sym) (match expr
	...
	(cons sym args) (cons (replace_window_refs sym windows row_sym)
...
(define same_partition_expr (lambda (left_sym right_sym part_keys)
	(if (equal? part_keys '())
		true
		(cons (quote and) (map part_keys (lambda (key)
			(list (quote equal??)
				(list (quote get_assoc) left_sym key)
				(list (quote get_assoc) right_sym key))))))))

(define same_order_key_expr (lambda (left_sym right_sym order_keys)
	(if (equal? order_keys '())
		true
		(cons (quote and) (map order_keys (lambda (key)
			(list (quote equal??)
				(list (quote get_assoc) left_sym key)
				(list (quote get_assoc) right_sym key))))))))

(define window_arg_expr (lambda (wf row_sym)
	(match (window_args wf)
		(cons arg _rest) (replace_window_refs arg '() row_sym)
		'() 1)))

(define window_offset_expr (lambda (wf)
...
(define window_aggregate_value_expr (lambda (fn val_expr)
	(match fn
		"COUNT" (list (quote if) (list (quote nil?) val_expr) 0 1)
		_ val_expr)))

(define window_aggregate_reducer (lambda (fn acc_expr val_expr)
	(match fn
		"SUM" (list (quote +) acc_expr val_expr)
		"COUNT" (list (quote +) acc_expr val_expr)
		"MIN" (list (quote min) acc_expr val_expr)
		"MAX" (list (quote max) acc_expr val_expr)
		_ (neumann_fail "build_queryplan" (concat "unsupported window aggregate: " fn)))))

(define window_aggregate_neutral (lambda (fn)
	(match fn
		"SUM" 0
		"COUNT" 0
		"MIN" nil
...
(define lower_window_value_expr (lambda (wf idx_sym rows_sym row_sym part_keys order_keys)
	(begin
...
(define scan_source_col_fields (lambda (cols alias)
	(merge (map cols (lambda (col)
		(list col (list (quote get_column) alias false col false)))))))

(define order_expr_windows (lambda (order)
	(dedupe_list (merge (map (coalesceNil order '()) (lambda (item) (match item
		'(expr _dir) (collect_window_funcs expr)
		_ '())))))))

(define order_expr_without_windows? (lambda (order)
	(equal? (order_expr_windows order) '())))

(define window_outer_order_compatible? (lambda (outer_order partition win_order)
	(or
		(equal? outer_order win_order)
		(equal? outer_order
			(merge (list
				(map partition (lambda (expr) (list expr <)))
				win_order))))))

(define window_validate (lambda (fields predicate order_node)
	(begin
		(if (and (not (nil? order_node)) (not (order_expr_without_windows? (qattr order_node (quote order) '()))))
			(neumann_fail "build_queryplan" "window functions are not allowed in ORDER BY")
			true)
		(define windows (collect_field_windows fields))
		(match windows
...
(define lower_project_scan_window_rows_with_outer (lambda (project_node scan_node predicate order_node outer_specs)
	(begin
		...
		(if (and (not (equal? outer_order '()))
				(reduce windows (lambda (found wf) (or found (offset_window_func? wf))) false)
				(not (window_outer_order_compatible? outer_order partition win_order)))
			(neumann_fail "build_queryplan" "LAG/LEAD with conflicting outer ORDER BY is not supported")
			true)
		(define source_cols (dedupe_list (merge (list
			(scan_fields_columns fields scan_node)
			(scan_expr_columns predicate scan_node)
			(dedupe_list (merge (map windows (lambda (wf) (window_func_columns_for_scan wf scan_node)))))
			(dedupe_list (merge (map partition (lambda (expr) (scan_expr_columns expr scan_node)))))
			(scan_order_columns win_order scan_node)
			(scan_order_columns outer_order scan_node)))))
		...
		(define input_fields (merge
			(scan_source_col_fields source_cols alias)
			part_fields
			win_order_fields
			outer_order_fields))
		(define input_project (qnode (quote project)
			(concat "window-input:" (qid project_node))
			(list (list (quote output-fields) input_fields))
			(list scan_node)
			'()))
		(define input_rows_sym (symbol (concat "__neumann_window_input_" (sha1 (string project_node)))))
		(define sorted_rows_sym (symbol (concat "__neumann_window_sorted_" (sha1 (string project_node)))))
		(define augmented_rows_sym (symbol (concat "__neumann_window_augmented_" (sha1 (string project_node)))))
		(define filtered_augmented_sym (symbol (concat "__neumann_window_augmented_filtered_" (sha1 (string project_node)))))
		(define result_rows_sym (symbol (concat "__neumann_window_result_" (sha1 (string project_node)))))
		(define sorted_result_sym (symbol (concat "__neumann_window_result_sorted_" (sha1 (string project_node)))))
		(define limited_result_sym (symbol (concat "__neumann_window_result_limited_" (sha1 (string project_node)))))
		(define idx_sym (symbol "__window_idx"))
		(define row_sym (symbol "__window_row"))
		(define out_row_sym (symbol "__window_out_row"))
		(define part_keys (window_key_names "__win_part_" (count partition)))
		...
		(list (quote begin)
						...
						(list (quote begin)
							...
							(reduce (produceN (count windows)) (lambda (row_expr i)
								(list (quote set_assoc) row_expr
									...
									(lower_window_value_expr (nth windows i) idx_sym sorted_rows_sym row_sym part_keys win_order_keys)))
								row_sym)))))
			...
			(list (quote define) result_rows_sym
				(list (quote map) filtered_augmented_sym
					(list (quote lambda) (list row_sym)
...
(define lower_project_scan_window (lambda (project_node scan_node predicate order_node)
	(begin
		...
		(define row_sym (symbol "__window_emit_row"))
		(list (quote begin)
			(list (quote define) rows_sym (lower_project_scan_window_rows project_node scan_node predicate order_node))
...
(define lower_project_scan (lambda (project_node scan_node predicate)
	(begin
		...
		(define fields (qattr project_node (quote output-fields) '()))
		(if (has_windows? fields)
			(lower_project_scan_window project_node scan_node predicate nil)
			(begin
...
(define lower_project_scan_order (lambda (project_node order_node scan_node predicate)
	(begin
		...
		(if (has_windows? fields)
			(lower_project_scan_window project_node scan_node predicate order_node)
			(begin
		(define outer_specs (list (list scan_node false true)))
		(define effective_predicate (scan_effective_predicate scan_node predicate))
		(validate_local_scan_columns effective_predicate scan_node)
	...
	(match (qchildren project_node)
		(cons child '()) (match (qop child)
			...
			(quote select) (match (qchildren child)
				...
				_ (neumann_fail "build_queryplan" "select expects one child"))
			(quote order_limit) (if (equal? (qattr child (quote limit) nil) 0)
				false
				(match (qchildren child)
					(cons grandchild '()) (match (qop grandchild)
						(quote scan) (lower_exists_scan grandchild true outer_specs)
						(quote select) (match (qchildren grandchild)
							(cons scan_child '()) (match (qop scan_child)
								(quote scan) (lower_exists_scan scan_child (qattr grandchild (quote predicate) true) outer_specs)
								(quote join) (lower_exists_join scan_child (qattr grandchild (quote predicate) true) outer_specs)
								_ (neumann_fail "build_queryplan" "EXISTS/order_limit/select lowerer only supports scan or join input yet"))
							_ (neumann_fail "build_queryplan" "select expects one child"))
						(quote join) (lower_exists_join grandchild true outer_specs)
						_ (neumann_fail "build_queryplan" "EXISTS/order_limit lowerer only supports scan/select/join input yet"))
					_ (neumann_fail "build_queryplan" "order_limit expects one child")))
			(quote join) (lower_exists_join child true outer_specs)
			_ (neumann_fail "build_queryplan" "EXISTS lowerer only supports scan/select/order_limit/join input yet"))
...
(define lower_project_group (lambda (project_node group_node order_node)
	(if (has_windows? (qattr project_node (quote output-fields) '()))
		(neumann_fail "build_queryplan" "window functions over GROUP BY are not ported yet")
			...
			(cons child '()) (match (qop child)
				...
				(quote select) (match (qchildren child)
					(cons grandchild '()) (if (equal? (qop grandchild) (quote scan))
						...
						(if (equal? (qop grandchild) (quote join))
							(lower_project_group_join project_node group_node order_node grandchild (qattr child (quote predicate) true))
							(neumann_fail "build_queryplan" "GROUP BY select lowerer only supports scan or join input yet")))
					_ (neumann_fail "build_queryplan" "select expects one child"))
				(quote join) (lower_project_group_join project_node group_node order_node child true)
				_ (neumann_fail "build_queryplan" "GROUP BY lowerer only supports scan or join input yet"))
...
(define scan_call_reduce (lambda (op scan_node filtercols filter_expr mapcols map_expr row_reducer neutral_expr shard_reducer is_outer order_node)
	(begin
		(define alias (qid scan_node))
		(define schema (qattr scan_node (quote schema) nil))
		(define tbl (qattr scan_node (quote table) nil))
		...
		(if (equal? op (quote scan_order))
			(list (quote scan_order)
				...
				(cons (quote list) mapcols)
				(list (quote lambda) map_params map_expr)
				row_reducer neutral_expr is_outer)
			(list (quote scan)
				'(session "__memcp_tx")
				(list (quote table) schema tbl)
				(cons (quote list) filtercols)
...
(define predicate_safe_for_prefix? (lambda (predicate rest_specs all_specs)
	(equal?
		(dedupe_list (merge (map rest_specs (lambda (rest_spec)
			(expr_columns_for_spec predicate rest_spec all_specs)))))
		'())))
	...
	(begin
		...
		(if (or
			...
			(begin
				...
				(define row_fields (merge fields order_fields))
				(define row_project (qnode (quote project)
					(concat "ordered-join:" (qid join_node))
					(list (list (quote output-fields) row_fields))
					(list join_node)
					'()))
				(define rows_sym (symbol (concat "__neumann_join_order_rows_" (sha1 (string project_node)))))
				(define sorted_sym (symbol (concat "__neumann_join_order_sorted_" (sha1 (string project_node)))))
				(define limited_sym (symbol (concat "__neumann_join_order_limited_" (sha1 (string project_node)))))
				(define row_sym (symbol (concat "__neumann_join_order_row_" (sha1 (string project_node)))))
				(list (quote begin)
					(list (quote define) rows_sym (lower_project_join_rows row_project join_node nil final_predicate))
					(list (quote define) sorted_sym (order_sort_rows_expr (list rows_sym sort_key_dirs order)))
					(list (quote define) limited_sym (group_limit_rows_expr sorted_sym
						(qattr order_node (quote limit) nil)
...
(define lower_project_scan_rows (lambda (project_node scan_node predicate order_node)
	(begin
		...
		(define fields (qattr project_node (quote output-fields) '()))
		(if (has_windows? fields)
			(lower_project_scan_window_rows project_node scan_node predicate order_node)
			(begin
		(define order (if (nil? order_node) '() (qattr order_node (quote order) '())))
		(define outer_specs (list (list scan_node false true)))
		(define effective_predicate (scan_effective_predicate scan_node predicate))
	...
	(match specs
		(cons spec rest) (begin
			...
			(define lowered_filter (lower_expr_for_specs filter_predicate (merge
				...
				outer_specs)))
			(define map_expr (if (equal? rest '())
				(build_row_assoc_expr_with_specs (map_assoc fields (lambda (_key expr)
					(lower_expr_for_specs expr (merge all_specs outer_specs))))
					(merge all_specs outer_specs))
			...
			(scan_call_reduce (if (and use_order (not (nil? order_node))) (quote scan_order) (quote scan))
				...
				(if (equal? rest '())
					(list (quote lambda) (list (quote acc) (quote row))
						(list (quote merge) (quote acc) (list (quote list) (quote row))))
					(list (quote lambda) (list (quote acc) (quote rows))
						(list (quote merge) (quote acc) (quote rows))))
				'()
				(list (quote lambda) (list (quote acc) (quote shard_rows))
					(list (quote if)
						(list (quote and)
							(list (quote list?) (quote shard_rows))
							(list (quote >) (list (quote count) (quote shard_rows)) 0)
							(list (quote string?) (list (quote car) (quote shard_rows))))
						(list (quote merge) (quote acc) (list (quote list) (quote shard_rows)))
						(list (quote merge) (quote acc) (quote shard_rows))))
				(spec_outer? spec)
				order_node))
		'() (neumann_fail "build_queryplan" "empty join scan sequence"))))
...
(define lower_project_global_aggregate_join (lambda (project_node join_node predicate)
	(begin
		...
		(define input_project (qnode (quote project)
			(concat "aggregate-input:" (qid join_node))
			(list (list (quote output-fields) input_fields))
			(list join_node)
			'()))
		(define rows_sym (symbol (concat "__neumann_join_agg_rows_" (sha1 (string project_node)))))
		(define agg_sym (symbol (concat "__neumann_join_agg_" (sha1 (string project_node)))))
		(define row_sym (symbol (concat "__neumann_join_agg_row_" (sha1 (string project_node)))))
		(list (quote begin)
			(list (quote define) rows_sym (lower_project_join_rows input_project join_node nil predicate))
			(list (quote define) agg_sym
				(list (quote reduce) rows_sym
					(list (quote lambda) (list (quote acc) row_sym)
						(cons (quote list) (map (produceN (count aggs)) (lambda (i)
...
(define lower_project_group_join (lambda (project_node group_node order_node join_node predicate)
	(begin
		(define fields (qattr project_node (quote output-fields) '()))
		(define groups (qattr group_node (quote keys) '()))
		(define having (qattr group_node (quote having) nil))
		(define aggs (collect_field_aggregates fields))
		(define inputs (map aggs aggregate_input_expr))
		(define hidden_fields (merge
			(indexed_expr_fields "__group_key_" groups)
			(indexed_expr_fields "__agg_in_" inputs)))
		(define hidden_project (qnode (quote project)
			(concat "group-input:" (qid join_node))
			(list (list (quote output-fields) hidden_fields))
			(list join_node)
			'()))
		(define rows_sym (symbol (concat "__neumann_join_group_input_" (sha1 (string project_node)))))
		(define groups_sym (symbol (concat "__neumann_join_groups_" (sha1 (string project_node)))))
		(define result_rows_sym (symbol (concat "__neumann_join_group_rows_" (sha1 (string project_node)))))
		(define sorted_sym (symbol (concat "__neumann_join_group_sorted_" (sha1 (string project_node)))))
		(define limited_sym (symbol (concat "__neumann_join_group_limited_" (sha1 (string project_node)))))
		(define row_sym (symbol (concat "__neumann_join_group_row_" (sha1 (string project_node)))))
		(define key_sym (symbol "__key"))
		(define agg_sym (symbol "__agg"))
		(define old_sym (symbol "__old"))
		(define row_key_expr (row_values_expr row_sym "__group_key_" (count groups)))
		(define row_vals_expr (row_values_expr row_sym "__agg_in_" (count aggs)))
		(define agg_neutral (cons (quote list) (map aggs aggregate_neutral_expr)))
		(define merge_group_aggs
			(list (quote lambda) (list (quote old) (quote new))
				(cons (quote list) (map (produceN (count aggs)) (lambda (i)
				...
				(list (quote reduce_assoc) groups_sym
					(list (quote lambda) (list (quote acc) key_sym agg_sym)
						(list (quote if) having_expr
							...
							(quote acc)))
					'()))
			(list (quote define) sorted_sym (group_sort_rows_expr result_rows_sym fields (if (nil? order_node) '() (qattr order_node (quote order) '()))))
			(list (quote define) limited_sym (group_limit_rows_expr sorted_sym
				(if (nil? order_node) nil (qattr order_node (quote limit) nil))
				(if (nil? order_node) nil (qattr order_node (quote offset) nil))))
			(list (quote map) limited_sym
		...
		(cons child '()) (match (qop child)
			...
			(quote select) (match (qchildren child)
				(cons grandchild '()) (match (qop grandchild)
					...
					(quote join) (if (has_aggregates? (qattr node (quote output-fields) '()))
						(lower_project_global_aggregate_join node grandchild (qattr child (quote predicate) true))
						(lower_project_join node grandchild nil (qattr child (quote predicate) true)))
			...
			(quote order_limit) (match (qchildren child)
				...
				_ (neumann_fail "build_queryplan" "order_limit expects one child"))
			(quote join) (if (has_aggregates? (qattr node (quote output-fields) '()))
				(lower_project_global_aggregate_join node child true)
				(lower_project_join node child nil true))
...
(define lower_dml_scan (lambda (kind fields scan_node predicate order_node)
	(begin
		(define alias (qid scan_node))
		(define schema (qattr scan_node (quote schema) nil))
		(define tbl (qattr scan_node (quote table) nil))
		...
		(define mapcols (dedupe_list (merge (list filtercols ordercols updatecols (list "$update")))))
		(define filter_params (map filtercols (lambda (col) (symbol (concat alias "." col)))))
		(define map_params (map mapcols (lambda (col) (if (equal? col "$update")
			(symbol "$update")
			(symbol (concat alias "." col))))))
		(define map_fn (list (quote lambda) map_params
...
(define lower_dml_join (lambda (kind target_alias fields join_node predicate)
	(begin
		(define specs (join_scan_specs join_node))
		(if (equal? specs '())
			(neumann_fail "build_queryplan" "DML join lowerer requires non-empty join specs")
			(begin
				(define target_spec (car specs))
				(define rest_specs (cdr specs))
				(define target_node (spec_node target_spec))
				(define alias (qid target_node))
				(if (and (not (nil? target_alias)) (not (equal?? alias target_alias)))
					(neumann_fail "build_queryplan" "DML join lowerer requires target table as first join input")
					true)
				(if (equal? rest_specs '())
					(neumann_fail "build_queryplan" "DML join lowerer requires at least one non-target join input")
					true)
				(define rows_sym (symbol (concat "__neumann_dml_join_rows_" (sha1 (string join_node)))))
				(define row_sym (symbol (concat "__neumann_dml_join_row_" (sha1 (string join_node)))))
				(define rows_expr (lower_scan_specs_rows_with_outer rest_specs specs fields nil predicate '() true (list target_spec)))
				(define has_match_expr (list (quote >) (list (quote count) rows_sym) 0))
				(define update_payload_expr (row_project_expr fields row_sym))
				(define target_cols (dedupe_list (merge (list
					(fields_columns_for_spec_in_join fields target_spec specs)
					(expr_columns_for_spec predicate target_spec specs)))))
				(define mapcols (dedupe_list (merge (list target_cols (list "$update")))))
				(define map_params (map mapcols (lambda (col) (if (equal? col "$update")
					(symbol "$update")
					(symbol (concat alias "." col))))))
				(define map_fn (list (quote lambda) map_params
					(list (quote begin)
						(list (quote define) rows_sym rows_expr)
						(list (quote if) has_match_expr
							(list (quote begin)
								(list (quote define) row_sym (list (quote car) rows_sym))
								(if (equal? kind (quote delete))
									(list (quote if) (list (quote $update)) 1 0)
									(list (quote if) (list (quote $update) update_payload_expr) 1 0)))
							0))))
				(list (quote scan)
					'(session "__memcp_tx")
					(list (quote table) (qattr target_node (quote schema) nil) (qattr target_node (quote table) nil))
					'()
...
(define lower_dml_qnode (lambda (ir)
	(match (ir_return ir)
		'(kind _target_table target_alias fields) (match (qop (ir_root ir))
			(quote project) (match (qchildren (ir_root ir))
				(cons child '()) (match (qop child)
					(quote scan) (lower_dml_scan kind (qattr (ir_root ir) (quote output-fields) fields) child true nil)
					(quote select) (match (qchildren child)
						(cons scan_child '()) (match (qop scan_child)
							(quote scan) (lower_dml_scan kind (qattr (ir_root ir) (quote output-fields) fields) scan_child (qattr child (quote predicate) true) nil)
							(quote join) (lower_dml_join kind target_alias (qattr (ir_root ir) (quote output-fields) fields) scan_child (qattr child (quote predicate) true))
							_ (neumann_fail "build_queryplan" "DML/select lowerer only supports scan or join input yet"))
						_ (neumann_fail "build_queryplan" "select expects one child"))
					(quote order_limit) (match (qchildren child)
						(cons ordered_child '()) (match (qop ordered_child)
							(quote scan) (lower_dml_scan kind (qattr (ir_root ir) (quote output-fields) fields) ordered_child true child)
							(quote select) (match (qchildren ordered_child)
								(cons scan_child '()) (if (equal? (qop scan_child) (quote scan))
									(lower_dml_scan kind (qattr (ir_root ir) (quote output-fields) fields) scan_child (qattr ordered_child (quote predicate) true) child)
									(neumann_fail "build_queryplan" "DML/order_limit/select lowerer only supports scan input yet"))
								_ (neumann_fail "build_queryplan" "select expects one child"))
							_ (neumann_fail "build_queryplan" "DML/order_limit lowerer only supports scan input yet"))
						_ (neumann_fail "build_queryplan" "order_limit expects one child"))
					(quote join) (lower_dml_join kind target_alias (qattr (ir_root ir) (quote output-fields) fields) child true)
					_ (neumann_fail "build_queryplan" "DML project lowerer only supports scan/join input yet"))
				_ (neumann_fail "build_queryplan" "project expects one child"))
