...
(define unnest_map_rule (lambda (tree subquery sq_cache target_expr us_single_tbl _us_nested_direct_tbls _us_base_aliases _us_base_tables us_has_stages _us_own_stages _us_inner_aliases tables2_us us_has_outer _us_inner_stages us_domain_cols _us_ria us_sq_prefix _us_lookup us_outer_parts _us_ror us_inner_cond_raw schemas2_us us_value_expr us_accessing_tags us_select_info) (begin
	...
	(if (and us_single_tbl (not _us_nested_direct_refs_base_aliases))
		(begin
			...
			(define us_stage_order_fallback (if us_has_stages (coalesceNil (stage_order_list (car _us_own_stages)) '()) '()))
			(define us_stage_limit_fallback (if us_has_stages (stage_limit_val (car _us_own_stages)) nil))
			(define us_stage_offset_fallback (if us_has_stages (stage_offset_val (car _us_own_stages)) nil))
...
(define unnest_groupby_rule (lambda (tree subquery sq_cache target_expr tables2_us _us_lookup us_alias_map _us_ria us_has_stages _us_own_stages _us_inner_stages us_domain_cols us_inner_cond_raw schemas2_us us_value_expr us_has_grp us_accessing_tags us_select_info) (begin
	...
	(define us_stage_having_fallback (if us_has_stages (stage_having_expr (car _us_own_stages)) nil))
	...
	(define _us_parts_for (lambda (alias) (begin
		...
		(if (equal? (count _my) 0) nil
			(if (equal? (count _my) 1) (car _my)
				(cons (quote and) _my))))))
	(define _us_merge_unique_and (lambda (expr_a expr_b)
		(combine_and_terms
			(reduce
				(merge
				...
				'(a s t io je) (if (not (reduce us_prefixed_aliases (lambda (f al) (or f (equal?? al a))) false)) td
					(begin
						...
						(if (equal? a _first_alias)
							(list a s t true
								(if (nil? _my_cond) us_dom_je
									(if (equal? us_dom_je true) _my_cond
										(_us_merge_unique_and us_dom_je _my_cond))))
							(list a s t io
								(if (nil? _my_cond) je
									(if (nil? je) _my_cond
