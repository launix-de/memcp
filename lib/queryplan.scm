...
(define untangle_query (lambda (schema tables fields condition group having order limit offset outer_schemas_param) (begin
								...
								(begin
										...
										(if (or us_has_agg us_has_grp)
											...
											(begin
												...
												(define us_stage_aliases us_inner_aliases)
	...
	(define subquery_outer_refs_are_direct_columns (lambda (query outer_schemas)
		(reduce (_subquery_outer_refs query outer_schemas) (lambda (all_ok ref)
			...
			true)))
	/* unnest_scalar_subselect: delegates to unnest_subselect which handles
	all cases (no-table, aggregate, correlated, ORDER/LIMIT, HAVING).
	Follows unnesting-pure: no gate checks, no Path B duplicate.
	Tables/groups are registered directly in sq_cache by unnest_subselect. */
	(define unnest_scalar_subselect (lambda (subquery outer_schemas) (match subquery
		'(_ _ flds _ _ _ _ _ _) (begin
			(define value_expr (match flds (cons _ (cons v _)) v nil))
			(if (not (nil? value_expr))
