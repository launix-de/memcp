...
(define sources_referenced_by_exprs (lambda (sources exprs)
	(filter (coalesceNil sources '()) (lambda (src)
		(reduce (coalesceNil exprs '()) (lambda (found expr)
			(or found (expr_refs_alias? nil (source_alias src) expr)))
			false)))))
...
(define make_exists_stage_rewrite (lambda (inner args)
	(begin
		...
		(define domain_sources (sources_referenced_by_exprs outer_sources lookup_keys))
		...
		(define stage (make_group_stage
			...
			(merge (list
				(list
					...
					(list (quote domain-sources) domain_sources)
