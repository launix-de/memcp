...
(define join_null_propagating? (lambda (src expr)
	(match expr
		...
		(cons head tail)
		(if (contains? '("sql_in" "sql_collated_in") (string head))
			(join_null_propagating? src (cadr tail))
			(equal? (string head) "sql_compare")
			(or (join_null_propagating? src (car tail))
				(join_null_propagating? src (cadr tail)))
			(contains? '("sql_typed_value" "sql_parameter_value" "sql_collation") (string head))
			(join_null_propagating? src (car tail))
