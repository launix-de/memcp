			...
			"/dashboard/api/logs" (begin
				(if (dashboard_check_admin req) (begin
					(define items (scan_order (session "__memcp_tx") (table "system_statistic" "logs")
						...
						'("datetime") '(>) 0 0 100
						'("datetime" "message")
						(lambda (acc dt msg) (begin
							(define item (json_encode_assoc (list "datetime" dt "message" msg)))
							(if (equal? acc "") item (concat acc "," item))))
						""))
				...
				(if (dashboard_check_admin req) (begin
					(define items (scan (session "__memcp_tx") (table "system_statistic" "errors")
						...
						(lambda (acc dt db usr qry err) (begin
							(define item (json_encode_assoc (list "datetime" dt "database" db "user" usr "query" qry "error" err)))
							(if (equal? acc "") item (concat acc "," item))))
						"" (lambda (a b) (if (equal? a "") b (if (equal? b "") a (concat a "," b))))))
					(dashboard_send_json res (concat "[" items "]"))
				) (dashboard_send_401 res))
...
(define log_trim_tick (lambda () (begin
	...
	(if (> max_print 0) (begin
		(try (lambda () (begin
			(define cnt (scan nil (table "system_statistic" "logs") '(369435906932736) '() '() (lambda () true) '() (lambda (acc) (+ acc 1)) 0 +))
			(if (> cnt max_print)
				(scan_order nil (table "system_statistic" "logs") '(369435906932736) '() '() (lambda () true) '("datetime") '(<) 0 0 (- cnt max_print) '("$update") (lambda (acc $update) (begin ($update) acc)) nil)
			)
		)) (lambda (e) true))
	))
	/* trim error log */
	...
	(if (> max_err 0) (begin
		(try (lambda () (begin
			(define cnt (scan nil (table "system_statistic" "errors") '(369435906932736) '() '() (lambda () true) '() (lambda (acc) (+ acc 1)) 0 +))
			(if (> cnt max_err)
				(scan_order nil (table "system_statistic" "errors") '(369435906932736) '() '() (lambda () true) '("datetime") '(<) 0 0 (- cnt max_err) '("$update") (lambda (acc $update) (begin ($update) acc)) nil)
			)
		)) (lambda (e) true))
	))
