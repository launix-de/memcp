...
(define sql_policy (lambda (username)
	(begin
		(define is_admin (scan_lookup nil (table "system" "user")
			'("scan_access" 1 "value" 1 -1 "equal" "username" 0 0 3 "" "admin") (list username)))
				...
				(begin
					...
					(if (equal?? schema "information_schema") true (begin
						...
						(define has_access (scan_lookup nil (table "system" "access")
							'("scan_access" 2 "exists" 0 -1 "equal" "username" 0 0 3 "" "equal" "database" 1 1 3 "") (list username schema)))
	...
	(if (has? (show "system") "user") (begin
		(if (has? (map (show "system" "user") (lambda (col) (get_assoc col "Field"))) "admin")
			...
			(begin
				...
				(scan nil (table "system" "user") '("scan_access" 0 "scan" 0 -1) '() '() (lambda () true) '("$update") (lambda (acc $update) (begin ($update '("admin" true)) acc)))
			)
		)
	) true)
)) (lambda (e) true))
...
(try (lambda () (begin
	(if (has? (show "system") "user")
		(scan nil (table "system" "user") '("scan_access" 1 "scan" 0 -1 "equal" "username" 0 0 3 "") '("root") '() (lambda () true) '("$update") (lambda (acc $update) (begin ($update '("admin" true)) acc)))
		true)
)) (lambda (e) true))
...
(sql_view_catalog_set_count
	(scan nil (table "system" "views") '("scan_access" 0 "scan" 0 -1) '() '() (lambda () true) '() (lambda (acc) (+ acc 1)) 0 +))
...
(define http_handler (begin
	...
	(define handle_query (lambda (req res schema query) (begin
		/* check for password */
		(set pw (scan_lookup nil (table "system" "user") '("scan_access" 1 "value" 1 -1 "equal" "username" 0 0 3 "" "password") (list (req "username"))))
...
(set mysql_auth (lambda (username_)
	(scan_lookup nil (table "system" "user") '("scan_access" 1 "value" 1 -1 "equal" "username" 0 0 3 "" "password") (list username_))))
