...
(define qpp-expr-has-window? (lambda (expr)
	(not (equal? (extract_window_funcs (coalesceNil expr true)) '()))))

(define qpp-tuple-has-window-shallow? (lambda (sub) (begin
	(define field-has-window
		(reduce (qpp-fields-to-pairs (qpp-tuple-fields sub)) (lambda (acc pair) (match pair
			'(_ expr) (or acc (qpp-expr-has-window? expr))
			acc)) false))
	(define order-has-window
		(reduce (coalesceNil (qpp-tuple-order sub) '()) (lambda (acc item) (match item
			'(expr _dir) (or acc (qpp-expr-has-window? expr))
			acc)) false))
	(or field-has-window
		(qpp-expr-has-window? (qpp-tuple-condition sub))
		(qpp-expr-has-window? (qpp-tuple-having sub))
		order-has-window))))

(define qpp-derived-can-inline? (lambda (sub)
	(and (qpp-tuple? sub)
		(not (qpp-tuple-has-window-shallow? sub))
