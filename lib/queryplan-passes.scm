...
/* ==================== Pass 0.5: alias_disambiguate_pass ==================== */
...
Why: the parser permits the same table-alias string at multiple SQL scope
levels (e.g. `SELECT … FROM gs_orders WHERE x = (SELECT amount FROM gs_orders
ORDER BY … LIMIT 1)` parses both scopes' `gs_orders` as the same atom).
SQL scope rules say they are DIFFERENT relations, but my Neumann pipeline
preserves the alias string in the lowered 7-tuple. The legacy build_queryplan
then can't tell them apart — wildcard expansion duplicates columns, refs
get mis-bound, etc. (test 52 'WHERE with LIMIT 1 subquery' is the canonical
breakage).

Architectural fix: rename inner-scope collisions BEFORE column_resolve_scoped
fires, so every subsequent pass sees unambiguous aliases. The renamed
alias preserves the original for legibility (`gs_orders:1`) and the visible
form per `visible_occurrence_alias` is the original — so user-facing
behaviour is unchanged.

The pass walks top-down accumulating outer-aliases. For each table whose
alias is in outer-aliases, generate a unique replacement; for each inner_*
marker, recurse with the merged outer set. Refs to renamed aliases inside
the sub get updated via expression walker.

Per FAQ §35 "canonical names always from physical source columns": renaming
is purely syntactic — the underlying table identity (schema + table name)
is unchanged. */
...
/* qpp-rename-refs-in-expr — walk expr, replace every (get_column tv …)
where tv ∈ rename-map's keys with the mapped tv. */
...
/* qpp-disambiguate-tuple-recursive — main worker. `outer-aliases` is the
set of alias strings visible from above; we use them to detect collisions
in this scope's tables. Returns the disambiguated tuple. */
(define qpp-disambiguate-tuple-recursive (lambda (t outer-aliases)
	(if (not (qpp-tuple? t)) t
		(begin
			...
			/* Decide renames for each table whose alias collides with outer. */
			...
			/* Apply rename-map to any joinExpr (5th slot) inside this scope's
			   tables — these run in the current scope, so refs to renamed
			   aliases must be updated. */
			(define renamed-tables (map renamed-tables-step1 (lambda (td) (match td
				'(alias tschema ttbl io je)
					(list alias tschema ttbl io
						(if (nil? je) nil (qpp-rename-refs-in-expr je rename-map)))
				td))))
			/* New visible set = outer + this scope's (post-rename) aliases. */
			(define this-scope-aliases (map renamed-tables (lambda (td) (match td
				'(alias tschema ttbl io je) (if (nil? alias) ttbl alias)
				nil))))
			(define new-visible (merge outer-aliases (filter this-scope-aliases
				(lambda (a) (not (nil? a))))))
			/* Recurse into sub-tuples in tables (derived) with NEW-VISIBLE as
			   outer. Apply renames-in-expr to top-level expressions. */
			(define tables-with-recursed-derived (map renamed-tables (lambda (td) (match td
				'(alias tschema ttbl io je)
					(if (qpp-tuple? ttbl)
						(list alias tschema
						...
						td)
				td))))
			/* Walk top-level expression slots: apply renames (refs to renamed
			   tables within this scope) + recurse into inner_select markers
			   with new-visible as outer scope. */
			...
			(qpp-rebuild-tuple
				(qpp-tuple-schema t)
				tables-with-recursed-derived
				(qpp-map-fields (qpp-tuple-fields t) rewrite-expr)
...
/* qpp-disambiguate-expr-recursive — walk expr; when encountering inner_select*
markers, recurse into their sub-tuples with outer-aliases as visible. */
